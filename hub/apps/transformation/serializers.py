...
class TransformationPipelineSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        """Decrypt pipeline_definition via model accessor."""
        ret = super().to_representation(instance)
        ret["pipeline_definition"] = instance.get_pipeline_definition()
        return ret
...
class TransformationPipelineCreateSerializer(serializers.Serializer):
    ...
    def validate_pipeline_definition(self, value):
        ...
        if not isinstance(value, dict):
            raise serializers.ValidationError("Pipeline definition must be a JSON object")
        ...
        if not isinstance(steps, list):
            raise serializers.ValidationError("Pipeline definition 'steps' must be a list")

        if len(steps) == 0:
            raise serializers.ValidationError("Pipeline definition must have at least one step")

        # Validate each step has required fields
        for i, step in enumerate(steps):
            if not isinstance(step, dict):
                raise serializers.ValidationError(f"Step {i} must be a JSON object")
...
class PipelineExecutionSerializer(serializers.ModelSerializer):
    ...
    result_asset_id = serializers.UUIDField(
        source="result_asset.id", read_only=True, allow_null=True
    )
    result_asset_name = serializers.CharField(
        source="result_asset.name", read_only=True, allow_null=True
    )
...
class PipelineExecutionProgressSerializer(serializers.Serializer):
    ...
    recent_logs = serializers.ListField(child=serializers.DictField(), allow_empty=True)
...
class PipelineExecutionResultSerializer(serializers.Serializer):
    ...
    metrics = serializers.DictField(allow_empty=True)
    execution_log = serializers.ListField(child=serializers.DictField(), allow_empty=True)
    started_at = serializers.DateTimeField(allow_null=True)
...
class WranglingOperationRequestSerializer(serializers.Serializer):
    ...
    operation = serializers.DictField(help_text="Operation definition with 'type' and 'parameters'")
...
class WranglingResultSerializer(serializers.Serializer):
    ...
    result = serializers.DictField(help_text="Operation result (sample_data, row counts, columns)")
