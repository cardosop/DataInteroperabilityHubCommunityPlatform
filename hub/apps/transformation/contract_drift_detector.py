...
class ContractDriftDetector:
        ...
        for pipeline in downstream:
            report = DriftReport(
                ...
                has_breaking_changes=any(
                    c.severity == DriftSeverity.BREAKING for c in changes
                ),
    ...
    def _compare_schemas(
        ...
        prev_fields: Dict[str, Dict[str, Any]] = {
            f["name"]: f for f in previous.get("fields", [])
        }
        curr_fields: Dict[str, Dict[str, Any]] = {
            f["name"]: f for f in current.get("fields", [])
        }
