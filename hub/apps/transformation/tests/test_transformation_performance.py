...
class TransformationPerformanceTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            email=f"t-{uid}@test.com", tenant=self.tenant, status=UserStatus.ACTIVE
        )
    ...
    def test_pipeline_creation_performance(self):
        ...
        pipeline_definition = {
            "version": "1.0.0",
            "steps": [
                {"type": "FILTER", "name": "filter_step", "config": {"condition": "column1 > 100"}}
            ],
        }
        ...
        self.assertLess(
            creation_time, 2.0, f"Pipeline creation took {creation_time:.2f}s, expected < 2.0s"
        )
    ...
    def test_preview_generation_performance(self):
        ...
        asset = Asset.objects.create(
            name="Test Asset", tenant=self.tenant, status=AssetStatus.ACTIVE
        )
        ...
        Dataset.objects.create(
            asset=asset, tenant=self.tenant, version=1, format="CSV", file=file_obj
        )
    ...
    def test_sequential_pipeline_creation_performance(self):
        ...
        self.assertLess(
            total_time, 10.0, f"Creating 5 pipelines took {total_time:.2f}s, expected < 10.0s"
        )
    ...
    def test_large_pipeline_definition_performance(self):
            ...
            steps.append(
                {
                    ...
                    "config": {"condition": f"column{i} > {i * 10}"},
                }
            )

        pipeline_definition = {"version": "1.0.0", "steps": steps}
        ...
        self.assertLess(
            creation_time,
            3.0,
            f"Large pipeline creation took {creation_time:.2f}s, expected < 3.0s",
        )
        self.assertIsNotNone(pipeline)
