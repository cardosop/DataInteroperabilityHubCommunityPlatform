...
class TransformationServiceIntegrationTest(TestCase):
    ...
    def test_service_workflow_integration(self):
        ...
        try:
            execution = self.service.execute_pipeline(
                ...
                tenant_id=str(self.tenant.id),
                user_id=str(self.user.id),
                execution_mode=ExecutionMode.ASYNC[0] if isinstance(ExecutionMode.ASYNC, tuple) else ExecutionMode.ASYNC[0] if isinstance(ExecutionMode.ASYNC, tuple) else ExecutionMode.ASYNC
            )
            ...
            if execution.execution_mode == ExecutionMode.ASYNC:
                ...
                self.assertEqual(workflow_instance.workflow_type, "transformation_pipeline")
        except Exception as e:
    ...
    def test_service_compliance_integration(self):
        ...
        try:
            ...
            # They block execution if compliance fails
            self.assertIsNotNone(execution.started_at)
    ...
    def test_service_job_queue_integration(self):
        ...
        job_type_value = (JobType.TRANSFORMATION_PIPELINE_EXECUTION[0]
                         if isinstance(JobType.TRANSFORMATION_PIPELINE_EXECUTION, tuple)
                         else JobType.TRANSFORMATION_PIPELINE_EXECUTION)
    ...
    def test_service_event_publishing_integration(self):
        ...
        # Event publishing is non-blocking, so we just verify pipeline was created
    ...
    def test_service_monitoring_integration(self):
        ...
        try:
            ...
            # Metrics are published to monitoring system asynchronously
            # We verify execution completed successfully
