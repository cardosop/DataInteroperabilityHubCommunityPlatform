...
class TransformationServiceTest(TestCase):
    ...
    def test_create_pipeline(self):
        ...
        self.assertEqual(pipeline.pipeline_definition, self.valid_pipeline_definition)
    ...
    def test_create_pipeline_with_defaults(self):
        ...
        self.assertEqual(pipeline.version, "1.0.0")
        self.assertEqual(pipeline.status, PipelineStatus.DRAFT)
        self.assertEqual(pipeline.pipeline_definition["version"], "1.0.0")
        # Default pipeline definition includes at least one step to pass validation
        self.assertGreaterEqual(len(pipeline.pipeline_definition["steps"]), 1)
        self.assertEqual(pipeline.pipeline_definition["steps"][0]["name"], "initial_step")
    ...
    def test_create_node(self):
        ...
        self.assertEqual(node.node_config, {"filter_expression": "age > 18"})
    ...
    def test_create_pipeline_integration_workflow(self):
        ...
        self.assertEqual(len(pipeline.pipeline_definition["steps"]), 3)
...
class PreviewTransformationTest(TestCase):
    ...
    def setUp(self):
        ...
        self.file = File.objects.create(
            ...
            storage_path=f"test/preview/{self.asset.id}/test_preview.csv",
            status=FileStatus.ACTIVE
        )
