...
class FakeDbtExecutor:
    ...
    def __init__(
        ...
        """
        self.results = results or copy.deepcopy(
            self._DEFAULT_RESULTS.get("run", [])
        )
        ...
        self.called_models: Optional[List[str]] = None
        self.called_target: Optional[str] = None
    ...
    def _respond(self, action: str, project_path: str) -> Dict[str, Any]:
        ...
        if self.fail_on and action == self.fail_on:
            return {
                ...
                "results": [],
                "elapsed_total": None,
                "error_logs": self.error_logs or [
                    f"Database Error in dbt {action}"
                ],
                "span_attributes": {},
            }
