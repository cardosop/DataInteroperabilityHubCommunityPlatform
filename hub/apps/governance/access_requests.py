...
class AccessRequestWorkflow:
    ...
    def create_access_request(
        ...
        approval_workflow: list[dict[str, Any]] | None = None,
    ...
    def approve_access_request(
        access_request_id: str, approved_by_id: str, step_index: int | None = None
    ) -> AccessRequest:
        """
    ...
    def reject_access_request(
        access_request_id: str, rejected_by_id: str, rejection_reason: str
    ) -> AccessRequest:
        """
    ...
    @staticmethod
    @transaction.atomic
    def revoke_access(access_request_id: str, revoked_by_id: str) -> AccessRequest:
        """
    ...
    def check_access_expiration():
        ...
        expired_requests = AccessRequest.objects.filter(
            status=AccessRequestStatus.APPROVED.value, expires_at__lte=now
        )

        count = expired_requests.update(status=AccessRequestStatus.EXPIRED.value)

        return count

    @staticmethod
    def get_user_access_requests(
        user_id: str, tenant_id: str | None = None, status: AccessRequestStatus | None = None
    ) -> list[AccessRequest]:
        """
    ...
    def get_pending_approvals(
        approver_id: str, tenant_id: str | None = None
    ) -> list[AccessRequest]:
        """
        ...
        queryset = AccessRequest.objects.filter(
            status=AccessRequestStatus.PENDING.value, requires_approval=True
        )
