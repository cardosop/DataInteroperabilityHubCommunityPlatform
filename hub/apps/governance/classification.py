...
from .models import ClassificationCategory, ClassificationStatus, DataClassification
...
class DataClassifier:
    ...
    def classify_field(
        ...
        compliance_service_client: ComplianceServiceClient | None = None,
    ...
    def classify_field_level(
        ...
        result = DataClassifier.classify_field(
            field_name=field_name, field_data=field_data, sample_values=sample_values
        )

        # Create or update classification
        classification, _created = DataClassification.objects.update_or_create(
            tenant_id=tenant_id,
            asset=asset,
            dataset=dataset,
    ...
    def classify_dataset(
        tenant_id: str, dataset_id: str, user_id: str | None = None
    ...
    @transaction.atomic
    def classify_asset(
        tenant_id: str, asset_id: str, user_id: str | None = None
    ) -> list[DataClassification]:
        """
        ...
        datasets = Dataset.objects.filter(tenant_id=tenant_id, asset_id=asset_id, is_current=True)

        classifications = []

        for dataset in datasets:
            dataset_classifications = DataClassifier.classify_dataset(
                tenant_id=tenant_id, dataset_id=str(dataset.id), user_id=user_id
            )
    ...
    def approve_classification(
        classification_id: str, user_id: str, notes: str | None = None
    ) -> DataClassification:
        """
    ...
    DataClassification.objects.update_or_create(
        **target_filter,
        defaults={
            ...
            "manual_review_notes": (
                f"Propagated from {source_type}:{source_id} (original category: {highest.category})"
            ),
        },
    )
