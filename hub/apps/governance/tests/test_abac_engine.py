...
def _mock_policy(
    effect="ALLOW", conditions=None, enabled=True, priority=10,
    asset_id=None, dataset_id=None,
):
...
def _mock_field_policy(
    access_type="READ", masking_strategy=None, enabled=True,
):
    fp = MagicMock()
...
class EvaluateConditionsTest(TestCase):
    ...
    def test_empty_conditions_always_match(self):
        assert ABACEngine._evaluate_conditions(
            {}, _USER_ATTRS, _RES_ATTRS, _ENV_ATTRS,
        ) is True

    def test_user_role_match(self):
        conditions = {"user": {"user_roles": ["DATA_CONSUMER"]}}
        assert ABACEngine._evaluate_conditions(
            conditions, _USER_ATTRS, _RES_ATTRS, _ENV_ATTRS,
        ) is True
    ...
    def test_resource_classification_matching(self):
        ...
        assert ABACEngine._evaluate_conditions(
            conditions, _USER_ATTRS, res, _ENV_ATTRS,
        ) is True
    ...
    def test_environment_time_outside_range(self):
        ...
        assert ABACEngine._evaluate_conditions(
            conditions, _USER_ATTRS, _RES_ATTRS, env,
        ) is False
...
class EvaluateAccessTest(TestCase):
    ...
    @patch.object(ABACEngine, "_get_environment_attributes",
                  return_value=_ENV_ATTRS)
    @patch.object(ABACEngine, "_get_resource_attributes",
                  return_value=_RES_ATTRS)
...
class FieldLevelAccessTest(TestCase):
    ...
    def test_field_level_masking_detected(
        self, mock_policies, _res, _env, mock_fp,
    ):
...
class ABACCacheTest(TestCase):
    ...
    def test_cache_populated_after_first_eval(self, mock_cache):
        ...
        ABACEngine._get_applicable_policies(
            str(tenant.id), "ASSET", str(uuid.uuid4()),
        )
    ...
    def test_cache_hit_skips_db(self, mock_cache):
        ...
        with patch(
            "hub.apps.governance.models.AccessPolicy.objects"
        ) as mock_qs:
