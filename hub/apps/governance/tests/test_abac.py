...
class ABACEngineTest(TestCase):
    ...
    def test_policy_caching(self):
        """Test policy caching returns consistent results via cache"""
        # Create policy
        AccessPolicy.objects.create(
            tenant=self.tenant,
            name="Cached Policy",
        ...
        # Verify cache was populated after evaluate_access
    ...
    def test_invalidate_policy_cache(self):
        ...
        cache_key = ABACEngine._get_cache_key(str(self.tenant.id), "DATASET", str(self.dataset.id))
        cached_data = cache.get(cache_key)
        self.assertIsNotNone(cached_data, "Cache should be populated after evaluate_access")
        self.assertGreater(len(cached_data), 0, "Cache should contain at least one policy ID")

        # Invalidate cache
        ABACEngine.invalidate_policy_cache(
            tenant_id=str(self.tenant.id), resource_type="DATASET", resource_id=str(self.dataset.id)
        )

        # Verify cache IS empty after invalidation
        cached_after = cache.get(cache_key)
        self.assertIsNone(cached_after, "Cache should be empty after invalidate_policy_cache")
