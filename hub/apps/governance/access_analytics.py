...
class AccessLog(models.Model):
    ...
    resource_type = models.CharField(
        max_length=50, help_text="Resource type (ASSET, DATASET, FILE, etc.)"
    )
    ...
    is_anomaly = models.BooleanField(
        default=False, help_text="Whether this access was flagged as anomalous"
    )
    anomaly_reason = models.TextField(null=True, blank=True, help_text="Reason for anomaly flag")
...
class AccessAnalyticsService:
    ...
    def get_access_patterns(
        ...
        patterns = (
            query.values("user", "resource_type", "resource_id", "action")
            .annotate(
    ...
    def get_security_events(
        ...
        """
        query = AccessLog.objects.filter(Q(result="DENIED") | Q(is_anomaly=True))
    ...
    def get_analytics_dashboard(
        ...
        top_resources = (
            query.values("resource_type", "resource_id")
            .annotate(access_count=Count("id"))
            .order_by("-access_count")[:10]
        )
        ...
        return {
            "summary": {
                ...
                "denial_rate": round(
                    (denied_accesses / total_accesses * 100) if total_accesses > 0 else 0.0, 2
                ),
                "anomaly_rate": round(
                    (anomaly_count / total_accesses * 100) if total_accesses > 0 else 0.0, 2
                ),
            },
            "top_users": [
                {
