...
class ComplianceReportGenerator:
    ...
    def generate_gdpr_report(
        ...
        pii_classifications = DataClassification.objects.filter(
            tenant_id=tenant_id,
            category__in=[
                ClassificationCategory.PII.value,
                ClassificationCategory.PHI.value
            ],
        ...
        approved_requests = access_requests.filter(status=AccessRequestStatus.APPROVED.value).count()
        rejected_requests = access_requests.filter(status=AccessRequestStatus.REJECTED.value).count()
        ...
        return {
            ...
            'pii_detection': {
                ...
                'high_risk_fields': pii_classifications.filter(
            ...
            'data_subject_rights': {
                ...
                'approval_rate': (approved_requests / access_requests_count * 100) if access_requests_count > 0 else 0
            },
    ...
    def generate_lgpd_report(
        ...
        return {
            ...
            'pii_detection': {
                'total_fields': pii_fields,
                'classified_fields': pii_fields
            },
            'data_subject_rights': {
                'access_requests': access_requests_count,
                'approved': approved_requests,
