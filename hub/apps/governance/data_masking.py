...
from .models import FieldAccessPolicy
...
class DataMasker:
    ...
    @staticmethod
    def _redact(value: Any, config: Optional[Dict[str, Any]] = None) -> str:
        """Redact value (replace with fixed string)"""
        redact_string = config.get('redact_string', '***REDACTED***') if config else '***REDACTED***'
        return redact_string
    
    @staticmethod
    def _hash(value: Any, config: Optional[Dict[str, Any]] = None) -> str:
        """Hash value"""
    ...
    @staticmethod
    def _partial(value: Any, config: Optional[Dict[str, Any]] = None) -> str:
        """Show partial value"""
        value_str = str(value)
    ...
    @staticmethod
    def _format_preserving(value: Any, config: Optional[Dict[str, Any]] = None) -> str:
        ...
        if re.match(phone_pattern, value_str.replace(' ', '').replace('-', '').replace('(', '').replace(')', '')):
