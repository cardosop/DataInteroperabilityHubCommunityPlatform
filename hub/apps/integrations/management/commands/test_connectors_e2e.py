...
class Command(BaseCommand):
    help = (
        "Manual end-to-end testing of marketplace connectors (CKAN, dados.gov.br, Snowflake, Azure)"
    )
    ...
    def handle(self, *args, **options):
        ...
        for source_name in sources_to_test:
            ...
            if skip_reason:
                ...
                self.stdout.write(
                    self.style.WARNING(f"  Skipping {source_name.upper()}: {skip_reason}")
                )
    ...
    def _check_credentials_available(self, source_name: str) -> str | None:
    ...
    def _get_tenant(self, tenant_spec: str | None) -> Tenant:
        ...
        if tenant_spec:
            ...
            if not tenant:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"   Created tenant: {tenant.name} ({tenant.id})")
                )
            return tenant

    def _get_user(self, user_spec: str | None, tenant: Tenant) -> User:
    ...
    def _test_source(
        ...
        skip_semantic: bool,
        data_strategy: str,
        download_resources: list[str] | None,
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if wait:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"   ✓ Sync job completed: {sync_job.status}\n")
                )
                ...
                if verify_assets:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f"   ✓ Verified {len(assets)} assets created\n")
                    )
            else:
        ...
        if source_name == "ckan":
            ...
            connector = DadosGovBrConnector(
                ...
                swagger_spec_url=getattr(
                    get_marketplace_instance_config("dados.gov.br"), "swagger_spec_url", None
                ),
            )
        elif source_name == "snowflake":
    ...
    def _sync_and_wait(
        ...
        if sync_job.status == SyncStatus.COMPLETED.value:
            ...
            self.stdout.write(
                self.style.WARNING(f"   ⚠️  Workflow still running (status: {sync_job.status})")
            )
    ...
    def _verify_assets(self, sync_job: MarketplaceSyncJob, tenant: Tenant) -> list[Asset]:
            ...
            if workflow_instance_id:
                try:
                    ...
                    if workflow_instance.state_data:
                        ...
                        if asset_ids:
                            assets = Asset.objects.filter(id__in=asset_ids, tenant=tenant).order_by(
                                "created_at"
                            )
        ...
        if assets.count() == 0:
            mappings = MarketplaceMapping.objects.filter(
                connection=sync_job.connection, tenant=tenant
            )
            if mappings.exists():
        ...
        if verified_count == assets.count():
            self.stdout.write(
                self.style.SUCCESS(f"   ✓ All {verified_count} asset(s) verified successfully")
            )
        else:
            self.stdout.write(
                self.style.WARNING(
    ...
    def _display_overall_summary(self, results: dict[str, dict[str, Any]]):
        ...
        for source_name, result in results.items():
            if result.get("skipped"):
                ...
                self.stdout.write(
                    f"  - Connection tested: {result.get('connection_tested', False)}"
                )
                self.stdout.write(
                    f"  - Listings discovered: {result.get('listings_discovered', 0)}"
                )
                ...
                if result.get("assets_created", 0) > 0:
                    ...
                    self.stdout.write(
                        f"  - Assets verified: {result.get('assets_verified', False)}"
                    )
            else:
        ...
        if failed == 0 and passed > 0:
            self.stdout.write(
                self.style.SUCCESS(f"✓ All run tests PASSED ({passed} passed, {skipped} skipped)")
            )
        elif failed > 0:
            self.stdout.write(
                self.style.ERROR(f"✗ {failed} test(s) FAILED ({passed} passed, {skipped} skipped)")
            )
        else:
            self.stdout.write(self.style.WARNING("⊘ All sources skipped (no credentials set)"))
