...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"  Using workflow: marketplace_sync_pull")
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"✓ Created sync job: {sync_job.id}")
            )
            self.stdout.write(f"  Workflow instance: {sync_job.metadata.get('workflow_instance_id', 'N/A')}")
            ...
            if wait:
                ...
                self.stdout.write(f"  The workflow will process in the background")
                self.stdout.write(f"  Monitor progress via: sync job {sync_job.id}")
                self.stdout.write(f"  Or check workflow instance: {sync_job.metadata.get('workflow_instance_id', 'N/A')}")
