...
class TestAWSDataExchangeConnectorInitialization(TestCase):
    ...
    def test_circuit_breaker_initialized(self):
        ...
        self.assertEqual(
            connector._circuit_breaker.service_name,
...
class TestAWSDataExchangeConnectorAuthentication(TestCase):
    ...
    @patch('hub.apps.integrations.connectors.aws_data_exchange_connector.AWSDataExchangeConnector.test_connection')
    ...
    def test_authenticate_with_role_arn(self, mock_test_connection):
        ...
        self.assertTrue(result)
        self.assertEqual(
            self.connector._role_arn,
            'arn:aws:iam::123456789012:role/DataExchangeRole'
        )
    ...
    def test_authenticate_missing_credentials(self):
        ...
        self.assertTrue(
            'credentials dictionary is required' in error_msg or
            ('role_arn' in error_msg and 'aws_access_key_id' in error_msg),
...
class TestAWSDataExchangeConnectorConnectionTest(TestCase):
    ...
    def test_test_connection_other_client_error(self, mock_boto3):
        ...
        self.assertTrue(
            'Unable to connect' in error_msg or 'AWS' in error_msg or 'connection' in error_msg.lower(),
    ...
    def test_test_connection_boto_core_error(self, mock_boto3):
        ...
        self.assertTrue(
            'Unable to connect' in error_msg or
            'AWS Data Exchange' in error_msg or
...
class TestAWSDataExchangeConnectorDiscoveryOperations(TestCase):
    ...
    def test_list_revisions_with_pagination(self, mock_boto3):
        ...
        result = self.connector._list_revisions('dataset-123', max_results=1, next_token='token-123')
    ...
    @patch('hub.apps.integrations.connectors.aws_data_exchange_connector.AWSDataExchangeConnector._get_dataset_details')
    @patch('hub.apps.integrations.connectors.aws_data_exchange_connector.boto3')
    def test_list_listings_success(self, mock_boto3, mock_get_details):
        ...
        mock_get_details.side_effect = [
            {'Id': 'dataset-1', 'Name': 'Dataset 1', 'Description': 'Desc 1', 'AssetType': 'S3_SNAPSHOT', 'CreatedAt': '2023-01-01T12:00:00Z', 'UpdatedAt': '2023-01-01T12:00:00Z'},
            {'Id': 'dataset-2', 'Name': 'Dataset 2', 'Description': 'Desc 2', 'AssetType': 'S3_SNAPSHOT', 'CreatedAt': '2023-01-01T12:00:00Z', 'UpdatedAt': '2023-01-01T12:00:00Z'}
        ]
    ...
    def test_list_listings_with_filters(self, mock_boto3, mock_get_details):
        ...
        mock_get_details.return_value = {
            'Id': 'dataset-1', 'Name': 'Dataset 1', 'Description': 'Desc 1',
            'AssetType': 'S3_SNAPSHOT', 'CreatedAt': '2023-01-01T12:00:00Z',
    ...
    @patch('hub.apps.integrations.connectors.aws_data_exchange_connector.AWSDataExchangeConnector._get_dataset_details')
    @patch('hub.apps.integrations.connectors.aws_data_exchange_connector.AWSDataExchangeConnector._list_revisions')
    @patch('hub.apps.integrations.connectors.aws_data_exchange_connector.boto3')
...
class TestAWSDataExchangeConnectorPullOperations(TestCase):
    ...
    @patch('hub.apps.integrations.connectors.aws_data_exchange_connector.AWSDataExchangeConnector.list_resources')
    ...
    @patch('hub.apps.integrations.connectors.aws_data_exchange_connector.AWSDataExchangeConnector.list_listings')
    @patch('hub.apps.integrations.connectors.aws_data_exchange_connector.AWSDataExchangeConnector.map_to_hub_asset')
    def test_sync_pull_success(self, mock_map_to_hub_asset, mock_list_listings):
        """Test sync_pull successfully syncs listings"""
        from hub.apps.integrations.base import SyncResult, SyncStatus, MarketplaceAssetMapping
        from hub.apps.assets.models import AssetSourceType
    ...
    @patch('hub.apps.integrations.connectors.aws_data_exchange_connector.AWSDataExchangeConnector.get_listing')
    @patch('hub.apps.integrations.connectors.aws_data_exchange_connector.AWSDataExchangeConnector.map_to_hub_asset')
    def test_sync_pull_with_listing_ids(self, mock_map_to_hub_asset, mock_get_listing):
        """Test sync_pull with specific listing IDs"""
        from hub.apps.integrations.base import SyncResult, MarketplaceAssetMapping
        from hub.apps.assets.models import AssetSourceType
    ...
    def test_subscribe_to_dataset_not_subscribed(self, mock_boto3):
        ...
        mock_client.list_data_set_revisions.side_effect = ClientError(error_response, 'list_data_set_revisions')
        mock_boto3.client.return_value = mock_client
    ...
    def test_create_export_job_missing_bucket(self, mock_boto3):
        ...
        with self.assertRaises(ValueError) as cm:
            self.connector._create_export_job(
                dataset_id='dataset-123',
                revision_id='rev-1'
            )
    ...
    def test_wait_for_job_completion_timeout(self, mock_boto3):
        ...
        with mock_patch('time.sleep'), mock_patch('time.time', side_effect=[0, 3700]):  # Exceed timeout
    ...
    @patch('hub.apps.integrations.connectors.aws_data_exchange_connector.AWSDataExchangeConnector._get_s3_client')
    ...
    @patch('hub.apps.integrations.connectors.aws_data_exchange_connector.AWSDataExchangeConnector._download_exported_assets')
    @patch('hub.apps.integrations.connectors.aws_data_exchange_connector.AWSDataExchangeConnector._wait_for_job_completion')
    @patch('hub.apps.integrations.connectors.aws_data_exchange_connector.AWSDataExchangeConnector._start_job')
    @patch('hub.apps.integrations.connectors.aws_data_exchange_connector.AWSDataExchangeConnector._create_export_job')
    @patch('hub.apps.integrations.connectors.aws_data_exchange_connector.AWSDataExchangeConnector._get_latest_revision')
    @patch('hub.apps.integrations.connectors.aws_data_exchange_connector.AWSDataExchangeConnector._subscribe_to_dataset')
    @patch('django.conf.settings')
    def test_download_resource_success(self, mock_settings, mock_subscribe, mock_get_revision,
                                       mock_create_job, mock_start_job, mock_wait, mock_download):
    ...
    def test_download_resource_subscription_warning(self, mock_subscribe):
        ...
        with mock_patch.object(self.connector, '_get_latest_revision', return_value='rev-1'), \
             mock_patch.object(self.connector, '_create_export_job', return_value='job-123'), \
             mock_patch.object(self.connector, '_start_job'), \
             mock_patch.object(self.connector, '_wait_for_job_completion', return_value={'State': 'COMPLETED'}), \
             mock_patch.object(self.connector, '_download_exported_assets', return_value='/tmp/test.csv'), \
...
class TestAWSDataExchangeConnectorRetryLogic(TestCase):
    ...
    @patch('time.sleep')
    @patch('hub.apps.integrations.connectors.aws_data_exchange_connector.AWSDataExchangeConnector._get_dataexchange_client')
...
class TestAWSDataExchangeConnectorCircuitBreaker(TestCase):
    ...
    def test_circuit_breaker_configuration(self):
        ...
        self.assertEqual(self.connector._circuit_breaker.service_name, 'aws-data-exchange-connector')
...
class TestAWSDataExchangeConnectorEdgeCases(TestCase):
    ...
    def test_list_listings_missing_fields(self, mock_get_client):
        ...
        with patch.object(self.connector, '_get_dataset_details', side_effect=get_dataset_details_side_effect):
            listings = self.connector.list_listings()
    ...
    def test_list_listings_pagination_edge_cases(self, mock_get_client):
        ...
        mock_client.list_data_sets.side_effect = [
            {
                'DataSets': [
                    {'Id': f'dataset-{i}', 'Name': f'Dataset {i}'}
                    for i in range(5)
                ],
                'NextToken': 'token-1'
            },
