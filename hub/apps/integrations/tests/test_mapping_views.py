...
class MarketplaceMappingViewSetTest(TestCase):
    ...
    def test_list_mappings_pagination(self):
        ...
        # Request first page
        response = self.client.get(
            '/api/v1/integrations/marketplace/mappings/?page=1&page_size=3'
        )
    ...
    def test_delete_mapping_success(self):
        ...
        self.assertFalse(
            MarketplaceMapping.objects.filter(id=self.mapping.id).exists()
        )
...
class MarketplaceMappingViewSetSecurityTest(TestCase):
    ...
    def test_tenant_isolation_delete(self):
        ...
        self.assertTrue(
            MarketplaceMapping.objects.filter(id=self.mapping2.id).exists()
        )
    ...
    def test_mapping_deletion_audit_log(self):
        ...
        # Verify mapping is deleted
        self.assertFalse(
            MarketplaceMapping.objects.filter(id=self.mapping1.id).exists()
        )
