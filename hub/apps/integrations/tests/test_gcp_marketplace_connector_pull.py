...
class TestGCPMarketplaceConnectorMapToHubAsset(TestCase):
    ...
    def test_map_to_hub_asset_with_missing_title(self, mock_list_resources):
        """Test map_to_hub_asset() error handling with missing title"""
        listing = MarketplaceListing(
            marketplace_id="test-listing-id",
            marketplace_type=MarketplaceType.GOOGLE_CLOUD_MARKETPLACE,
            title="",  # Empty title
        ...
        # Should handle missing title gracefully (may use marketplace_id as fallback)
        mapping = self.connector.map_to_hub_asset(listing)
        self.assertIsInstance(mapping, MarketplaceAssetMapping)
        self.assertIsNotNone(mapping.asset_data["name"])
    ...
    @patch(
        "hub.apps.integrations.connectors.gcp_marketplace_connector.GCPMarketplaceConnector.map_to_hub_asset"
    )
    @patch(
        "hub.apps.integrations.connectors.gcp_marketplace_connector.GCPMarketplaceConnector.list_resources"
    )
    def test_sync_pull_with_empty_listing_ids(
        self, mock_list_resources, mock_map_to_hub_asset, mock_get_listing
    ):
    ...
    def test_sync_pull_with_none_listing_ids(
        self, mock_list_resources, mock_map_to_hub_asset, mock_get_listing
    ):
    ...
    def test_sync_pull_with_zero_limit(
        self, mock_list_resources, mock_map_to_hub_asset, mock_get_listing
    ):
    ...
    def test_sync_pull_with_none_options(
        self, mock_list_resources, mock_map_to_hub_asset, mock_get_listing
    ):
...
class TestGCPMarketplaceConnectorSyncPull(TestCase):
    ...
    @patch(
        "hub.apps.integrations.connectors.gcp_marketplace_connector.GCPMarketplaceConnector.list_listings"
    )
    @patch(
        "hub.apps.integrations.connectors.gcp_marketplace_connector.GCPMarketplaceConnector.map_to_hub_asset"
    )
    ...
    def test_sync_pull_with_filters(
        self, mock_list_resources, mock_map_to_hub_asset, mock_list_listings
    ):
    ...
    def test_sync_pull_dry_run(self, mock_map_to_hub_asset, mock_get_listing):
        ...
        result = self.connector.sync_pull(
            listing_ids=["test-listing-id"], options={"dry_run": True}
        )
...
class TestGCPMarketplaceConnectorExtractSchema(TestCase):
    ...
    @patch(
        "hub.apps.integrations.connectors.gcp_marketplace_connector.GCPMarketplaceConnector._get_bigquery_client"
    )
    ...
    def test_extract_schema_not_found(self, mock_get_client):
        ...
        with self.assertRaises(NotFoundError):
            self.connector._extract_schema_from_bigquery_dataset(
                "test_dataset", "non_existent_table"
            )
...
class TestGCPMarketplaceConnectorSubscribeToListing(TestCase):
    ...
    @patch(
        "hub.apps.integrations.connectors.gcp_marketplace_connector.ANALYTICSHUB_AVAILABLE", True
    )
    @patch(
        "hub.apps.integrations.connectors.gcp_marketplace_connector.GCPMarketplaceConnector._get_analyticshub_client"
    )
    ...
    def test_subscribe_to_listing_already_subscribed(self, mock_get_listing, mock_get_client):
        ...
        mock_listing = MarketplaceListing(
            ...
            metadata={
                "analytics_hub_listing": {
                    "bigquery_dataset": {"dataset": "test-project.test_dataset"}
                }
            },
        )
