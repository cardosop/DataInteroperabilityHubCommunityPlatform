...
class MarketplaceMetricsTest(TestCase):
    ...
    def test_connection_status_metrics_recorded_on_update(self):
        ...
        service.update_connection(connection_id=str(connection.id), is_active=False)
    ...
    def test_connection_metrics_recorded_on_delete(self):
        ...
        # Delete connection
        service.delete_connection(connection_id=str(connection.id))
        ...
        if response.status_code == 200:
            ...
            for line in lines:
                if "marketplace_connections_total" in line and not line.startswith("#"):
                    ...
                    self.assertTrue(
                        "{" in line or " " in line,
                        f"Metric line should have labels or value: {line}",
                    )
                    break
            ...
            if marketplace_metric_found:
                self.assertTrue(
                    marketplace_metric_found, "Marketplace metric found with valid format"
                )
