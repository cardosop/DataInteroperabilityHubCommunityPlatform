...
class MarketplaceDashboardsTest(TestCase):
    ...
    def test_marketplace_dashboards_exist(self):
        ...
        for dashboard_name in required_dashboards:
            ...
            self.assertTrue(dashboard_path.exists(), f"Dashboard {dashboard_name} should exist")
            ...
            with self.subTest(dashboard=dashboard_name):
                try:
                    ...
                    self.assertIn(
                        "dashboard",
                        dashboard,
                        f"Dashboard {dashboard_name} should have 'dashboard' key",
                    )
    ...
    def test_dashboards_have_required_structure(self):
        ...
        for dashboard_name in dashboard_files:
            ...
            with self.subTest(dashboard=dashboard_name):
                ...
                self.assertIn(
                    "title", dashboard_obj, f"Dashboard {dashboard_name} should have title"
                )
                self.assertIn(
                    "panels", dashboard_obj, f"Dashboard {dashboard_name} should have panels"
                )
                self.assertIn(
                    "schemaVersion",
                    dashboard_obj,
                    f"Dashboard {dashboard_name} should have schemaVersion",
                )
    ...
    def test_dashboards_have_panels(self):
        ...
        for dashboard_name in dashboard_files:
            ...
            with self.subTest(dashboard=dashboard_name):
                ...
                self.assertGreater(
                    len(panels), 0, f"Dashboard {dashboard_name} should have at least one panel"
                )
                ...
                if "templating" in dashboard["dashboard"]:
                    ...
                    if "list" in templating:
                        ...
                        self.assertGreater(
                            len(variables),
                            0,
                            f"Dashboard {dashboard_name} should have templating variables",
                        )
    ...
    def test_marketplace_alerts_valid_yaml(self):
        ...
        try:
            ...
            self.assertGreater(
                len(alerts["groups"]), 0, "Alerts file should have at least one group"
            )

            # Check that marketplace alerts group exists
            group_names = [g.get("name") for g in alerts["groups"]]
            self.assertIn(
                "marketplace_integration_alerts",
                group_names,
                "Should have marketplace_integration_alerts group",
            )
            ...
            self.assertGreater(
                len(marketplace_group["rules"]),
                0,
                "Marketplace alerts should have at least one rule",
            )
    ...
    def test_marketplace_alerts_have_required_rules(self):
        ...
        # Find marketplace alerts group
        marketplace_group = next(
            (g for g in alerts["groups"] if g.get("name") == "marketplace_integration_alerts"), None
        )
        ...
        if marketplace_group:
            ...
            for alert_name in required_alerts:
                self.assertIn(
                    alert_name, rule_names, f"Marketplace alerts should have {alert_name} rule"
                )
                ...
                for panel in dashboard_obj.get("panels", []):
                    ...
                    for target in panel.get("targets", []):
                        if "expr" in target:
                            ...
                            # marketplace-connections-overview uses generic
                            # connection metrics (not marketplace-specific);
                            # other dashboards MUST reference marketplace metrics.
                            if dashboard_name == "marketplace-connections-overview.json":
                                continue
                            self.assertTrue(
                                has_marketplace_metric,
                                "Panel should reference marketplace metrics",
                            )
