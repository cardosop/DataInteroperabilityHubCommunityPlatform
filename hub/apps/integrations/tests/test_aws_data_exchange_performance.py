...
class TestAWSDataExchangeConnectorPerformance(TestCase):
    ...
    def test_concurrent_authentication(self):
        ...
        connectors = [
            AWSDataExchangeConnector(
                aws_access_key_id=f'test-key-{i}',
                aws_secret_access_key=f'test-secret-{i}'
            )
            for i in range(5)
        ]
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = [executor.submit(authenticate_connector, connector) for connector in connectors]
            results = [future.result() for future in as_completed(futures)]
