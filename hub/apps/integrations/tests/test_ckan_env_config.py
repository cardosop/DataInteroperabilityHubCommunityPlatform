...
class TestCKANEnvironmentVariables(TestCase):
    ...
    def test_ckan_dados_gov_br_api_key_env_var_exists(self):
        ...
        try:
            ...
            self.assertEqual(
                env_value, test_key, "Should be able to set and read CKAN_DADOS_GOV_BR_API_KEY"
            )
        finally:
    ...
    def test_ckan_test_api_key_env_var_exists(self):
        ...
        try:
            ...
            self.assertEqual(
                env_value, test_key, "Should be able to set and read CKAN_TEST_API_KEY"
            )
        finally:
            if original_value is not None:
...
class TestCKANDadosGovBrIntegration(TestCase):
    ...
    def test_dados_gov_br_config_exists(self):
        ...
        self.assertIsNotNone(config, "dados.gov.br should be registered in MARKETPLACE_INSTANCES")
        ...
        self.assertEqual(
            config.connector_type, "swagger"
        )  # dados.gov.br uses Swagger API, NOT CKAN
        self.assertTrue(config.is_production, "dados.gov.br should be marked as production")
    ...
    def test_dados_gov_br_api_key_resolution(self):
        ...
        try:
            ...
            os.environ["CKAN_DADOS_GOV_BR_API_KEY"] = test_key
            api_key = config.get_api_key()
            self.assertEqual(api_key, test_key, "Should return API key from environment")
        finally:
    ...
    def test_dados_gov_br_in_fallback_list(self):
        ...
        self.assertIsNotNone(
            config, "dados.gov.br should be accessible via get_test_marketplace_config"
        )
        assert config is not None  # Type narrowing for linter
        self.assertEqual(config.base_url, "https://dados.gov.br")
...
class TestCKANTestHelpersIntegration(TestCase):
    ...
    def test_get_test_ckan_url_uses_fallback(self):
        ...
        try:
            ...
            self.assertIn(
                url, registered_urls, f"URL {url} should be one of the registered instance URLs"
            )
        finally:
            if original_url is not None:
                os.environ["CKAN_TEST_URL"] = original_url
    ...
    def test_all_ckan_env_vars_available(self):
        ...
        for var_name in required_vars:
            ...
            try:
                ...
                self.assertEqual(
                    env_value, test_value, f"{var_name} should be settable and readable"
                )
            finally:
                if original_value is not None:
...
class TestCKANEnvironmentConfigurationIntegration(TestCase):
    ...
    def test_full_configuration_flow(self):
        ...
        try:
            ...
            api_key = config.get_api_key()
            self.assertEqual(api_key, test_key, "API key should be resolved from environment")

            # 4. Test helpers can use it
            test_config = get_test_marketplace_config("dados.gov.br")
            self.assertIsNotNone(test_config, "Test helpers should be able to access config")
            assert test_config is not None  # Type narrowing for linter
            self.assertEqual(
                test_config.base_url, "https://dados.gov.br", "Test config should have correct URL"
            )
        finally:
            if original_primary is not None:
                os.environ["DADOS_GOV_BR_API_KEY"] = original_primary
