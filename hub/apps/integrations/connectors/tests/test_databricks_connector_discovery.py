    ...
    def test_list_listings_basic(self):
        ...
        with patch.object(self.connector, "_request_with_retry") as mock_request:
            ...
            with patch.object(self.connector, "_get_share_details") as mock_get_details:
                ...
                self.assertEqual(
                    listings[0].marketplace_type, MarketplaceType.DATABRICKS_MARKETPLACE
                )

    def test_list_listings_with_limit(self):
        """Test list_listings with limit parameter"""
        mock_shares_response = {
            "shares": [{"name": f"share_{i}", "comment": f"Share {i}"} for i in range(10)]
        }
    ...
    def test_list_listings_with_offset(self):
        ...
        with patch.object(self.connector, "_request_with_retry") as mock_request:
            ...
            with patch.object(self.connector, "_get_share_details") as mock_get_details:
                ...
                self.assertEqual(
                    len(listings), 5, "offset=5 with 10 total shares should return 5 listings"
                )
                # _get_share_details is called once per listing returned
                self.assertTrue(mock_get_details.called)
...
class TestDatabricksConnectorListResources(TestCase):
    ...
    def test_list_resources_success(self):
        ...
        mock_share_details = {
            "name": "test_share",
            "comment": "Test Share",
            "schemas": [{"name": "schema1", "catalog_name": "test_catalog"}],
        }
