...
class MarketplaceConnectionSerializer(serializers.ModelSerializer):
    ...
    tenant_name = serializers.CharField(
        source="tenant.name", read_only=True, help_text="Tenant name"
    )
    tenant = serializers.UUIDField(source="tenant.id", read_only=True, help_text="Tenant ID")
...
class MarketplaceConnectionCreateSerializer(serializers.Serializer):
    ...
    is_active = serializers.BooleanField(
        required=False, default=True, help_text="Whether this connection is active and can be used"
    )
...
class MarketplaceConnectionUpdateSerializer(serializers.Serializer):
    ...
    name = serializers.CharField(
        max_length=255, required=False, help_text="Human-readable name for this connection"
    )
    config = serializers.JSONField(
        required=False,
        write_only=True,
    ...
    is_active = serializers.BooleanField(
        required=False, help_text="Whether this connection is active and can be used"
    )
...
class MarketplaceConnectionTestResponseSerializer(serializers.Serializer):
    ...
    error = serializers.CharField(
        required=False, allow_null=True, allow_blank=True, help_text="Error message if test failed"
    )
    tested_at = serializers.DateTimeField(
        help_text="Timestamp when the test was performed (ISO format)"
    )
    connection_id = serializers.UUIDField(help_text="ID of the tested connection")
...
class MarketplaceSyncJobSerializer(serializers.ModelSerializer):
    ...
    connection_id = serializers.UUIDField(
        source="connection.id", read_only=True, help_text="Connection ID"
    )
    connection_name = serializers.CharField(
        source="connection.name", read_only=True, help_text="Connection name"
    )
    direction_display = serializers.CharField(
        source="get_direction_display", read_only=True, help_text="Human-readable sync direction"
    )
    status_display = serializers.CharField(
        source="get_status_display", read_only=True, help_text="Human-readable sync status"
    )
...
class MarketplaceMappingSerializer(serializers.ModelSerializer):
    ...
    hub_asset_id = serializers.UUIDField(
        source="hub_asset.id", read_only=True, help_text="Hub asset ID"
    )
    hub_asset_name = serializers.CharField(
        source="hub_asset.name", read_only=True, help_text="Hub asset name"
    )
    hub_asset_key = serializers.CharField(
        source="hub_asset.key", read_only=True, help_text="Hub asset key"
    )
