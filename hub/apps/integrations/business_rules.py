...
class MarketplaceIntegrationRuleExecutionContext(RuleExecutionContext):
    ...
    mapping: MarketplaceMapping | None = None
    asset: Any | None = None  # Using Any to avoid circular import
    marketplace_listing: Any | None = None  # Using Any to avoid circular import
        ...
        base_dict.update(
            {
                ...
                if self.asset and hasattr(self.asset, "id")
                else None,
                "marketplace_listing_id": (
                    self.marketplace_listing.marketplace_id
                    if self.marketplace_listing
                    and hasattr(self.marketplace_listing, "marketplace_id")
                    else None
                ),
            }
        )
...
class MarketplaceIntegrationBusinessRules(BusinessRules):
    ...
    def validate(
        ...
        if isinstance(context, MarketplaceIntegrationRuleExecutionContext):
            ...
            if context and hasattr(context, "metadata") and isinstance(context.metadata, dict):
                ...
                marketplace_listing = marketplace_listing or context.metadata.get(
                    "marketplace_listing"
                )
        ...
        if not any([connection, sync_job, mapping, asset, marketplace_listing]):
            return ValidationResult(
                is_valid=False,
                errors=[
                    "At least one resource (connection, sync_job, mapping, asset, or marketplace_listing) must be provided"
                ],
                details=details,
            )
    ...
    def _validate_connection(self, connection: MarketplaceConnection) -> ValidationResult:
        ...
        if not isinstance(connection.config, dict):
            ...
            try:
                ...
                warnings.append(f"Could not decrypt connection config: {e!s}")
    ...
    def _validate_sync_job(
        self, sync_job: MarketplaceSyncJob, connection: MarketplaceConnection | None = None
    ) -> ValidationResult:
        """
    ...
    def _validate_mapping(
        ...
        if not effective_asset:
            ...
            details["asset_id"] = (
                str(effective_asset.id) if hasattr(effective_asset, "id") else None
            )
    ...
    def _validate_marketplace_listing(self, marketplace_listing: Any) -> ValidationResult:
        ...
        if (
            not hasattr(marketplace_listing, "marketplace_id")
            or not marketplace_listing.marketplace_id
        ):
        ...
        # Validate marketplace_type
        if (
            not hasattr(marketplace_listing, "marketplace_type")
            or not marketplace_listing.marketplace_type
        ):
    ...
    def validate_sync_direction(
        self, sync_direction: SyncDirection, connection: MarketplaceConnection
    ) -> ValidationResult:
        """
        ...
        details: dict[str, Any] = {
            ...
            if isinstance(sync_direction, SyncDirection)
            else str(sync_direction),
            "connection_id": str(connection.id),
            "marketplace_type": connection.marketplace_type,
        }
        ...
        try:
            ...
            try:
                ...
                connector = factory.create_connector(
                    marketplace_type=marketplace_type, config=config
                )
                supported_directions = connector.supported_sync_directions
                details["connector_supported_directions"] = [
                    sd.value for sd in supported_directions
                ]
                ...
                errors.append(f"Failed to create connector to validate sync direction: {e!s}")
                details["connector_creation_error"] = str(e)
                details["direction_supported"] = None

        except ValueError as e:
            errors.append(f"Invalid marketplace type {connection.marketplace_type}: {e!s}")
            details["marketplace_type_valid"] = False
        except Exception as e:
            errors.append(f"Error validating sync direction: {e!s}")
    ...
    def validate_sync_eligibility(
        ...
        sync_job: MarketplaceSyncJob | None = None,
        exclude_sync_job_id: str | None = None,
    ) -> ValidationResult:
        """
        ...
        if not connection.is_active:
            errors.append(
                f"Connection {connection.id} is not active - cannot be used for sync operations"
            )
            details["connection_active"] = False
        else:
            details["connection_active"] = True
        ...
        try:
            ...
            if factory.is_supported(marketplace_type):
                try:
                    ...
                    try:
                        ...
                        errors.append(f"Connection {connection.id} test error: {test_error!s}")
                        details["connection_tested"] = False
                        details["connection_test_error"] = str(test_error)
                except Exception as connector_error:
                    errors.append(
                        f"Failed to create connector for connection {connection.id}: {connector_error!s}"
                    )
            ...
            errors.append(f"Failed to decrypt connection config or create connector: {e!s}")
    ...
    def validate_sync_resources(
        ...
        if sync_direction == SyncDirection.PUSH:
                ...
                for asset_id in asset_ids:
                    try:
                        ...
                        if hasattr(asset, "status"):
                            ...
                            if asset.status == AssetStatus.RETIRED:
                                inaccessible_assets.append(
                                    {"asset_id": asset_id, "reason": "Asset is retired (deleted)"}
                                )
                            else:
                        ...
                        inaccessible_assets.append(
                            {
                                "asset_id": asset_id,
                                "reason": f"Asset not found or not accessible: {e!s}",
                            }
                        )
            ...
            if not listing_ids:
                warnings.append(
                    "No listing IDs provided for PULL sync - will sync all available listings"
                )
                ...
                try:
                    ...
                    if factory.is_supported(marketplace_type):
                        ...
                        for listing_id in listing_ids:
                            try:
                                connector.get_listing(listing_id)
                                accessible_listings.append(listing_id)
                            except Exception as e:
                                inaccessible_listings.append(
                                    {
                                        "listing_id": listing_id,
                                        "reason": f"Listing not found or not accessible: {e!s}",
                                    }
                                )
                    ...
                    errors.append(f"Failed to validate marketplace listings: {e!s}")
            ...
            if asset_ids:
                ...
                for asset_id in asset_ids:
                    try:
                        ...
                        errors.append(f"Asset {asset_id} is not accessible: {e!s}")
                    ...
                    if factory.is_supported(marketplace_type):
                        ...
                        for listing_id in listing_ids:
                            try:
                                ...
                                errors.append(f"Listing {listing_id} is not accessible: {e!s}")
            ...
            total_items = (len(asset_ids) if asset_ids else 0) + (
                len(listing_ids) if listing_ids else 0
            )
            max_items = self._get_max_items_per_sync(connection.tenant)
    ...
    def validate_sync_mapping(
        self, mapping: MarketplaceMapping, connection: MarketplaceConnection | None = None
    ) -> ValidationResult:
        """
        ...
        if not mapping.hub_asset:
            ...
            try:
                ...
                asset = asset_service.get_asset(
                    str(mapping.hub_asset.id), tenant_id=str(mapping.tenant.id)
                )
                ...
                errors.append(f"Asset {mapping.hub_asset.id} is not accessible: {e!s}")
                details["asset_accessible"] = False
        ...
        if not mapping.external_listing_id or not mapping.external_listing_id.strip():
            ...
            try:
                ...
                if factory.is_supported(marketplace_type):
                    ...
                    try:
                        connector.get_listing(mapping.external_listing_id)
                        details["listing_exists"] = True
                        details["listing_accessible"] = True
                    except Exception as e:
                        errors.append(
                            f"External listing {mapping.external_listing_id} is not accessible: {e!s}"
                        )
                ...
                errors.append(f"Failed to validate external listing: {e!s}")
                details["listing_validation_error"] = str(e)
        ...
        if mapping.tenant and mapping.hub_asset:
            ...
            conflict_query = MarketplaceMapping.objects.filter(
                tenant=mapping.tenant, connection=effective_connection, hub_asset=mapping.hub_asset
            )
            if mapping.id:
            ...
            if schedule_type not in valid_types:
                ...
                # Validate interval if type is interval
                    ...
                    if not interval:
                        ...
                        errors.append(f"Invalid interval: {interval}. Must be a positive number")
                    else:
    ...
    def _get_max_items_per_sync(self, tenant: Any) -> int:
        ...
        if tenant and hasattr(tenant, "settings") and isinstance(tenant.settings, dict):
    ...
    def validate_dual_contract_creation(
        self, asset_mapping: "MarketplaceAssetMapping", tenant_id: str | None = None
    ) -> ValidationResult:
        """
        ...
        if not asset_mapping.odps_metadata or not isinstance(asset_mapping.odps_metadata, dict):
            ...
            if tenant_id:
                try:
                    ...
                    ContractService(tenant_id=tenant_id)
                    ...
                    try:
                        ...
                        errors.append(f"ODPS metadata is not valid JSON structure: {e!s}")
                        details["odps_parseable"] = False
                        details["odps_parse_error"] = str(e)

                except Exception as e:
                    warnings.append(f"Could not validate ODPS metadata structure: {e!s}")
        ...
        if asset_mapping.odcs_metadata is None:
            ...
            warnings.append("ODCS metadata not provided - will use defaults or extract from ODPS")
        else:
            ...
            if not isinstance(asset_mapping.odcs_metadata, dict):
                ...
                if tenant_id:
                    try:
                        ...
                        errors.append(f"ODCS metadata is not valid JSON structure: {e!s}")
        ...
        if asset_mapping.odps_metadata and asset_mapping.odcs_metadata:
            ...
            try:
                ...
                warnings.append(f"Could not validate contract linking compatibility: {e!s}")
            ...
            if asset_mapping.odcs_metadata is None:
                warnings.append("ODCS metadata not provided - contract linking will use defaults")
    ...
    def validate_resource_download(
        ...
        max_resource_size = self._get_max_resource_size(
            connection.tenant if connection.tenant else None, tenant_id
        )
        ...
        for idx, resource in enumerate(resources):
            ...
            if not resource.resource_id or not resource.resource_id.strip():
                errors.append(f"Resource {idx + 1} (name: {resource.name}) is missing resource_id")
                non_downloadable_resources.append(
                    {**resource_details, "reason": "Missing resource_id"}
                )
                continue

            if not resource.name or not resource.name.strip():
                warnings.append(f"Resource {idx + 1} (id: {resource.resource_id}) is missing name")
            ...
            if resource.format:
                ...
                warnings.append(f"Resource {resource.resource_id} does not specify format")
            ...
            try:
                ...
                if factory.is_supported(marketplace_type):
                    config = connection.get_config()
                    factory.create_connector(marketplace_type=marketplace_type, config=config)
                ...
                errors.append(
                    f"Failed to validate resource {resource.resource_id} downloadability: {e!s}"
                )
                non_downloadable_resources.append(
                    {**resource_details, "reason": f"Validation error: {e!s}", "error": str(e)}
                )
    ...
    def _get_max_resource_size(self, tenant: Any, tenant_id: str | None = None) -> int:
        """
    ...
    def validate_connection_config(
        ...
        try:
            ...
            errors.append(f"Error checking marketplace type support: {e!s}")
        ...
        if not isinstance(config, dict):
            ...
            try:
                ...
                if marketplace_type_enum is not None:
                    validate_marketplace_config(config, marketplace_type=marketplace_type_enum)
        ...
        for field in credential_fields:
            if field in config:
                if not isinstance(config[field], str) or not config[field].strip():
                    errors.append(f"Authentication credential '{field}' must be a non-empty string")
        ...
        if not connection_name or not connection_name.strip():
            ...
            try:
                ...
                query = MarketplaceConnection.objects.filter(
                    tenant_id=tenant_id, name=connection_name.strip()
                )
                ...
                errors.append(f"Error checking connection name uniqueness: {e!s}")
    ...
    def validate_connection_access(self, user_id: str, tenant_id: str) -> ValidationResult:
        """
            ...
            try:
                ...
                if user.is_platform_admin:
                    ...
                    if str(user.tenant_id) != tenant_id:
                        errors.append(f"User {user_id} does not belong to tenant {tenant_id}")
                        details["tenant_match"] = False
                ...
                errors.append(f"Error checking user permissions: {e!s}")
        ...
        if valid_tenant_id:
            try:
                ...
                try:
                    ...
                    details["tenant_kyc_status"] = (
                        tenant.kyc_status.value
                        if hasattr(tenant.kyc_status, "value")
                        else str(tenant.kyc_status)
                    )
                ...
                errors.append(f"Error checking tenant marketplace integration: {e!s}")
        ...
        try:
            ...
            errors.append(f"Error checking resource quotas: {e!s}")
    ...
    def validate_connection_test(
        self, connection: MarketplaceConnection, test_results: dict[str, Any] | None = None
    ) -> ValidationResult:
        """
        ...
        if not connection.is_active:
            errors.append(
                f"Connection {connection.id} is not active. Inactive connections cannot be tested."
            )
            details["is_active"] = False
        else:
            details["is_active"] = True
            ...
            try:
                ...
                try:
                    ...
                    validate_marketplace_config(
                        decrypted_config, marketplace_type=marketplace_type_enum
                    )
                    details["config_structure_valid"] = True
                except Exception as e:
                    # Try to get message attribute if it exists (for MarketplaceError)
                    error_msg = getattr(e, "message", str(e))
                    warnings.append(f"Connection config structure validation warning: {error_msg}")
                    details["config_structure_valid"] = False
                    details["config_structure_error"] = error_msg
            except Exception as e:
                errors.append(f"Connection config cannot be decrypted or is invalid: {e!s}")
                details["config_decryptable"] = False
        ...
        if test_results is not None:
            ...
            if not isinstance(test_results, dict):
                ...
                if "success" in test_results:
                    ...
                    if not test_success:
                        error_message = test_results.get(
                            "error", "Test failed without error message"
                        )
                        errors.append(f"Connection test failed: {error_message}")
