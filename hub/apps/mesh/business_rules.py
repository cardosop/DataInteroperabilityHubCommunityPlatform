...
class DataMeshRuleExecutionContext(RuleExecutionContext):
    ...
    domain: DataMeshDomain | None = None
...
class DataMeshBusinessRules(BusinessRules):
    ...
    def validate(
        ...
        if isinstance(context, DataMeshRuleExecutionContext):
            ...
            if not domain:
                if (
                    ...
                    and isinstance(context.resource, DataMeshDomain)
                ):
        ...
        if validation_type in ("ownership", "all"):
            ...
            if new_owner_id is not None or (
                context
                and hasattr(context, "metadata")
                and isinstance(context.metadata, dict)
                and context.metadata.get("new_owner_id") is not None
            ):
                # Ownership transfer validation
                if (
                ...
                ownership_result = self.validate_ownership_transfer(
                    domain=domain, new_owner_id=new_owner_id, raise_on_error=False
                )
            else:
    ...
    def validate_domain_structure(
        self, domain: DataMeshDomain, raise_on_error: bool = False
    ) -> ValidationResult:
        """
        ...
        if hasattr(domain, "owner_id") and domain.owner_id is not None:
            try:
                ...
                errors.append("Domain owner not found or does not belong to tenant")
    ...
    def validate_ownership_transfer(
        self, domain: DataMeshDomain, new_owner_id: str | None, raise_on_error: bool = False
    ) -> ValidationResult:
        """
        ...
        if new_owner_id is None:
            return ValidationResult(is_valid=True, errors=[], warnings=warnings, details=details)
        ...
        if domain.status == DomainStatus.ARCHIVED:
            warnings.append("Transferring ownership of an archived domain may have limited effect")
    ...
    def validate_boundaries(
        self, boundaries: dict[str, Any] | None, raise_on_error: bool = False
    ) -> ValidationResult:
        """
        ...
        if domain.boundaries and isinstance(domain.boundaries, dict):
            ...
            if tenant_id:
                ...
                other_domains = DataMeshDomain.objects.filter(
                    tenant_id=tenant_id, status__in=[DomainStatus.ACTIVE, DomainStatus.INACTIVE]
                ).exclude(id=domain.id)

                # Check for overlaps in data_products
                if "data_products" in domain.boundaries and isinstance(
                    domain.boundaries["data_products"], list
                ):
                    ...
                    for other_domain in other_domains:
                        if other_domain.boundaries and isinstance(other_domain.boundaries, dict):
                            if "data_products" in other_domain.boundaries and isinstance(
                                other_domain.boundaries["data_products"], list
                            ):
                ...
                if "schemas" in domain.boundaries and isinstance(
                    domain.boundaries["schemas"], list
                ):
                    domain_schemas = set(domain.boundaries["schemas"])
                    for other_domain in other_domains:
                        if other_domain.boundaries and isinstance(other_domain.boundaries, dict):
                            if "schemas" in other_domain.boundaries and isinstance(
                                other_domain.boundaries["schemas"], list
                            ):
                ...
                if "access_patterns" in domain.boundaries and isinstance(
                    domain.boundaries["access_patterns"], list
                ):
                    domain_access_patterns = set(domain.boundaries["access_patterns"])
                    for other_domain in other_domains:
                        if other_domain.boundaries and isinstance(other_domain.boundaries, dict):
                            if "access_patterns" in other_domain.boundaries and isinstance(
                                other_domain.boundaries["access_patterns"], list
                            ):
                                other_access_patterns = set(
                                    other_domain.boundaries["access_patterns"]
                                )
    ...
    def validate_asset_domain_boundary(
        self, asset: Asset, domain: DataMeshDomain, raise_on_error: bool = False
    ) -> ValidationResult:
        """
        ...
        if domain.boundaries and isinstance(domain.boundaries, dict):
            if "data_products" in domain.boundaries and isinstance(
                ...
                asset_in_boundary = (
                    asset.key in data_products
                    or asset.name in data_products
                    or str(asset.id) in data_products
                )
    ...
    def validate_cross_domain_access(
        ...
        if resource_id:
            ...
            try:
                ...
                warnings.append(
                    f"Failed to evaluate ABAC policies: {e!s}. Proceeding with basic validation."
                )
    ...
    def validate_domain_resource_quota(
        ...
        if domain.resource_quota:
            ...
            for key, value in domain.resource_quota.items():
                if not isinstance(value, (int, float)):
                    errors.append(
                        f"Resource quota '{key}' must be a number, got {type(value).__name__}"
                    )
                elif value < 0:
        ...
        if domain.resource_usage:
            ...
            for key, value in domain.resource_usage.items():
                if not isinstance(value, (int, float)):
                    errors.append(
                        f"Resource usage '{key}' must be a number, got {type(value).__name__}"
                    )
                elif value < 0:
        ...
        if domain.resource_quota and domain.tenant:
            try:
                ...
                governance_service = GovernanceService(
                    tenant_id=str(domain.tenant.id), user_id=self.user_id
                )
                ...
                try:
                    ...
                    governance_service.check_tenant_resource_limits(
                        tenant_id=str(domain.tenant.id), requested_quota=validated_quota
                    )
                    ...
                    if "storage" in error_message.lower() or "storage_gb" in error_message.lower():
                        ...
                        "compute" in error_message.lower()
                        or "compute_hours" in error_message.lower()
                    ):
                ...
                warnings.append(
                    f"Could not validate domain resource quota via GovernanceService: {e!s}. "
    ...
    def validate_asset_ownership_transfer(
        self, domain: DataMeshDomain, asset: Asset, raise_on_error: bool = False
    ) -> ValidationResult:
        """
        ...
        try:
            ...
            details["transfer_checks"]["has_dependencies"] = (
                upstream_count > 0 or downstream_count > 0
            )
            ...
            logger.warning(
                f"Failed to check asset dependencies for asset {asset.id}: {e}", exc_info=True
            )
            warnings.append(
                f"Could not fully validate asset dependencies: {e!s}. "
    ...
    def validate_policy_conflicts(
        self, domain: DataMeshDomain, new_policy: "AccessPolicy", raise_on_error: bool = False
    ) -> ValidationResult:
        """
        ...
        domain_applied_policy_ids = PolicyApplication.objects.filter(domain=domain).values_list(
            "policy_id", flat=True
        )
                ...
                existing_policies.append(
                    {
                        "policy": app.policy,
                        "application": app,
                        "effective_effect": app.overrides.get("effect", app.policy.effect)
                        if isinstance(app.overrides, dict)
                        else app.policy.effect,
                        "effective_priority": app.overrides.get("priority", app.policy.priority)
                        if isinstance(app.overrides, dict)
                        else app.policy.priority,
    ...
    def validate_policy_compatibility(
        ...
        result = ValidationResult(
            ...
            details={**conflict_result.details, "compatible": is_compatible},
        )
    ...
    def _resolve_policy_precedence(
        self, existing_priority: int, new_priority: int, existing_source: str
    ) -> dict[str, Any]:
        """
    ...
    def _check_condition_overlap_detailed(
        ...
        if not isinstance(conditions1, dict) or not isinstance(conditions2, dict):
            return {"overlaps": False, "overlapping_keys": [], "details": {}}
        ...
        return {
            ...
            "details": {"total_overlaps": len(overlapping_keys)},
        }
...
class PolicyBusinessRules(BusinessRules):
    ...
    def validate_policy_application(
        ...
        if not isinstance(overrides_dict, dict):
            ...
            for key in overrides_dict.keys():
                if key not in valid_override_keys:
                    warnings.append(
                        f"Unknown override key '{key}' (allowed: {', '.join(valid_override_keys)})"
                    )
        ...
        existing_app = PolicyApplication.objects.filter(
            domain=domain, policy=policy, status=PolicyApplicationStatus.APPLIED
        ).first()
        if existing_app:
            warnings.append(
    ...
    def check_compliance(
        ...
        if asset:
            ...
            domain_assets = AssetModel.objects.filter(
                tenant_id=domain.tenant_id, domain=domain.name
            )
        ...
        return {"compliance_status": compliance_status, "violations": violations}
...
class TopologyBusinessRules(BusinessRules):
    ...
    def validate(
        ...
        errors: list[str] = []
        warnings: list[str] = []
        details: dict[str, Any] = {"validation_type": "topology"}
            ...
            for domain2 in domains[i + 1 :]:
                ...
                domain1_policies = set(
                    PolicyApplication.objects.filter(
                        domain=domain1, status=PolicyApplicationStatus.APPLIED
                    ).values_list("policy_id", flat=True)
                )
                domain2_policies = set(
                    PolicyApplication.objects.filter(
                        domain=domain2, status=PolicyApplicationStatus.APPLIED
                    ).values_list("policy_id", flat=True)
                )
