...
class WebhookDeliveryValidator:
    ...
    def validate_delivery_retry(
        delivery: WebhookDelivery, webhook: Webhook | None = None
    ) -> ValidationResult:
        """
        ...
        if webhook is None:
            try:
                ...
                errors.append(f"Failed to fetch webhook for delivery: {e!s}")
        ...
        if webhook.max_retries < 0:
            errors.append(f"Webhook max_retries must be non-negative, got: {webhook.max_retries}")
        ...
        if delivery.status == DeliveryStatus.FAILED:
            ...
            if delivery.attempt_number < webhook.max_retries:
                ...
                elif delivery.next_retry_at <= timezone.now():
                    warnings.append(
                ...
                if delivery.next_retry_at is not None:
                    warnings.append(
                        "Delivery with max retries exceeded should not have next_retry_at set"
                    )
    ...
    def validate_delivery_timeout(
        ...
        if timeout_seconds <= 0:
            errors.append(f"Webhook delivery timeout must be positive, got: {timeout_seconds}")
            ...
            if "timeout" in error_lower or "timed out" in error_lower:
                ...
                if delivery.status == DeliveryStatus.FAILED:
                    if delivery.attempt_number < webhook.max_retries:
                        ...
                        details["timeout_retry_scheduled"] = (
                            "true"  # Max retries exceeded, no retry needed
                        )
                else:
    ...
    @staticmethod
    def validate_delivery_status(delivery: WebhookDelivery) -> ValidationResult:
        """
        ...
        if delivery.status not in valid_statuses:
            errors.append(
                f"Invalid delivery status: {delivery.status}. Valid statuses are: {valid_statuses}"
            )
        ...
        if delivery.status == DeliveryStatus.PENDING:
            ...
            if delivery.delivered_at is not None:
                errors.append(
                    "Delivery with status PENDING should not have delivered_at timestamp set"
                )
            ...
            if delivery.delivered_at is None:
                errors.append("Delivery with status SUCCESS must have delivered_at timestamp set")
            ...
            if delivery.next_retry_at is not None:
                warnings.append("Delivery with status SUCCESS should not have next_retry_at set")
                details["success_constraints_valid"] = details.get(
                    "success_constraints_valid", True
                )

            # SUCCESS deliveries should not have error_message
            if delivery.error_message:
                warnings.append("Delivery with status SUCCESS should not have error_message set")
                details["success_constraints_valid"] = details.get(
                    "success_constraints_valid", True
                )
            ...
            if not delivery.error_message:
                warnings.append(
                    "Delivery with status FAILED should have error_message set for debugging"
                )
            ...
            if delivery.delivered_at is not None:
                errors.append(
                    "Delivery with status FAILED should not have delivered_at timestamp set"
                )
            ...
            if delivery.next_retry_at is not None:
                warnings.append(
                    "Delivery with status DEAD_LETTER should not have next_retry_at set"
                )
            ...
            if not delivery.error_message:
                warnings.append(
                    "Delivery with status DEAD_LETTER should have error_message set for debugging"
                )
                details["dead_letter_constraints_valid"] = details.get(
                    "dead_letter_constraints_valid", True
                )

            # DEAD_LETTER deliveries should not have delivered_at set
            if delivery.delivered_at is not None:
                errors.append(
                    "Delivery with status DEAD_LETTER should not have delivered_at timestamp set"
                )
    ...
    def validate_dead_letter_queue(
        ...
        if delivery.status == DeliveryStatus.DEAD_LETTER:
            ...
            if delivery.next_retry_at is not None:
                errors.append("Delivery in DEAD_LETTER status should not have next_retry_at set")
            ...
            if not delivery.error_message:
                warnings.append(
                    "Delivery in DEAD_LETTER status should have error_message set for debugging"
                )
            ...
            if delivery.delivered_at is not None:
                errors.append(
                    "Delivery in DEAD_LETTER status should not have delivered_at timestamp set"
                )
    ...
    @staticmethod
    def validate_all(delivery: WebhookDelivery, webhook: Webhook | None = None) -> ValidationResult:
        """
        ...
        if webhook is None:
            try:
                ...
                return ValidationResult(
                    is_valid=False,
                    errors=[f"Failed to fetch webhook for delivery: {e!s}"],
                    details={"webhook_fetch_error": str(e)},
                )
        ...
        # Combine all results
        combined_result = (
            retry_result.combine(timeout_result).combine(status_result).combine(dlq_result)
        )
