...
class WebhookRuleExecutionContext(RuleExecutionContext):
    ...
    webhook_subscription: Webhook | None = None
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        base_dict.update(
            {
                "webhook_id": str(self.webhook_subscription.id)
                if self.webhook_subscription
                else None,
                "webhook_name": self.webhook_subscription.name
                if self.webhook_subscription
                else None,
                "webhook_status": self.webhook_subscription.status
                if self.webhook_subscription
                else None,
...
class WebhooksBusinessRules(BusinessRules):
    ...
    def validate(
        ...
        if isinstance(context, WebhookRuleExecutionContext):
            ...
            if context and hasattr(context, "metadata") and isinstance(context.metadata, dict):
                webhook_subscription = webhook_subscription or context.metadata.get(
    ...
    def _validate_webhook_subscription(
        self, webhook: Webhook, tenant: Any | None = None, user: User | None = None
    ) -> ValidationResult:
        """
            ...
            if not isinstance(webhook.retry_intervals, list):
                ...
                for interval in webhook.retry_intervals:
                    if not isinstance(interval, (int, float)) or interval < 0:
                        errors.append(
                            f"Webhook retry interval must be a non-negative number: {interval}"
                        )
    ...
    def _validate_subscription_url(self, webhook: Webhook) -> ValidationResult:
        """
        ...
        try:
            ...
            errors.append(f"Webhook subscription URL is invalid: {e!s}")
            details["url_format_valid"] = False
            ...
            with httpx.Client(timeout=5.0, follow_redirects=True) as client:
                try:
                    ...
                    if response.status_code >= 500:
                        ...
                        warnings.append(
                            "Webhook URL returned 404. Endpoint may not exist or may require authentication."
                        )
                    ...
                    warnings.append(f"Webhook URL accessibility check failed: {e!s}")
                    details["url_accessible"] = False
                    details["accessibility_error"] = str(e)
                except OSError as e:
                    warnings.append(f"Webhook URL accessibility check failed (OS error): {e!s}")
                    details["url_accessible"] = False
                    details["accessibility_error"] = str(e)
        except ImportError:
            warnings.append(
    ...
    def _validate_subscription_event_types(self, webhook: Webhook) -> ValidationResult:
        """
    ...
    def _validate_subscription_filters(self, webhook: Webhook) -> ValidationResult:
        """
        ...
        errors = []
        warnings = []
        details = {"filter_validation": "subscription_filters", "filters_valid": False}
    ...
    def _validate_subscription_security(self, webhook: Webhook) -> ValidationResult:
        """
        ...
        try:
            ...
            errors.append(f"Webhook signature generation failed: {e!s}")
            details["signature_generation_valid"] = False
    ...
    def _validate_delivery(
        self, delivery: WebhookDelivery, tenant: Any | None = None, user: User | None = None
    ) -> ValidationResult:
        """
        ...
        if not delivery.webhook_id:
            ...
            try:
                ...
                result.is_valid = False
                result.errors.append(f"Error accessing webhook delivery's webhook: {e!s}")
    ...
    def _validate_tenant_context(
        ...
        if delivery and delivery.webhook and delivery.webhook.tenant_id != tenant.id:
            result.is_valid = False
            result.errors.append(
                f"Delivery's webhook tenant_id ({delivery.webhook.tenant_id}) "
                f"does not match provided tenant ({tenant.id})"
    ...
    def _validate_payload(
        self, payload: dict[str, Any] | Any, delivery: WebhookDelivery | None = None
    ) -> ValidationResult:
        """
    ...
    def _validate_payload_size(self, payload: Any) -> ValidationResult:
        """
        ...
        try:
            ...
            result.is_valid = False
            result.errors.append(f"Failed to serialize payload for size validation: {e!s}")
            result.details["serialization_error"] = str(e)

        return result

    def _validate_payload_structure(self, payload: Any) -> ValidationResult:
        """
        ...
        if not isinstance(payload, dict):
            result.is_valid = False
            result.errors.append(f"Payload must be a dictionary, got {type(payload).__name__}")
        ...
        try:
            ...
            result.is_valid = False
            result.errors.append(f"Payload cannot be serialized to JSON: {e!s}")
            result.details["serialization_error"] = str(e)
            return result
    ...
    def _validate_payload_content(
        self, payload: dict[str, Any], delivery: WebhookDelivery | None = None
    ) -> ValidationResult:
        """
        ...
        if delivery and event_type and delivery.event_type != event_type:
            result.warnings.append(
