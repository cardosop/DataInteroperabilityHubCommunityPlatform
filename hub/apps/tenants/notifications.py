...
def notify_upgrade_confirmation(tenant_id, old_plan_slug: str = "",
                                new_plan_slug: str = "",
    ...
    logger.info(
        ...
        tenant_id, old_plan_slug, new_plan_slug, new_plan_tier,
    )


def notify_downgrade_confirmation(tenant_id, old_plan_slug: str = "",
                                  new_plan_slug: str = "") -> None:
    """Notify tenant admins of a successful plan downgrade."""
    logger.info(
        "notify_downgrade_confirmation: tenant=%s old=%s new=%s",
        tenant_id, old_plan_slug, new_plan_slug,
    )


def notify_price_change(tenant_id, plan_name: str = "",
                        old_price: int = 0, new_price: int = 0) -> None:
    """Notify tenant admins of an upcoming price change."""
    logger.info(
        "notify_price_change: tenant=%s plan=%s old=%d new=%d",
        tenant_id, plan_name, old_price, new_price,
    )
