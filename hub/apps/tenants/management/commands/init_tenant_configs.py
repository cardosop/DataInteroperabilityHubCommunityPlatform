    ...
    def handle(self, *args, **options):
        ...
        if tenant_id:
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(f'Invalid tenant ID: {e}')
                )
                return
        else:
        ...
        if dry_run:
            self.stdout.write(self.style.WARNING('DRY RUN - No TenantConfig records will be created.'))
        ...
        if error_count > 0:
            self.stdout.write(
                self.style.ERROR(f'  - Errors: {error_count} tenant(s)')
            )
        self.stdout.write(self.style.SUCCESS('=' * 60))
