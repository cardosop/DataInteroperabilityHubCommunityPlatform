    ...
    def test_unique_slugs(self):
        ...
        with pytest.raises(IntegrityError), transaction.atomic():
            TenantPlan.objects.create(
                slug=slug, name="B", tier=PlanTier.FREE, order=0, is_active=True
            )
...
class PlanSeedTests(TestCase):
    ...
    def test_create_nine_plans_with_profiles(self):
        ...
        for slug, name, tier, order, cat, price in plans_data:
            plan, _created = TenantPlan.objects.get_or_create(
                slug=slug,
                defaults={
                    "name": name,
                    "tier": tier,
            ...
            TierProfile.objects.get_or_create(
                plan=plan,
                defaults={"headline": f"{name} plan", "is_public": True},
            )
    ...
    def test_seed_idempotent(self):
        ...
        p1, c1 = TenantPlan.objects.get_or_create(
            slug=slug,
            defaults={"name": "Test", "tier": PlanTier.FREE, "order": 0, "is_active": True},
        )
...
class TierProfileTests(TestCase):
    ...
    def setUp(self):
        self.plan = TenantPlan.objects.create(
            slug=f"tp-plan-{uuid.uuid4().hex[:8]}",
            name="TP Plan",
            tier=PlanTier.PRO,
            order=1,
            is_active=True,
...
class PlanLimitsCoverageTests(TestCase):
    ...
    def test_all_categories_have_limits(self):
        ...
        self.assertTrue(
            base_plans.exists(),
            "Expected at least one active BASE plan to validate limits coverage",
        )
        for plan in base_plans:
            self.assertIsNotNone(
                plan.limits_json,
                f"Plan {plan.slug} must have non-null limits_json",
            )
    ...
    def test_limits_json_is_dict(self):
        plan = TenantPlan.objects.create(
            slug=f"limits-{uuid.uuid4().hex[:8]}",
            name="Limits",
            tier=PlanTier.PRO,
            order=1,
            is_active=True,
