...
from hub.apps.tenants.validators import PLATFORM_MAX_RATE_LIMITS, get_platform_defaults
...
class TenantConfigViewSetTest(TestCase):
    ...
    def setUp(self):
        ...
        self.auditor_role, _ = Role.objects.get_or_create(
            tenant=self.tenant1, name="AUDITOR", defaults={"description": "Auditor"}
        )
    ...
    def test_get_returns_platform_defaults_when_config_doesnt_exist(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["default_dq_profile"], self.platform_defaults["default_dq_profile"]
        )
        self.assertEqual(
            response.data["allowed_compliance_regimes"],
    ...
    def test_get_returns_tenant_specific_config_when_exists(self):
        ...
        TenantConfig.objects.create(
            tenant=self.tenant1, default_dq_profile="intake_basic_soda", data_retention_days=1825
        )
    ...
    def test_get_timestamps_iso_format(self):
        ...
        TenantConfig.objects.create(tenant=self.tenant1)
    ...
    def test_patch_update_single_field(self):
        ...
        response = self.client.patch(
            f"/api/v1/tenants/{self.tenant1.id}/config/", data, format="json"
        )
    ...
    def test_patch_partial_update_only_provided_fields(self):
        ...
        TenantConfig.objects.create(
            tenant=self.tenant1, default_dq_profile="intake_basic_gx", data_retention_days=2555
        )
    ...
    def test_patch_404_tenant_not_found(self):
        ...
        data = {"default_dq_profile": "intake_basic_soda"}
        response = self.client.patch(
            f"/api/v1/tenants/{fake_tenant_id}/config/", data, format="json"
        )
    ...
    def test_patch_with_empty_request_body(self):
        ...
        config = TenantConfig.objects.create(
            tenant=self.tenant1, default_dq_profile="intake_basic_gx"
        )

        self.client.force_authenticate(user=self.tenant1_admin)
        response = self.client.patch(
            f"/api/v1/tenants/{self.tenant1.id}/config/", {}, format="json"
        )
    ...
    def test_patch_rate_limits_empty_dict_clears_all(self):
        ...
        config = TenantConfig.objects.create(
            tenant=self.tenant1, rate_limits={"dq_runs": {"burst_per_10s": 20}}
        )
    ...
    def test_patch_with_null_values(self):
        ...
        data = {"default_dq_profile": None, "data_retention_days": None}
        response = self.client.patch(
            f"/api/v1/tenants/{self.tenant1.id}/config/", data, format="json"
        )
        ...
        self.assertEqual(
            response.data["data_retention_days"], self.platform_defaults["data_retention_days"]
        )
    ...
    def test_audit_events_are_queryable(self):
        ...
        data = {"default_dq_profile": "intake_basic_soda"}
        self.client.patch(f"/api/v1/tenants/{self.tenant1.id}/config/", data, format="json")

        # Query audit events
        events = AuditEvent.objects.filter(action="TENANT_CONFIG_UPDATED", tenant=self.tenant1)
        self.assertTrue(events.exists())

        # Verify we can filter by actor
        events_by_actor = AuditEvent.objects.filter(
            action="TENANT_CONFIG_UPDATED", actor_user=self.tenant1_admin
        )
