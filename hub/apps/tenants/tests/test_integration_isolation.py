...
class MultiTenantIsolationTest(TestCase):
    ...
    def setUp(self):
        ...
        self.tenant_a = Tenant.objects.create(
            ...
            slug=f"tenant-a-{_suffix}",
            kyc_status=KYCStatus.VERIFIED,
        )

        self.user_a = User.objects.create_user(
            email=f"user_a-{_suffix}@example.com",
        ...
        self.tenant_b = Tenant.objects.create(
            ...
            slug=f"tenant-b-{_suffix}",
            kyc_status=KYCStatus.VERIFIED,
        )

        self.user_b = User.objects.create_user(
            email=f"user_b-{_suffix}@example.com",
            password="testpass123",
    ...
    def test_tenant_cannot_update_other_tenant_asset(self):
        ...
        response = self.client_a.patch(
            f"/api/v1/assets/{self.asset_b.id}/", {"name": "Hacked Asset"}, format="json"
        )
    ...
    def test_tenant_cannot_create_contract_for_other_tenant_asset(self):
        ...
        self.assertIn(
            ...
            f"Tenant isolation should return 403/404, got {response.status_code}: {body}",
        )
