...
class SearchIndex(models.Model):
    ...
    title = models.CharField(
        max_length=255, null=True, blank=True, help_text="Title or name of the resource"
    )
    description = models.TextField(null=True, blank=True, help_text="Description of the resource")
    ...
    schema_text = models.TextField(
        null=True, blank=True, help_text="Flattened schema text for search"
    )
    ...
    lineage_text = models.TextField(
        null=True, blank=True, help_text="Flattened lineage text for search"
    )
    tags = models.JSONField(
        default=list, null=True, blank=True, help_text="Tags associated with the resource"
    )
    tags_text = models.TextField(null=True, blank=True, help_text="Flattened tags text for search")
    ...
    owner_id = models.UUIDField(null=True, blank=True, help_text="Owner user ID")
    owner_email = models.EmailField(null=True, blank=True, help_text="Owner email for search")
    ...
    quality_status = models.CharField(
        max_length=20, null=True, blank=True, help_text="Quality status (PASS, WARN, FAIL, UNKNOWN)"
    )
    ...
    search_vector = SearchVectorField(
        null=True, help_text="PostgreSQL tsvector for full-text search"
    )
    # Metadata
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["tenant", "resource_type", "resource_id"], name="unique_search_index"
            ),
        ]
...
class SearchAnalytics(models.Model):
    ...
    no_results = models.BooleanField(
        default=False, help_text="Whether the query returned no results"
    )
    # Click tracking
    clicked_result_id = models.UUIDField(
        null=True, blank=True, help_text="ID of the result that was clicked (if any)"
    )
    clicked_result_type = models.CharField(
        max_length=50, null=True, blank=True, help_text="Type of the result that was clicked"
    )
    clicked_at = models.DateTimeField(
        null=True, blank=True, help_text="When the result was clicked"
    )
    # Metadata
    session_id = models.CharField(
        max_length=255, null=True, blank=True, help_text="Session ID for tracking user sessions"
    )
