...
class SearchViewsEventPublishingE2ETest(TestCase):
    ...
    def test_e2e_search_endpoint_publishes_search_query_event(self):
        ...
        initial_count = Event.objects.filter(event_type="search.query", tenant_id=self.tenant.id).count()
        ...
        events = Event.objects.filter(event_type="search.query", tenant_id=self.tenant.id).order_by('-created_at')
    ...
    def test_e2e_search_endpoint_with_filters_publishes_event(self):
        ...
        event = Event.objects.filter(event_type="search.query", tenant_id=self.tenant.id).order_by('-created_at').first()
        self.assertIsNotNone(event)
    ...
    def test_e2e_rebuild_index_endpoint_publishes_index_rebuilt_event(self):
        ...
        # Get initial event count
        initial_count = Event.objects.filter(event_type="search.index.rebuilt", tenant_id=self.tenant.id).count()
        ...
        events = Event.objects.filter(event_type="search.index.rebuilt", tenant_id=self.tenant.id).order_by('-created_at')
        self.assertGreaterEqual(events.count(), initial_count + 1)
        ...
        self.assertGreaterEqual(event_data.get("resource_count"), 3)  # At least asset, dataset, contract
    ...
    def test_e2e_search_endpoint_cached_results_still_publishes_event(self):
        ...
        first_search_count = Event.objects.filter(event_type="search.query", tenant_id=self.tenant.id).count()
        ...
        second_search_count = Event.objects.filter(event_type="search.query", tenant_id=self.tenant.id).count()
    ...
    def test_e2e_search_endpoint_multiple_searches_publish_multiple_events(self):
        ...
        final_count = Event.objects.filter(event_type="search.query", tenant_id=self.tenant.id).count()
        self.assertGreaterEqual(final_count, initial_count + 3)

        # Verify each event has correct query
        events = Event.objects.filter(event_type="search.query", tenant_id=self.tenant.id).order_by('-created_at')[:3]
    ...
    def test_e2e_rebuild_index_endpoint_event_includes_rebuild_metadata(self):
        ...
        event = Event.objects.filter(event_type="search.index.rebuilt", tenant_id=self.tenant.id).order_by('-created_at').first()
        self.assertIsNotNone(event)
        ...
        self.assertEqual(response.data['resource_types'], event_data.get("resource_types"))
