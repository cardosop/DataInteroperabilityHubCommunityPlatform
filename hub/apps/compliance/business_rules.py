...
class ComplianceRuleExecutionContext(RuleExecutionContext):
    ...
    compliance_run: ComplianceRun | None = None
    assessment: dict[str, Any] | None = None
    resource: Any | None = None  # Using Any to avoid circular import
        ...
        base_dict.update(
            {
                ...
                "assessment_type": self.assessment.get("type")
                if self.assessment and isinstance(self.assessment, dict)
                else None,
...
class ComplianceBusinessRules(BusinessRules):
    ...
    def validate(
        ...
        if assessment and validation_type in ("risk_assessment", "all"):
            assessment_result = self._validate_risk_assessment(
                assessment, compliance_run, tenant, user
            )
        ...
        if not validated_items:
            return ValidationResult(
                is_valid=False,
                errors=[
                    "At least one of compliance_run, assessment, resource, tenant, or user must be provided"
                ],
    ...
    def _validate_compliance_run(
        self, compliance_run: ComplianceRun, tenant: Any | None = None, user: User | None = None
    ) -> ValidationResult:
        """
        ...
        if not compliance_run.asset and not compliance_run.dataset and not compliance_run.file:
            errors.append(
                "Compliance run must have at least one resource (asset, dataset, or file)"
            )
            ...
            if compliance_run.tenant_id:
                res = compliance_run.asset or compliance_run.dataset or compliance_run.file
        ...
        if compliance_run.regulations:
            if not isinstance(compliance_run.regulations, list):
                ...
                invalid_regulations = [
                    r for r in compliance_run.regulations if not isinstance(r, str) or not r.strip()
                ]
    ...
    def _validate_risk_assessment(
        ...
        if risk_level:
            valid_risk_levels = [choice[0] for choice in RiskLevel.choices]
            # NONE is not a meaningful assessment risk level — filter it
            # out so callers that mistakenly submit NONE receive a
            # validation error rather than a warning.
            valid_risk_levels = [rl for rl in valid_risk_levels if rl != "NONE"]
    ...
    def _validate_risk_calculation(self, risk_level: str, risk_score: float) -> ValidationResult:
        """
    ...
    def _validate_risk_mitigation(
        self, mitigation_actions: Any, risk_level: str | None = None
    ) -> ValidationResult:
        """
    ...
    def validate_compliance_run_execution(
        ...
        if validation_type in ("all", "eligibility"):
            eligibility_result = self._validate_compliance_run_eligibility(
                compliance_run, user, tenant
            )
    ...
    def _validate_compliance_run_eligibility(
        self, compliance_run: ComplianceRun, user: User | None = None, tenant: Any | None = None
    ) -> ValidationResult:
        """
        ...
        if user:
            ...
            warnings.append(
                "No user provided for eligibility validation - cannot verify permissions"
            )
        ...
        if compliance_run.dataset:
            ...
            if not compliance_run.dataset:
                ...
                if not compliance_run.dataset.file:
                    errors.append(
                        f"Dataset {resource_id} has no associated file for compliance run"
                    )
            ...
            if not compliance_run.asset:
                ...
                if hasattr(compliance_run.asset, "datasets"):
                    ...
                    if dataset_count == 0:
                        warnings.append(
                            f"Asset {resource_id} has no datasets - compliance run may fail"
                        )
            ...
            if not compliance_run.file:
                ...
                if not compliance_run.file.storage_path:
                    errors.append(
                        f"File {resource_id} has no storage path - cannot run compliance check"
                    )
    ...
    def _validate_compliance_run_resource_quota(
        self, compliance_run: ComplianceRun, tenant: Any | None = None
    ) -> ValidationResult:
        """
        ...
        has_quota, quota_info = QuotaManager.check_quota(
            tenant_id=tenant_id, category=EndpointCategory.COMPLIANCE_RUN, window=TimeWindow.DAILY
        )
        ...
        if hasattr(effective_tenant, "config") and effective_tenant.config:
            ...
            if hasattr(tenant_config, "compliance_max_concurrent_runs"):
                max_concurrent_runs = (
                    tenant_config.compliance_max_concurrent_runs or max_concurrent_runs
                )
        ...
        if new_status:
            ...
            if current_status == pending_val and new_status == running_val:
                ...
                if not compliance_run.completed_at:
                    ...
                    if (
                        compliance_run.started_at
                        and compliance_run.completed_at < compliance_run.started_at
                    ):
                        errors.append(
    ...
    def validate_scan_configuration(
        self, scan_config: dict[str, Any], tenant: Any | None = None, user: User | None = None
    ) -> ValidationResult:
        """
    ...
    def _validate_scan_type(
        self, scan_config: dict[str, Any], tenant: Any | None = None
    ) -> ValidationResult:
        """
        ...
        scan_types = (
            scan_config.get("scan_type")
            or scan_config.get("scan_types")
            or scan_config.get("applicable_regulations")
            or scan_config.get("regulations")
        )
        ...
        if scan_types:
            ...
            if tenant:
                try:
                    ...
                    warnings.append(
                        f"Could not validate tenant compliance regime restrictions: {e}"
                    )
        ...
        if schedule_config is not None:
            if not isinstance(schedule_config, dict):
                ...
                if schedule_frequency:
                    ...
                    missing_fields = [
                        f for f in required_schedule_fields if f not in schedule_config
                    ]
                    if missing_fields:
                        warnings.append(
    ...
    def _validate_scan_resource(
        ...
        if not asset_id and not dataset_id and not file_id:
            errors.append("At least one of asset_id, dataset_id, or file_id must be provided")
            details["has_resource"] = False
            return ValidationResult(
                is_valid=False, errors=errors, warnings=warnings, details=details
            )
            ...
            try:
                ...
                if tenant and asset.tenant_id:
                    if str(asset.tenant_id) != str(tenant.id):
                        errors.append(f"Asset {asset_id} does not belong to tenant {tenant.id}")
            ...
            try:
                ...
                if tenant and dataset.tenant_id:
                    if str(dataset.tenant_id) != str(tenant.id):
                        errors.append(f"Dataset {dataset_id} does not belong to tenant {tenant.id}")
            ...
            try:
                ...
                if tenant and file_obj.tenant_id:
                    if str(file_obj.tenant_id) != str(tenant.id):
                        errors.append(f"File {file_id} does not belong to tenant {tenant.id}")
        ...
        if tenant:
            ...
            if asset_id:
                ...
                if scan_config.get("exclude_run_id"):
                    conflicting_runs = conflicting_runs.exclude(
                        id=scan_config.get("exclude_run_id")
                    )
            ...
            if schedule_frequency:
                try:
                    ...
                    warnings.append(
                        "ComplianceReport model not available. Cannot check for conflicting scheduled reports."
                    )
                    details["has_conflicting_reports"] = None
