...
class FilesBusinessRules(BusinessRules):
    ...
    def validate(
        ...
        if tenant and validation_type in ("storage_quota", "all"):
            storage_quota_result = self.validate_storage_quota(
                file=file, tenant=tenant, file_size=file.size if file else None
            )
    ...
    def _validate_file(
        self, file: File, tenant: Any | None = None, user: User | None = None
    ) -> ValidationResult:
        """
    ...
    def _validate_user_permissions(
        self, file: File | None, tenant: Any | None, user: User
    ) -> ValidationResult:
        """
        ...
        if not file:
            ...
            if tenant and self.tenant_id and str(tenant.id) != self.tenant_id:
                warnings.append(
                    f"Provided tenant id ({tenant.id}) does not match "
                    f"instance tenant_id ({self.tenant_id})"
                )
    ...
    def _validate_file_access(
        self, file: File, user: User, access_type: str = "READ"
    ) -> ValidationResult:
        """
        ...
        if access_type == "WRITE":
            ...
            if (
                ...
                and not (has_role("TENANT_ADMIN") or has_role("DATA_PROVIDER"))
            ):
        ...
        details["abac_result"] = {
            ...
            "policy_id": str(abac_result.policy.id)
            if hasattr(abac_result, "policy") and abac_result.policy
            else None,
            "masking_required": abac_result.masking_required
            if hasattr(abac_result, "masking_required")
            else False,
        }
        ...
        if access_request_result.is_valid:
            ...
            if abac_result.masking_required:
                warnings.append(f"Access allowed but data masking is required for file '{file.id}'")
        elif is_same_tenant:
    ...
    def _validate_tenant_isolation(self, file: File, user: User) -> ValidationResult:
        """
    ...
    def _validate_abac_access(self, file: File, user: User, access_type: str) -> Any:
        """
    ...
    def _validate_access_request(
        self, file: File, user: User, access_type: str
    ) -> ValidationResult:
        """
    ...
    def validate_file_read_access(self, file: File, user: User | None = None) -> ValidationResult:
        """
    ...
    def validate_file_write_access(self, file: File, user: User | None = None) -> ValidationResult:
        """
    ...
    def _validate_active_filename_unique(self, tenant: Any, name: str) -> ValidationResult:
    ...
    def validate_file_for_destroy(
        ...
        if status_str in (FileStatus.DELETING.value, FileStatus.DELETED.value):
            errors.append("File is already deleted or pending purge (DELETING/DELETED).")
    ...
    def validate_upload(
        ...
        if validation_type in ("size", "all"):
            size_result = self._validate_file_size(
                file_size=file_size, upload_method=upload_method, tenant=tenant
            )
        ...
        if validation_type in ("type", "all"):
            type_result = self._validate_file_type(filename=filename, content_type=content_type)
    ...
    def _validate_file_size(
        self, file_size: int, upload_method: str = "browser", tenant: Any | None = None
    ) -> ValidationResult:
        """
        ...
        if tenant:
            ...
            try:
                ...
                warnings.append(f"Could not retrieve tenant file size limit: {e!s}")
        ...
        if tenant_limit:
            ...
            effective_limit = getattr(
                settings, "MAX_FILE_SIZE", 10 * 1024 * 1024 * 1024
            )  # 10GB default
        ...
        if file_size > 0 and file_size > max_size:
            errors.append(f"File size ({file_size} bytes) exceeds {limit_name} ({max_size} bytes)")
            details["size_valid"] = False
        ...
        if tenant:
            try:
                ...
                warnings.append(f"Could not validate tenant quota: {e!s}")
        ...
        if tenant_id:
            try:
                ...
                if details.get("size_valid", True):
                    warnings.append(f"Django validator check: {e!s}")
    ...
    def validate_storage_quota(
        self, file: File | None = None, tenant: Any | None = None, file_size: int | None = None
    ) -> ValidationResult:
        """
        ...
        if effective_file_size is not None:
            storage_quota_result = self._validate_storage_quota(
                file_size=effective_file_size, tenant=effective_tenant
            )
            result = result.combine(storage_quota_result)
        ...
        file_count_result = self._validate_file_count_quota(tenant=effective_tenant, file=file)
        ...
        quota_exceeded_result = self._validate_quota_exceeded_handling(
            tenant=effective_tenant, file_size=effective_file_size
        )
    ...
    def _validate_storage_quota(self, file_size: int, tenant: Any) -> ValidationResult:
        """
        ...
        try:
            ...
            current_usage_bytes = (
                File.objects.filter(tenant_id=tenant.id, status=FileStatus.ACTIVE).aggregate(
            ...
            requested_quota = {"storage_gb": requested_storage_gb}
            ...
            try:
                ...
                governance_service = GovernanceService(
                    tenant_id=str(tenant.id), user_id=self.user_id if self.user_id else None
                )
                ...
                validated_quota = governance_service.validate_resource_quota_allocation(
                    tenant_id=str(tenant.id), requested_quota=requested_quota
                )

                # Enforce tenant-level resource limits
                governance_service.check_tenant_resource_limits(
                    tenant_id=str(tenant.id), requested_quota=validated_quota
                )
                ...
                if "storage" in error_message.lower() or "storage_gb" in error_message.lower():
                    ...
                    warnings.append(f"GovernanceService validation failed: {error_message}")
                ...
                warnings.append(
                    f"Could not validate storage quota via GovernanceService: {e!s}. "
            ...
            errors.append(f"Could not check tenant storage quota: {e!s}")
            details["quota_check_error"] = str(e)
            details["storage_quota_valid"] = False
    ...
    def _validate_file_count_quota(self, tenant: Any, file: File | None = None) -> ValidationResult:
        """
        ...
        try:
            ...
            current_file_count = File.objects.filter(
                tenant_id=tenant.id, status=FileStatus.ACTIVE
            ).count()
            ...
            try:
                ...
                if hasattr(tenant, "config") and tenant.config:
                    if hasattr(tenant.config, "max_files_per_tenant"):
                        file_count_limit = (
                            tenant.config.max_files_per_tenant or default_file_count_limit
                        )
                    elif isinstance(tenant.config, dict):
                        file_count_limit = tenant.config.get(
                            "max_files_per_tenant", default_file_count_limit
                        )
            except Exception as e:
                logger.debug(f"Could not get file count limit from tenant config: {e}")

            details["file_count_limit"] = file_count_limit
            details["file_count_limit_source"] = (
                "tenant_config" if file_count_limit != default_file_count_limit else "default"
            )
            ...
            errors.append(f"Could not check tenant file count quota: {e!s}")
    ...
    def _validate_quota_exceeded_handling(
        self, tenant: Any, file_size: int | None = None
    ) -> ValidationResult:
        """
        Validate quota exceeded handling.
        ...
        try:
            ...
            warnings.append(f"Could not validate quota exceeded handling: {e!s}")
    ...
    def _validate_tenant_quota(self, file_size: int, tenant: Any) -> ValidationResult:
        """
        ...
        try:
            ...
            try:
                ...
                if not storage_result.is_valid and storage_result.errors:
                    ...
                    for error in storage_result.errors:
                        if (
                            "storage quota exceeded" in error.lower()
                            or "storage_gb" in error.lower()
                        ):
                            errors.append(error)
            ...
            warnings.append(f"Could not check tenant storage quota: {e!s}")
            details["quota_check_error"] = str(e)
    ...
    def _validate_file_type(
        ...
        allowed_types = [
            t.lower()
            for t in getattr(
                settings, "ALLOWED_FILE_TYPES", ["csv", "json", "parquet", "txt", "xlsx", "xls"]
            )
        ]
    ...
    def _validate_file_name(self, filename: str) -> ValidationResult:
        """
    ...
    def _validate_file_content(
        ...
        try:
            ...
            if not is_healthy:
                ...
                try:
                    scan_result = compliance_client.scan_file(
                        file_content=file_content, file_format=file_format, scan_mode="internal"
                    )
                    ...
                    warnings.append(f"Could not complete malware scan: {e!s}")
            ...
            warnings.append(f"Could not initialize malware scanning: {e!s}")
            details["malware_scan_skipped"] = True
