...
class FilesBusinessRulesInitializationTest(TestCase):
...
class FilesRuleExecutionContextTest(TestCase):
...
class FileSizeValidationTest(FileUploadValidationTest):
    ...
    def test_validate_tenant_quota_exceeds_limit(self):
        ...
        result = self.rules._validate_tenant_quota(
            file_size=int(quota_limit * 0.1),  # Try to add 10% more
            tenant=self.tenant
        )
...
class FilesBusinessRulesAccessValidationTest(TestCase):
    ...
    def test_validate_file_read_access_cross_tenant(self):
        ...
        self.assertTrue(result.details['tenant_isolation_valid'])  # Validation passes, but cross-tenant
...
class FilesBusinessRulesAccessValidationIntegrationTest(TestCase):
...
class FilesBusinessRulesStorageQuotaValidationTest(TestCase):
    ...
    def test_validate_storage_quota_within_limit(self):
        ...
        result = self.rules.validate_storage_quota(
            tenant=self.tenant,
            file_size=file_size
        )
    ...
    def test_validate_storage_quota_integrates_with_governance_service(self):
        ...
        file_size = 1024 * 1024 * 100  # 100 MB
        result = self.rules._validate_storage_quota(
            file_size=file_size,
            tenant=self.tenant
        )
    ...
    def test_validate_file_count_quota_within_limit(self):
        ...
        result = self.rules._validate_file_count_quota(
            tenant=self.tenant,
            file=None
        )
        self.assertTrue(result.is_valid, f"Validation failed with errors: {result.errors}")
    ...
    def test_validate_file_count_quota_calculates_current_count(self):
        ...
        # Should have correct current file count
        self.assertEqual(result.details['current_file_count'], 5)
        self.assertEqual(result.details['projected_file_count'], 5)  # No new file
    ...
    def test_validate_file_count_quota_increments_for_new_file(self):
        ...
        # Should increment projected count
        self.assertEqual(result.details['current_file_count'], 3)
        self.assertEqual(result.details['projected_file_count'], 4)  # 3 + 1 new file
