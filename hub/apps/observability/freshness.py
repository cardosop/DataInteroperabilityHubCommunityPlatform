...
class FreshnessMonitor:
    ...
    @staticmethod
    def get_sla_seconds(sla: str) -> int | None:
        """Get SLA threshold in seconds"""
        return FreshnessMonitor.SLA_SECONDS.get(sla)

    @staticmethod
    def calculate_freshness_age(last_update_time: timezone.datetime | None) -> int | None:
        """
    ...
    @staticmethod
    def is_stale(freshness_age_seconds: int | None, sla_seconds: int | None) -> bool:
        """
    ...
    def record_metric(
        ...
        last_update_time: timezone.datetime | None = None,
            ...
            results.append(
                {
                    ...
                    "last_update_time": metric.last_update_time.isoformat()
        ...
        return {
            ...
            "summary": {
                ...
                "stale_percentage": round(
                    (stale_count / total_metrics * 100) if total_metrics > 0 else 0, 2
                ),
                "avg_freshness_age_seconds": int(avg_freshness["avg_age"])
                if avg_freshness["avg_age"]
                else None,
    ...
    def detect_stale_data(
        cls, tenant_id: str, dataset_id: str | None = None, asset_id: str | None = None
    ) -> list[dict[str, Any]]:
        """
        ...
        """
        queryset = DataObservabilityMetric.objects.filter(tenant_id=tenant_id, is_stale=True)
        ...
        if dataset_id:
                ...
                stale_records.append(
                    {
                        ...
                        "last_update_time": latest.last_update_time.isoformat()
                        if latest.last_update_time
                        else None,
            ...
            # For datasets
            dataset_latest = (
                queryset.filter(dataset__isnull=False)
                .values("dataset_id")
            ...
            for item in dataset_latest:
                latest = queryset.filter(
                    dataset_id=item["dataset_id"], recorded_at=item["latest_recorded"]
                ).first()
                if latest:
                    stale_records.append(
                        {
            ...
            # For assets
            asset_latest = (
                queryset.filter(asset__isnull=False, dataset__isnull=True)
                .values("asset_id")
                .annotate(latest_recorded=Max("recorded_at"))
            )
            ...
            for item in asset_latest:
                latest = queryset.filter(
                    asset_id=item["asset_id"], recorded_at=item["latest_recorded"]
                ).first()
                if latest:
                    stale_records.append(
                        {
