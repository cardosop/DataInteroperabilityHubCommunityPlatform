...
from hub.apps.observability.models import PipelineExecution, DataSLA, DataIncident
...
class PipelineMonitoringE2ETest(TestCase):
    ...
    def test_pipeline_sync_from_jobs_workflow(self):
        ...
        # Sync from jobs
        count = PipelineMonitor.sync_from_jobs(
            tenant_id=str(self.tenant.id),
            limit=100
        )
...
class DataSLAsE2ETest(TestCase):
    ...
    def test_complete_sla_workflow(self):
        ...
        sla_in_results = next(
            (s for s in response.data['results'] if s['id'] == str(sla.id)),
            None
        )
...
class DataIncidentsE2ETest(TestCase):
    ...
    def test_complete_incident_lifecycle_workflow(self):
        ...
        response = self.client.post('/api/v1/observability/incidents/', {
