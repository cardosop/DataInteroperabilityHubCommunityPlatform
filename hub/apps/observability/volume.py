...
class VolumeMonitor:
    ...
    def aggregate_hourly_trends(
        ...
        ).values('hour', 'dataset_id', 'asset_id').annotate(
        ...
        for item in hourly_data:
            ...
            trend, created = VolumeTrend.objects.update_or_create(
                ...
                defaults={
                    ...
                    'avg_size_bytes': int(item['avg_size_bytes']) if item['avg_size_bytes'] else None,
    ...
    def aggregate_daily_trends(
        ...
        ).values('day', 'dataset_id', 'asset_id').annotate(
            avg_row_count=Avg('row_count'),
            min_row_count=Min('row_count'),
            max_row_count=Max('row_count'),
            avg_size_bytes=Avg('size_bytes'),
    ...
    def _detect_anomalies(
        ...
        historical = VolumeTrend.objects.filter(
            historical_filter,
            period_start__lt=trend.period_start
        ...
        if trend.avg_row_count is not None:
            baseline_row_counts = [h.avg_row_count for h in historical if h.avg_row_count is not None]
            if len(baseline_row_counts) >= 3:
                mean_row = statistics.mean(baseline_row_counts)
                stdev_row = statistics.stdev(baseline_row_counts) if len(baseline_row_counts) > 1 else 0
        ...
        if trend.avg_size_bytes is not None:
            baseline_size_bytes = [h.avg_size_bytes for h in historical if h.avg_size_bytes is not None]
            if len(baseline_size_bytes) >= 3:
                mean_size = statistics.mean(baseline_size_bytes)
                stdev_size = statistics.stdev(baseline_size_bytes) if len(baseline_size_bytes) > 1 else 0
    ...
    def get_volume_dashboard(
        ...
        """
        queryset = VolumeTrend.objects.filter(
            tenant_id=tenant_id,
        ...
        return {
            ...
            "summary": {
                ...
                "anomaly_percentage": round((anomaly_count / total_trends * 100) if total_trends > 0 else 0, 2),
                "volume_stats": {
                    "avg_row_count": int(volume_stats["avg_row_count"]) if volume_stats["avg_row_count"] else None,
                    "max_row_count": int(volume_stats["max_row_count"]) if volume_stats["max_row_count"] else None,
                    "min_row_count": int(volume_stats["min_row_count"]) if volume_stats["min_row_count"] else None,
                    "avg_size_bytes": int(volume_stats["avg_size_bytes"]) if volume_stats["avg_size_bytes"] else None,
                    "max_size_bytes": int(volume_stats["max_size_bytes"]) if volume_stats["max_size_bytes"] else None,
                    "min_size_bytes": int(volume_stats["min_size_bytes"]) if volume_stats["min_size_bytes"] else None,
                }
            }
        }
