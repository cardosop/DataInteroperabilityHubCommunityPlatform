...
class PipelineMonitor:
    ...
    def update_execution(
        ...
        if completed_at is not None:
            ...
            if execution.started_at:
                execution.execution_time_seconds = (completed_at - execution.started_at).total_seconds()
    ...
    def get_pipeline_dashboard(
        ...
        success_rate = (completed_executions / terminal_executions * 100) if terminal_executions > 0 else 0.0
        
        # Calculate error rate
        error_rate = (failed_executions / terminal_executions * 100) if terminal_executions > 0 else 0.0
        
        # Calculate average execution time
        avg_execution_time = stats_query.filter(
            status="COMPLETED",
            execution_time_seconds__isnull=False
        ).aggregate(avg=Avg('execution_time_seconds'))['avg'] or 0.0
        
        # Calculate average latency
        avg_latency = stats_query.filter(
            status="COMPLETED",
            latency_ms__isnull=False
        ).aggregate(avg=Avg('latency_ms'))['avg'] or 0.0
        
        # Calculate average throughput
        avg_throughput = stats_query.filter(
            status="COMPLETED",
            throughput_items_per_second__isnull=False
        ).aggregate(avg=Avg('throughput_items_per_second'))['avg'] or 0.0
        
        # Get error codes distribution
        error_codes = stats_query.filter(
            status="FAILED",
            error_code__isnull=False
        ).values('error_code').annotate(count=Count('id')).order_by('-count')
        
        # Get pipeline type distribution
        pipeline_types = stats_query.values('pipeline_type').annotate(
            count=Count('id'),
        ...
        for job in jobs:
            ...
            if not created:
                ...
                if job.started_at and job.completed_at:
                    execution.execution_time_seconds = (job.completed_at - job.started_at).total_seconds()
        ...
        for run in runs:
            ...
            execution, created = PipelineExecution.objects.get_or_create(
                ...
                defaults={
                    ...
                    'resource_id': str(run.scheduled_ingestion.asset.id) if run.scheduled_ingestion.asset else None,
            ...
            if not created:
                ...
                if run.started_at and run.completed_at:
                    execution.execution_time_seconds = (run.completed_at - run.started_at).total_seconds()
