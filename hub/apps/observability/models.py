...
class DataObservabilityMetric(models.Model):
    ...
    last_update_time = models.DateTimeField(
        null=True, blank=True, help_text="Last time the data was updated"
    )
    freshness_age_seconds = models.BigIntegerField(
        null=True, blank=True, help_text="Age of data in seconds (time since last update)"
    )
    ...
    freshness_sla_seconds = models.BigIntegerField(
        null=True, blank=True, help_text="Freshness SLA in seconds"
    )
    is_stale = models.BooleanField(
        default=False, db_index=True, help_text="Whether data is stale (exceeds SLA)"
    )
    # Volume metrics
    ...
    schema_json = models.JSONField(
        null=True, blank=True, help_text="Schema JSON snapshot for comparison"
    )
    # Metadata
    recorded_at = models.DateTimeField(
        auto_now_add=True, db_index=True, help_text="When this metric was recorded"
    )
...
class VolumeTrend(models.Model):
    ...
    avg_row_count = models.BigIntegerField(
        null=True, blank=True, help_text="Average row count during period"
    )
    min_row_count = models.BigIntegerField(
        null=True, blank=True, help_text="Minimum row count during period"
    )
    max_row_count = models.BigIntegerField(
        null=True, blank=True, help_text="Maximum row count during period"
    )
    avg_size_bytes = models.BigIntegerField(
        null=True, blank=True, help_text="Average size in bytes during period"
    )
    min_size_bytes = models.BigIntegerField(
        null=True, blank=True, help_text="Minimum size in bytes during period"
    )
    max_size_bytes = models.BigIntegerField(
        null=True, blank=True, help_text="Maximum size in bytes during period"
    )
    sample_count = models.IntegerField(default=0, help_text="Number of samples in this period")
    # Anomaly detection
    is_anomaly = models.BooleanField(
        default=False, db_index=True, help_text="Whether this period contains anomalies"
    )
    ...
    anomaly_score = models.FloatField(null=True, blank=True, help_text="Anomaly score (0.0 to 1.0)")
...
class SchemaDrift(models.Model):
    ...
    previous_schema_hash = models.CharField(
        max_length=64, null=True, blank=True, help_text="Hash of previous schema"
    )
    current_schema_hash = models.CharField(
        max_length=64, db_index=True, help_text="Hash of current schema"
    )
    previous_schema_json = models.JSONField(null=True, blank=True, help_text="Previous schema JSON")
    current_schema_json = models.JSONField(null=True, blank=True, help_text="Current schema JSON")
    # Drift details
    new_fields = models.JSONField(
        default=list, null=True, blank=True, help_text="List of new fields added"
    )
    removed_fields = models.JSONField(
        default=list, null=True, blank=True, help_text="List of fields removed"
    )
    ...
    is_within_tolerance = models.BooleanField(
        default=True, db_index=True, help_text="Whether drift is within configured tolerance"
    )
    # Metadata
    detected_at = models.DateTimeField(
        auto_now_add=True, db_index=True, help_text="When this drift was detected"
    )
...
class PipelineExecution(models.Model):
    ...
    pipeline_id = models.UUIDField(
        db_index=True, help_text="ID of the pipeline (scheduled_ingestion_id, job_id, etc.)"
    )
    pipeline_name = models.CharField(
        max_length=255, null=True, blank=True, help_text="Name of the pipeline"
    )
    # Execution tracking
    ...
    started_at = models.DateTimeField(
        null=True, blank=True, db_index=True, help_text="When execution started"
    )
    completed_at = models.DateTimeField(
        null=True, blank=True, db_index=True, help_text="When execution completed"
    )
    execution_time_seconds = models.FloatField(
        null=True, blank=True, help_text="Execution time in seconds"
    )
    # Performance metrics
    latency_ms = models.FloatField(null=True, blank=True, help_text="Latency in milliseconds")
    throughput_items_per_second = models.FloatField(
        null=True, blank=True, help_text="Throughput (items processed per second)"
    )
    ...
    resource_id = models.UUIDField(null=True, blank=True, help_text="ID of the resource")
    # Metadata
    result_json = models.JSONField(
        null=True, blank=True, default=dict, help_text="Execution result data"
    )
...
class DataSLA(models.Model):
    ...
    # SLA definition
    name = models.CharField(max_length=255, help_text="SLA name")
    description = models.TextField(null=True, blank=True, help_text="SLA description")
    ...
    # Freshness SLA
    freshness_sla_seconds = models.BigIntegerField(
        null=True, blank=True, help_text="Freshness SLA in seconds (maximum age of data)"
    )
    # Quality SLA
    ...
    # Compliance tracking
    is_active = models.BooleanField(default=True, db_index=True, help_text="Whether SLA is active")
    ...
    last_compliance_check = models.DateTimeField(
        null=True, blank=True, help_text="Last time compliance was checked"
    )
    is_violated = models.BooleanField(
        default=False, db_index=True, help_text="Whether SLA is currently violated"
    )
    violation_count = models.IntegerField(
        default=0, help_text="Number of times SLA has been violated"
    )
    # Metadata
...
class DataIncident(models.Model):
    ...
    title = models.TextField(help_text="Incident title (supports long descriptions)")
    ...
    detected_at = models.DateTimeField(
        auto_now_add=True, db_index=True, help_text="When incident was detected"
    )
    triaged_at = models.DateTimeField(null=True, blank=True, help_text="When incident was triaged")
    in_progress_at = models.DateTimeField(
        null=True, blank=True, help_text="When incident was marked as in progress"
    )
    resolved_at = models.DateTimeField(
        null=True, blank=True, db_index=True, help_text="When incident was resolved"
    )
    resolution_time_seconds = models.BigIntegerField(
        null=True, blank=True, help_text="Time to resolve in seconds"
    )
    # Assignment
    ...
    resource_id = models.UUIDField(null=True, blank=True, help_text="ID of the resource")
    # Incident details
    root_cause = models.TextField(null=True, blank=True, help_text="Root cause analysis")
    resolution_notes = models.TextField(null=True, blank=True, help_text="Resolution notes")
    metadata_json = models.JSONField(
        null=True, blank=True, default=dict, help_text="Additional incident metadata"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
