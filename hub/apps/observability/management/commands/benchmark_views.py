...
BASELINE_PATH = os.path.join(
    ...
    "scripts",
    "perf",
    "view_benchmark_baseline.json",
)
BASELINE_PATH = os.path.abspath(BASELINE_PATH)

VIEW_QUERIES: dict[str, str] = {
    "asset_list": ("SELECT * FROM assets ORDER BY created_at DESC LIMIT 20"),
    "contract_list": ("SELECT * FROM contracts ORDER BY created_at DESC LIMIT 20"),
    "marketplace_list": (
        "SELECT * FROM listings WHERE status = 'PUBLISHED' ORDER BY created_at DESC LIMIT 20"
    ),
    "search_fts": (
    ...
    "sparql_basic": ("SELECT * FROM sparql_queries ORDER BY created_at DESC LIMIT 1"),
}
...
def compare_results(
    ...
    for r in results:
        ...
        if r.view_name in baseline:
            ...
            if baseline_total > 0:
                delta_pct = (r.total_time_ms - baseline_total) / baseline_total * 100
...
def run_benchmarks(
    ...
    for view_name, query in targets.items():
        ...
        if not json_output:
            ...
            print(
                ...
                f"rows={result.plan_rows}" + (f" ERROR: {result.error}" if result.error else "")
            )
                ...
                json.dumps(
                    {
                        "status": "no_baseline",
                        "results": [
                            {"view": r.view_name, "total_time_ms": r.total_time_ms} for r in results
                        ],
                    },
                    indent=2,
                )
    ...
    if json_output:
        ...
        for entry in report:
            ...
            print(f"  {flag} {entry['view']}: {entry['total_time_ms']:.2f}ms ({pct})")
        if has_regression:
