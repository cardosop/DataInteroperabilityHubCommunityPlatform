...
class IncidentManager:
    ...
    def update_incident_status(
        ...
        if status == "RESOLVED" and not incident.resolved_at:
            ...
            if incident.detected_at:
                incident.resolution_time_seconds = int((now - incident.detected_at).total_seconds())
    ...
    @staticmethod
    def assign_incident(incident_id: str, assigned_to_id: str) -> DataIncident:
        """
        ...
        if incident.status in ["DETECTED", "TRIAGED"]:
            ...
            if not incident.in_progress_at:
                ...
                incident.save(
                    update_fields=["assigned_to", "status", "in_progress_at", "updated_at"]
                )
            else:
    ...
    def get_incidents_dashboard(
        ...
        incident_types = (
            stats_query.values("incident_type")
            .annotate(count=Count("id"), resolved_count=Count("id", filter=Q(status="RESOLVED")))
            .order_by("-count")
        )

        # Get severity distribution
        severities = (
            stats_query.values("severity")
            .annotate(count=Count("id"), resolved_count=Count("id", filter=Q(status="RESOLVED")))
            .order_by("-count")
        )
        ...
        avg_resolution_time = stats_query.filter(
            status="RESOLVED", resolution_time_seconds__isnull=False
        ).aggregate(avg=Avg("resolution_time_seconds"))["avg"]

        # Get unassigned incidents
        unassigned_incidents = stats_query.filter(
            assigned_to__isnull=True, status__in=["DETECTED", "TRIAGED", "IN_PROGRESS"]
        ).count()
            ...
            "results": [
                {
                    ...
                    "in_progress_at": incident.in_progress_at.isoformat()
                    if incident.in_progress_at
                    else None,
                    "resolved_at": incident.resolved_at.isoformat()
                    if incident.resolved_at
                    else None,
                    "resolution_time_seconds": incident.resolution_time_seconds,
                    "assigned_to_id": str(incident.assigned_to.id)
                    if incident.assigned_to
                    else None,
                    "detected_by_id": str(incident.detected_by.id)
                    if incident.detected_by
                    else None,
                    "resolved_by_id": str(incident.resolved_by.id)
    ...
    def auto_detect_incidents(tenant_id: str) -> dict[str, Any]:
        ...
        violated_slas = DataSLA.objects.filter(tenant=tenant, is_active=True, is_violated=True)
        ...
        for sla in violated_slas:
            ...
            existing_incident = DataIncident.objects.filter(
                ...
                else "QUALITY_VIOLATION"
                if sla.sla_type == "QUALITY"
                else "AVAILABILITY_VIOLATION"
