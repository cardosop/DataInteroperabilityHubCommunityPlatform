...
from .models import DataObservabilityMetric, SchemaDrift
...
class SchemaDriftDetector:
    ...
    @staticmethod
    def calculate_schema_hash(schema_json: dict[str, Any] | None) -> str | None:
        """
    ...
    @classmethod
    def compare_schemas(
        cls, previous_schema: dict[str, Any] | None, current_schema: dict[str, Any] | None
    ) -> dict[str, Any]:
        """
        ...
        for field_name in previous_field_map:
            if field_name in current_field_map:
                ...
                if prev_type != curr_type:
                    type_changes.append(
                        {"field_name": field_name, "old_type": prev_type, "new_type": curr_type}
                    )
        ...
        has_changes = (
            ...
            or len(type_changes) > 0
            or len(nullable_changes) > 0
        )
    ...
    def detect_drift(
        ...
        current_schema_json: dict[str, Any] | None = None,
        tolerance_config: dict[str, bool] | None = None,
    ) -> SchemaDrift | None:
        """
        ...
        # Get previous schema
        previous_metric = DataObservabilityMetric.objects.filter(tenant_id=tenant_id)
        ...
        previous_metric = (
            previous_metric.exclude(schema_hash__isnull=True)
            .exclude(
        ...
        return {
            ...
            "summary": {
                ...
                "breaking_percentage": round(
                    (breaking_count / total_drifts * 100) if total_drifts > 0 else 0, 2
                ),
            },
        }
