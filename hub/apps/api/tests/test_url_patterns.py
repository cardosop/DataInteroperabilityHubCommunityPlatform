...
from django.urls import reverse, resolve, Resolver404, NoReverseMatch, get_resolver
...
class URLPatternNoDuplicationTest(TestCase):
    ...
    def test_no_duplicate_service_names(self):
        ...
        duplication_errors = [
            ...
            if e.rule == "no_duplication"
        ]

        if duplication_errors:
            error_messages = "\n".join([
                f"  - {e.pattern}: {e.message}"
                for e in duplication_errors
            ])
            self.fail(
...
class URLPatternResolutionTest(TestCase):
    ...
    def test_all_api_v1_patterns_resolve(self):
        ...
        if unresolved_patterns:
            ...
            self.fail(
                ...
                f"{error_details}"
                + (f"\n  ... and {len(unresolved_patterns) - 10} more" if len(unresolved_patterns) > 10 else "")
            )

        if resolution_errors:
            error_details = "\n".join([
                f"  - {p['pattern']}: {p['error']}"
                for p in resolution_errors[:10]
            ])
            self.fail(
    ...
    def test_pattern_resolution_with_parameters(self):
        ...
        self.assertGreater(
            resolved_count, 0,
            "At least some patterns with parameters should resolve correctly"
        )
        ...
        for url_name in test_url_names:
            try:
                ...
                if resolved:
                    ...
                    if url_name in (resolved.url_name or '') or (resolved.url_name or '').endswith(url_name.split('-')[-1]):
                        consistent_count += 1
            except (NoReverseMatch, Resolver404):
                pass

        # At least some should be consistent
        self.assertGreater(consistent_count, 0, "At least some reverse/resolve pairs should be consistent")
...
class URLPatternComprehensiveTest(TestCase):
    ...
    def test_all_patterns_extracted(self):
        ...
        self.assertGreater(
            self.result.total_patterns, 0,
            "Should extract at least some URL patterns"
        )
    ...
    def test_all_api_v1_patterns_follow_standards(self):
        ...
        self.assertGreater(
            len(api_v1_patterns), 0,
            "Should have at least some /api/v1/ patterns"
        )

        # Check that validation passed (or log issues)
        if not self.result.passed:
            error_summary = "\n".join([
                f"  - {e.rule}: {e.pattern} - {e.message}"
                for e in self.result.errors[:10]
            ])
            self.fail(
                f"URL pattern validation failed with {len(self.result.errors)} errors:\n"
                f"{error_summary}"
                + (f"\n  ... and {len(self.result.errors) - 10} more errors" if len(self.result.errors) > 10 else "")
            )
    ...
    def test_no_missing_url_names_for_important_patterns(self):
        ...
        patterns_without_name = [
            p for p in important_patterns
            if not p.get('url_name')
        ]
    ...
    def test_pattern_naming_standards(self):
        ...
        kebab_case_errors = [
            ...
            if e.rule == "kebab_case"
        ]

        plural_errors = [
            e for e in self.result.errors
            if e.rule == "plural_resources"
        ]

        if kebab_case_errors:
            error_details = "\n".join([
                f"  - {e.pattern}: {e.message}"
                for e in kebab_case_errors[:5]
            ])
            self.fail(
                f"Found {len(kebab_case_errors)} kebab-case violations:\n{error_details}"
            )

        if plural_errors:
            error_details = "\n".join([
                f"  - {e.pattern}: {e.message}"
                for e in plural_errors[:5]
            ])
            self.fail(
                f"Found {len(plural_errors)} plural resource violations:\n{error_details}"
            )
...
class URLPatternValidatorEdgeCasesTest(TestCase):
    ...
    def test_format_errors_with_warnings(self):
        ...
        from hub.apps.api.utils.url_pattern_validator import URLPatternValidationResult, URLPatternValidationError
