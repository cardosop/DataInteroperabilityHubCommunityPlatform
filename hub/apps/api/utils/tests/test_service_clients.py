...
class DQServiceClientTest(TestCase):
    ...
    def test_health_check_failure(self):
        ...
        self.client.client = httpx.Client(
            base_url=self.client.base_url, timeout=self.client.timeout
        )
...
class SemanticServiceClientTest(TestCase):
    ...
    def test_map_contract_endpoint_construction(self):
        ...
        def handler(request: httpx.Request) -> httpx.Response:
            ...
            return httpx.Response(
                200, json={"contract_uri": "http://example.com/contract/123"}, request=request
            )
    ...
    def test_resolve_uri_endpoint_construction(self):
        ...
        def handler(request: httpx.Request) -> httpx.Response:
            ...
            recorded_requests.append(request)
            return httpx.Response(
                200, json={"@context": {}, "@id": "http://example.com/resource"}, request=request
            )
...
class DataContractCLIClientTest(TestCase):
    ...
    def test_validate_endpoint_construction(self):
        ...
        def handler(request: httpx.Request) -> httpx.Response:
            """Record request and return mock response"""
            recorded_requests.append(request)
            return httpx.Response(200, json={"status": "valid"}, request=request)
        ...
        test_client = httpx.Client(
            transport=httpx.MockTransport(handler), base_url=self.client.base_url
        )
