...
class APIAnalyticsService:
    ...
    def get_endpoint_popularity(
        ...
        results = query.values('endpoint_path', 'method').annotate(
    ...
    def get_usage_trends(
        ...
        results = query.values('period').annotate(
            request_count=Count('id'),
            avg_latency_ms=Avg('latency_ms'),
            error_count=Count('id', filter=Q(status_code__gte=400))
        ).order_by('period')
    ...
    def get_performance_metrics(
        ...
        # P95 latency
        latencies = list(query.exclude(latency_ms__isnull=True).values_list('latency_ms', flat=True))
        p95_latency = 0.0
        if latencies:
            latencies.sort()
        ...
        return {
            ...
            'success_rate': round(((total_requests - total_errors) / total_requests * 100) if total_requests > 0 else 0.0, 2)
        }
