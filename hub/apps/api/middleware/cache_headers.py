...
def get_last_modified(instance) -> datetime | None:
    """
...
def check_if_modified_since(request: HttpRequest, last_modified: datetime) -> bool:
    ...
    try:
        ...
        if_modified_since_dt = datetime.fromtimestamp(
            if_modified_since_timestamp, tz=ZoneInfo("UTC")
        )
...
class CacheHeadersMiddleware(MiddlewareMixin):
    ...
    def process_response(self, request: HttpRequest, response: HttpResponse) -> HttpResponse:
        ...
        is_public = (
            "/api/v1/marketplace/listings/" in request.path
            or "/api/v1/marketplace/search" in request.path
        )
