...
from .models import ScheduledIngestion, ScheduledIngestionRun, IngestionCost
...
class CostTrackingManager:
    ...
    def calculate_run_costs(
        ...
        storage_rate = storage_cost_per_gb_month or CostTrackingManager.DEFAULT_STORAGE_COST_PER_GB_MONTH
    ...
    def get_cost_report(
        ...
        query = IngestionCost.objects.filter(
        ...
        # Aggregate costs
        total_storage_cost = query.aggregate(
            total=Sum('storage_cost_usd')
        )['total'] or Decimal('0.0')
        
        total_compute_cost = query.aggregate(
            total=Sum('compute_cost_usd')
        )['total'] or Decimal('0.0')
        
        total_network_cost = query.aggregate(
            total=Sum('network_cost_usd')
        )['total'] or Decimal('0.0')
        
        total_cost = total_storage_cost + total_compute_cost + total_network_cost
        
        # Average costs
        avg_storage_cost = query.aggregate(
            avg=Avg('storage_cost_usd')
        )['avg'] or Decimal('0.0')
        
        avg_compute_cost = query.aggregate(
            avg=Avg('compute_cost_usd')
        )['avg'] or Decimal('0.0')
        
        avg_total_cost = query.aggregate(
            avg=Avg('total_cost_usd')
        )['avg'] or Decimal('0.0')
        
        # Cost by ingestion
        cost_by_ingestion = query.values(
            'scheduled_ingestion__id',
            'scheduled_ingestion__name'
        ).annotate(
        ...
        if start_date and end_date:
            ...
            while current_date <= end_date_only:
                ...
                daily_costs.append({
                    ...
                    'storage_cost_usd': float(day_query.aggregate(total=Sum('storage_cost_usd'))['total'] or Decimal('0.0')),
                    'compute_cost_usd': float(day_query.aggregate(total=Sum('compute_cost_usd'))['total'] or Decimal('0.0')),
                    'network_cost_usd': float(day_query.aggregate(total=Sum('network_cost_usd'))['total'] or Decimal('0.0'))
                })
