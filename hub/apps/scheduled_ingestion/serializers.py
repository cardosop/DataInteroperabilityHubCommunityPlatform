...
class ScheduledIngestionSerializer(serializers.ModelSerializer):
    ...
    def validate_file_pattern(self, value):
        ...
        try:
            ...
            raise serializers.ValidationError(f"Invalid file pattern regex: {e!s}")
        return value
    ...
    def validate_source_config(self, value):
        ...
        if source_type == SourceType.S3:
            ...
            if not value.get("host") or not value.get("database") or not value.get("schema"):
                raise serializers.ValidationError(
                    "Snowflake host, database, and schema are required"
                )
            ...
            if (
                not value.get("host")
                or not value.get("http_path")
                or not value.get("catalog")
                or not value.get("schema")
            ):
                raise serializers.ValidationError(
                    "Databricks host, http_path, catalog, and schema are required"
                )
    ...
    def validate(self, attrs):
        ...
        if asset_id is not None:
            ...
            try:
                ...
                raise serializers.ValidationError(
                    {"asset_id": f"Asset with id {asset_id} does not exist"}
                )
...
class ScheduledIngestionRunSerializer(serializers.ModelSerializer):
    ...
    scheduled_ingestion_name = serializers.CharField(
        source="scheduled_ingestion.name", read_only=True
    )
        ...
        if test_connection:
            ...
            if source_type and source_config:
                try:
                    ...
                    raise serializers.ValidationError(
                        {"source_config": f"Connection test failed: {e!s}"}
                    )
