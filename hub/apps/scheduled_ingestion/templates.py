...
from .models import ScheduledIngestion, ScheduleType, SourceType
...
class IngestionTemplate(models.Model):
    ...
    description = models.TextField(null=True, blank=True, help_text="Template description")
    template_type = models.CharField(
        max_length=50,
        help_text="Template type (e.g., 'S3_DAILY_FILES', 'API_POLLING', 'DATABASE_REPLICATION')",
    )
    is_system_template = models.BooleanField(
        default=False, help_text="True if this is a system-wide template (available to all tenants)"
    )
    source_type = models.CharField(
        max_length=50, choices=SourceType.choices, help_text="Source type for this template"
    )
...
class IngestionTemplateManager:
    ...
    def create_from_template(
        ...
        if template.ingestion_config_template:
            ingestion_config = IngestionTemplateManager._resolve_template_variables(
                template.ingestion_config_template, template_variables
            )
    ...
    def _resolve_template_variables(
        ...
        resolved_str = IngestionTemplateManager._resolve_string_template(template_str, variables)
        return json.loads(resolved_str)

    @staticmethod
    def _resolve_string_template(template: str, variables: dict[str, Any]) -> str:
        """
    ...
    templates = [
        {
            ...
            "schedule_config_template": {"time": "00:00", "timezone": "UTC"},
