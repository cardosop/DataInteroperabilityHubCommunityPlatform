...
from hub.apps.scheduled_ingestion.models import (
    ...
    IngestionCost, DeadLetterQueueItem,
...
class ScheduledIngestionModelTest(TestCase):
    ...
    def _create_ingestion(self, **overrides):
        defaults = dict(
            tenant=self.tenant, name=f"Ing {uuid.uuid4().hex[:6]}",
            source_type=SourceType.S3, source_config={"bucket": "b", "region": "us-east-1"},
            schedule_type=ScheduleType.DAILY, schedule_config={"time": "03:00", "timezone": "UTC"},
    ...
    def test_create_with_valid_cron_expression(self):
        ing = self._create_ingestion(schedule_type=ScheduleType.CUSTOM_CRON, schedule_config={"cron": "0 2 * * *", "timezone": "UTC"})
        self.assertIsNotNone(ing.id)
    ...
    def test_failure_tracking_fields(self):
        ...
        ing.save(update_fields=["consecutive_failure_count", "last_error_at", "error_message", "updated_at"])
        ing.refresh_from_db()
...
class ScheduledIngestionRunModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.ingestion = ScheduledIngestion.objects.create(
            tenant=self.tenant, name=f"Ing {uid}", source_type=SourceType.S3,
    ...
    def test_run_status_transitions(self):
        ...
        run = ScheduledIngestionRun.objects.create(scheduled_ingestion=self.ingestion, status=ScheduledIngestionRunStatus.PENDING)
    ...
    def test_run_file_tracking(self):
        ...
        run = ScheduledIngestionRun.objects.create(
            scheduled_ingestion=self.ingestion, status=ScheduledIngestionRunStatus.COMPLETED,
            files_found=10, files_processed=8, files_failed=2, datasets_created=3,
        )
    ...
    def test_run_duration_tracking(self):
        ...
        run = ScheduledIngestionRun.objects.create(
            scheduled_ingestion=self.ingestion, status=ScheduledIngestionRunStatus.COMPLETED,
            started_at=start, completed_at=start + timezone.timedelta(minutes=5),
        )
    ...
    def test_run_dlq_sync_status(self):
        ...
        run = ScheduledIngestionRun.objects.create(scheduled_ingestion=self.ingestion, status=ScheduledIngestionRunStatus.COMPLETED, dlq_sync_status="SYNCED")
...
class IngestionCostModelTest(TestCase):
    ...
    def test_cost_auto_calculation(self):
        ...
        cost = IngestionCost.objects.create(
            ...
            storage_cost_usd=1.50, compute_cost_usd=2.00, network_cost_usd=0.50,
