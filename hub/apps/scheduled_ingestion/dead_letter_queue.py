...
from .models import DeadLetterQueueItem, ScheduledIngestion

logger = structlog.get_logger(__name__)


def _parse_datetime_aware(value: str | None) -> datetime:
...
class DeadLetterQueueManager:
        ...
        for failure in permanent_failures:
            ...
            dlq_item, created = DeadLetterQueueItem.objects.get_or_create(
                ...
                defaults={
                    ...
                    "permanently_failed_at": _parse_datetime_aware(
                        failure.get("permanently_failed_at")
                    ),
    ...
    @staticmethod
    @transaction.atomic
    def retry_file(dlq_item_id: str, user_id: str | None = None) -> bool:
        """
    ...
    def resolve_item(
        ...
        logger.info(
            "DLQ item resolved", dlq_item_id=dlq_item_id, resolution_status=resolution_status
        )
    ...
    def get_dlq_dashboard(
        ...
        query = DeadLetterQueueItem.objects.filter(scheduled_ingestion__tenant=tenant)
        ...
        error_code_stats = query.values("error_code").annotate(count=Count("id")).order_by("-count")
