...
class ScheduledIngestionBusinessRules(BusinessRules):
        ...
        if schedule_type == ScheduleType.CUSTOM_CRON:
            ...
            if not isinstance(days_of_week, list):
                ...
                if not days_of_week:
                    ...
                    invalid_days = [d for d in days_of_week if not isinstance(d, int) or not (0 <= d <= 6)]
                    if invalid_days:
                        errors.append(
    ...
    def _validate_schedule_resource(
        self,
        schedule: ScheduledIngestion
    ) -> ValidationResult:
        """
        ...
        if source_type == SourceType.S3:
            ...
            if 'url' not in source_config:
                errors.append(f"{source_type} source config requires 'url' field")
    ...
    def _validate_ingestion_run(
        ...
        warnings = []
        details = {
            'ingestion_run_validation': 'ingestion_run',
            'validation_checks': {}
        }
        ...
        if ingestion_run.files_failed < 0:
            errors.append(
                f"Files failed must be non-negative, got: {ingestion_run.files_failed}"
            )
    ...
    def _validate_run_status_transition(
        ...
        if status == ScheduledIngestionRunStatus.PENDING:
            ...
            if not ingestion_run.started_at:
                errors.append(
                    "Run with status RUNNING must have started_at timestamp"
                )
            ...
            if not ingestion_run.started_at:
                errors.append(
                    f"Run with status {status} must have started_at timestamp"
                )
            if not ingestion_run.completed_at:
                errors.append(
                    f"Run with status {status} must have completed_at timestamp"
                )
            ...
            details['timestamps_consistent'] = len(errors) == timestamp_errors_before
            if details['timestamps_consistent']:
                details['started_at'] = ingestion_run.started_at.isoformat() if ingestion_run.started_at else None
                details['completed_at'] = ingestion_run.completed_at.isoformat() if ingestion_run.completed_at else None
    ...
    def _validate_incremental_ingestion(
        ...
        try:
            ...
            warnings.append("IncrementalStateManager not available - incremental validation skipped")
        ...
        if not isinstance(processed_files, list):
            errors.append(
                f"processed_files must be a list, got {type(processed_files).__name__}"
            )
        ...
        if not isinstance(failed_files, list):
            errors.append(
                f"failed_files must be a list, got {type(failed_files).__name__}"
            )
            ...
            for i, failed_file in enumerate(failed_files):
                if not isinstance(failed_file, dict):
                    ...
                    errors.append(
                        f"failed_files[{i}] must have 'file_path' key"
                    )
    ...
    def _validate_run_retry(
        ...
        if ingestion_run.job_id:
            try:
                ...
                from hub.apps.jobs.utils import get_job_max_retries, JobType
        ...
        if retry_count < 0:
            errors.append(
                f"Retry count must be non-negative, got: {retry_count}"
            )
        ...
        if retry_count > max_retries:
            errors.append(
                f"Retry count ({retry_count}) exceeds max retries ({max_retries})"
            )
    ...
    def _validate_source(
        ...
        if isinstance(source, ScheduledIngestion):
            ...
            source_config = source.get('source_config', source)  # If no source_config key, use entire dict
            schedule = source.get('schedule')  # Optional schedule reference
        else:
            errors.append(f"Source must be a dictionary or ScheduledIngestion instance, got {type(source).__name__}")
        ...
        if source_type:
            ...
            details['validation_checks']['connection'] = {'skipped': True, 'reason': 'source_type_not_provided'}
        ...
        if source_type:
            ...
            details['validation_checks']['accessibility'] = {'skipped': True, 'reason': 'source_type_not_provided'}
        ...
        if target_asset and source_type:
            ...
            details['validation_checks']['schema'] = {'skipped': True, 'reason': 'No target asset provided'}
    ...
    def _validate_source_connection(
        ...
        # Validate required fields based on source type
        required_fields_result = self._validate_source_config_required_fields(source_type, source_config)
    ...
    def _validate_source_config_required_fields(
        ...
        field_requirements = {
            SourceType.S3.value: {  # 'S3'
                'required': ['bucket'],
                'optional': ['prefix', 'access_key_id', 'secret_access_key', 'endpoint_url', 'region']
            },
        ...
        if not requirements:
            warnings.append(f"Unknown source type '{source_type}' - cannot validate required fields")
        ...
        for field in required_fields:
            if field not in source_config or source_config[field] is None or source_config[field] == '':
        ...
        present_optional_fields = [f for f in optional_fields if f in source_config and source_config[f]]
        ...
        if source_type == SourceType.S3.value and 'region' not in source_config:
            warnings.append("S3 source config should include 'region' field")
        elif source_type in [SourceType.FTP.value, SourceType.SFTP.value] and 'port' not in source_config:
            warnings.append(f"{source_type} source config should include 'port' field")
    ...
    def _validate_source_accessibility(
            ...
            if bucket:
                try:
                    ...
                    try:
                        ...
                        warnings.append("Connector factory not available - accessibility check skipped")
                ...
                try:
                    ...
                    with httpx.Client(timeout=5.0) as client:
                        ...
                        if response.status_code >= 500:
                            errors.append(f"Source URL returned error status {response.status_code}")
        ...
        if details.get('accessibility_tested') and details.get('accessibility_successful') is not False:
    ...
    def _validate_source_schema_compatibility(
        ...
        try:
            ...
            if not isinstance(target_fields, list):
                warnings.append("Target asset schema 'fields' is not a list - schema compatibility check skipped")
                details['schema_compatibility_checked'] = False
            ...
            details['target_schema_fields'] = [f.get('name') for f in target_fields if f.get('name')]
