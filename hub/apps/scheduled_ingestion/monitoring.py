...
class IngestionMonitoringDashboard:
    ...
    @staticmethod
    def get_dashboard(
        tenant_id: str, scheduled_ingestion_id: str | None = None, days: int = 30
    ) -> dict[str, Any]:
        """
        ...
        if scheduled_ingestion_id:
            ingestions = ScheduledIngestion.objects.filter(tenant=tenant, id=scheduled_ingestion_id)
        else:
        ...
        runs_query = ScheduledIngestionRun.objects.filter(
            scheduled_ingestion__tenant=tenant, created_at__gte=start_date
        )
        ...
        if completed_runs_with_times.exists():
            ...
            for run in completed_runs_with_times:
                if run.started_at and run.completed_at:
                    execution_times.append((run.completed_at - run.started_at).total_seconds())
            if execution_times:
                avg_execution_time = sum(execution_times) / len(execution_times)

        # File processing metrics
        total_files_found = runs_query.aggregate(total=Sum("files_found"))["total"] or 0

        total_files_processed = runs_query.aggregate(total=Sum("files_processed"))["total"] or 0

        total_files_failed = runs_query.aggregate(total=Sum("files_failed"))["total"] or 0

        total_datasets_created = runs_query.aggregate(total=Sum("datasets_created"))["total"] or 0

        # Calculate file processing success rate
        file_success_rate = (
            (total_files_processed / total_files_found * 100) if total_files_found > 0 else 0.0
        )
        ...
        for ingestion in ingestions:
            ...
            ingestion_success_rate = (
                (ingestion_completed / ingestion_total * 100) if ingestion_total > 0 else 0.0
            )
            ...
            ingestion_stats.append(
                {
                    ...
                    "next_run_at": ingestion.next_run_at.isoformat()
                    if ingestion.next_run_at
                    else None,
                }
            )

        # Trend data (daily aggregates)
        trend_data = IngestionMonitoringDashboard._get_trend_data(runs_query, start_date, days)
    ...
    def _calculate_health_status(
        success_rate: float, file_success_rate: float, failed_runs: int, total_runs: int
    ) -> str:
        """
    ...
    @staticmethod
    def _get_trend_data(
        runs_query: models.QuerySet, start_date: datetime, days: int
    ) -> list[dict[str, Any]]:
        """
        ...
        for i in range(days):
            ...
            day_runs = runs_query.filter(created_at__gte=day_start, created_at__lt=day_end)

            day_completed = day_runs.filter(status=ScheduledIngestionRunStatus.COMPLETED).count()

            day_failed = day_runs.filter(status=ScheduledIngestionRunStatus.FAILED).count()

            day_files_processed = day_runs.aggregate(total=Sum("files_processed"))["total"] or 0

            day_datasets_created = day_runs.aggregate(total=Sum("datasets_created"))["total"] or 0
