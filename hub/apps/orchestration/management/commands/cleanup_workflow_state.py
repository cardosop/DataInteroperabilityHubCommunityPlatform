...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--tenant-id", type=str, help="Process workflows for a specific tenant only"
        )
        parser.add_argument(
            "--cleanup-orphaned",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        archived_count = self._archive_completed_workflows(archive_days, tenant_id, dry_run)

        # Delete old archived workflows
        deleted_count = self._delete_archived_workflows(delete_days, tenant_id, dry_run)
    ...
    def _archive_completed_workflows(
        self, archive_days: int, tenant_id: str | None, dry_run: bool
    ) -> int:
        ...
        if not dry_run:
            ...
            with transaction.atomic():
                for workflow in queryset:
                    ...
                    logger.info(
                        f"Archived workflow {workflow.id} (completed_at={workflow.completed_at})"
                    )

        return count

    def _delete_archived_workflows(
        self, delete_days: int, tenant_id: str | None, dry_run: bool
    ) -> int:
        """Delete archived workflows older than delete_days."""
        cutoff_date = timezone.now() - timedelta(days=delete_days)

        queryset = WorkflowInstance.objects.filter(
            state_data__archived=True, completed_at__lt=cutoff_date
        )
    ...
    def _cleanup_orphaned_states(self, tenant_id: str | None, dry_run: bool) -> int:
        ...
        if not dry_run:
            ...
            WorkflowState.objects.filter(workflow_instance_id__in=orphaned_ids).delete()
