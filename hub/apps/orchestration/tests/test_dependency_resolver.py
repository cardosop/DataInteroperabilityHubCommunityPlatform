...
class TestPipelineDependencyResolver(TestCase):
    ...
    def test_are_dependencies_met_no_deps(self):
        self.assertTrue(self.resolver.are_dependencies_met("dq", str(uuid.uuid4()), {}))
    ...
    def test_validate_no_cycles_empty_graph(self):
        """Empty graph should not raise — validates base case."""
        self.resolver.validate_no_cycles()
        # Implicit: no CycleDetectedError raised → empty graph is cycle-free.

    @pytest.mark.integration
    def test_validate_no_cycles_dag(self):
        """Single-edge DAG should not raise a cycle error."""
        ...
        self.resolver.validate_no_cycles()
        # Implicit: no CycleDetectedError raised → DAG is cycle-free.
