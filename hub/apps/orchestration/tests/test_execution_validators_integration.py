...
class WorkflowExecutionValidatorIntegrationTest(TestCase):
    ...
    def test_integration_step_execution_validation_with_engine(self):
        ...
        # Execute workflow (will execute steps).  The test task may raise
        # ControlledWorkflowException or WorkflowExecutionError depending on
        # engine configuration; either way the workflow instance and its
        # steps should be present in the database for validation.
        try:
            workflow = self.engine.execute_instance(str(workflow.id))
        except (WorkflowExecutionError, ControlledWorkflowException):
            workflow.refresh_from_db()
        except Exception as exc:
            self.fail(
                f"Unexpected exception during workflow execution: {exc}"
            )
                ...
                "steps": [
                    {
                        ...
                        "type": "task",
                        "task": "test_task",
                        "compensation": {"type": "task", "task": "compensation_task"},
                    }
                ],
            },
            created_by=self.user,
    ...
    def test_integration_validation_after_workflow_completion(self):
        ...
        for step in steps:
            WorkflowExecutionValidator.validate_step_execution(step, workflow)
            # Steps should be in terminal states
            self.assertIn(
                step.status, [StepStatus.COMPLETED, StepStatus.FAILED, StepStatus.SKIPPED]
            )
