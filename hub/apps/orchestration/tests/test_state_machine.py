...
class WorkflowStateMachineTest(TestCase):
    ...
    def test_can_transition_draft_to_running(self):
        ...
        self.assertTrue(
            WorkflowStateMachine.can_transition(WorkflowStatus.DRAFT, WorkflowStatus.RUNNING)
        )

    def test_can_transition_running_to_completed(self):
        """Test RUNNING → COMPLETED transition"""
        self.assertTrue(
            WorkflowStateMachine.can_transition(WorkflowStatus.RUNNING, WorkflowStatus.COMPLETED)
        )

    def test_can_transition_running_to_failed(self):
        """Test RUNNING → FAILED transition"""
        self.assertTrue(
            WorkflowStateMachine.can_transition(WorkflowStatus.RUNNING, WorkflowStatus.FAILED)
        )

    def test_can_transition_running_to_rolling_back(self):
        """Test RUNNING → ROLLING_BACK transition"""
        self.assertTrue(
            WorkflowStateMachine.can_transition(WorkflowStatus.RUNNING, WorkflowStatus.ROLLING_BACK)
        )

    def test_cannot_transition_completed_to_running(self):
        """Test COMPLETED → RUNNING transition (invalid)"""
        self.assertFalse(
            WorkflowStateMachine.can_transition(WorkflowStatus.COMPLETED, WorkflowStatus.RUNNING)
        )

    def test_cannot_transition_draft_to_completed(self):
        """Test DRAFT → COMPLETED transition (invalid)"""
        self.assertFalse(
            WorkflowStateMachine.can_transition(WorkflowStatus.DRAFT, WorkflowStatus.COMPLETED)
        )

    def test_validate_transition_valid(self):
        """Test validate_transition with valid transition"""
        try:
            WorkflowStateMachine.validate_transition(WorkflowStatus.DRAFT, WorkflowStatus.RUNNING)
