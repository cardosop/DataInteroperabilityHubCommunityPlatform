...
class WorkflowMetricsTest(TestCase):
    ...
    def test_workflow_instances_created_total_metric(self):
        ...
        self.assertIsNotNone(workflow_instances_created_total)
        self.assertTrue(hasattr(workflow_instances_created_total, "labels"))

        # Call the metric with real wrapper
        labeled_metric = workflow_instances_created_total.labels(
            workflow_name="test_workflow", workflow_version="1.0.0", tenant_id="test_tenant"
        )

        # Verify labeled metric is callable and has inc method
        self.assertIsNotNone(labeled_metric)
        self.assertTrue(hasattr(labeled_metric, "inc"))

        # Call inc() - should not raise exception
        labeled_metric.inc()

    def test_workflow_instances_started_total_metric(self):
        """Test workflow_instances_started_total metric"""
        # Test real metric wrapper object
        self.assertIsNotNone(workflow_instances_started_total)
        labeled_metric = workflow_instances_started_total.labels(
            workflow_name="test_workflow", workflow_version="1.0.0", tenant_id="test_tenant"
        )
        self.assertIsNotNone(labeled_metric)
        self.assertTrue(hasattr(labeled_metric, "inc"))
    ...
    def test_workflow_instances_completed_total_metric(self):
        ...
        # Test real metric wrapper object
        self.assertIsNotNone(workflow_instances_completed_total)
        labeled_metric = workflow_instances_completed_total.labels(
    ...
    def test_workflow_instances_failed_total_metric(self):
        ...
        # Test real metric wrapper object
        self.assertIsNotNone(workflow_instances_failed_total)
        labeled_metric = workflow_instances_failed_total.labels(
            workflow_name="test_workflow",
            workflow_version="1.0.0",
    ...
    def test_workflow_execution_duration_seconds_metric(self):
        ...
        # Test real metric wrapper object
        self.assertIsNotNone(workflow_execution_duration_seconds)
        labeled_metric = workflow_execution_duration_seconds.labels(
            workflow_name="test_workflow",
            workflow_version="1.0.0",
            status="COMPLETED",
            tenant_id="test_tenant",
        ...
        self.assertIsNotNone(labeled_metric)
        self.assertTrue(hasattr(labeled_metric, "observe"))
        labeled_metric.observe(10.5)

    def test_workflow_steps_started_total_metric(self):
        """Test workflow_steps_started_total metric"""
        # Test real metric wrapper object
        self.assertIsNotNone(workflow_steps_started_total)
        labeled_metric = workflow_steps_started_total.labels(
            workflow_name="test_workflow",
            workflow_version="1.0.0",
            step_name="step1",
            step_type="task",
    ...
    def test_workflow_steps_completed_total_metric(self):
        ...
        # Test real metric wrapper object
        self.assertIsNotNone(workflow_steps_completed_total)
        labeled_metric = workflow_steps_completed_total.labels(
            workflow_name="test_workflow",
            workflow_version="1.0.0",
            step_name="step1",
            step_type="task",
    ...
    def test_workflow_steps_failed_total_metric(self):
        ...
        # Test real metric wrapper object
        self.assertIsNotNone(workflow_steps_failed_total)
        labeled_metric = workflow_steps_failed_total.labels(
            workflow_name="test_workflow",
            workflow_version="1.0.0",
            step_name="step1",
            step_type="task",
