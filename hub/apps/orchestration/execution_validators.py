...
class WorkflowExecutionValidator:
    ...
    def validate_status_transition(
        workflow_instance: WorkflowInstance, target_status: WorkflowStatus | None = None
    ) -> ValidationResult:
        """
        ...
        if current_status == WorkflowStatus.DRAFT:
            ...
            if workflow_instance.started_at is not None:
                errors.append("Workflow with status DRAFT should not have started_at timestamp set")
            ...
            if workflow_instance.completed_at is not None:
                errors.append(
                    "Workflow with status DRAFT should not have completed_at timestamp set"
                )
                details["draft_constraints_valid"] = "false"

            # DRAFT workflows should not have error_message
            if workflow_instance.error_message:
                warnings.append("Workflow with status DRAFT should not have error_message set")
            ...
            if workflow_instance.started_at is None:
                errors.append("Workflow with status RUNNING must have started_at timestamp set")
            ...
            if workflow_instance.completed_at is not None:
                errors.append(
                    "Workflow with status RUNNING should not have completed_at timestamp set"
                )
                details["running_constraints_valid"] = "false"
            ...
            if workflow_instance.output_data:
                warnings.append(
                    "Workflow with status RUNNING should not have output_data set (not completed yet)"
                )
                details["running_constraints_valid"] = details.get(
                    "running_constraints_valid", "true"
                )
            ...
            # Validate completed_at is after started_at
                ...
                and workflow_instance.completed_at < workflow_instance.started_at
            ):
                errors.append(
            ...
            if current_status == WorkflowStatus.COMPLETED:
                if workflow_instance.output_data is None:
                    warnings.append("Workflow with status COMPLETED should have output_data set")
                    details["terminal_constraints_valid"] = details.get(
                        "terminal_constraints_valid", "true"
                    )

                # COMPLETED workflows should not have error_message
                if workflow_instance.error_message:
                    warnings.append(
                        "Workflow with status COMPLETED should not have error_message set"
                    )
                    details["terminal_constraints_valid"] = details.get(
                        "terminal_constraints_valid", "true"
                    )
            ...
            if workflow_instance.started_at is None:
                warnings.append("Workflow with status PAUSED should have started_at timestamp set")
            ...
            if workflow_instance.completed_at is not None:
                errors.append(
                    "Workflow with status PAUSED should not have completed_at timestamp set"
                )
            ...
            if workflow_instance.started_at is None:
                errors.append(
                    "Workflow with status ROLLING_BACK must have started_at timestamp set"
                )
            ...
            if workflow_instance.completed_at is not None:
                warnings.append(
                    "Workflow with status ROLLING_BACK should not have completed_at timestamp set (rollback in progress)"
                )
                details["rolling_back_constraints_valid"] = details.get(
                    "rolling_back_constraints_valid", "true"
                )
    ...
    def validate_step_execution(
        step: WorkflowStep | None, workflow_instance: WorkflowInstance | None = None
    ) -> ValidationResult:
        """
        ...
        if workflow_instance is None:
            try:
                ...
                errors.append(f"Failed to fetch workflow instance for step: {e!s}")
        ...
        if step.status not in valid_step_statuses:
            errors.append(
                f"Invalid step status: {step.status}. Valid statuses are: {valid_step_statuses}"
            )
        ...
        if step.step_index > 0:
            previous_steps = workflow_instance.steps.filter(step_index__lt=step.step_index).exclude(
                status__in=[StepStatus.COMPLETED, StepStatus.SKIPPED]
            )

            incomplete_previous_steps = list(
                previous_steps.values_list("step_index", "step_name", "status")
            )
        ...
        if workflow_instance.status == WorkflowStatus.RUNNING:
            ...
            if step.status != StepStatus.PENDING:
                warnings.append(
                    f"Step status {step.status} is unusual for a DRAFT workflow. Expected: PENDING"
                )
                details["status_workflow_match"] = "false"
            else:
                details["status_workflow_match"] = "true"
        elif workflow_instance.status in [
            WorkflowStatus.COMPLETED,
            WorkflowStatus.FAILED,
            WorkflowStatus.CANCELLED,
        ]:
        ...
        if step.status == StepStatus.RUNNING:
            ...
            if step.started_at is None:
                errors.append("Step with status RUNNING must have started_at timestamp set")
            ...
            if step.completed_at is not None:
                errors.append("Step with status RUNNING should not have completed_at timestamp set")
                details["step_timestamps_valid"] = "false"
            ...
            if step.started_at is None:
                errors.append(f"Step with status {step.status} must have started_at timestamp set")
                details["step_timestamps_valid"] = "false"
            else:
                details["step_timestamps_valid"] = "true"
            ...
            if step.completed_at is None:
                ...
                details["step_timestamps_valid"] = "false"
            # Validate completed_at is after started_at
            elif step.started_at and step.completed_at < step.started_at:
                errors.append(
        ...
        if step.status == StepStatus.COMPLETED:
            ...
            if step.output_data is None:
                warnings.append("Step with status COMPLETED should have output_data set")
                details["step_constraints_valid"] = "true"

            # COMPLETED steps should not have error_message
            if step.error_message:
                warnings.append("Step with status COMPLETED should not have error_message set")
                details["step_constraints_valid"] = "true"
            ...
            if not step.error_message:
                warnings.append(
                    "Step with status FAILED should have error_message set for debugging"
                )
                details["step_constraints_valid"] = "true"
    ...
    def validate_compensation(
        workflow_instance: WorkflowInstance, step: WorkflowStep | None = None
    ) -> ValidationResult:
        """
        ...
        try:
            ...
            errors.append(f"Failed to fetch workflow definition: {e!s}")
                        ...
                        if compensation_type not in valid_compensation_types:
                            ...
                            if compensation_type == "task":
                                if "task" not in step_compensation:
                                    errors.append(
                                        "Compensation type 'task' requires 'task' field to be specified"
                                    )
                                    details["step_compensation_valid"] = "false"
                            elif compensation_type == "script":
                                if "script" not in step_compensation:
                                    errors.append(
                                        "Compensation type 'script' requires 'script' field to be specified"
                                    )
                                    details["step_compensation_valid"] = "false"
                else:
    ...
    @staticmethod
    def validate_retry(workflow_instance: WorkflowInstance) -> ValidationResult:
        """
    ...
    def validate_all(
        ...
        if step is not None:
            step_result = WorkflowExecutionValidator.validate_step_execution(
