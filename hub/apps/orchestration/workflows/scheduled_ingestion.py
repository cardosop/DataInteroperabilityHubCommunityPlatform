...
class ScheduledIngestionWorkflow:
    ...
    def register_tasks(cls, engine: WorkflowEngine) -> None:
        ...
        """
        engine.register_task("scheduled_ingestion.validate_ingestion_config", cls._validate_ingestion_config_task)
        ...
        engine.register_task("scheduled_ingestion.create_dataset_version", cls._create_dataset_version_task)
        engine.register_task("scheduled_ingestion.index_dataset", cls._index_dataset_task)
        engine.register_task("scheduled_ingestion.mark_file_processed", cls._mark_file_processed_task)
        engine.register_task("scheduled_ingestion.update_ingestion_state", cls._update_ingestion_state_task)
        engine.register_task("scheduled_ingestion.handle_failures_dlq", cls._handle_failures_dlq_task)
        engine.register_task("scheduled_ingestion.send_completion_notification", cls._send_completion_notification_task)
        engine.register_task("scheduled_ingestion.audit_logging", cls._audit_logging_task)
        
        # Compensation tasks
        engine.register_task("scheduled_ingestion.rollback_connection", cls._rollback_connection_task)
        engine.register_task("scheduled_ingestion.rollback_file_download", cls._rollback_file_download_task)
        engine.register_task("scheduled_ingestion.rollback_dataset_creation", cls._rollback_dataset_creation_task)
        engine.register_task("scheduled_ingestion.rollback_indexing", cls._rollback_indexing_task)
    
    @staticmethod
    def _validate_ingestion_config_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        """
        ...
        file_pattern = scheduled_ingestion.file_pattern
        if not file_pattern:
    ...
    @staticmethod
    def _connect_to_source_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        """
    ...
    @staticmethod
    def _discover_files_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        """
    ...
    @staticmethod
    def _filter_files_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        """
    ...
    @staticmethod
    @transaction.atomic
    def _download_file_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        """
    ...
    @staticmethod
    def _validate_file_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        """
    ...
    @staticmethod
    def _run_dq_check_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if quality_score < min_quality_score:
                error_message = f"DQ quality score {quality_score:.2%} below threshold {min_quality_score:.2%}"
                logger.warning(
            ...
            return {
                "dq_check_failed": True,
                "error": str(e)
            }
    
    @staticmethod
    @transaction.atomic
    def _create_dataset_version_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        """
        ...
        if not asset and scheduled_ingestion.auto_create_asset:
            ...
            asset = Asset.objects.create(
                ...
                description=scheduled_ingestion.description or f"Asset created from scheduled ingestion {scheduled_ingestion.name}",
        ...
        if dq_result:
            metadata_json["pre_ingestion_dq"] = {
                ...
                "passed_checks": len([c for c in dq_result.get("checks", []) if c.get("status", "").upper() in ["PASS", "SUCCESS"]]),
                "failed_checks": len([c for c in dq_result.get("checks", []) if c.get("status", "").upper() not in ["PASS", "SUCCESS"]])
            }
    ...
    @staticmethod
    @transaction.atomic
    def _index_dataset_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        """
    ...
    @staticmethod
    @transaction.atomic
    def _mark_file_processed_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        """
    ...
    @staticmethod
    @transaction.atomic
    def _update_ingestion_state_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        """
    ...
    @staticmethod
    @transaction.atomic
    def _handle_failures_dlq_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        """
    ...
    @staticmethod
    def _send_completion_notification_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        """
...
Workflow Instance: {instance.id}
    ...
    def _audit_logging_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        ...
        scheduled_ingestion = ScheduledIngestion.objects.get(id=scheduled_ingestion_id)
        user_id = str(scheduled_ingestion.created_by.id) if scheduled_ingestion.created_by else None
    ...
    # Compensation tasks
    @staticmethod
    def _rollback_connection_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        """Rollback connection (no-op, connection is stateless)"""
        return {"rolled_back": True}
    
    @staticmethod
    def _rollback_file_download_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        """Rollback file download by cleaning up temp file"""
        temp_path = input_data.get("temp_path")
        if temp_path and os.path.exists(temp_path):
            try:
                os.unlink(temp_path)
    ...
    @staticmethod
    @transaction.atomic
    def _rollback_dataset_creation_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
    ...
    def execute(
        ...
        workflow_instance = engine.create_instance(
            ...
            tenant_id=str(scheduled_ingestion.tenant.id),
            created_by_id=str(scheduled_ingestion.created_by.id) if scheduled_ingestion.created_by else None
        )
