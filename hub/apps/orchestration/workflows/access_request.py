...
class AccessRequestWorkflow:
        ...
        workflow_dsl = {
            ...
            "steps": [
                ...
                },
                {
                    "name": "audit_logging",
                    "type": "task",
                    "task": "access_request.audit_logging"
                }
            ],
            "compensation": {
                "enabled": True
    ...
    def register_tasks(cls, engine: WorkflowEngine) -> None:
        ...
        """
        engine.register_task("access_request.create_access_request", cls._create_access_request_task)
        ...
        engine.register_task("access_request.wait_for_step_approval", cls._wait_for_step_approval_task)
    ...
    @staticmethod
    def _create_access_request_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        """
    ...
    @staticmethod
    def _classify_request_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        """
        ...
        if access_request.asset:
            ...
            # Files inherit classification from dataset
            if access_request.file.dataset:
                classifications = DataClassification.objects.filter(
                    tenant_id=access_request.tenant_id,
                    dataset_id=str(access_request.file.dataset.id),
                    status="APPROVED"
                )
            else:
        ...
        if classifications.exists():
            ...
            highest = max(
                classifications,
                key=lambda c: classification_priority.get(c.category, 0)
            )
            highest_classification = highest.category
        
        # Determine if approval is required based on classification
        # Higher classifications require approval, lower may auto-approve
        requires_approval = True
        if highest_classification in [ClassificationCategory.PUBLIC, ClassificationCategory.INTERNAL]:
            # Check if access type is READ only
            if access_request.requested_access_type == "READ":
                # May auto-approve for low-risk resources
    ...
    @staticmethod
    def _route_to_approvers_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        """
        ...
        if highest_classification in [ClassificationCategory.CONFIDENTIAL, ClassificationCategory.RESTRICTED]:
            ...
            elif access_request.dataset and access_request.dataset.asset and access_request.dataset.asset.created_by:
        ...
        elif highest_classification in [ClassificationCategory.RESTRICTED, ClassificationCategory.PII, ClassificationCategory.PHI, ClassificationCategory.PCI]:
            ...
            if not approvers:
                ...
                if tenant_admin_role:
                    ...
                    approvers = [str(ur.user.id) for ur in tenant_admin_users[:1]]  # Take first admin
                else:
    ...
    @staticmethod
    def _wait_for_step_approval_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        """
        ...
        timeout_hours = step_info.get("timeout_hours", AccessRequestWorkflow.DEFAULT_APPROVAL_TIMEOUT_HOURS)
    ...
    @staticmethod
    def _escalate_approval_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        """
                ...
                if approver_email:
                    send_email_async(
                        ...
                        context={
                            "access_request_id": str(access_request.id),
                            "step_index": step_index,
                            "resource": str(access_request.asset or access_request.dataset or access_request.file),
                            "requested_by": getattr(access_request.requested_by, 'email', 'Unknown') if access_request.requested_by else "Unknown"
                        },
    ...
    @staticmethod
    def _grant_access_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        """
        ...
        return {
            "access_request_id": str(access_request.id),
            "access_granted": True,
            "access_granted_at": access_request.access_granted_at.isoformat() if access_request.access_granted_at else None,
            "expires_at": access_request.expires_at.isoformat() if access_request.expires_at else None
        }
    
    @staticmethod
    def _revoke_access_task(input_data: Dict[str, Any], instance: WorkflowInstance, step) -> Dict[str, Any]:
        """
        ...
        if access_request.requested_by:
            ...
            if requester_email:
                try:
                    ...
                    status_text = "approved" if access_request.status == AccessRequestStatus.APPROVED.value else "rejected" if access_request.status == AccessRequestStatus.REJECTED.value else "pending"
                    send_email_async(
                        email_type=EmailType.JOB_COMPLETION,
                        ...
                        context={
                            ...
                            "resource": str(access_request.asset or access_request.dataset or access_request.file),
                            "rejection_reason": access_request.rejection_reason if access_request.status == AccessRequestStatus.REJECTED.value else None
                        },
                        tenant_id=str(access_request.tenant.id),
            ...
            audit_event = create_audit_event(
                ...
                details={
                    ...
                    "resource": {
                        ...
                        "dataset_id": str(access_request.dataset.id) if access_request.dataset else None,
                        "file_id": str(access_request.file.id) if access_request.file else None
                    },
                    "approved_by": str(access_request.approved_by.id) if access_request.approved_by else None,
                    "rejected_by": str(access_request.rejected_by.id) if access_request.rejected_by else None,
                    "expires_at": access_request.expires_at.isoformat() if access_request.expires_at else None
                }
            )
    ...
    def approve_access_request(
        ...
        if access_request.approval_workflow and len(access_request.approval_workflow) > 1:
            ...
            if step_index < len(access_request.approval_workflow) - 1:
                ...
                access_request.approvers = access_request.approval_workflow[step_index + 1].get("approvers", [])
                access_request.save()
