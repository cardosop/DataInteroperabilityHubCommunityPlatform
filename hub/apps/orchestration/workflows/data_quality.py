...
class DataQualityCheckWorkflow:
    ...
    def register_workflow(cls, registry: WorkflowRegistry) -> None:
        ...
        workflow_dsl = {
            ...
            "dependencies": [],
            "steps": [
                {"name": "create_dq_run", "type": "task", "task": "data_quality.create_dq_run"},
                {
                ...
                {"name": "generate_alerts", "type": "task", "task": "data_quality.generate_alerts"},
                {
                    "name": "update_quality_metrics",
                    "type": "task",
                    "task": "data_quality.update_quality_metrics",
                },
                {"name": "store_results", "type": "task", "task": "data_quality.store_results"},
                {"name": "audit_logging", "type": "task", "task": "data_quality.audit_logging"},
            ],
    ...
    def register_tasks(cls, engine: WorkflowEngine) -> None:
        ...
        """
        engine.register_task("data_quality.create_dq_run", cls._create_dq_run_task)
        engine.register_task(
            "data_quality.load_contract_and_dataset", cls._load_contract_and_dataset_task
        )
        engine.register_task("data_quality.execute_quality_rules", cls._execute_quality_rules_task)
        engine.register_task(
            "data_quality.calculate_quality_scores", cls._calculate_quality_scores_task
        )
        engine.register_task("data_quality.detect_anomalies", cls._detect_anomalies_task)
        engine.register_task("data_quality.generate_alerts", cls._generate_alerts_task)
        engine.register_task(
            "data_quality.update_quality_metrics", cls._update_quality_metrics_task
        )
        engine.register_task("data_quality.store_results", cls._store_results_task)
        engine.register_task("data_quality.audit_logging", cls._audit_logging_task)
    ...
    def _create_dq_run_task(
        ...
        # Validate DQ run creation using DQBusinessRules
        dq_rules = DQBusinessRules(
        ...
        # Validate created DQ run using DQBusinessRules
        dq_run_validation_result = dq_rules.validate_dq_run_execution(
            dq_run=dq_run, user=triggered_by, tenant=tenant, validation_type="all"
        )

        if not dq_run_validation_result.is_valid:
            error_messages = dq_run_validation_result.errors
            # Log errors but don't fail - DQ run is already created
            logger.warning(
                "DQ run validation errors after creation",
                workflow_instance_id=str(instance.id),
                dq_run_id=str(dq_run.id),
                errors=error_messages,
            )
        elif dq_run_validation_result.warnings:
            logger.warning(
                "DQ run validation warnings",
                workflow_instance_id=str(instance.id),
                dq_run_id=str(dq_run.id),
                warnings=dq_run_validation_result.warnings,
            )
    ...
    def _execute_quality_rules_task(
        ...
        # Validate DQ run execution
        dq_run_execution_result = dq_rules.validate_dq_run_execution(
            dq_run=dq_run, user=user, tenant=tenant, validation_type="execution"
        )

        if not dq_run_execution_result.is_valid:
            error_messages = dq_run_execution_result.errors
            raise ValueError(f"DQ run execution validation failed: {'; '.join(error_messages)}")

        # Log validation warnings if any
        if dq_run_execution_result.warnings:
            logger.warning(
                "DQ run execution validation warnings",
                workflow_instance_id=str(instance.id),
                dq_run_id=str(dq_run.id),
                warnings=dq_run_execution_result.warnings,
            )
    ...
    def _detect_anomalies_task(
        ...
        # Detect anomalies
        anomalies = anomaly_detector.detect_anomalies(dq_run=dq_run, metric_type="quality_score")
        ...
        for metric_type in anomaly_metric_types:
            if metric_type != "quality_score":
                metric_alerts = alerting_service.evaluate_rules(
                    dq_run=dq_run, metric_type=metric_type
                )
    ...
    def _update_quality_metrics_task(
        ...
        if dq_run.asset:
            scorecard_data = scorecard_service.get_asset_scorecard(
                asset_id=str(dq_run.asset.id), tenant_id=str(dq_run.tenant.id), days=30
            )
        else:
            scorecard_data = None
    ...
    def execute(
        ...
        if not is_healthy:
            logger.warning(
                ...
                extra={"error": error_msg, "tenant_id": tenant_id, "asset_id": asset_id},
            )
