...
class MarketplacePublicationWorkflow:
    ...
    def register_tasks(cls, engine: WorkflowEngine) -> None:
        ...
        engine.register_task(
            "marketplace_publication.configure_pricing_model", cls._configure_pricing_model_task
        )
        engine.register_task(
            "marketplace_publication.configure_license_information",
            cls._configure_license_information_task,
        )
        engine.register_task("marketplace_publication.publish_listing", cls._publish_listing_task)
        engine.register_task(
            "marketplace_publication.index_for_marketplace_search",
            cls._index_for_marketplace_search_task,
        )
        engine.register_task(
            "marketplace_publication.send_notifications", cls._send_notifications_task
        )
        engine.register_task("marketplace_publication.audit_logging", cls._audit_logging_task)

        # Compensation tasks
        engine.register_task(
            "marketplace_publication.rollback_listing_creation", cls._rollback_listing_creation_task
        )
        engine.register_task(
            "marketplace_publication.rollback_publication", cls._rollback_publication_task
        )
        engine.register_task(
            "marketplace_publication.rollback_indexing", cls._rollback_indexing_task
        )

    @staticmethod
    def _validate_asset_eligibility_task(
        ...
        elif active_contract.validation_status not in ["VALID", "WARNING_ONLY"]:
            validation_results["blockers"].append(
    ...
    @staticmethod
    @transaction.atomic
    def _create_marketplace_listing_task(
        input_data: dict[str, Any], instance: WorkflowInstance, step
    ) -> dict[str, Any]:
        """
        ...
        tenant_id = (
            instance.state_data.get("tenant_id")
            or input_data.get("tenant_id")
            or instance.tenant_id
        )
        ...
        if existing_listing:
            ...
            return {
                ...
                "state": {"listing_id": str(existing_listing.id), "existing_listing": True},
            }
        ...
        return {
            ...
            "listing_id": str(listing.id),
            "state": {"listing_id": str(listing.id), "listing_created": True},
        }

    @staticmethod
    @transaction.atomic
    def _configure_pricing_model_task(
        input_data: dict[str, Any], instance: WorkflowInstance, step
    ) -> dict[str, Any]:
        """
        ...
        if pricing_model == PricingModel.REQUEST_APPROVAL:
            if not price_amount or price_amount <= 0:
                raise ValueError("REQUEST_APPROVAL pricing model requires a valid price_amount > 0")
        ...
        return {
            ...
            "price_amount": price_amount,
            "currency": currency,
            "state": {"pricing_configured": True, "pricing_model": pricing_model},
        }

    @staticmethod
    @transaction.atomic
    def _configure_license_information_task(
        input_data: dict[str, Any], instance: WorkflowInstance, step
    ) -> dict[str, Any]:
        """
        ...
        return {
            ...
            "state": {"license_configured": True, "license_info": license_info},
        }

    @staticmethod
    @transaction.atomic
    def _publish_listing_task(
        input_data: dict[str, Any], instance: WorkflowInstance, step
    ) -> dict[str, Any]:
        """
    ...
    @staticmethod
    def _index_for_marketplace_search_task(
        input_data: dict[str, Any], instance: WorkflowInstance, step
    ) -> dict[str, Any]:
        """
    ...
    def _send_notifications_task(
        ...
        if not listing_id:
            logger.warning(
                "Notifications skipped (missing listing_id)", workflow_instance_id=str(instance.id)
            )
            return {"notifications_sent": False, "reason": "Missing listing_id"}
    ...
    def _audit_logging_task(
        ...
        tenant_id = input_data.get("tenant_id") or instance.tenant_id
        listing_id = instance.state_data.get("listing_id")
        instance.state_data.get("asset_id")
    ...
    @staticmethod
    @transaction.atomic
    def _rollback_listing_creation_task(
        input_data: dict[str, Any], instance: WorkflowInstance, step
    ) -> dict[str, Any]:
    ...
    @staticmethod
    @transaction.atomic
    def _rollback_publication_task(
        input_data: dict[str, Any], instance: WorkflowInstance, step
    ) -> dict[str, Any]:
