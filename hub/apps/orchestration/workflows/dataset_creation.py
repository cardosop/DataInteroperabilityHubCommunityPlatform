...
class DatasetCreationWorkflow:
    ...
    def register_workflow(cls, registry: WorkflowRegistry) -> None:
        ...
        workflow_dsl = {
            ...
            "steps": [
                ...
                {"name": "infer_schema", "type": "task", "task": "dataset_creation.infer_schema"},
                {
                ...
                },
                {
                    ...
                    "compensation": {"type": "task", "task": "dataset_creation.rollback_indexing"},
                },
                {
                ...
                {"name": "audit_logging", "type": "task", "task": "dataset_creation.audit_logging"},
            ],
    ...
    def register_tasks(cls, engine: WorkflowEngine) -> None:
        ...
        engine.register_task(
            "dataset_creation.validate_file_format", cls._validate_file_format_task
        )
        engine.register_task(
            "dataset_creation.upload_file_to_storage", cls._upload_file_to_storage_task
        )
        engine.register_task("dataset_creation.infer_schema", cls._infer_schema_task)
        engine.register_task("dataset_creation.extract_sample_data", cls._extract_sample_data_task)
        engine.register_task(
            "dataset_creation.create_dataset_record", cls._create_dataset_record_task
        )
        engine.register_task(
            "dataset_creation.link_dataset_to_asset", cls._link_dataset_to_asset_task
        )
        engine.register_task("dataset_creation.index_for_search", cls._index_for_search_task)
        engine.register_task("dataset_creation.send_notifications", cls._send_notifications_task)
        engine.register_task("dataset_creation.audit_logging", cls._audit_logging_task)

        # Compensation tasks
        engine.register_task(
            "dataset_creation.rollback_file_upload", cls._rollback_file_upload_task
        )
        engine.register_task(
            "dataset_creation.rollback_dataset_record", cls._rollback_dataset_record_task
        )
        engine.register_task("dataset_creation.rollback_indexing", cls._rollback_indexing_task)

    @staticmethod
    def _validate_file_format_task(
        ...
        input_data.get("triggered_by_id") or instance.created_by_id
    ...
    @staticmethod
    def _upload_file_to_storage_task(
        input_data: dict[str, Any], instance: WorkflowInstance, step
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            raise ValueError(f"File not found in storage: {e!s}")
    ...
    def _infer_schema_task(
        ...
        return {
            ...
            "row_count_estimated": schema_json.get("row_count_estimated")
    ...
    def _extract_sample_data_task(
        ...
        return {
            ...
            "state": {
                "sample_data_json": sample_data_json,
                "sample_rows_count": len(sample_data_json)
    ...
    @staticmethod
    def _create_dataset_record_task(
        input_data: dict[str, Any], instance: WorkflowInstance, step
    ) -> dict[str, Any]:
        """
        ...
        if asset:
            latest_dataset = (
                Dataset.objects.filter(tenant=tenant, asset=asset)
        ...
        return {
            "dataset_id": str(dataset.id),
            "version": dataset.version,
            "state": {"dataset_id": str(dataset.id), "version": dataset.version},
        }

    @staticmethod
    def _link_dataset_to_asset_task(
        input_data: dict[str, Any], instance: WorkflowInstance, step
    ) -> dict[str, Any]:
        """
        ...
        if asset_id and not dataset.asset:
            ...
            return {
                "linked": True,
                "asset_id": str(dataset.asset.id),
                "state": {"linked": True, "asset_id": str(dataset.asset.id)},
            }
        elif dataset.asset:
            logger.info(
            ...
            return {"linked": False, "state": {"linked": False}}
    ...
    def _send_notifications_task(
        ...
        """
        dataset_id = instance.state_data.get("dataset_id")
        instance.state_data.get("triggered_by_id")
    ...
    @staticmethod
    def _rollback_file_upload_task(
        input_data: dict[str, Any], instance: WorkflowInstance, step
    ) -> dict[str, Any]:
        """Rollback file upload (no-op, file upload is idempotent)"""
        logger.info("File upload rollback (no-op)", workflow_instance_id=str(instance.id))
        return {"rolled_back": True}

    @staticmethod
    def _rollback_dataset_record_task(
        input_data: dict[str, Any], instance: WorkflowInstance, step
    ) -> dict[str, Any]:
        ...
        try:
            try:
                ...
                is_migration_issue = (
                    "access_certifications" in error_str
                    or "does not exist" in error_str.lower()
                    or "ProgrammingError" in error_type
                )

                # Check if this is a transaction error
                is_transaction_error = (
                    "transaction" in error_str.lower() or "TransactionManagementError" in error_type
                )
                ...
                if is_migration_issue or is_transaction_error:
                    ...
                    return {
                        "rolled_back": True,
                        "warning": "Database migration issue - dataset deletion attempted",
                    }
                else:
            ...
            if "transaction" in error_str.lower() or "TransactionManagementError" in error_type:
                ...
                return {
                    "rolled_back": True,
                    "warning": "Transaction error - dataset deletion attempted",
                }
            else:
                logger.error(
