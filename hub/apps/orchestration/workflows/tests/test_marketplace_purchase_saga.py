...
class MarketplacePurchaseSagaTest(TestCase):
    ...
    def setUp(self):
        ...
        self.seller_tenant = Tenant.objects.create(
            name="Seller Tenant", slug="seller-tenant", kyc_status="VERIFIED"
        )
        ...
        self.listing = Listing.objects.create(
            asset=self.asset,
            tenant=self.seller_tenant,
            metadata_json={"title": "Test Listing", "price": 100.00, "currency": "USD"},
        ...
        result = compensate_order_creation(
            {
                ...
                "step_output": {"order_id": str(order.id), "order_status": OrderStatus.REQUESTED},
            }
        )
    ...
    def test_send_notification_failure_non_critical(self):
        ...
        with patch(
            "hub.apps.orchestration.workflows.marketplace_purchase_saga.logger.info"
        ) as mock_log:
    ...
    def test_full_saga_execution_failure_order_creation(self):
        ...
        with patch(
            "hub.apps.orchestration.workflows.marketplace_purchase_saga.Order.objects.create"
        ) as mock_create:
    ...
    def test_full_saga_execution_failure_entitlement_creation(self):
        ...
        with patch(
            "hub.apps.orchestration.workflows.marketplace_purchase_saga.Entitlement.objects.create"
        ) as mock_create:
            mock_create.side_effect = Exception("Database error")
