...
class WorkflowAlerting:
    ...
    def check_workflow_timeouts(
        self, timeout_threshold_seconds: int = 3600, workflow_name: str | None = None
    ) -> list[dict[str, Any]]:
        """
        ...
        query = Q(status=WorkflowStatus.RUNNING, started_at__lt=threshold_time)

        if workflow_name:
            query &= Q(workflow_name=workflow_name)

        timed_out_workflows = WorkflowInstance.objects.filter(query).select_related(
            "workflow_definition"
        )
    ...
    def check_failed_workflows(
        ...
        query = Q(status=WorkflowStatus.FAILED, completed_at__gte=window_start)
        ...
        failure_counts = (
            WorkflowInstance.objects.filter(query)
    ...
    def check_retry_exhaustion(self) -> list[dict[str, Any]]:
        ...
        for instance in exhausted_workflows:
            ...
            if (
                max_retries == 3
                and instance.workflow_definition
                and instance.workflow_definition.dsl_json
            ):
    ...
    def check_stuck_workflows(
        self, stuck_threshold_minutes: int = 30, workflow_name: str | None = None
    ) -> list[dict[str, Any]]:
        """
        ...
        query = Q(status=WorkflowStatus.RUNNING, updated_at__lt=threshold_time)

        if workflow_name:
            query &= Q(workflow_name=workflow_name)

        stuck_workflows = WorkflowInstance.objects.filter(query).select_related(
            "workflow_definition"
        )
        ...
        for instance in stuck_workflows:
            ...
            if current_step:
                ...
                step_started_at = (
                    current_step.started_at.isoformat() if current_step.started_at else None
                )
    ...
    def _is_validation_related_failure(
        ...
        if error_message and (
            ...
            return True
        if error_details:
            msg = str(error_details.get("error_message") or error_details.get("message") or "")
    ...
    def check_validation_failure_rate(
        ...
        started_by = {
            (r["workflow_name"], r["workflow_version"]): r["started"] for r in started_in_window
        }
    ...
    def check_step_failure_rate(
        ...
        query = Q(status=StepStatus.FAILED, completed_at__gte=window_start)
        ...
        step_stats = (
            WorkflowStep.objects.filter(query)
            .values(
                "workflow_instance__workflow_name",
