        ...
        base_dict.update(
            {
                ...
                "workflow_definition_id": str(self.workflow_definition.id)
...
class OrchestrationBusinessRules(BusinessRules):
    ...
    def validate(
        ...
        if isinstance(context, OrchestrationRuleExecutionContext):
            ...
            if context and hasattr(context, "metadata") and isinstance(context.metadata, dict):
                ...
                workflow_definition = workflow_definition or context.metadata.get(
                    "workflow_definition"
                )
        ...
        if workflow and validation_type in ("state_management", "all"):
            ...
            state_result = self.validate_state_management(
                workflow, validation_type=state_management_type, tenant=tenant, user=user
            )
    ...
    def _validate_workflow_instance(
        self, workflow: WorkflowInstance, tenant: Any | None = None, user: User | None = None
    ) -> ValidationResult:
        """
            ...
            try:
                ...
                if workflow_def is None:
                    ...
                    result.errors.append(
                        "Workflow instance references a non-existent workflow_definition"
                    )
                else:
                ...
                result.is_valid = False
                result.errors.append(f"Error accessing workflow instance's definition: {e!s}")
    ...
    def _validate_workflow_step(
        self, step: WorkflowStep, tenant: Any | None = None, user: User | None = None
    ) -> ValidationResult:
        """
        ...
        if not step.workflow_instance_id:
            ...
            try:
                ...
                if workflow is None:
                    result.is_valid = False
                    result.errors.append(
                        "Workflow step references a non-existent workflow_instance"
                    )
            except Exception as e:
                result.is_valid = False
                result.errors.append(f"Error accessing workflow step's workflow_instance: {e!s}")
        ...
        if step.workflow_instance and step.step_index == step.workflow_instance.current_step_index:
            ...
            if (
                ...
                and step.workflow_instance.status != WorkflowStatus.RUNNING
            ):
                result.warnings.append(
        ...
        if user and step.workflow_instance:
            ...
            if (
                step.workflow_instance.tenant_id
                and user.tenant_id != step.workflow_instance.tenant_id
            ):
                result.warnings.append(
                    f"User ({user.id}) belongs to different tenant ({user.tenant_id}) "
    ...
    def _validate_state_persistence(
        ...
        if workflow is None:
            result.is_valid = False
            result.errors.append(
                "Workflow instance cannot be None for state persistence validation"
            )
            return result
        ...
        try:
            ...
            result.is_valid = False
            result.errors.append(f"Failed to persist workflow state: {e!s}")
            return result
        ...
        try:
            ...
            result.is_valid = False
            result.errors.append(f"Failed to reload workflow from database: {e!s}")
            return result
        ...
        if not isinstance(workflow.state_data, dict):
            result.is_valid = False
            result.errors.append(
                "Workflow state_data is not a dictionary after reload from database"
            )
            return result
        ...
        try:
            ...
            result.is_valid = False
            result.errors.append(f"Workflow state_data is not JSON serializable: {e!s}")
            return result
    ...
    def _validate_state_recovery(
        ...
        try:
            ...
            result.is_valid = False
            result.errors.append(f"Failed to persist workflow state before recovery test: {e!s}")
            return result
        ...
        try:
            ...
            result.is_valid = False
            result.errors.append(f"Failed to reload workflow from database for recovery: {e!s}")
            return result
        ...
        if workflow.id != workflow_id:
            result.is_valid = False
            result.errors.append(
                f"Workflow ID mismatch after recovery: expected {workflow_id}, got {workflow.id}"
            )
            return result
        ...
        try:
            ...
            result.is_valid = False
            result.errors.append(f"Failed to modify state_data after recovery: {e!s}")
            return result
    ...
    def _validate_state_consistency(
        ...
        if workflow is None:
            result.is_valid = False
            result.errors.append(
                "Workflow instance cannot be None for state consistency validation"
            )
            return result
        ...
        try:
            ...
            if (
                hasattr(workflow, "_prefetched_objects_cache")
                and "steps" in workflow._prefetched_objects_cache
            ):
            ...
            logger.warning(
                f"Error fetching workflow steps: {e!s}", extra={"workflow_id": str(workflow.id)}
            )
            result.warnings.append(f"Could not fetch workflow steps: {e!s}")
            steps = []  # Empty list to prevent errors below
        ...
        if steps and "current_step_name" in workflow.state_data:
            ...
            try:
                ...
                result.warnings.append(f"Could not validate step name consistency: {e!s}")
        ...
        if steps:
            try:
                ...
                result.warnings.append(f"Could not validate step output_data consistency: {e!s}")
        ...
        try:
            ...
            result.warnings.append(f"Could not validate state data size: {e!s}")
    ...
    def _validate_workflow_definition(
        ...
        errors = []
        warnings = []
        details = {"workflow_definition_validation": "workflow_definition", "validation_checks": {}}
    ...
    def _validate_workflow_structure(
        ...
        errors = []
        warnings = []
        details = {"structure_validation": "structure", "dsl_valid": False}
        ...
        try:
            ...
            errors.append(f"Invalid workflow DSL structure: {e!s}")
    ...
    def _validate_workflow_steps(self, workflow_definition: WorkflowDefinition) -> ValidationResult:
        """
        ...
        errors = []
        warnings = []
        details = {"steps_validation": "steps", "steps_valid": False}
        ...
        for i, step in enumerate(steps):
            ...
            if step_type == "task":
                if "task" not in step:
                    step_errors.append(
                        f"Step '{step_name or f'index_{i}'}' (type: task) must have 'task' field"
                    )
                else:
                    task_name = step.get("task")
                    if not isinstance(task_name, str) or not task_name.strip():
                        step_errors.append(
                            f"Step '{step_name or f'index_{i}'}' task field must be a non-empty string"
                        )

            elif step_type == "parallel":
                if "steps" not in step:
                    step_errors.append(
                        f"Step '{step_name or f'index_{i}'}' (type: parallel) must have 'steps' field"
                    )
                elif not isinstance(step.get("steps"), list) or len(step.get("steps", [])) == 0:
                    step_errors.append(
                        f"Step '{step_name or f'index_{i}'}' (type: parallel) must have at least one step in 'steps'"
                    )

            elif step_type == "conditional":
                if "condition" not in step:
                    step_errors.append(
                        f"Step '{step_name or f'index_{i}'}' (type: conditional) must have 'condition' field"
                    )
                if "then" not in step:
                    step_errors.append(
                        f"Step '{step_name or f'index_{i}'}' (type: conditional) must have 'then' field"
                    )
                ...
                if "steps" not in step:
                    step_errors.append(
                        f"Step '{step_name or f'index_{i}'}' (type: loop) must have 'steps' field"
                    )

            elif step_type == "retry":
                if "max_retries" not in step:
                    step_errors.append(
                        f"Step '{step_name or f'index_{i}'}' (type: retry) must have 'max_retries' field"
                    )
                else:
                    max_retries = step.get("max_retries")
                    if not isinstance(max_retries, int) or max_retries < 0:
                        step_errors.append(
                            f"Step '{step_name or f'index_{i}'}' max_retries must be a non-negative integer"
                        )
                if "steps" not in step:
                    step_errors.append(
                        f"Step '{step_name or f'index_{i}'}' (type: retry) must have 'steps' field"
                    )
    ...
    def _validate_workflow_cycles(
        ...
        errors = []
        warnings = []
        details = {"cycles_validation": "cycles", "has_cycles": False}
    ...
    def _validate_workflow_resources(
        ...
        errors = []
        warnings = []
        details = {"resources_validation": "resources", "resources_valid": False}
        ...
        for dep_name in dependencies:
            if not isinstance(dep_name, str):
                continue
            try:
                dep_def = WorkflowDefinition.objects.filter(name=dep_name, is_active=True).first()
                if not dep_def:
                    missing_workflows.append(dep_name)
            except Exception as e:
                warnings.append(f"Error checking workflow dependency '{dep_name}': {e!s}")
    ...
    def validate_step_input(
        ...
        try:
            ...
            result.is_valid = False
            result.errors.append(f"Step input is not JSON serializable: {e!s}")
    ...
    def validate_step_output(
        ...
        try:
            ...
            result.is_valid = False
            result.errors.append(f"Step output is not JSON serializable: {e!s}")
    ...
    def _workflow_state_cache_key(
        ...
        * the user id (for permission checks).
        ...
        """
        try:
            state_repr = json.dumps(workflow.state_data or {}, sort_keys=True, default=str)
