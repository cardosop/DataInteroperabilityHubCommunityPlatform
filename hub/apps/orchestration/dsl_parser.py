...
class WorkflowDSLParser:
    ...
    def parse(cls, dsl_content: str, format: str = "json") -> Dict[str, Any]:
        ...
        try:
            ...
            raise ValidationError(f"Invalid {format.upper()} format: {str(e)}")
    ...
    def _validate_workflow(cls, workflow_def: Dict[str, Any]) -> None:
        ...
        if missing_fields:
            raise ValidationError(
                f"Missing required workflow fields: {', '.join(missing_fields)}"
            )
    ...
    def _validate_conditional_step(cls, step: Dict[str, Any], step_index: int) -> None:
        ...
        if "condition" not in step:
            raise ValidationError(f"Step {step_index} (type: conditional) must have 'condition' field")
        if "then" not in step:
