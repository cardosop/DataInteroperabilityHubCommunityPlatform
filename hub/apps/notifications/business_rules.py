...
class NotificationsRuleExecutionContext(RuleExecutionContext):
    ...
    notification: EmailDelivery | None = None
...
class NotificationsBusinessRules(BusinessRules):
    ...
    def validate(
        ...
        if notification and validation_type in (
        ...
        if not validated_items:
            return ValidationResult(
                is_valid=False,
                errors=[
                    "At least one of notification, template, recipient, tenant, or user must be provided"
                ],
    ...
    def _validate_notification_basic(
        self, notification: EmailDelivery, tenant: Any | None = None, user: User | None = None
    ) -> ValidationResult:
        """
    ...
    def _validate_template_basic(self, template: str) -> ValidationResult:
        """
    ...
    def _validate_recipient_basic(self, recipient: str) -> ValidationResult:
        """
    ...
    def validate_delivery_channel(
        self, channel: str, notification: EmailDelivery | None = None
    ) -> ValidationResult:
        """
        ...
        errors = []
        warnings = []
        details = {"channel": channel, "validation_type": "delivery_channel"}
        ...
        if channel_upper == "EMAIL":
            ...
            if notification and not notification.to_email:
                errors.append("EMAIL channel requires a valid recipient email address")
            ...
            warnings.append(
                "IN_APP channel requires a valid user ID. User ID validation not yet implemented."
            )
    ...
    def validate_recipient(self, recipient: str, channel: str = "EMAIL") -> ValidationResult:
        """
        ...
        errors = []
        warnings = []
        details = {"recipient": recipient, "channel": channel, "validation_type": "recipient"}
        ...
        if channel_upper == "EMAIL":
            ...
            try:
                ...
                errors.append(f"Invalid email address '{recipient}': {', '.join(e.messages)}")
                details["email_format_valid"] = False
            ...
            if "@" not in recipient:
                errors.append(
                    f"Recipient '{recipient}' does not appear to be a valid email address"
                )
            else:
                ...
                if not domain or "." not in domain:
                    errors.append(
                        f"Email domain '{domain}' is invalid. Must contain at least one dot."
                    )
                elif len(domain) > 255:
                    errors.append(
                        f"Email domain '{domain}' is too long. Maximum length is 255 characters."
                    )
            ...
            if re.match(phone_pattern, recipient.replace(" ", "").replace("-", "")):
                ...
                errors.append(
                    f"Invalid phone number '{recipient}'. Expected E.164 format (e.g., +1234567890)"
                )
            ...
            try:
                ...
                errors.append(f"Invalid user ID '{recipient}'. User ID must be a valid UUID.")
                details["user_id_format_valid"] = False

        else:
            errors.append(
                f"Unknown delivery channel '{channel}'. Cannot validate recipient format."
            )
    ...
    def validate_delivery_rate_limiting(
        ...
        RATE_LIMIT_PER_USER = getattr(settings, "NOTIFICATION_RATE_LIMIT_PER_USER", 20)  # per hour
        ...
        if channel.upper() == "EMAIL":
            recent_deliveries = EmailDelivery.objects.filter(
                to_email=recipient, created_at__gte=window_start
            ).count()
    ...
    def validate_delivery_status(
        self, notification: EmailDelivery, new_status: str | None = None
    ) -> ValidationResult:
        """
    ...
    def validate_preference_structure(self, preferences: dict[str, Any]) -> ValidationResult:
        """
        ...
        if not isinstance(preferences, dict):
            errors.append(f"Preferences must be a dictionary, got {type(preferences).__name__}")
        ...
        if "email_types" in preferences:
            ...
            if not isinstance(email_types, dict):
                ...
                for email_type_key, enabled in email_types.items():
                    ...
                    if hasattr(email_type_key, "value") and not isinstance(email_type_key, str):
                        ...
                        email_type = (
                            email_type_key[0] if len(email_type_key) > 0 else str(email_type_key)
                        )
                    else:
            ...
            warnings.append(
                "No email type preferences specified. Defaulting to all email types enabled."
            )
            details["email_types"] = []
        ...
        if "opt_out" in preferences:
            ...
            if not isinstance(opt_out, bool):
                errors.append(f"opt_out preference must be a boolean, got {type(opt_out).__name__}")
    ...
    def validate_preference_update(
        self, user: User, preferences: dict[str, Any], requesting_user: User | None = None
    ) -> ValidationResult:
        """
        ...
        if requesting_user:
            ...
            # Check if requesting user is platform admin
                ...
                hasattr(requesting_user, "is_platform_admin") and requesting_user.is_platform_admin
            ):
                details["can_update"] = True
                details["update_reason"] = "admin_update"
            # Check if requesting user is in the same tenant and has admin privileges
            elif (
                hasattr(requesting_user, "tenant_id")
                and hasattr(user, "tenant_id")
                and requesting_user.tenant_id == user.tenant_id
            ):
                ...
                is_admin = getattr(requesting_user, "is_staff", False) or getattr(
                    requesting_user, "is_tenant_admin", False
                )
                if is_admin:
                    details["can_update"] = True
