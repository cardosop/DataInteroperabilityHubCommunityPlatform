...
class SendGridEmailServiceTest(EmailServiceTest):
    ...
    def test_sendgrid_missing_api_key(self):
        ...
        with self.settings(EMAIL_BACKEND="sendgrid", SENDGRID_API_KEY=None):
    ...
    def test_sendgrid_send_failure(self, mock_client_class):
        ...
        with self.settings(
            ...
            service = SendGridEmailService()
            with (
                self.assertRaises(EmailServiceError) as cm,
...
class SESEmailServiceTest(EmailServiceTest):
    ...
    def test_send_email_success(self, mock_boto_client):
        ...
        self.assertEqual(ses_kwargs["Destination"]["ToAddresses"], [self.to_email])
        self.assertEqual(ses_kwargs["Message"]["Subject"]["Data"], self.subject)

    def test_ses_missing_region(self):
        """Test error when AWS_SES_REGION is missing"""
        with self.settings(EMAIL_BACKEND="ses", AWS_SES_REGION=None):
            with self.assertRaises(EmailServiceError) as cm:
                SESEmailService()
...
class EmailServiceFactoryTest(TestCase):
    ...
    def test_get_email_service_ses(self):
        ...
        with self.settings(
            EMAIL_PROVIDER="ses", AWS_SES_REGION="us-east-1", AWS_SES_FROM_EMAIL="test@example.com"
        ):
            service = get_email_service()
    ...
    def test_get_email_service_smtp(self):
        ...
        with self.settings(
            EMAIL_PROVIDER="smtp", SMTP_HOST="localhost", SMTP_FROM_EMAIL="test@example.com"
        ):
            service = get_email_service()
            self.assertIsInstance(service, SMTPEmailService)

    def test_get_email_service_invalid_backend(self):
        """Test error for invalid EMAIL_PROVIDER"""
        with self.settings(EMAIL_PROVIDER="invalid"):
            with self.assertRaises(EmailServiceError) as cm:
                get_email_service()
            self.assertIn("Invalid EMAIL_BACKEND", str(cm.exception))

    def test_get_email_service_missing_backend(self):
        """Test error when neither EMAIL_PROVIDER nor EMAIL_BACKEND are set"""
        with self.settings(EMAIL_PROVIDER=None, EMAIL_BACKEND=None):
            with self.assertRaises(EmailServiceError) as cm:
                get_email_service()
