...
class DQRuleExecutionContext(RuleExecutionContext):
    ...
    dataset: Any | None = None  # Using Any to avoid circular import
        ...
        base_dict.update(
            {
                ...
                "check_name": self.check.get("name")
                if self.check and isinstance(self.check, dict)
                else None,
...
class DQBusinessRules(BusinessRules):
    ...
    def _validate_dq_run(
        self, dq_run: DQRun, tenant: Any | None = None, user: User | None = None
    ) -> ValidationResult:
        """
        Validate DQ run.
    ...
    def _validate_dq_check_config(
        ...
        if not isinstance(check, dict):
            ...
            return ValidationResult(
                is_valid=False, errors=result.errors, warnings=result.warnings, details=details
            )
    ...
    def _validate_check_type(self, check: dict[str, Any]) -> ValidationResult:
        """
    ...
    def _validate_check_configuration(self, check: dict[str, Any]) -> ValidationResult:
        """
    ...
    def _validate_check_schema_compatibility(
        self, check: dict[str, Any], dataset: Any
    ) -> ValidationResult:
        """
        ...
        if not isinstance(schema_fields, list):
            warnings.append(
                "Dataset schema_json.fields is not a list - cannot validate schema compatibility"
            )
        ...
        if target_level == "COLUMN":
            if target_column:
                ...
                try:
                    ...
                    errors.append(f"DQ check has invalid column pattern '{target_pattern}': {e!s}")
    ...
    def validate_scorecard_calculation(
        ...
        # Validate thresholds
        threshold_result = self._validate_scorecard_thresholds(scorecard_data, tenant, asset)
        result = result.combine(threshold_result)
        details["thresholds_validated"] = True

        # Validate update triggers
        triggers_result = self._validate_scorecard_update_triggers(tenant, asset)
    ...
    def _validate_scorecard_aggregation_rules(
        ...
        scorecard_data: dict[str, Any] | None,
        tenant: Any | None,
        asset: Any | None,
    ) -> ValidationResult:
        """
    ...
    def _validate_scorecard_thresholds(
        self, scorecard_data: dict[str, Any] | None, tenant: Any | None, asset: Any | None
    ) -> ValidationResult:
        """
    ...
    def _validate_scorecard_update_triggers(
        self, tenant: Any | None, asset: Any | None
    ) -> ValidationResult:
        """
        ...
        if tenant:
            ...
            try:
                ...
                dq_runs_count = DQRun.objects.filter(
                    tenant=tenant, status=DQRunStatus.SUCCEEDED
                ).count()
                ...
                warnings.append(f"Could not validate tenant DQ runs: {e!s}")
        ...
        if asset:
            ...
            try:
                ...
                asset_dq_runs_count = DQRun.objects.filter(
                    asset=asset, status=DQRunStatus.SUCCEEDED
                ).count()
                ...
                if asset_dq_runs_count == 0:
                    ...
                    recent_dq_runs_count = DQRun.objects.filter(
                        asset=asset, status=DQRunStatus.SUCCEEDED, completed_at__gte=recent_date
                    ).count()
                ...
                warnings.append(f"Could not validate asset DQ runs: {e!s}")
    ...
    def _validate_dq_run_eligibility(
        self, dq_run: DQRun, user: User | None = None, tenant: Any | None = None
    ) -> ValidationResult:
        """
    ...
    def _validate_dq_run_resource_quota(
        self, dq_run: DQRun, tenant: Any | None = None
    ) -> ValidationResult:
        """
        ...
        has_quota, quota_info = QuotaManager.check_quota(
            tenant_id=tenant_id, category=EndpointCategory.DQ_RUN, window=TimeWindow.DAILY
        )
        ...
        concurrent_runs = (
            DQRun.objects.filter(
                tenant_id=tenant_id, status__in=[DQRunStatus.PENDING, DQRunStatus.RUNNING]
            )
    ...
    def _validate_dq_run_schedule(
        ...
        if not effective_tenant:
            ...
            recent_runs_count = (
                DQRun.objects.filter(tenant_id=tenant_id, created_at__gte=recent_time)
                .exclude(id=dq_run.id if dq_run.id else None)
                .count()
            )
    ...
    def _validate_dq_run_status_transition(
        self, dq_run: DQRun, current_status: str | None = None, new_status: str | None = None
    ) -> ValidationResult:
        """
        ...
        if new_status:
            ...
            allowed_transitions = {
                ...
                DQRunStatus.RUNNING: [DQRunStatus.SUCCEEDED, DQRunStatus.FAILED],
        ...
        elif current_status == DQRunStatus.PENDING:
