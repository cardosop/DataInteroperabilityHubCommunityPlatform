...
from hub.apps.dq.models import DQAnomaly, DQAnomalySeverity, DQEngine, DQRun, DQRunStatus
...
class RootCauseAnalyzerTest(TestCase):
    ...
    def test_analyze_root_cause_check_failure(self):
        ...
        dq_run = self._create_dq_run(
            quality_score=60.0, overall_status="FAIL", checks_json=checks_json
        )
    ...
    def test_analyze_root_cause_schema_change(self):
        ...
        dq_run = self._create_dq_run(quality_score=70.0, overall_status="FAIL")
        ...
        self.assertEqual(details["added_fields"], ["col2"], "col2 was added in the child schema")
        self.assertTrue(details["has_changes"], "schema difference must set has_changes=True")
    ...
    def test_analyze_root_cause_volume_change(self):
        ...
        self.assertEqual(
            details["current_count"], 5000, "current row_count must be the updated 5000"
        )
        self.assertEqual(
            details["average_count"],
            1000,
    ...
    def test_analyze_root_cause_anomaly_correlation(self):
        ...
        anomaly_correlations = [
            c for c in analysis["root_causes"] if c["type"] == "ANOMALY_CORRELATION"
        ]
        self.assertGreaterEqual(len(anomaly_correlations), 1)
        details = anomaly_correlations[0]["details"]
        self.assertEqual(details["anomaly_count"], 3, "must report all 3 created anomalies")
        self.assertEqual(details["high_severity_count"], 3, "all 3 anomalies have HIGH severity")

    def test_analyze_root_cause_confidence_scores(self):
        """Test confidence score calculation"""
        checks_json = [{"name": "test_check", "type": "column", "status": "FAIL", "result": False}]
    ...
    def test_analyze_root_cause_recommendations(self):
        ...
        checks_json = [
            {"name": "expect_column_values_to_not_be_null", "type": "column", "status": "FAIL"}
        ]
