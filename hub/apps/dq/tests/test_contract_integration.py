...
class ContractQualityRulesExtractionTest(TestCase):
    ...
    def test_get_contract_profile_key_with_default(self):
        ...
        profile_key = ContractQualityRulesExtractor.get_contract_profile_key(
            self.contract_with_rules, fallback="intake_basic_gx"
        )
    ...
    def test_get_contract_profile_key_without_default(self):
        ...
        profile_key = ContractQualityRulesExtractor.get_contract_profile_key(
            self.contract_without_rules, fallback="intake_basic_gx"
        )
    ...
    def test_parse_rule_expression_greater_than(self):
        ...
        parsed = ContractQualityRulesExtractor.parse_rule_expression("id > 0", field_name="id")
    ...
    def test_parse_rule_expression_in_values(self):
        ...
        parsed = ContractQualityRulesExtractor.parse_rule_expression(
            "status IN ('active', 'pending')", field_name="status"
        )
...
class ContractQualityRulesTransformationTest(TestCase):
    """Test transformation of contract quality rules into DQ structures (GAP-8.2.1).

    Tests the :class:`ContractQualityRulesExtractor` data-transformation
    layer — converting contract ``quality.rules`` entries into
    ``DQCheck`` objects and resolving profile keys.  Does NOT call the
    DQ service; that end-to-end path is exercised by the integration
    tests.
    """
    ...
    def test_extract_quality_rules_and_checks_from_contract(self):
        ...
        profile_key = ContractQualityRulesExtractor.get_contract_profile_key(
            self.contract, fallback="intake_basic_gx"
        )
        self.assertEqual(profile_key, "custom_profile")
    ...
    def test_custom_rules_properly_formatted_for_dq_service(self):
        ...
        # Verify the check can be serialized for the DQ service wire format
        # (value accessors must not raise, and all required keys must be
        # present).
        check_dict = {
        ...
        # Verify required keys have non-None values.
        for key in ("check_id", "category", "severity", "expectation_type"):
            self.assertIsNotNone(check_dict.get(key), f"Serialized key '{key}' is None")
    ...
    def test_contract_profile_key_with_fallback(self):
        ...
        contract_no_profile = Contract.objects.create(
            ...
            status=ContractStatus.DRAFT,
            hub_contract_version="1.0.0",
            hub_contract_json={"info": {"name": "Test Contract"}, "schema": {"fields": []}},
        )

        profile_key_fallback = ContractQualityRulesExtractor.get_contract_profile_key(
            contract_no_profile, fallback="intake_basic_gx"
        )
