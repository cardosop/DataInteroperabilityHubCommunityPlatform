...
class DQBusinessRulesInitializationTest(TestCase):
...
class CheckTypeValidationTest(TestCase):
    ...
    def test_validate_check_type_all_valid_types(self):
        ...
        valid_types = ['COMPLETENESS', 'ACCURACY', 'CONSISTENCY', 'VALIDITY', 'UNIQUENESS', 'TIMELINESS']
...
class CheckConfigurationValidationTest(TestCase):
...
class CheckSchemaCompatibilityValidationTest(TestCase):
...
class CheckResourceValidationTest(TestCase):
    ...
    def test_validate_check_resource_dataset_direct(self):
        ...
        result = self.rules._validate_check_resource(check, None, self.dataset, self.tenant, self.user)
...
class DQCheckConfigurationValidationIntegrationTest(TestCase):
    ...
    def test_validate_dq_check_config_comprehensive_invalid(self):
        ...
        check = {
            ...
            'target': {
                'level': 'COLUMN',
                'column': 'nonexistent_column'  # Column doesn't exist
            },
            'expectation': {
                'type': 'expect_column_values_to_not_be_null',
                'params': {
...
class DQRunExecutionValidationTest(TestCase):
    ...
    def test_validate_dq_run_eligibility_dataset_no_file(self):
        ...
        # Skip this test - Dataset model requires file_id, cannot be null
        # This test validates the business rule logic, but the model constraint
        # prevents creating a dataset without a file
        # In practice, this scenario would be caught at the model level
        self.skipTest("Dataset model requires file_id - cannot test dataset without file scenario")

        dq_run = DQRun.objects.create(
            tenant=self.tenant,
            dataset=dataset_with_file,
            job=self.job,
            profile_key="intake_basic_gx",
            engine=DQEngine.GREAT_EXPECTATIONS,
            status=DQRunStatus.PENDING
...
class DQRunExecutionIntegrationTest(TestCase):
...
class ScorecardCalculationValidationTest(TestCase):
    ...
    def test_validate_scorecard_aggregation_rules_valid_config(self):
        ...
        aggregation_config = {
            'functions': ['Avg', 'Count', 'Sum', 'Min', 'Max']
        }
    ...
    def test_validate_scorecard_aggregation_rules_invalid_functions(self):
        ...
        aggregation_config = {
            'functions': ['InvalidFunction', 'AnotherInvalid']
        }
    ...
    def test_validate_scorecard_aggregation_rules_score_out_of_range(self):
        ...
        scorecard_data = {
            'summary': {
                'avg_quality_score': 150.0  # Out of range (0-100)
            }
        }
...
class ScorecardThresholdValidationTest(TestCase):
    ...
    def test_validate_scorecard_thresholds_default_thresholds(self):
        ...
        result = self.rules._validate_scorecard_thresholds(
            None, self.tenant, None
        )
    ...
    def test_validate_scorecard_thresholds_custom_thresholds(self):
        ...
        result = self.rules._validate_scorecard_thresholds(
            scorecard_data, self.tenant, None
        )
    ...
    def test_validate_scorecard_thresholds_status_matching(self):
        ...
        scorecard_data = {
            'summary': {
                'avg_quality_score': 85.0  # Should be PASS (>= 80)
            },
            'overall_status': 'PASS',
            'thresholds': {
                'pass_threshold': 80.0,
...
class ScorecardUpdateTriggersValidationTest(TestCase):
    ...
    def test_validate_scorecard_update_triggers_with_tenant(self):
        ...
        result = self.rules._validate_scorecard_update_triggers(
            self.tenant, None
        )
...
class ScorecardCalculationValidationIntegrationTest(TestCase):
