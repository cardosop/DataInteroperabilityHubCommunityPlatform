...
class DQScorecardServiceTest(TestCase):
    ...
    def _create_dq_run(
        self, quality_score: float, overall_status: str = "PASS", completed_at=None
    ) -> DQRun:
    ...
    def test_get_asset_scorecard_recent_runs(self):
        ...
        self.assertGreaterEqual(
            len(recent_runs), 2, "Expected at least 2 recent runs for ordering check"
        )
    ...
    def test_drill_down_dataset(self):
        ...
        # Drill down
        drill_down = DQScorecardService.drill_down(
            str(self.tenant.id), dataset_id=str(self.dataset.id), days=30
        )
