...
class TrendAnalyzer:
    ...
    def calculate_trend(
        ...
        dq_runs = TrendAnalyzer._get_dq_runs(
            asset_id=asset_id, dataset_id=dataset_id, tenant_id=tenant_id, periods=periods
        )
        ...
        period_groups = TrendAnalyzer._group_by_period(
            dq_runs, period_type=period_type, metric_type=metric_type
        )
        ...
        for period_start, period_data in sorted(period_groups.items()):
            ...
            trend = DQTrend(
                ...
                metadata={"run_count": period_data["count"], "period_index": len(trends)},
            )
            trends.append(trend)
    ...
    @staticmethod
    def _get_dq_runs(
        asset_id: str | None, dataset_id: str | None, tenant_id: str, periods: int
    ) -> list[DQRun]:
        """Get DQ runs for trend analysis"""
        filter_q = Q(tenant_id=tenant_id, status=DQRunStatus.SUCCEEDED)
        ...
        return list(
            DQRun.objects.filter(filter_q, completed_at__gte=cutoff_date).order_by("completed_at")
        )

    @staticmethod
    def _group_by_period(
        dq_runs: list[DQRun], period_type: str, metric_type: str
        ...
        for run in dq_runs:
            ...
            # Get period start
            period_start = TrendAnalyzer._get_period_start(run.completed_at, period_type)
            ...
            if period_start not in period_groups:
                period_groups[period_start] = {"values": [], "count": 0}
    ...
    def _calculate_trend_strength(
        ...
        current_index = (
            sorted_periods.index(current_period) if current_period in sorted_periods else -1
        )
        ...
        for i in range(1, len(recent_periods)):
            ...
            if prev_period in period_groups and curr_period in period_groups:
                ...
                if (
                    (direction == DQTrendDirection.IMPROVING and curr_value > prev_value)
                    or (direction == DQTrendDirection.DEGRADING and curr_value < prev_value)
                    or (direction == DQTrendDirection.STABLE and abs(curr_value - prev_value) < 1.0)
                ):
                    consistent_count += 1
    ...
    @staticmethod
    def get_trend_visualization(trends: list[DQTrend], format: str = "json") -> Any:
        """
        ...
        if format == "json":
            ...
            lines = [
                "period_start,period_end,current_value,previous_value,change_amount,change_percent,direction,trend_strength,forecast_value"
            ]
            for trend in trends:
                lines.append(
