...
class DQScorecardService:
    ...
    @staticmethod
    def get_executive_dashboard(tenant_id: str, days: int = 30) -> dict[str, Any]:
        """
        ...
        dq_runs = DQRun.objects.filter(
            tenant_id=tenant_id, status=DQRunStatus.SUCCEEDED, completed_at__gte=start_date
        )

        # Aggregate metrics
        total_runs = dq_runs.count()
        avg_quality_score = dq_runs.aggregate(avg_score=Avg("quality_score"))["avg_score"] or 0.0
    ...
    @staticmethod
    def get_asset_scorecard(asset_id: str, tenant_id: str, days: int = 30) -> dict[str, Any]:
        """
        ...
        trends = DQTrend.objects.filter(
            tenant_id=tenant_id, asset_id=asset_id, period_start__gte=start_date
    ...
    def _get_trend_summary(tenant_id: str, days: int) -> dict[str, Any]:
        ...
        trends = DQTrend.objects.filter(
            tenant_id=tenant_id, period_start__gte=start_date, metric_type="quality_score"
        )
        ...
        return {
            ...
            "stable": stable_count,
            "improving_percent": round(
                (improving_count / total_trends * 100) if total_trends > 0 else 0.0, 2
            ),
            "degrading_percent": round(
                (degrading_count / total_trends * 100) if total_trends > 0 else 0.0, 2
            ),
        }
    ...
    def drill_down(
        ...
        query = Q(tenant_id=tenant_id, status=DQRunStatus.SUCCEEDED, completed_at__gte=start_date)
