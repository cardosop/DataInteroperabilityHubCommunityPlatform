...
from .models import DQAnomaly, DQRun, DQRunStatus
...
class RootCauseAnalyzer:
    ...
    @staticmethod
    def analyze_root_cause(dq_run: DQRun, lookback_days: int = 30) -> dict[str, Any]:
        """
    ...
    @staticmethod
    def _check_schema_changes(dq_run: DQRun, historical_runs: Any) -> dict[str, Any] | None:
        ...
        if dq_run.dataset.parent_version:
            ...
            if current_schema != previous_schema:
                # Compare schemas
                schema_diff = RootCauseAnalyzer._compare_schemas(previous_schema, current_schema)
    ...
    @staticmethod
    def _check_volume_changes(dq_run: DQRun, historical_runs: Any) -> dict[str, Any] | None:
        ...
        if current_row_count:
            ...
            if std_dev == 0:
                ...
                change_percent = (
                    ((current_row_count - avg_count) / avg_count * 100) if avg_count > 0 else 0
                )
    ...
    @staticmethod
    def _check_anomaly_correlation(dq_run: DQRun, historical_runs: Any) -> dict[str, Any] | None:
        """Check for correlation with anomalies"""
        # Get recent anomalies
        start_date = timezone.now() - timedelta(days=7)

        anomalies = DQAnomaly.objects.filter(tenant=dq_run.tenant, detected_at__gte=start_date)
        ...
        if anomaly_count > 0:
            ...
            high_severity_anomalies = anomalies.filter(severity__in=["CRITICAL", "HIGH"]).count()
    ...
    @staticmethod
    def _compare_schemas(old_schema: dict[str, Any], new_schema: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            or len(removed_fields) > 0
            or len(changed_fields) > 0,
    ...
    @staticmethod
    def _calculate_confidence(cause: dict[str, Any], historical_runs: Any) -> float:
    ...
    @staticmethod
    def _generate_recommendations(root_causes: list[dict[str, Any]]) -> list[str]:
        ...
        for cause in root_causes:
            ...
            if cause_type == "SCHEMA_CHANGE":
                ...
                recommendations.append(f"Review and fix the failing check: {check_name}")
        ...
        if not recommendations:
            recommendations.append("Review DQ run results and check-level failures for details")

        return recommendations

    @staticmethod
    def generate_root_cause_report(
        tenant_id: str, asset_id: str | None = None, days: int = 30
    ) -> dict[str, Any]:
        """
    ...
    @staticmethod
    def _get_common_recommendations(analyses: list[dict[str, Any]]) -> list[str]:
