...
class VirtualizationURLRoutingTest(TestCase):
    ...
    def test_virtual_dataset_custom_action_urls(self):
        ...
        url = reverse('virtual-dataset-validate-dataset', kwargs={'id': str(self.virtual_dataset.id)})
        self.assertEqual(url, f'/api/v1/virtualization/datasets/{self.virtual_dataset.id}/validate/')
    ...
    def test_query_execution_custom_action_urls(self):
        ...
        url = reverse('query-execution-cancel-execution', kwargs={'id': str(self.query_execution.id)})
    ...
    def test_no_duplicate_basenames(self):
        ...
        for basename in expected_basenames:
            self.assertIn(basename, basenames,
                         f"Expected basename '{basename}' not found in router registry")
    ...
    def test_all_viewset_actions_have_urls(self):
        ...
        if 'virtual-dataset' in viewsets:
            ...
            standard_actions = {
                ...
                'detail': {'id': str(self.virtual_dataset.id)},  # retrieve, update, partial_update, destroy
            }
