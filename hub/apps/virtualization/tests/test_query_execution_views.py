...
class QueryExecutionViewSetTest(TestCase):
    ...
    def setUp(self):
        ...
        self.query_execution = QueryExecution.objects.create(
            ...
            metrics={"duration_ms": 1500, "rows_processed": 100},
        )
    ...
    def test_cancel_query_execution(self):
        ...
        response = self.client.post(
            f"/api/v1/virtualization/queries/{running_execution.id}/cancel/", format="json"
        )
    ...
    def test_cancel_query_execution_not_cancellable(self):
        ...
        response = self.client.post(
            f"/api/v1/virtualization/queries/{self.query_execution.id}/cancel/", format="json"
        )
    ...
    def test_cancel_query_execution_tenant_isolation(self):
        ...
        response = self.client.post(
            f"/api/v1/virtualization/queries/{other_execution.id}/cancel/", format="json"
        )
    ...
    def test_get_query_execution_result(self):
        ...
        cache.set(
            cache_key,
            {"data": [{"id": 1, "name": "Test", "age": 25}], "row_count": 1},
            timeout=3600,
        )
    ...
    def test_get_query_execution_result_with_pagination(self):
        ...
        cache_key = f"query_result_{self.query_execution.id}"
        cache.set(cache_key, {"data": test_data, "row_count": len(test_data)}, timeout=3600)
        self.query_execution.result_cache_key = cache_key
        self.query_execution.save()
    ...
    def test_get_query_execution_progress_tenant_isolation(self):
        ...
        response = self.client.get(f"/api/v1/virtualization/queries/{other_execution.id}/progress/")
    ...
    def test_list_query_executions_with_status_filter(self):
        ...
        response = self.client.get("/api/v1/virtualization/queries/", {"status": "COMPLETED"})
    ...
    def test_list_query_executions_with_dataset_filter(self):
        ...
        response = self.client.get(
            "/api/v1/virtualization/queries/", {"virtual_dataset_id": str(self.virtual_dataset.id)}
        )
...
class VirtualDatasetQueryExecutionTest(TestCase):
    ...
    def test_execute_query_on_virtual_dataset(self):
        ...
        response = self.client.post(
            ...
            {"parameters": {"min_age": 18}, "execution_mode": QueryExecutionMode.SYNC},
            format="json",
        )
    ...
    def test_execute_query_on_virtual_dataset_minimal(self):
        ...
        response = self.client.post(
            f"/api/v1/virtualization/datasets/{self.virtual_dataset.id}/queries/", {}, format="json"
        )
    ...
    def test_execute_query_on_inactive_dataset(self):
        ...
        response = self.client.post(
            f"/api/v1/virtualization/datasets/{inactive_dataset.id}/queries/", {}, format="json"
        )
    ...
    def test_execute_query_tenant_isolation(self):
        ...
        response = self.client.post(
            f"/api/v1/virtualization/datasets/{other_dataset.id}/queries/", {}, format="json"
        )

        # Cross-tenant execute_query: the tenant-scoped queryset cannot find the
        # dataset in the user's tenant, so get_object() returns 404.
        self.assertEqual(
            response.status_code,
            status.HTTP_404_NOT_FOUND,
            f"Expected 404 for cross-tenant query execution; got {response.status_code}",
        )
    ...
    def test_abac_policy_denial_prevents_access(self):
        ...
        self.assertEqual(
            ...
            f"ABAC DENY policy should prevent query execution access. "
    ...
    def test_stream_result_sse_format(self):
        ...
        self.assertEqual(
            response.status_code,
            status.HTTP_200_OK,
            f"Streaming endpoint should return 200; got {response.status_code}: "
    ...
    def test_execute_query_with_legacy_parameter_binding(self):
        """Test query execution with legacy parameter interpolation mode.

        When PARAMETERIZED_VIRTUAL_QUERIES is disabled, the service falls back
        to _apply_parameters_legacy() for SQL injection prevention.
        """
        from django.test import override_settings

        self.client.force_authenticate(user=self.user)

        with override_settings(PARAMETERIZED_VIRTUAL_QUERIES=False):
            response = self.client.post(
                f"/api/v1/virtualization/datasets/{self.virtual_dataset.id}/queries/",
                {"parameters": {"min_age": 25, "status": "active"},
                 "execution_mode": QueryExecutionMode.SYNC},
                format="json",
            )

        # Legacy path should produce a valid response (201/200/202/400), never 500
        self.assertLess(
            response.status_code,
            status.HTTP_500_INTERNAL_SERVER_ERROR,
            f"Legacy parameter binding caused server error: {response.status_code} "
            f"{getattr(response, 'data', '')}",
        )
