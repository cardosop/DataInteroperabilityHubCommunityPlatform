...
class VirtualizationService(BaseService, VirtualizationEventPublisher):
    ...
    def _apply_parameters(
        ...
        Supports both :param and %(param)s style parameter substitution.
        ...
        Returns:
            Query string with parameters applied
        """
        if not parameters:
            return query
        ...
        try:
            if source_type == "postgresql":
                ...
                with engine.connect() as conn:
                    parameterized_query = self._apply_parameters(query, parameters)
                    result = conn.execute(text(parameterized_query))
                    columns = result.keys()
