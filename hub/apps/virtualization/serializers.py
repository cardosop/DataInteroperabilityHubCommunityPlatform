...
class VirtualDatasetCreateSerializer(serializers.Serializer):
    ...
    def validate_version(self, value):
        ...
        try:
            ...
            raise serializers.ValidationError(
                "Version parts must be numeric (e.g., '1.0.0')"
            )
...
class VirtualDatasetUpdateSerializer(serializers.Serializer):
    ...
    sources = serializers.JSONField(
        required=False,
        allow_null=True,
        help_text="Source system configurations"
    )
    version = serializers.CharField(
        max_length=50,
        required=False,
    ...
    status = serializers.ChoiceField(
        choices=VirtualDatasetStatus.choices,
        required=False,
        help_text="Virtual dataset status"
    )
...
class QueryExecutionSerializer(serializers.ModelSerializer):
    ...
    virtual_dataset_name = serializers.CharField(
        source='virtual_dataset.name',
        read_only=True,
        help_text="Name of the virtual dataset"
    )
    virtual_dataset = serializers.UUIDField(
        source='virtual_dataset.id',
        read_only=True,
        help_text="Virtual dataset ID"
    )
...
class QueryExecutionProgressSerializer(serializers.Serializer):
    ...
    progress_percentage = serializers.FloatField(
        required=False,
        allow_null=True,
        help_text="Progress percentage (0-100) if available"
    )
...
class VirtualizationHealthMetricsSerializer(serializers.Serializer):
    ...
    total_executions = serializers.IntegerField(
        help_text="Total number of query executions"
    )
    completed_executions = serializers.IntegerField(
        help_text="Number of completed executions"
    )
    ...
    recent_success_rate = serializers.FloatField(
        allow_null=True,
        help_text="Recent success rate (last 24 hours) percentage (0-100)"
    )
    ...
    is_active = serializers.BooleanField(
        help_text="Whether dataset is active"
    )
...
class VirtualizationTopologyNodeSerializer(serializers.Serializer):
    ...
    created_by_id = serializers.UUIDField(
        allow_null=True,
        help_text="User ID who created the dataset"
    )
    created_at = serializers.DateTimeField(
        allow_null=True,
        help_text="Dataset creation timestamp"
    )
    updated_at = serializers.DateTimeField(
        allow_null=True,
        help_text="Dataset last update timestamp"
    )
...
class VirtualizationTopologySummarySerializer(serializers.Serializer):
    ...
    average_health_score = serializers.FloatField(
        allow_null=True,
        help_text="Average health score across all datasets"
    )
...
class VirtualizationTopologySerializer(serializers.Serializer):
    ...
    edges = VirtualizationTopologyEdgeSerializer(many=True, help_text="List of dataset relationships")
...
class DatasetTopologySerializer(serializers.Serializer):
    ...
    relationships = VirtualizationTopologyEdgeSerializer(
        many=True,
        help_text="Relationships for this dataset"
    )
    health_metrics = VirtualizationHealthMetricsSerializer(
        allow_null=True,
        help_text="Dataset health metrics"
    )
