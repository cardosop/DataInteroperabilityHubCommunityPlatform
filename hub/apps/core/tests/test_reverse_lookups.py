...
class ReverseLookupTest(TestCase):
    ...
    def test_all_reverse_calls_resolvable(self):
        ...
        possible_paths = [
            Path(__file__).parent.parent.parent.parent / "reverse_lookup_analysis.json",  # From hub/apps/core/tests/
            Path(__file__).parent.parent.parent.parent.parent / "reverse_lookup_analysis.json",  # From project root
        ...
        for reverse_call in reverse_calls:
            ...
            if kwargs:
                ...
                has_expression_kwargs = any(
                    isinstance(v, str) and (
                        any(expr in v for expr in ['self.', 'uuid.', 'str(', 'int(', '.id', '.uuid4()']) or
                ...
                if has_expression_kwargs:
                    ...
                    try:
                        ...
                        error_msg = str(e)
                        if "with no arguments" in error_msg or "with keyword arguments" in error_msg:
                        ...
                        failures.append({
                            ...
                            "error": f"URL pattern not found: {str(e)}",
                        })
                        ...
                        failures.append({
                            ...
                            "file": reverse_call.get("file"),
                            "line": reverse_call.get("line"),
                            "error": f"Error resolving with dummy kwargs: {str(e)}",
                        })
                    continue
            ...
            if not kwargs and ("-detail" in url_name or "-analytics" in url_name or url_name.count("-") >= 2):
        ...
        if failures:
            failure_msg = "\n".join([
                f"  - {f['url_name']} in {f['file']}:{f['line']}: {f['error']}"
                for f in failures
            ])
    ...
    def test_reverse_lookup_mapping(self):
        ...
        if failures:
            failure_msg = "\n".join([
                f"  - {f['url_name']} -> {f['pattern_name']} (in {f['source']})"
                for f in failures
            ])
    ...
    def test_unmapped_lookups_are_documented(self):
        ...
        if problematic:
            failure_msg = "\n".join([
                f"  - {p['url_name']} (used in {len(p['sources'])} places)"
