...
class RlsHelpersTests(TestCase):
    def setUp(self):
        ...
        self.tenant_a = Tenant.objects.create(
            name="RLS-A", slug=f"rls-a-{uuid.uuid4().hex[:8]}",
            status=TenantStatus.ACTIVE,
        )
        self.tenant_b = Tenant.objects.create(
            name="RLS-B", slug=f"rls-b-{uuid.uuid4().hex[:8]}",
            status=TenantStatus.ACTIVE,
        )
    ...
    def test_assert_tenant_isolation_with_tenantplan(self):
        ...
        plan_a = TenantPlan.objects.create(
            slug=f"iso-a-{uuid.uuid4().hex[:8]}", name="Isolation Plan A",
            tier="FREE", order=0, is_active=True,
        )
