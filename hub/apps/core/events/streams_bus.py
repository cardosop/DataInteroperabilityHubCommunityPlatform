...
class EventStreamsBus:
    ...
    def __init__(self, redis_client: redis.Redis | None = None):
        ...
        self.consumer_group_prefix = getattr(
            settings, "EVENT_BUS_CONSUMER_GROUP_PREFIX", "event_consumers"
        )
        ...
        self.stream_max_length = getattr(
            settings, "EVENT_BUS_STREAM_MAX_LENGTH", 10000
    ...
    def _create_redis_client(self) -> redis.Redis:
        ...
        try:
            ...
            logger.error(
                "redis_connection_error", error=str(e), error_type=error_type, exc_info=True
            )
            raise
    ...
    def publish(
        ...
        try:
            ...
            logger.error(
                "event_stream_publish_error", event_type=event_type, error=str(e), exc_info=True
            )
    ...
    def create_consumer_group(self, stream_name: str, group_name: str, start_id: str = "0") -> bool:
        ...
        try:
            ...
            if "BUSYGROUP" in str(e):
                ...
                logger.debug(
                    "consumer_group_exists", stream_name=stream_name, group_name=group_name
                )
                return False
            raise
    ...
    def subscribe(
        ...
        try:
            ...
            _subscription, _created = EventSubscription.objects.update_or_create(
    ...
    def _listen_streams(
        ...
        while True:
            try:
                ...
                streams_with_ids = dict.fromkeys(streams.keys(), ">")
    ...
    def _handle_event(
        self, subscriber_name: str, event: dict[str, Any], handler: Callable[[dict[str, Any]], None]
    ) -> None:
        """
        ...
        while retries <= self.max_retries:
            try:
                ...
                event_handler_retry_count.labels(
                    event_type=event.get("event_type", "unknown"), subscriber_name=subscriber_name
                ).inc()
    ...
    def _send_to_dlq(self, subscriber_name: str, event: dict[str, Any], error: str) -> None:
        ...
        try:
            ...
            logger.error("dlq_error", subscriber_name=subscriber_name, error=str(e), exc_info=True)

    def get_pending_messages(
        self, stream_name: str, consumer_group: str, consumer_name: str | None = None
    ) -> list[dict[str, Any]]:
        """
    ...
    def replay_events(
        self, stream_name: str, start_id: str = "0", end_id: str = "+", count: int | None = None
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.error(
                "replay_events_error", stream_name=stream_name, error=str(e), exc_info=True
            )
            return []


def get_event_streams_bus(redis_client: redis.Redis | None = None) -> EventStreamsBus:
    """
