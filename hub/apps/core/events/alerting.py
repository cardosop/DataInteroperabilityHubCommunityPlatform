...
from .models import DeadLetterQueue, EventSubscription, Event
...
class EventBusAlerting:
    ...
    def check_publish_failures(
        ...
        try:
            ...
            recent_dlq = DeadLetterQueue.objects.filter(
            ...
            for entry in recent_dlq:
                alerts.append({
                    ...
                    'severity': 'warning' if entry['count'] < min_failure_count * 2 else 'critical',
    ...
    def check_consume_failures(
        ...
        try:
            ...
            ).values('subscriber', 'event_type').annotate(
                count=Count('id')
            ).filter(count__gte=min_failure_count)
    ...
    def check_dlq_size(
        ...
        try:
            unresolved_dlq_count = DeadLetterQueue.objects.filter(
                resolved_at__isnull=True
            ).count()
            
            if unresolved_dlq_count > max_size:
                # Get breakdown by event type and subscriber
                dlq_breakdown = DeadLetterQueue.objects.filter(
                    resolved_at__isnull=True
                ).values('event_type', 'subscriber').annotate(
                    count=Count('id')
                ).order_by('-count')[:10]
                ...
                alerts.append({
                    ...
                    'severity': 'critical' if unresolved_dlq_count > max_size * 2 else 'warning',
                    'timestamp': timezone.now().isoformat()
                })
        except Exception as e:
    ...
    def check_redis_connection_issues(
        self,
    ...
    def check_all_alerts(
        ...
        return {
            'publish_failures': self.check_publish_failures(
                min_failure_count=publish_failure_threshold,
                time_window_minutes=time_window_minutes
            ),
            'consume_failures': self.check_consume_failures(
                min_failure_count=consume_failure_threshold,
                time_window_minutes=time_window_minutes
            ),
