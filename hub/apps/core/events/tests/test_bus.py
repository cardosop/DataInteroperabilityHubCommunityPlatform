...
from hub.apps.core.events.bus import EventBus, EventPublishError
...
class EventBusTest(TestCase):
    ...
    def test_publish_event_success(self):
        ...
        payload = json.loads(
            call_args[0][1] if len(call_args[0]) > 1 else call_args[1].get("message")
        )
    ...
    def test_publish_event_invalid(self):
        ...
        with self.assertRaises(EventPublishError):
            self.event_bus.publish(event_type="InvalidEventType", data={})
    ...
    def test_replay_events_with_time_range(self):
        ...
        events = self.event_bus.replay_events(
            event_type=unique_type, start_time=now - timedelta(hours=1, minutes=30), end_time=now
        )
