...
class ErrorTrackerTest(TestCase):
    ...
    def test_tracker_initialization_with_sentry(
        self, mock_sentry, mock_redis, mock_logging, mock_django
    ):
    ...
    def test_set_user(self, mock_sentry):
        ...
        tracker.set_user(
            "user-id", email=f"user-{uuid.uuid4().hex[:8]}@example.com", username="user"
        )
