...
class ErrorRecoveryTest(TestCase):
    ...
    def test_calculate_backoff_exponential(self):
        ...
        delay1 = ErrorRecovery.calculate_backoff(
            0, RetryStrategy.EXPONENTIAL_BACKOFF, base_delay=1.0
        )
        delay2 = ErrorRecovery.calculate_backoff(
            1, RetryStrategy.EXPONENTIAL_BACKOFF, base_delay=1.0
        )
        delay3 = ErrorRecovery.calculate_backoff(
            2, RetryStrategy.EXPONENTIAL_BACKOFF, base_delay=1.0
        )
...
class CircuitBreakerTest(TestCase):
    ...
    def test_circuit_breaker_half_open_after_timeout(self):
        ...
        # Wait for timeout
        time.sleep(0.2)  # noqa: sleep-needed  # INTENTIONAL: wait for circuit breaker timeout to expire
