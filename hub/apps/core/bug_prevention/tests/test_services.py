...
class IdempotencyServiceTest(TestCase):
    ...
    def test_validate_key_format_invalid_characters(self):
        ...
        self.assertIsNotNone(error)
        self.assertIn(
            "characters", error.lower(),
            f"Error message should mention characters, got: {error}"
        )
    ...
    def test_check_idempotency_expired(self):
        ...
        record = IdempotencyKey.objects.create(
            ...
            method=self.method,
            path=self.path,
            request_fingerprint=IdempotencyKey.compute_fingerprint(self.method, self.path, self.body),
    ...
    def test_store_idempotency_update_existing(self):
        ...
        self.assertEqual(IdempotencyKey.objects.filter(
            tenant_id=self.tenant.id,
            idempotency_key=self.idempotency_key
        ).count(), 1)
...
class RequestDeduplicationServiceTest(TestCase):
    ...
    def test_cleanup_expired(self):
        ...
        self.assertEqual(deleted_count, 1)
        self.assertFalse(RequestDeduplication.objects.filter(request_fingerprint="expired-fingerprint").exists())
        self.assertTrue(RequestDeduplication.objects.filter(request_fingerprint="active-fingerprint").exists())
    ...
    def test_cleanup_expired_boundary_not_deleted(self):
        ...
        self.assertEqual(deleted_count, 0)
        self.assertTrue(RequestDeduplication.objects.filter(request_fingerprint="boundary-fingerprint").exists())
