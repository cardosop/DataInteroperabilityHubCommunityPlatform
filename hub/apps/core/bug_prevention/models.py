...
class IdempotencyKey(models.Model):
    ...
    idempotency_key = models.CharField(
        max_length=256, db_index=True, help_text="Client-provided idempotency key"
    )
    ...
    request_fingerprint = models.CharField(
        max_length=64, help_text="SHA-256 hash of method + path + body"
    )
    ...
    expires_at = models.DateTimeField(
        db_index=True, help_text="Expiry time (24 hours from creation)"
    )
    ...
    @classmethod
    def compute_fingerprint(cls, method: str, path: str, body: dict[str, Any] | None = None) -> str:
        """
        ...
        fingerprint_data = {"method": method.upper(), "path": normalized_path, "body": body or {}}
...
class RequestDeduplication(models.Model):
    ...
    tenant_id = models.UUIDField(db_index=True, help_text="Tenant UUID")
    request_fingerprint = models.CharField(
        max_length=64, db_index=True, unique=True, help_text="SHA-256 hash of request"
    )
    method = models.CharField(max_length=10, help_text="HTTP method")
    path = models.CharField(max_length=512, help_text="Request path")
    request_body_hash = models.CharField(max_length=64, help_text="SHA-256 hash of request body")
    created_at = models.DateTimeField(auto_now_add=True, db_index=True)
    expires_at = models.DateTimeField(
        db_index=True, help_text="Expiry time (5 minutes from creation)"
    )
