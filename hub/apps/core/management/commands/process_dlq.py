...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help='Filter by subscriber name'
        )
        parser.add_argument(
            '--max-entries',
            type=int,
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help='Retry a specific DLQ entry by ID'
        )
    ...
    def handle(self, *args, **options):
        ...
        if total_entries_found == 0:
            self.stdout.write(
                self.style.WARNING('No unresolved DLQ entries found matching the specified filters.')
            )
            return
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING('DRY RUN MODE: No entries will be processed')
            )
        ...
        while offset < max_entries and offset < total_entries_found:
            ...
            for entry in batch:
                if not should_retry_dlq_entry(entry, max_retries=max_retries):
                    ...
                    next_retry_time = entry.last_attempt_at + timedelta(seconds=delay) if entry.last_attempt_at else None
                ...
                # Retry entry
                success, error_msg = retry_dlq_entry(str(entry.id), user_id=None, max_retries=max_retries)

                entries_processed += 1
                if success:
                    entries_succeeded += 1
                    self.stdout.write(
                        self.style.SUCCESS(f'  Retried: {entry.id} ({entry.event_type} -> {entry.subscriber})')
                    )
                else:
                    entries_failed += 1
                    self.stdout.write(
                        self.style.ERROR(f'  Failed: {entry.id} - {error_msg}')
                    )
    ...
    def _handle_retry(self, entry_id: str, max_retries: Optional[int]):
