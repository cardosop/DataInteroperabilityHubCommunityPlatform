...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--tenant-id", type=str, help="Tenant UUID to warm cache for")
        parser.add_argument("--all-tenants", action="store_true", help="Warm cache for all tenants")
        parser.add_argument("--assets-only", action="store_true", help="Only warm asset list cache")
        parser.add_argument(
            "--contracts-only", action="store_true", help="Only warm contract list cache"
        )
        parser.add_argument(
            "--marketplace-only", action="store_true", help="Only warm marketplace listings cache"
        )
    ...
    def handle(self, *args, **options):
        ...
        if marketplace_only:
            ...
            try:
                ...
                self.stdout.write(self.style.ERROR(f"Failed to warm marketplace cache: {e}"))
        ...
        if all_tenants:
            ...
            try:
                ...
                self.stdout.write(self.style.ERROR(f"Failed to warm cache for all tenants: {e}"))
        ...
        if not tenant_id:
            raise CommandError(
                "Must specify --tenant-id when not using --all-tenants or --marketplace-only"
            )
        ...
        try:
            if assets_only:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"Successfully warmed {count} asset list cache entries")
                )
            elif contracts_only:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"Successfully warmed {count} contract list cache entries")
                )
            else:
            ...
            self.stdout.write(self.style.ERROR(f"Failed to warm cache for tenant: {e}"))
