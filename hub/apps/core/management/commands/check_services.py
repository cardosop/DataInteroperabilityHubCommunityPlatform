...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if service_name:
            ...
            if is_available:
                self.stdout.write(
                    self.style.SUCCESS(f'✓ {service_name} is available and healthy')
                )
            else:
                self.stdout.write(
                    self.style.ERROR(f'✗ {service_name} is not available')
                )
            ...
            for service_name, (is_available, error_msg) in results.items():
                if is_available:
                    self.stdout.write(
                        self.style.SUCCESS(f'✓ {service_name}')
                    )
                else:
                    self.stdout.write(
                        self.style.ERROR(f'✗ {service_name}')
                    )
                    if verbose and error_msg:
                        self.stdout.write(f'  Error: {error_msg}')
            ...
            if available_count < total_count:
                unavailable = [
                    name for name, (is_avail, _) in results.items()
                    if not is_avail
                ]
                self.stdout.write(
                    self.style.WARNING(
