...
from hub.apps.core.services.base import BaseService, NotFoundError, ServiceError
...
class ServiceCommunicationConfig:
    ...
    @classmethod
    def get_timeout(cls, service_name: Optional[str] = None) -> float:
        """
    ...
    @classmethod
    def get_retry_count(cls, service_name: Optional[str] = None) -> int:
        """
    ...
    @classmethod
    def get_retry_delay(cls, service_name: Optional[str] = None) -> float:
        """
    ...
    @classmethod
    def get_retry_strategy(cls, service_name: Optional[str] = None) -> RetryStrategy:
        """
        ...
        try:
            for attempt in range(self.retry_count):
                try:
                    ...
                    if attempt < self.retry_count - 1 and ServiceCommunicationConfig.should_retry(response.status_code):
                        retry_count += 1
                    ...
                    if attempt < self.retry_count - 1 and ServiceCommunicationConfig.should_retry(e.response.status_code):
                        retry_count += 1
                        delay = ServiceCommunicationConfig.calculate_retry_delay(
                            attempt=attempt,
                            base_delay=self.retry_delay,
                    ...
                    raise ServiceError(
                        ...
                        details={"path": path, "method": method, "status_code": e.response.status_code}
                    )
    ...
    def _build_headers(
        ...
        if span:
            ...
            if span_context.is_valid:
                request_headers['traceparent'] = f"00-{span_context.trace_id:032x}-{span_context.span_id:016x}-01"
...
def get_contract_client(tenant_id: Optional[str] = None) -> ServiceClient:
...
def get_asset_client(tenant_id: Optional[str] = None, use_cache: bool = True) -> ServiceClient:
...
def get_dataset_client(tenant_id: Optional[str] = None) -> ServiceClient:
