...
class ReferenceService(BaseService):
    ...
    @classmethod
    def get_model_class(cls, app_name: str, model_name: str) -> Optional[Type[models.Model]]:
        """
    ...
    def bulk_validate(
        ...
        **filters
        ...
        # Execute batch query
        existing_ids = set(
            str(id) for id in model_class.objects.filter(**query_filters).values_list("id", flat=True)
        )
