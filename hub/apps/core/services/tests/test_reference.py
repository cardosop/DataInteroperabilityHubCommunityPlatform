...
class ReferenceServiceTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            email=f"test-{uid}@example.com", password="password", tenant=self.tenant
        )
        self.service = ReferenceService(tenant_id=str(self.tenant.id), user_id=str(self.user.id))
    ...
    def test_validate_reference_unregistered_model(self):
        ...
        with self.assertRaises(ValidationError) as cm:
            self.service.validate_reference(
                app_name="nonexistent", model_name="Model", resource_id=str(uuid.uuid4())
            )
    ...
    def test_validate_multiple_references(self):
        ...
        results = self.service.validate_multiple_references(
            references, tenant_id=str(self.tenant.id)
        )
