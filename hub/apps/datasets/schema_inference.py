...
def infer_format_from_values(values: list[Any], field_name: str = "") -> str | None:
    """
...
def infer_pattern_from_values(values: list[Any]) -> str | None:
    """
...
def infer_enum_from_values(values: list[Any], max_enum_size: int = 20) -> list[Any] | None:
    """
    ...
    if len(unique_values) <= max_enum_size and len(unique_values) >= 2:
        ...
        if (
            (len(non_null_values) <= 10 and 2 <= len(unique_values) <= 5)
            or unique_ratio <= 0.5
            or avg_samples_per_value >= 2.0
        ):
...
def infer_semantic_type_from_field(field_name: str, values: list[Any]) -> str | None:
    """
...
def infer_type_from_values(values: list[Any]) -> dict[str, Any]:
    ...
    if not non_null_values:
        return {"data_type": "string", "nullable": True}
    ...
    if all(str(v).strip() in bool_patterns for v in non_null_values):
        return {"data_type": "boolean", "nullable": len(non_null_values) < len(values)}

    # Check for integer
    try:
        [int(float(v)) for v in non_null_values]
        # Check if all are actually integers (no decimals)
        if all(float(v) == int(float(v)) for v in non_null_values):
            return {"data_type": "integer", "nullable": len(non_null_values) < len(values)}
    except (ValueError, TypeError):
        pass
    ...
    try:
        ...
        return {"data_type": "float", "nullable": len(non_null_values) < len(values)}
    except (ValueError, TypeError):
        pass
    ...
    if date_matches > 0:
        return {"data_type": "datetime", "nullable": len(non_null_values) < len(values)}

    # Default to string
    return {"data_type": "string", "nullable": len(non_null_values) < len(values)}
...
def infer_schema_from_csv(
    ...
    for field_name in field_names:
        ...
        if hashable_values:
            if len(hashable_values) == len(set(hashable_values)) and len(hashable_values) == len(
                non_null_values
            ) == len(values):
        ...
        if field_name.lower().endswith("_id") or field_name.lower().endswith("id"):
            index_recommendations.append(
                {"field": field_name, "reason": "ID field pattern", "priority": "high"}
            )
        elif (
            "timestamp" in field_name.lower()
            or "created_at" in field_name.lower()
            or "updated_at" in field_name.lower()
        ):
            index_recommendations.append(
                {"field": field_name, "reason": "Timestamp field", "priority": "medium"}
            )
    ...
    try:
        ...
        for column in df.columns:
            ...
            if column.lower().endswith("_id") or column.lower().endswith("id"):
                index_recommendations.append(
                    {"field": column, "reason": "ID field pattern", "priority": "high"}
                )
            elif (
                "timestamp" in column.lower()
                or "created_at" in column.lower()
                or "updated_at" in column.lower()
            ):
                index_recommendations.append(
                    {"field": column, "reason": "Timestamp field", "priority": "medium"}
                )
        ...
        raise ValueError(f"Failed to read Parquet file: {e!s}")
...
def infer_schema_from_excel(
    ...
    try:
        ...
        raise ImportError("openpyxl is required for Excel schema inference (pip install openpyxl)")
    buf = io.BytesIO(file_content)
    try:
        df = pd.read_excel(buf, engine="openpyxl")
    except Exception as e:
        raise ValueError(f"Failed to read Excel file: {e!s}")
    ...
    for column in df.columns:
        ...
        values = [None if (pd.isna(v) or (isinstance(v, float) and v != v)) else v for v in values]
        type_info = infer_type_from_values(values)
        ...
        elif any(x in str(column).lower() for x in ("timestamp", "created_at", "updated_at")):
            index_recommendations.append(
                {
                    "field": str(column),
...
def extract_sample_data(
    file_content: bytes, format: str, sample_size: int = DEFAULT_SAMPLE_DATA_SIZE
) -> list[dict[str, Any]]:
    """
...
def infer_schema_with_encoding_gate(
    ...
    raise ValidationError(
        f"Unsupported file format: {file_format}. Supported formats: CSV, JSON, PARQUET.",
