...
class Dataset(models.Model):
    ...
    sample_data_json = models.JSONField(
        null=True, blank=True, help_text="Sample data (first 100 rows) as JSON array"
    )
    row_count = models.BigIntegerField(
        null=True, blank=True, help_text="Total number of rows in the dataset"
    )
    ...
    retired_at = models.DateTimeField(
        null=True, blank=True, help_text="When the dataset was retired"
    )
    format = models.CharField(max_length=20, help_text="File format: CSV, JSON, PARQUET")
    version = models.IntegerField(
        default=1, help_text="Dataset version (per-asset version counter)"
    )
    ...
    snapshot_metadata = models.JSONField(
        default=dict, null=True, blank=True, help_text="Additional metadata for version snapshot"
    )
    is_current = models.BooleanField(
        default=True, db_index=True, help_text="Whether this is the current version for the asset"
    )
    archived_at = models.DateTimeField(
        null=True, blank=True, db_index=True, help_text="Timestamp when version was archived"
    )
...
class SchemaVersion(models.Model):
    ...
    status = models.CharField(max_length=20, default="ACTIVE", help_text="Schema version status")
    ...
    change_summary = models.JSONField(
        default=dict, help_text="Summary of changes (counts by change type)"
    )
...
class DatasetSnapshot(models.Model):
    ...
    status = models.CharField(
        max_length=20, default="ACTIVE", help_text="Snapshot status: ACTIVE, ARCHIVED, EXPIRED"
    )
    created_at = models.DateTimeField(auto_now_add=True)
