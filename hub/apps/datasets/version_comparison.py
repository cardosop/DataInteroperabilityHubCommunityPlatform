...
from .schema_evolution import SchemaDiff, SchemaEvolutionTracker
...
class VersionComparisonService:
    ...
    def compare_versions(
        old_version: Dataset, new_version: Dataset, include_data_diff: bool = True
    ) -> VersionComparison:
        """
        ...
        schema_diff = SchemaEvolutionTracker.calculate_schema_diff(
            old_version.schema_json or {}, new_version.schema_json or {}
        )

        # Calculate data diff if requested
        data_diff = None
        if include_data_diff:
            data_diff = VersionComparisonService._calculate_data_diff(old_version, new_version)

        # Generate side-by-side comparison
        side_by_side = VersionComparisonService._generate_side_by_side(
            old_version, new_version, schema_diff
        )
    ...
    @staticmethod
    def _calculate_data_diff(old_version: Dataset, new_version: Dataset) -> DataDiff:
        """
    ...
    def _generate_side_by_side(
        old_version: Dataset, new_version: Dataset, schema_diff: SchemaDiff
    ) -> dict[str, Any]:
        """
    ...
    @staticmethod
    def visualize_schema_diff(schema_diff: SchemaDiff, format: str = "json") -> Any:
        """
    ...
    @staticmethod
    def visualize_data_diff(data_diff: DataDiff, format: str = "json") -> Any:
        """
        ...
        if format == "json":
            ...
            for field_name, stats in data_diff.field_statistics.items():
                ...
                if stats.get("type_changed"):
                    lines.append(
                        f"- **Type Changed:** {stats.get('old_type')} → {stats.get('new_type')}"
                    )
                if stats.get("sample_values_changed"):
                    lines.append("- **Sample Values Changed:** Yes")
                lines.append("")
            ...
            for field_name, stats in data_diff.field_statistics.items():
                ...
                if stats.get("type_changed"):
                    html += (
                        f"<li>Type Changed: {stats.get('old_type')} → {stats.get('new_type')}</li>"
                    )
                html += "</ul></li>"
    ...
    @staticmethod
    def visualize_version_diff(comparison: VersionComparison, format: str = "json") -> Any:
        """
        ...
        if format == "json":
            ...
            lines.extend(["## Side-by-Side Field Comparison", ""])
