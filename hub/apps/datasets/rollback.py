...
class VersionRollbackManager:
    ...
    def check_rollback_conditions(
        ...
        if not config.enable_auto_rollback:
            return {
                ...
                "reason": "Auto-rollback is disabled",
                "triggers": []
            }
        ...
        return {
            ...
            "reason": "Quality or compliance failure detected" if should_rollback else "No failures detected",
            "triggers": triggers,
            "requires_approval": config.require_approval
        }
    
    @staticmethod
    def _check_quality_failure(
        dataset: Dataset,
    ...
    @staticmethod
    def _check_compliance_failure(dataset: Dataset) -> Optional[Dict[str, Any]]:
        """Check if compliance failure occurred"""
        # Get latest compliance run for this dataset
        try:
            from hub.apps.compliance.models import ComplianceRun, ComplianceRunStatus
            
            latest_compliance_run = ComplianceRun.objects.filter(
                tenant=dataset.tenant,
                dataset=dataset,
    ...
    def execute_rollback(
        dataset: Dataset,
        approved_by: Optional[Any] = None,  # User object
