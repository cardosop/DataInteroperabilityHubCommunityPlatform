...
class JobsBusinessRules(BusinessRules):
    ...
    def validate(
        ...
        **kwargs
        ...
        if job and validation_type in ('job_creation', 'job_execution', 'status_transition', 'all'):
            ...
            validated_items.append('job_basic' if validation_type != 'job_creation' else 'job_creation')
    ...
    def _validate_job_basic(
        ...
        if self.tenant_id and job.tenant_id:
            if str(job.tenant_id) != str(self.tenant_id):
                errors.append(
                    f"Job tenant ({job.tenant_id}) does not match "
        ...
        if user and job.created_by:
            if str(user.id) != str(job.created_by.id):
                warnings.append(
                    ...
                    f"job creator ({job.created_by.id})"
                )
    ...
    def _validate_job_configuration(
        ...
        if job.type and job.details_json is not None and isinstance(job.details_json, dict):
            ...
            if job_type_str == JobType.CONTRACT_VALIDATION[0] or job_type_str == JobType.CONTRACT_VALIDATION:
            ...
            if job_type_str == JobType.COMPLIANCE_RUN[0] or job_type_str == JobType.COMPLIANCE_RUN:
                if 'scan_type' not in job.details_json:
                    warnings.append(
                        "COMPLIANCE_RUN job should have 'scan_type' in details_json"
                    )
    ...
    def _validate_job_resource(
        ...
        try:
            ...
            warnings.append(
                f"Could not import resource model for '{job.resource_type}': {str(e)}"
            )
        ...
        try:
            ...
            errors.append(
                f"Resource {job.resource_type}:{job.resource_id} does not exist"
            )
    ...
    def _validate_job_quota(
        ...
        try:
            ...
            if not can_create_job:
                ...
                concurrency_usage_pct = (running_count / max_concurrency * 100) if max_concurrency > 0 else 0
            ...
            logger.warning(
                f"Job quota validation failed: {str(e)}",
                extra={'tenant_id': tenant_id},
                exc_info=True
            )
            warnings.append(
                f"Could not validate job quota: {str(e)}. Job creation may proceed."
            )
    ...
    def _validate_job_status_transition(
        ...
        if new_status:
            ...
            allowed_transitions = {
                ...
                running_val: [completed_val, failed_val, cancelled_val],  # RUNNING can go to COMPLETED, FAILED, or CANCELLED
            ...
            if current_status == pending_val and new_status == running_val:
            ...
            elif current_status == running_val and new_status in [completed_val, failed_val, cancelled_val]:
                ...
                if not job.completed_at:
                    warnings.append(
                        "Cancelling job - completed_at should be set"
                    )
                    details['completed_at_set'] = False
                else:
                    details['completed_at_set'] = True
    ...
    def _validate_job_cancellation(
        self,
        job: Job
    ) -> ValidationResult:
        """
        ...
        if job.status in terminal_statuses:
            errors.append(
                f"Job is in terminal state ({job.status}) and cannot be cancelled"
            )
    ...
    def _validate_job_retry(
        ...
        if max_retries < 0:
            errors.append(
                f"max_retries ({max_retries}) cannot be negative"
            )
        ...
        if retry_count < 0:
            errors.append(
                f"retry_count ({retry_count}) cannot be negative"
            )
        ...
        if retry_count > max_retries:
            errors.append(
                f"retry_count ({retry_count}) exceeds max_retries ({max_retries})"
            )
    ...
    def _validate_job_timeout(
        ...
        if job.status == running_val and job.started_at:
            ...
            if elapsed_seconds > timeout_seconds:
                ...
                timeout_usage_pct = (elapsed_seconds / timeout_seconds * 100) if timeout_seconds > 0 else 0
    ...
    def validate_job_priority(
        ...
        if effective_user:
            priority_assignment_result = self._validate_priority_assignment(job, effective_user, effective_tenant)
