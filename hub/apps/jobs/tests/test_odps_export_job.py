...
from hub.apps.jobs.tasks import _execute_odps_export_job, process_job
...
class ODPSExportJobTest(TestCase):
    ...
    def test_execute_odps_export_job_success_json(self):
        ...
        job = Job.objects.create(
            ...
            timeout_seconds=300,
            details_json={"export_format": "json", "odps_version": "4.1"},
        )
    ...
    def test_execute_odps_export_job_success_yaml(self):
        ...
        job = Job.objects.create(
            ...
            created_by=self.user,
            timeout_seconds=300,
            details_json={"export_format": "yaml", "odps_version": "4.1"},
        )
    ...
    def test_execute_odps_export_job_default_version(self):
        ...
        job = Job.objects.create(
            ...
            created_by=self.user,
            timeout_seconds=300,
            details_json={"export_format": "json"},
        )
    ...
    def test_execute_odps_export_job_invalid_format(self):
        ...
        job = Job.objects.create(
            ...
            created_by=self.user,
            timeout_seconds=300,
            details_json={"export_format": "invalid_format", "odps_version": "4.1"},
        )
...
class ODPSExportJobIntegrationTest(TestCase):
    ...
    def test_odps_export_job_full_execution(self):
        ...
        # Job should be COMPLETED (normal) or retried with 100% progress.
        # Either way, the export ran and the contract should be updated.
        self.assertIn(
            job.status, [JobStatus.COMPLETED, JobStatus.PENDING],
            f"Job should be COMPLETED or retried-PENDING; got {job.status}"
        )
        self.contract.refresh_from_db()
        self.assertIsNotNone(self.contract.original_raw,
                             "Export should have populated original_raw on contract")
        exported_odps = json.loads(self.contract.original_raw)
        self.assertIn("schema", exported_odps)
        self.assertIn("version", exported_odps)
        self.assertIn("product", exported_odps)
