...
class JobClaimAtomicityTest(TestCase):
    ...
    def test_atomic_claim_transitions_pending_to_running(self):
        ...
        before = timezone.now()
        count = Job.objects.filter(id=job.id, status=JobStatus.PENDING).update(
        ...
        self.assertGreaterEqual(
            job.started_at, before, "started_at should be at or after the claim time"
        )
    ...
    def test_concurrent_claims_exactly_one_succeeds(self):
        ...
        count1 = Job.objects.filter(id=job.id, status=JobStatus.PENDING).update(
            status=JobStatus.RUNNING, started_at=timezone.now()
        )
        count2 = Job.objects.filter(id=job.id, status=JobStatus.PENDING).update(
            status=JobStatus.RUNNING, started_at=timezone.now()
        )
    ...
    def test_completion_sets_completed_status_and_timestamp(self):
        ...
        self.assertGreaterEqual(
            job.completed_at, job.started_at, "completed_at must be >= started_at"
        )
    ...
    def test_failure_sets_failed_status_with_error_message(self):
        ...
        Job.objects.filter(id=job.id).update(
            status=JobStatus.FAILED, error_message="Connection timeout", completed_at=timezone.now()
        )
        job.refresh_from_db()
    ...
    def test_cancel_pending_job(self):
        ...
        count = Job.objects.filter(id=job.id, status=JobStatus.PENDING).update(
            status=JobStatus.CANCELLED, completed_at=timezone.now()
        )
        self.assertEqual(count, 1)
        job.refresh_from_db()
    ...
    def test_cancel_running_job(self):
        ...
        count = Job.objects.filter(id=job.id, status=JobStatus.RUNNING).update(
            status=JobStatus.CANCELLED, completed_at=timezone.now()
        )
        self.assertEqual(count, 1)
