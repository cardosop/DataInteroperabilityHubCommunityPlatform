...
class JobLifecycleIntegrationTest(TestCase):
    ...
    def test_job_cancellation_during_lifecycle(self):
        ...
        with patch("hub.apps.jobs.tasks_base._execute_job_logic"):
            process_job(str(job.id), JobType.DQ_RUN)
            # Should not execute logic if already cancelled
            job.refresh_from_db()
            # A CANCELLED job must not be picked up by process_job —
            # the status must remain CANCELLED after processing.
            self.assertEqual(
                job.status, JobStatus.CANCELLED,
                "CANCELLED job must not be processed by process_job",
            )
