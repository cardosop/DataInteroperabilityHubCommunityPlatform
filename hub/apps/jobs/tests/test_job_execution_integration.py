...
class JobExecutionIntegrationTest(TestCase):
    ...
    def test_dq_run_job_execution_real(
        self, mock_health_check, mock_run_dq, mock_storage_client_class
    ):
        ...
        mock_run_dq.return_value = {
            "overall_status": "PASS",
            "quality_score": 0.95,
            "checks": [{"name": "check1", "status": "PASS"}, {"name": "check2", "status": "PASS"}],
    ...
    def test_compliance_run_job_execution_real(
        self, mock_health_check, mock_scan_file, mock_storage_client_class
    ):
        ...
        mock_scan_file.assert_called_once()  # scan_file called once in sync path
        mock_health_check.assert_called_once()
    ...
    @patch("hub.apps.dq.service_client.DQServiceClient.run_dq")
    @patch("hub.apps.dq.service_client.DQServiceClient.health_check")
    def test_dq_run_job_execution_transient_failure_retry(
        self, mock_health_check, mock_run_dq, mock_storage_client_class
    ):
    ...
    @patch("hub.apps.dq.service_client.DQServiceClient.run_dq")
    @patch("hub.apps.dq.service_client.DQServiceClient.health_check")
    def test_dq_run_job_execution_non_transient_failure(
        self, mock_health_check, mock_run_dq, mock_storage_client_class
    ):
