...
class WarehouseConnectionViewSet(viewsets.ModelViewSet):
    ...
    def test_connection(self, request, id=None):
        ...
        try:
            ...
            connector = connector_cls(
                config, tenant_id=str(connection.tenant_id),
            )
    ...
    @action(detail=False, methods=["get"], url_path="residency-mismatches",
            throttle_classes=[ScopedRateThrottle])
    def residency_mismatches(self, request):
        ...
        tenant_mismatches = [
            ...
            if m.get("tenant_id") == str(tenant.id)
        ] if tenant else mismatches
...
class LiveQueryViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        try:
            asset = Asset.objects.select_related("warehouse_connection").get(
                id=asset_id, data_strategy=DataStrategy.LIVE_QUERY,
    ...
    def _do_query(self, request, asset, tenant_id_str):
        ...
        try:
            ...
            next_cursor = (
                b64encode(str(next_offset).encode()).decode()
            ...
            try:
                ...
                set_cached_result(
                    str(tenant.id), asset_id, cache_key,
