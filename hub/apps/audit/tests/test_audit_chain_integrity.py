...
class AuditChainIntegrityTests(TestCase):
    ...
    def test_prev_chain_hash_links_to_prior_event(self):
        ...
        e1 = AuditEvent.objects.create(
            tenant=self.tenant, resource_type="TEST", action="CHAIN_LINK_1",
            resource_id=str(uuid.uuid4()),
        )
        e2 = AuditEvent.objects.create(
            tenant=self.tenant, resource_type="TEST", action="CHAIN_LINK_2",
            resource_id=str(uuid.uuid4()),
        )
    ...
    def test_genesis_event_has_null_prev_hash(self):
        ...
        event = AuditEvent.objects.create(
            tenant=tenant2, resource_type="TEST", action="GENESIS",
            resource_id=str(uuid.uuid4()),
        )
    ...
    def test_tenant_isolation_separate_chains(self):
        ...
        tenant_b = Tenant.objects.create(
            name=f"Test-B-{uuid.uuid4().hex[:8]}", slug=f"tb-{uuid.uuid4().hex[:8]}", status="ACTIVE",
        )
        e_a = AuditEvent.objects.create(
            tenant=self.tenant, resource_type="TEST", action="ISO_A",
            resource_id=str(uuid.uuid4()),
        )
        e_b = AuditEvent.objects.create(
            tenant=tenant_b, resource_type="TEST", action="ISO_B",
            resource_id=str(uuid.uuid4()),
        )
        self.assertNotEqual(e_a.chain_hash, e_b.chain_hash,
                            "Different tenants should have different chain hashes")
        self.assertIsNone(e_b.prev_chain_hash,
                          "First event in tenant B should be genesis (NULL prev)")

    @pytest.mark.integration
    def test_chain_sequence_increments(self):
        """chain_sequence should increment for each event in a tenant."""
        e1 = AuditEvent.objects.create(
            tenant=self.tenant, resource_type="TEST", action="SEQ_1",
            resource_id=str(uuid.uuid4()),
        )
        e2 = AuditEvent.objects.create(
            tenant=self.tenant, resource_type="TEST", action="SEQ_2",
            resource_id=str(uuid.uuid4()),
        )
        self.assertIsNotNone(e1.chain_sequence)
        self.assertIsNotNone(e2.chain_sequence)
        # Sequence should be monotonically increasing
        self.assertGreater(e2.chain_sequence, e1.chain_sequence,
                           "chain_sequence should increment between events")
