...
class AssetsBusinessRulesInitializationTest(TestCase):
    ...
    def test_rule_registration(self):
        """Test that AssetsBusinessRules is registered in the registry."""
...
class AssetsBusinessRulesValidationTest(TestCase):
    ...
    def test_validate_tenant_context(self):
        ...
        result = self.rules.validate(
            context=None,
            asset=asset,
            validation_type='tenant_context'
        )
    ...
    def test_validate_permissions_user_tenant_mismatch(self):
        ...
        self.assertIsInstance(result, ValidationResult)
        self.assertFalse(result.is_valid)
        self.assertTrue(any("User tenant" in error and "does not match asset tenant" in error for error in result.errors))
...
class AssetsBusinessRulesLifecycleValidationTest(TestCase):
    ...
    def test_status_transition_invalid_retired_to_active(self):
        ...
        result = self.rules._validate_status_transition(
            asset,
            old_status=AssetStatus.RETIRED,
            new_status=AssetStatus.ACTIVE
        )
...
class AssetsBusinessRulesContractAttachmentValidationTest(TestCase):
    ...
    def test_contract_version_compatibility_auto_increment(self):
        ...
        result = self.rules._validate_contract_version_compatibility(asset, contract, proposed_version=None)
    ...
    def test_contract_version_compatibility_valid_increment(self):
        ...
        result = self.rules._validate_contract_version_compatibility(asset, contract, proposed_version=2)
    ...
    def test_contract_version_compatibility_invalid_increment(self):
        ...
        result = self.rules._validate_contract_version_compatibility(asset, contract, proposed_version=1)
    ...
    def test_validate_contract_attachment_multiple_errors(self):
        """Test contract attachment validation with multiple validation errors"""
        from hub.apps.contracts.models import Contract, ContractStatus, ValidationStatus, NormalizationStatus, OriginalSpecType, OriginalFormat
        ...
        self.assertFalse(result.is_valid)
        self.assertGreaterEqual(len(result.errors), 2)  # At least validation and normalization errors
...
class AssetsBusinessRulesHealthScoreTest(TestCase):
    ...
    def test_validate_health_score_calculation_active_with_fail_dq(self):
        ...
        self.assertFalse(result.is_valid)
        self.assertTrue(any("ACTIVE status requires DQ status PASS or WARN" in error for error in result.errors))
    ...
    def test_validate_health_score_calculation_active_with_fail_compliance(self):
        ...
        self.assertFalse(result.is_valid)
        self.assertTrue(any("ACTIVE status requires compliance status PASS or WARN" in error for error in result.errors))
    ...
    def test_validate_health_score_calculation_active_without_contract(self):
        ...
        self.assertFalse(result.is_valid)
        self.assertTrue(any("ACTIVE status requires an ACTIVE contract" in error for error in result.errors))
    ...
    def test_validate_health_score_calculation_health_score_out_of_range(self):
        ...
        self.assertFalse(result.is_valid)
        self.assertTrue(any("Health score must be between 0.0 and 100.0" in error for error in result.errors))
    ...
    def test_validate_health_score_thresholds_public_meets_threshold(self):
        ...
        result = self.rules.validate_health_score_thresholds(
            asset=asset,
            target_status=AssetStatus.PUBLIC,
            raise_on_error=False
        )
    ...
    def test_validate_health_score_thresholds_low_score_warning(self):
        ...
        result = self.rules.validate_health_score_thresholds(
            asset=asset,
            target_status=AssetStatus.DRAFT,
            raise_on_error=False
        )
    ...
    def test_validate_health_score_update_triggers_no_recalculation_needed(self):
        ...
        result = self.rules.validate_health_score_update_triggers(
            asset=asset,
            raise_on_error=False
        )

        self.assertTrue(result.is_valid)
        self.assertFalse(result.details["update_trigger_checks"].get("recalculation_required", False))
    ...
    def test_validate_health_score_update_triggers_stale_score(self):
        ...
        self.assertTrue(result.details["update_trigger_checks"].get("recalculation_recommended", False))
    ...
    def test_validate_health_score_update_triggers_active_without_score(self):
        ...
        self.assertFalse(result.is_valid)
        self.assertTrue(any("must have a calculated health score" in error for error in result.errors))
        self.assertTrue(result.details["update_trigger_checks"].get("recalculation_required", False))
    ...
    def test_validate_health_score_update_triggers_raises_on_error(self):
        ...
        from hub.apps.core.services.base import ValidationError
        with self.assertRaises(ValidationError) as context:
            self.rules.validate_health_score_update_triggers(
                asset=asset,
                raise_on_error=True
            )
    ...
    def test_validate_health_score_calculation_with_contract(self):
        ...
        self.assertTrue(result.details["health_score_checks"]["contract_validation_status_appropriate"])
