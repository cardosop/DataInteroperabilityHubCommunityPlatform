...
from .models import Asset, AssetStatus, DQStatus, ComplianceStatus
...
class AssetHealthScoreService:
    ...
    def _calculate_freshness_score(asset: Asset) -> float:
        ...
        if DataObservabilityMetric:
            freshness_metric = DataObservabilityMetric.objects.filter(
                tenant=asset.tenant,
                asset=asset,
                metric_type="freshness"
            ).order_by('-created_at').first()
        
        if freshness_metric and freshness_metric.metadata:
            # Extract freshness age from metadata
            freshness_age_hours = freshness_metric.metadata.get("age_hours", 0)
        ...
        return {
            ...
            "components": {
                ...
                "compliance": {
                    ...
                    "weighted_score": round(compliance_score * AssetHealthScoreService.COMPLIANCE_WEIGHT, 2),
                    "status": asset.compliance_status
                },
                "freshness": {
                    "score": round(freshness_score, 2),
                    "weight": AssetHealthScoreService.FRESHNESS_WEIGHT,
                    "weighted_score": round(freshness_score * AssetHealthScoreService.FRESHNESS_WEIGHT, 2)
                },
                "usage": {
