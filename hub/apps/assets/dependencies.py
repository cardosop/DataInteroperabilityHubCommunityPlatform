...
class AssetDependencyGraph:
    ...
    def add_node(self, asset_id: str, asset: Asset, metadata: dict[str, Any] | None = None):
    ...
    def add_edge(
        ...
        edge = {"source": source_id, "target": target_id, "type": edge_type, **(metadata or {})}
    ...
    def to_d3_format(self) -> dict[str, Any]:
        ...
        return {
            "nodes": [
                {**node, "group": self._get_node_group(node)} for node in self.nodes.values()
            ],
            "links": [
                {
                    "source": edge["source"],
...
class AssetDependencyService:
    ...
    def generate_dependency_graph(
        ...
        if direction in ["upstream", "both"]:
            AssetDependencyService._traverse_upstream(
                source_asset, graph, tenant_id, max_depth, include_metadata
            )
    ...
    def _traverse_upstream(
        ...
        contracts = Contract.objects.filter(tenant_id=tenant_id, asset=asset)
    ...
    def get_dependency_stats(graph: AssetDependencyGraph) -> dict[str, Any]:
        ...
        return {
            ...
            "upstream_count": sum(
                1 for e in graph.edges if e.get("metadata", {}).get("direction") == "upstream"
            ),
            "downstream_count": sum(
                1 for e in graph.edges if e.get("metadata", {}).get("direction") == "downstream"
            ),
        }
