...
class AssetRecommendationService:
    ...
    @staticmethod
    def _get_usage_pattern_recommendations(
        tenant_id: str,
        limit: int = 10
    ) -> List[Dict[str, Any]]:
        ...
        viewed_assets = Asset.objects.filter(
            tenant_id=tenant_id,
            status__in=[AssetStatus.ACTIVE, AssetStatus.PUBLIC]
        ).annotate(
            total_interactions=F('view_count') + F('download_count')
    ...
    def _get_lineage_recommendations(
        ...
        source_contracts = Contract.objects.filter(
            tenant_id=tenant_id,
            asset=source_asset
        )
