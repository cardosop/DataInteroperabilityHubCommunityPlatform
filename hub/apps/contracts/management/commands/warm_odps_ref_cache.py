...
from hub.apps.contracts.ref_warming import get_frequently_accessed_refs, warm_ref_cache
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not ref_urls:
            self.stdout.write(
                self.style.WARNING("No frequently accessed refs found. Cache warming skipped.")
            )
            return
        ...
        if not ref_urls:
            self.stdout.write(self.style.WARNING("No refs to warm after filtering."))
            return

        if dry_run:
            self.stdout.write(self.style.SUCCESS(f"DRY-RUN: Would warm {len(ref_urls)} refs:"))
        ...
        # Warm cache
        try:
            result = warm_ref_cache(ref_urls=ref_urls, tenant_id=tenant_id, batch_size=batch_size)
        except Exception as e:
        ...
        self.stdout.write(self.style.SUCCESS(f"  Warmed: {result['warmed']}"))
        self.stdout.write(self.style.WARNING(f"  Skipped (already cached): {result['skipped']}"))
        if result["failed"] > 0:
        ...
        if result["failed"] > 0:
            self.stdout.write(
                self.style.WARNING(
                    f"\nWarning: {result['failed']} refs failed to warm. Check logs for details."
                )
            )
