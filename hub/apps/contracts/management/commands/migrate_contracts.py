...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        queryset = Contract.objects.filter(
            hub_contract_json__isnull=False, hub_contract_version__isnull=False
        )
        ...
        if not force:
            ...
            for contract in queryset:
                if (
                    ...
                    or contract.hub_contract_version != target_version
                ) and can_migrate(contract.hub_contract_version, target_version):
            ...
            for contract in batch:
                try:
                    if dry_run:
                        ...
                        migrated, _hub_contract, warnings = (
                    ...
                    self.stdout.write(
                        self.style.ERROR(f"  Contract {contract.id}: Migration failed - {e!s}")
                    )
