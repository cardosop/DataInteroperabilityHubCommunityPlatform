...
def get_normalizer(
    ...
    # For ODCS with unknown versions, try fallback to latest (3.1.0) before using default
    # For other cases, use default normalizer if found
    if spec_type == OriginalSpecType.ODCS and spec_version not in ("3.1.0", "3.0.2"):
    ...
    # try to find the latest version normalizer (3.1.0) to ensure backward
    ...
    if spec_type == OriginalSpecType.ODCS and spec_version not in ("3.1.0", "3.0.2"):
        ...
        # Try to find 3.1.0 normalizer first (latest version), then 3.0.2
        ...
        for normalizer in reversed(normalizers):
            ...
            if normalizer_class_name == "ODCSNormalizer" or isinstance(
                ...
                continue
            # Prefer v3.1.0 normalizer (current latest)
            if isinstance(normalizer, ODCSNormalizerV3_1_0):
                return normalizer
        # Fall back to 3.0.2 if 3.1.0 not found
        for normalizer in reversed(normalizers):
            normalizer_class_name = normalizer.__class__.__name__
            if normalizer_class_name == "ODCSNormalizer" or isinstance(
                normalizer, ODCSNormalizerDefault
...
try:
    ...
    register_normalizer(ODCSNormalizerV3_1_0())
except ImportError:
    # ODCS 3.1.0 normalizer not available (should not happen in production)
    pass
...
# Register Bitol ODPS normalizer (before pre-Bitol version-specific)
try:
    from hub.apps.contracts.normalization.odps_normalizer_bitol_v1 import (
        ODPSBitolNormalizerV1_0_0,
    )

    register_normalizer(ODPSBitolNormalizerV1_0_0())
except ImportError:
    pass
...
try:
    ...
    register_normalizer(ODPSNormalizerV4_2())
except ImportError:
    pass
