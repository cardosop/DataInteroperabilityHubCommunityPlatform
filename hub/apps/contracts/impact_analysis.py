...
from .lineage import LineageReference, LineageTraverser
...
class ImpactAnalyzer:
    ...
    def __init__(
        ...
        self.visited_fields: Set[Tuple[str, str, str]] = set()  # (contract_id, model_name, field_name)
    ...
    def analyze_impact(
        ...
        root_node = ImpactNode(
            resource_type="CONTRACT" if not model_name else ("MODEL" if not field_name else "FIELD"),
        ...
        result = {
            "source": {
                "contract_id": contract_id,
                "contract_name": source_contract.hub_contract_json.get("info", {}).get("name") if isinstance(source_contract.hub_contract_json, dict) else None,
                "model_name": model_name,
                "field_name": field_name
            },
    ...
    def _traverse_reverse_lineage(
        ...
        if path_key in self.cycle_detection:
            logger.warning(
                "Cycle detected in impact analysis",
                path=node.path,
                        ...
                        if ref_name == source_contract_name:
                            if not source_namespace or ref_namespace == source_namespace:
                                dependents.append({
                                    ...
                                    "reference_path": f"{ref_namespace}/{ref_name}" if ref_namespace else ref_name
                                })
            ...
            for model in models_list:
                ...
                if self.include_fields and field_name:
                    ...
                    for field in fields:
                        ...
                        if field_lineage:
                            input_fields = field_lineage.get("input_fields") or field_lineage.get("inputFields", [])
                            for input_field in input_fields:
                                if isinstance(input_field, dict):
                                    input_name = input_field.get("name")
                                    input_model = input_field.get("model") or input_field.get("model_name")
                                    input_field_name = input_field.get("field")
                                    
                                    # Match field reference
                                    if (input_name == source_contract_name and
                                        input_model == model_name and
    ...
    def _calculate_impact_scores(
        ...
        node.impact_score = base_score + (children_score * 0.5)  # Children contribute 50% of their score
    ...
    def _build_summary(self, node: ImpactNode) -> Dict[str, Any]:
        ...
        def traverse(node: ImpactNode):
            ...
            summary["severity_distribution"][node.severity] = summary["severity_distribution"].get(node.severity, 0) + 1
...
class ImpactScorer:
    ...
    @staticmethod
    def get_asset_criticality(contract_id: str, tenant_id: Optional[str] = None) -> str:
    ...
    @staticmethod
    def get_usage_frequency(contract_id: str, tenant_id: Optional[str] = None) -> str:
