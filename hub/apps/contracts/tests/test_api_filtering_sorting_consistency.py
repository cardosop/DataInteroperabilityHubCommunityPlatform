...
class APIFilteringSortingConsistencyTest(TestCase):
    """
    ...
    def setUp(self):
        ...
        for i in range(10):
            contract = Contract.objects.create(
                ...
                normalization_status=NormalizationStatus.NORMALIZED_OK if i % 3 == 0 else NormalizationStatus.NOT_NORMALIZED
            )
    ...
    def test_contracts_list_endpoint_supports_filtering(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK,
                        "Contracts list should support filtering")
    ...
    def test_filter_parameter_format_is_consistent(self):
        ...
        for filter_name, filter_value in filters.items():
            ...
            self.assertEqual(response.status_code, status.HTTP_200_OK,
                           f"Filter {filter_name} should work")
    ...
    def test_contracts_list_endpoint_supports_sorting(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK,
                        "Contracts list should support sorting")
    ...
    def test_sort_parameter_format_is_consistent(self):
        ...
        for sort_param in sort_params:
            ...
            self.assertEqual(response.status_code, status.HTTP_200_OK,
                           f"Sort parameter {sort_param} should work")
            self.assertIn('results', response.data,
                         f"Response should include results for {sort_param} sort")
    ...
    def test_filter_and_sort_combinations_work_correctly(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK,
                        "Filter and sort combination should work")
    ...
    def test_sort_validation_and_error_messages(self):
        """Test sort validation and error messages"""
        ...
        if response.status_code == status.HTTP_400_BAD_REQUEST:
            ...
            # If it returns 200, should use default sort
            self.assertEqual(response.status_code, status.HTTP_200_OK,
    ...
    def test_multiple_filter_parameters_work(self):
        ...
        if response.data.get('results'):
            for result in response.data['results']:
                self.assertEqual(result['status'], 'ACTIVE',
                               "All results should match status filter")
                self.assertEqual(result['original_spec_type'], 'ODPS',
    ...
    def test_multiple_sort_fields_work(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK,
                        "Multiple sort fields should work")
        self.assertIn('results', response.data,
    ...
    def test_filtering_and_sorting_performance(self):
        ...
        start = time.time()
        response2 = self.client.get('/api/v1/contracts/', {
            'status': 'ACTIVE',
            'ordering': '-created_at'
        })
        ...
        self.assertLess(time_with, time_without * 10,
                       "Filtering and sorting should not be excessively slow")
    ...
    def test_empty_filter_results(self):
        ...
        self.assertEqual(response.data.get('count', 0), 0,
                        "Count should be 0 when no matches")
