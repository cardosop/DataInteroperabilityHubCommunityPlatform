...
class ODPSDashboardsValidationTest(TestCase):
    ...
    def test_all_required_dashboards_exist(self):
        ...
        for dashboard_file in self.required_dashboards:
            ...
            self.assertTrue(
                dashboard_path.exists(), f"Required dashboard file not found: {dashboard_file}"
            )
    ...
    def test_dashboards_have_required_structure(self):
        ...
        for dashboard_file in self.required_dashboards:
            ...
            self.assertIn(
                "dashboard", config, f"Dashboard {dashboard_file} must have 'dashboard' key"
            )
            ...
            self.assertIsInstance(
                dashboard["panels"], list, f"Dashboard {dashboard_file} panels must be a list"
            )
            self.assertGreater(
                len(dashboard["panels"]),
                0,
                f"Dashboard {dashboard_file} must have at least one panel",
            )
    ...
    def test_dashboards_have_required_metadata(self):
        ...
        for dashboard_file in self.required_dashboards:
            ...
            self.assertIsInstance(
                dashboard["tags"], list, f"Dashboard {dashboard_file} tags must be a list"
            )
            ...
            self.assertTrue(
                has_task_tag,
                f"Dashboard {dashboard_file} must have a task tag (e.g., 'task-6.2.5' or 'task-6.6.8')",
            )

            self.assertIn(
                "schemaVersion", dashboard, f"Dashboard {dashboard_file} must have 'schemaVersion'"
            )
    ...
    def test_odps_adoption_dashboard_content(self):
        ...
        self.assertTrue(
            any("Ingestion Rate" in title for title in panel_titles),
            "Dashboard should have ingestion rate panel",
        )
        self.assertTrue(
            any("Adoption" in title for title in panel_titles),
    ...
    def test_odps_version_distribution_dashboard_content(self):
        ...
        self.assertGreater(
            len(version_exprs), 0, "Dashboard should have version distribution queries"
        )
    ...
    def test_odcs_version_distribution_dashboard_content(self):
        ...
        self.assertGreater(
            len(odcs_exprs), 0, "Dashboard should have ODCS version distribution queries"
        )
    ...
    def test_ref_resolution_performance_dashboard_content(self):
        ...
        duration_exprs = [
            expr for expr in all_exprs if "odps_ref_resolution_duration_seconds" in expr
        ]
    ...
    def test_rate_limiting_dashboard_content(self):
        ...
        rate_limit_exprs = [
            expr for expr in all_exprs if "odps_rate_limit_violations_total" in expr
        ]
        self.assertGreater(
            len(rate_limit_exprs), 0, "Dashboard should have rate limit violation queries"
        )
    ...
    def test_regression_detection_dashboard_content(self):
        ...
        regression_exprs = [
            expr for expr in all_exprs if "odcs_normalization_regression_total" in expr
        ]
    ...
    def test_performance_targets_dashboard_content(self):
        ...
        target_exprs = [
            expr for expr in all_exprs if "0.010" in expr or "0.050" in expr or "5.0" in expr
        ]
        ...
        for dashboard_file in self.required_dashboards:
            ...
            for panel in dashboard.get("panels", []):
                for target in panel.get("targets", []):
                    ...
                    self.assertGreater(
                        len(expr), 0, f"Query expression should not be empty in {dashboard_file}"
                    )
                    ...
                    has_prometheus_function = any(
                        keyword in expr for keyword in prometheus_functions
                    )
                    has_metric_with_labels = bool(expr) and expr[0].isalpha() and "{" in expr
                    is_bare_metric = (
                        bool(expr)
                        and expr[0].isalpha()
                        and expr.replace("_", "").replace(".", "").isalnum()
                    )
                    ...
                    has_arithmetic = any(op in expr for op in ["+", "-", "*", "/", "%"]) and (
                        "(" in expr or ")" in expr or any(char.isalpha() for char in expr)
                    )

                    self.assertTrue(
                        is_numeric_constant
                        or has_prometheus_function
                        or is_metric_selector
                        or has_arithmetic,
    ...
    def test_dashboards_have_descriptions(self):
        ...
        for dashboard_file in self.required_dashboards:
            ...
            for i, panel in enumerate(dashboard.get("panels", [])):
                panel_name = f"{dashboard_file} panel {i + 1}"
                self.assertIn("description", panel, f"{panel_name} should have a description")
                self.assertGreater(
    ...
    def test_marketplace_integration_dashboard_content(self):
        ...
        access_sqls = [
            sql for sql in all_sqls if "access_method" in sql.lower() or "accessMethod" in sql
        ]
    ...
    def test_creation_flow_performance_dashboard_content(self):
        ...
        self.assertTrue(
            any(
                "Duration" in title and ("P50" in title or "P95" in title or "P99" in title)
        ...
        self.assertGreater(
            len(workflow_exprs), 0, "Dashboard should have product_creation workflow queries"
        )

        # Should have duration queries
        duration_exprs = [
            expr for expr in all_exprs if "workflow_execution_duration_seconds" in expr
        ]
        self.assertGreater(
            len(duration_exprs), 0, "Dashboard should have workflow execution duration queries"
        )

        # Should have step duration queries
        step_exprs = [
            expr for expr in all_exprs if "workflow_step_execution_duration_seconds" in expr
        ]
        self.assertGreater(
            len(step_exprs), 0, "Dashboard should have workflow step execution duration queries"
        )
        ...
        success_exprs = [
            ...
            if "workflow_instances_completed_total" in expr
            or "workflow_instances_started_total" in expr
        ]
        self.assertGreater(
            len(success_exprs), 0, "Dashboard should have workflow instance success queries"
        )

        failure_exprs = [expr for expr in all_exprs if "workflow_instances_failed_total" in expr]
        self.assertGreater(
            len(failure_exprs), 0, "Dashboard should have workflow instance failure queries"
        )
        ...
        self.assertGreater(
            len(found_steps),
            0,
            f"Dashboard should have queries for at least some workflow steps. Found: {found_steps}",
        )
