...
class ODPSAuditIntegrationTestBase(TestCase):
    ...
    def setUp(self):
        ...
        self.tenant = Tenant.objects.create(
            name="ODPS Audit Test Tenant",
            slug="odps-audit-test",
        ...
        self.user = User.objects.create_user(
            email="odps-audit-test@example.com",
            password="testpass123",
            tenant=self.tenant,
            status=UserStatus.ACTIVE,
            display_name="ODPS Audit Test User",
        )
...
class ODPSNormalizeAuditTest(ODPSAuditIntegrationTestBase):
    ...
    def test_normalize_odps_creates_audit_event(self):
        ...
        hub_contract = odps_service.normalize_odps(
            odps_doc=self.base_odps_contract,
            odps_version="4.1",
            tenant_id=str(self.tenant.id)
        )
