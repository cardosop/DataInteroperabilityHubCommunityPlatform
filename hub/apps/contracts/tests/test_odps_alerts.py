...
class ODPSAlertsConfigurationTest(TestCase):
    ...
    def test_alert_structure_is_valid(self):
        ...
        for rule in rules:
            ...
            self.assertEqual(
                labels["component"], "odps", f"Alert '{alert_name}' component should be 'odps'"
            )
    ...
    def test_normalization_failure_alert_configuration(self):
        ...
        self.assertIn(
            ...
            alert["expr"],
            "Alert should reference odps_normalization_total metric",
        )
        self.assertIn(
            'status=~"failed|error"', alert["expr"], "Alert should filter for failed/error status"
        )
    ...
    def test_ref_resolution_error_alert_configuration(self):
        ...
        self.assertIn(
            ...
            alert["expr"],
            "Alert should reference odps_ref_resolution_total metric",
        )
    ...
    def test_external_fetch_failure_alert_configuration(self):
        ...
        self.assertIn(
            ...
            alert["expr"],
            "Alert should reference odps_external_fetch_failures_total metric",
        )
        self.assertIn(
            'odps_ref_resolution_total{ref_type="external"}',
            alert["expr"],
            "Alert should reference external ref resolution metric",
        )
        self.assertEqual(alert["labels"]["severity"], "warning", "Alert severity should be warning")
    ...
    def test_rate_limit_violations_alert_configuration(self):
        ...
        self.assertIn(
            ...
            alert["expr"],
            "Alert should reference odps_rate_limit_violations_total metric",
        )
        self.assertIn("[1h]", alert["expr"], "Alert should use 1 hour rate window")
        self.assertEqual(
            alert["labels"]["severity"], "critical", "Alert severity should be critical"
        )
    ...
    def test_performance_degradation_alerts_configuration(self):
        ...
        self.assertIsNotNone(
            internal_alert, "ODPSRefResolutionPerformanceDegradationInternal alert should exist"
        )
        self.assertIn(
            'odps_ref_resolution_duration_seconds_bucket{ref_type="internal"}',
            internal_alert["expr"],
            "Alert should reference internal ref duration histogram",
        )
        self.assertIn(
            "histogram_quantile(0.95", internal_alert["expr"], "Alert should use P95 percentile"
        )
        ...
        self.assertIsNotNone(
            local_alert, "ODPSRefResolutionPerformanceDegradationLocal alert should exist"
        )
        self.assertIn(
            'odps_ref_resolution_duration_seconds_bucket{ref_type="local"}',
            local_alert["expr"],
        ...
        self.assertIsNotNone(
            external_alert, "ODPSRefResolutionPerformanceDegradationExternal alert should exist"
        )
        self.assertIn(
            'odps_ref_resolution_duration_seconds_bucket{ref_type="external"}',
            external_alert["expr"],
            "Alert should reference external ref duration histogram",
        )
        self.assertIn(
            "> 10.0", external_alert["expr"], "Alert threshold should be 10s (2x target of 5s)"
        )
    ...
    def test_cache_hit_rate_alert_configuration(self):
        ...
        self.assertIn(
            "odps_ref_cache_hits_total", alert["expr"], "Alert should reference cache hits metric"
        )
        self.assertIn(
            "odps_ref_cache_misses_total",
            alert["expr"],
            "Alert should reference cache misses metric",
        )
    ...
    def test_export_failure_alert_configuration(self):
        ...
        self.assertIn(
            "odps_export_total", alert["expr"], "Alert should reference odps_export_total metric"
        )
        self.assertIn('status="failure"', alert["expr"], "Alert should filter for failure status")
        self.assertEqual(alert["labels"]["severity"], "warning", "Alert severity should be warning")
        self.assertIn("0.05", alert["expr"], "Alert threshold should be 5% (0.05)")
    ...
    def test_linking_failure_alert_configuration(self):
        ...
        self.assertIn(
            ...
            alert["expr"],
            "Alert should reference odps_linking_failures_total metric",
        )
        self.assertIn(
            "odps_linking_total", alert["expr"], "Alert should reference odps_linking_total metric"
        )
    ...
    def test_semantic_mapping_failure_alert_configuration(self):
        ...
        self.assertIn(
            ...
            alert["expr"],
            "Alert should reference odps_semantic_mapping_total metric",
        )
        self.assertIn(
            'status=~"failure|error"', alert["expr"], "Alert should filter for failure/error status"
        )
        self.assertEqual(alert["labels"]["severity"], "warning", "Alert severity should be warning")
        ...
        self.assertIn(
            "threshold: 5%", annotations["description"], "Description should mention 5% threshold"
        )
