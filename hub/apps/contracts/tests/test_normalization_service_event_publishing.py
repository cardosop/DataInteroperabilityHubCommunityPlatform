...
class NormalizationServiceEventPublishingIntegrationTest(ContractsTestBase):
    ...
    def test_normalize_contract_publishes_started_and_completed_events(self):
        ...
        started_event = (
            Event.objects.filter(
                event_type="normalization.started", data__contract_id=str(self.contract.id)
            )
            .order_by("-timestamp")
            .first()
        )
        ...
        completed_event = (
            Event.objects.filter(
                event_type="normalization.completed", data__contract_id=str(self.contract.id)
            )
            .order_by("-timestamp")
            .first()
        )
    ...
    def test_normalize_contract_failure_publishes_started_and_failed_events(self):
        ...
        failed_event = (
            Event.objects.filter(
                event_type="normalization.failed", data__contract_id=str(self.contract.id)
            )
            .order_by("-timestamp")
            .first()
        )
    ...
    def test_normalize_contract_event_publishing_failure_does_not_block_normalization(self):
        ...
        service = NormalizationService(tenant_id="invalid-uuid", user_id=str(self.user.id))
