...
class ODCSAlertsConfigurationTest(TestCase):
    ...
    def setUp(self):
        ...
        self.alerts_file = Path(__file__).parent.parent.parent.parent.parent / "monitoring" / "prometheus" / "alerts" / "odcs-alerts.yml"
    ...
    def test_alert_structure_is_valid(self):
        ...
        for rule in rules:
            ...
            self.assertEqual(labels['component'], 'odcs', f"Alert '{alert_name}' component should be 'odcs'")
    ...
    def test_normalization_failure_alert_configuration(self):
        ...
        self.assertIn('odcs_normalization_total', alert['expr'], "Alert should reference odcs_normalization_total metric")
        self.assertIn('status=~"NORMALIZATION_FAILED|failed|error"', alert['expr'], "Alert should filter for failed/error status")
        ...
        self.assertIn('version', alert['expr'], "Alert should track by version for all ODCS versions")
    ...
    def test_version_specific_normalization_errors_alert_configuration(self):
        ...
        self.assertIn('odcs_normalization_total', alert['expr'], "Alert should reference odcs_normalization_total metric")
        self.assertIn('version=~"3.0.2|3.0.1|3.0.0"', alert['expr'], "Alert should filter for stable versions")
        self.assertEqual(alert['labels']['severity'], 'warning', "Alert severity should be warning")
        self.assertIn('0.10', alert['expr'], "Alert threshold should be 10% (0.10) for version-specific errors")
    ...
    def test_regression_detection_alert_configuration(self):
        ...
        self.assertIn('odcs_normalization_regression_total', alert['expr'], "Alert should reference odcs_normalization_regression_total metric")
    ...
    def test_regression_rate_alert_configuration(self):
        ...
        self.assertIn('odcs_normalization_regression_total', alert['expr'], "Alert should reference regression metric")
        self.assertIn('odcs_normalization_total', alert['expr'], "Alert should compare with total normalization")
        self.assertIn('regression_type', alert['expr'], "Alert should track regression type")
        self.assertEqual(alert['labels']['severity'], 'critical', "Alert severity should be critical for high regression rate")
        self.assertIn('0.05', alert['expr'], "Alert threshold should be 5% (0.05)")
    ...
    def test_version_distribution_anomaly_alert_configuration(self):
        ...
        self.assertIn('odcs_version_distribution_total', alert['expr'], "Alert should reference version distribution metric")
        self.assertIn('version="3.0.2"', alert['expr'], "Alert should check 3.0.2 version distribution")
    ...
    def test_prometheus_config_includes_odcs_alerts(self):
        ...
        self.assertIn('alerts/odcs-alerts.yml', config_content,
    ...
    def test_alerts_reference_correct_metrics(self):
        ...
        expected_metrics = {
            ...
            'ODCSNormalizationRegressionRate': ['odcs_normalization_regression_total', 'odcs_normalization_total'],
            ...
            if alert_name in expected_metrics:
                for metric in expected_metrics[alert_name]:
                    self.assertIn(metric, expr,
                                 f"Alert '{alert_name}' should reference metric '{metric}'")
