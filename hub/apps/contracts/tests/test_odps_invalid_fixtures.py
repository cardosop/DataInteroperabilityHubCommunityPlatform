        ...
        for fixture_path in invalid_files:
            with self.subTest(fixture=fixture_path.name):
                try:
                    ...
                    failed_validations.append(f"{fixture_path.name}: Invalid JSON: {e}")
                except Exception as e:
                    # Other errors are also validation failures
                    failed_validations.append(f"{fixture_path.name}: Unexpected error: {e}")

        # All invalid fixtures should have failed validation
        if failed_validations:
            self.fail(
                "Some invalid fixtures did not fail validation as expected:\n"
