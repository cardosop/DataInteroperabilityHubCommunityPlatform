...
class SchemeRejectionTests(TestCase):
    ...
    def test_file_scheme_rejected(self):
        self.assertFalse(is_safe_url("file:///etc/passwd"), "file:// scheme must be rejected")

    def test_gopher_scheme_rejected(self):
        self.assertFalse(
            is_safe_url("gopher://evil.example/_payload"), "gopher:// scheme must be rejected"
        )

    def test_dict_scheme_rejected(self):
        self.assertFalse(
            is_safe_url("dict://localhost:6379/INFO"), "dict:// scheme must be rejected"
        )

    def test_ftp_scheme_rejected(self):
        self.assertFalse(
            is_safe_url("ftp://internal.fileshare.example/file.json"),
    ...
    def test_data_scheme_rejected(self):
        ...
        self.assertFalse(
            is_safe_url("data:text/plain;base64,SGVsbG8="), "data: scheme must be rejected"
        )

    def test_javascript_scheme_rejected(self):
        self.assertFalse(is_safe_url("javascript:alert(1)"), "javascript: scheme must be rejected")
...
class BlockedHostsTests(TestCase):
    ...
    def test_gcp_imds_blocked(self):
        ...
        with _mock_dns("169.254.169.254"):
            self._expect_security_violation("http://metadata.google.internal/computeMetadata/v1/")
...
class HelperWithBlockedURLTests(TestCase):
    ...
    def test_blocked_contractURL_yields_no_model_and_warning(self):
        ...
        self.assertEqual(hub["models"], [], f"Expected no models; got {hub['models']!r}")
        # … and a STRUCTURELESS warning surfaces so ops can find it.
        self.assertTrue(
            any("STRUCTURELESS_PORT_NO_RESOLVABLE_PAYLOAD" in w for w in warnings),
...
class HelperWithMockedResolverTests(TestCase):
    def test_public_https_contractURL_resolves_to_model(self):
        ...
        self.assertEqual(warnings, [], f"Expected no warnings; got {warnings!r}")
