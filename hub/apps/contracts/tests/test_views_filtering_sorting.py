...
class ContractViewFilteringTest(ContractsAPITestBase):
    ...
    def setUp(self):
        ...
        self.contract2 = Contract.objects.create(
            ...
            hub_contract_json={
                ...
                "schema": {"fields": []},
                "quality": {"default_profile_key": "profile2"},
                "privacy_compliance": {"jurisdictions": ["CCPA"]},
            },
        )
    ...
    def test_list_contracts_without_filter(self):
        ...
        self.assertEqual(
            all_contracts.count(), 2, f"Expected 2 contracts in DB, got {all_contracts.count()}"
        )
        ...
        self.assertEqual(
            len(results),
            2,
            f"Expected 2 contracts, got {len(results)}. Results: {[r.get('id') for r in results]}",
        )
...
class ContractViewSortingTest(ContractsAPITestBase):
    ...
    def test_sort_by_multiple_fields(self):
        ...
        response = self.client.get(
            "/api/v1/contracts/", {"ordering": "-compliance_risk,-created_at"}
        )
