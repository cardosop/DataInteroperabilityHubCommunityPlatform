...
class APIPaginationConsistencyTest(TestCase):
    """
    ...
    def test_contracts_list_endpoint_supports_pagination(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK,
                        "Contracts list should support pagination")
    ...
    def test_pagination_metadata_is_complete(self):
        ...
        has_next_indicator = (
            response.data.get('next') is not None or
            response.data.get('has_next') is True or
            response.data.get('next_page') is not None
        )
        self.assertTrue(has_next_indicator, "Should indicate there's a next page")
        # Check for previous indicator
        has_previous = (
            response.data.get('previous') is not None or
            response.data.get('has_previous') is True or
            response.data.get('previous_page') is not None
        )
    ...
    def test_pagination_edge_case_empty_results(self):
        ...
        self.assertIn(response.data['total_pages'], [0, 1],
                     "Total pages should be 0 or 1 for empty results")
    ...
    def test_pagination_error_handling_excessive_page_size(self):
        ...
        if response.status_code == status.HTTP_200_OK:
            self.assertLessEqual(response.data['page_size'], 100,
                               "Page size should be capped at maximum")
        else:
