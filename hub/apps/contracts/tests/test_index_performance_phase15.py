...
class IndexPerformanceTestCase(TestCase):
        ...
        for i in range(100):
            contract = Contract.objects.create(
                ...
                hub_contract_json={
                    ...
                    "servicelevels": [
                        {"property": "availability", "target": 99.0 + (i % 10) * 0.1}
                    ],
    ...
    def _explain_query(self, queryset):
        ...
        try:
            with db_tx.atomic(), connection.cursor() as cursor:
                cursor.execute(
    ...
    def test_contact_email_filter_index_usage(self):
        ...
        queryset = Contract.objects.filter(
            tenant=self.tenant, hub_contract_json__contact__email="contact1@example.com"
        )
    ...
    def test_server_type_filter_index_usage(self):
        ...
        queryset = Contract.objects.filter(
            tenant=self.tenant, hub_contract_json__servers__type="s3"
        )
    ...
    def test_model_name_filter_index_usage(self):
        ...
        queryset = Contract.objects.filter(
            tenant=self.tenant, hub_contract_json__models__name="model-1"
        )
    ...
    def test_lineage_query_performance(self):
        ...
        for i, contract in enumerate(self.contracts[:10]):
            contract.hub_contract_json["lineage"] = {
                "entries": [{"namespace": "ns1", "name": f"source-{i}", "model_name": "model1"}]
            }
            contract.save()

        # Query contracts with lineage
        queryset = Contract.objects.filter(
            tenant=self.tenant, hub_contract_json__lineage__isnull=False
        )
