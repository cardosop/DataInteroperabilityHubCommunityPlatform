...
class ODPSExportMetricsTestBase(TestCase):
    ...
    def setUp(self):
        ...
        # Authenticate client BEFORE creating contract (matches working test pattern)
        ...
        self.contract = Contract.objects.create(
            ...
            original_raw=json_module.dumps(self.hub_contract),  # Include original_raw for consistency
        ...
        self.assertEqual(self.contract.tenant_id, self.tenant.id,
                        f"Contract tenant_id ({self.contract.tenant_id}) should match tenant.id ({self.tenant.id})")
        self.assertEqual(self.contract.tenant_id, self.user.tenant_id,
        ...
        self.assertTrue(filtered_contracts.filter(id=self.contract.id).exists(),
...
class ODPSExportMetricsCollectionTest(ODPSExportMetricsTestBase):
    ...
    def test_export_json_collects_metrics(self):
        ...
        # Verify contract exists in database
        self.assertTrue(Contract.objects.filter(id=self.contract.id, tenant=self.tenant).exists(),
        ...
        self.assertEqual(self.user.tenant_id, self.contract.tenant_id,
        ...
        self.assertTrue(contract_in_queryset,
        ...
        self.assertIsNotNone(db_user.tenant_id,
        ...
        self.assertEqual(db_user.tenant_id, self.contract.tenant_id,
