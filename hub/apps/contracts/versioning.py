...
def parse_version(version_string: str) -> SemanticVersion | None:
    """
...
def validate_version(version_string: str) -> tuple[bool, str | None]:
    """
    ...
    if parsed is None:
        return (
            False,
            f"Invalid version format: '{version_string}'. Expected format: MAJOR.MINOR.PATCH (e.g., '1.0.0')",
        )

    return True, None


def detect_version(contract_data: dict[str, Any]) -> str | None:
    """
...
def ensure_version(contract_data: dict[str, Any]) -> dict[str, Any]:
    ...
    if version:
        is_valid, _error = validate_version(version)
        if is_valid:
...
class VersionMigrationFramework:
    ...
    @staticmethod
    def migrate(
        contract_data: dict[str, Any], from_version: str, to_version: str
    ...
    def migrate_to_latest(contract_data: dict[str, Any]) -> tuple[dict[str, Any], list]:
        ...
        return VersionMigrationFramework.migrate(
            contract_data, current_version, CURRENT_HUBCONTRACT_VERSION_STRING
        )
