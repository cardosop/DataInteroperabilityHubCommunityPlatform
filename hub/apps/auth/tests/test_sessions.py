...
class TestListSessions(TestCase):
    ...
    def test_session_has_created_at(self):
        ...
        client.credentials(HTTP_AUTHORIZATION=f"Bearer {token}")
        resp = client.get("/api/v1/auth/sessions/")
        self.assertIn("created_at", resp.data[0])
    ...
    def test_session_has_expires_at(self):
        ...
        client.credentials(HTTP_AUTHORIZATION=f"Bearer {token}")
        resp = client.get("/api/v1/auth/sessions/")
        self.assertIn("expires_at", resp.data[0])
    ...
    def test_session_has_revoked_at(self):
        ...
        client.credentials(HTTP_AUTHORIZATION=f"Bearer {token}")
        resp = client.get("/api/v1/auth/sessions/")
        self.assertIn("revoked_at", resp.data[0])
    ...
    def test_session_has_is_current(self):
        ...
        client.credentials(HTTP_AUTHORIZATION=f"Bearer {token}")
        resp = client.get("/api/v1/auth/sessions/")
        self.assertIn("is_current", resp.data[0])
    ...
    def test_session_has_non_null_id(self):
        ...
        client.credentials(HTTP_AUTHORIZATION=f"Bearer {token}")
        resp = client.get("/api/v1/auth/sessions/")
        self.assertIsNotNone(resp.data[0]["id"])
    ...
    def test_after_logout_includes_revoked_sessions(self):
        ...
        self.assertGreaterEqual(len(revoked), 1)
...
class TestRevokeSession(TestCase):
    def test_unauthenticated_returns_401(self):
        client, *_ = _make_test_data()
    ...
    def test_sets_revoked_at(self):
        ...
        revoked = next((s for s in after.data if s["id"] == sid), None)
    ...
    def test_other_user_session_returns_404(self):
        ...
        self.assertEqual(other_sessions.status_code, status.HTTP_200_OK)
        self.assertGreaterEqual(len(other_sessions.data), 1)
        other_sid = other_sessions.data[0]["id"]
        client.force_authenticate(user=user)
        resp = client.post(f"/api/v1/auth/sessions/{other_sid}/revoke/")
    ...
    def test_invalid_uuid_format(self):
        ...
        client, _, user, _ = _make_test_data()
        client.force_authenticate(user=user)
        resp = client.post("/api/v1/auth/sessions/invalid-uuid/revoke/")
        self.assertEqual(resp.status_code, status.HTTP_404_NOT_FOUND)
...
class TestEndAllOtherSessions(TestCase):
    def test_unauthenticated_returns_401(self):
        client, *_ = _make_test_data()
    ...
    def test_has_no_other_sessions_returns_zero_count(self):
        ...
        client.force_authenticate(user=user)
        resp = client.post("/api/v1/auth/sessions/end-all-others/")
        self.assertIn("revoked_count", resp.data)
        self.assertEqual(resp.data["revoked_count"], 0)
    ...
    def test_revokes_other_active_sessions(self):
        ...
        resp = client.post("/api/v1/auth/sessions/end-all-others/")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertGreaterEqual(resp.data["revoked_count"], 1)

        # Verify: only the current session should remain valid
        sessions = client.get("/api/v1/auth/sessions/")
        valid_sessions = [s for s in sessions.data if s.get("revoked_at") is None]
        self.assertGreaterEqual(len(valid_sessions), 1)
    ...
    def test_current_session_not_revoked(self):
        ...
        current_before = next(s for s in sessions_before.data if s.get("is_current") is True)
        ...
        current_after = next(s for s in sessions_after.data if s["id"] == current_before["id"])
        self.assertIsNone(current_after["revoked_at"])
    ...
    def test_returns_message_and_count(self):
        ...
        resp = client.post("/api/v1/auth/sessions/end-all-others/")
        self.assertIn("message", resp.data)
        self.assertIsInstance(resp.data["revoked_count"], int)
    ...
    def test_is_idempotent(self):
        ...
        self.assertEqual(resp2.data["revoked_count"], 0)
