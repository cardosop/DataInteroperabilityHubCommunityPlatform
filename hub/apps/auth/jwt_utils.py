...
class JWTTokenGenerator:
    ...
    def generate_access_token(
        ...
        # Link to refresh token so end-all-other-sessions can exclude
        # the current session (277.B.068).
        if refresh_token_id:
        ...
        token = jwt.encode(payload, _sign_key, algorithm=settings.JWT_ALGORITHM)

        return token

    @staticmethod
    def decode_access_token(token: str, *, verify_version: bool = True) -> dict[str, Any] | None:
        """
        ...
        if verify_version and not JWTTokenGenerator._validate_token_version(payload):
            return None
    ...
    def _validate_token_version(payload: dict[str, Any]) -> bool:
        ...
        if not user_id:
            return False
        try:
            db_version = User.objects.values_list("token_version", flat=True).get(id=user_id)
    ...
    @staticmethod
    def get_user_from_token(payload: dict[str, Any]) -> User | None:
        """
