...
@extend_schema(
    ...
    responses={201: RegisterResponseSerializer, 400: OpenApiResponse(description='Validation error or email already exists'), 429: OpenApiResponse(description='Rate limit exceeded')},
    tags=['Authentication']
)
@api_view(['POST'])
@permission_classes([permissions.AllowAny])
...
def register(request):
    ...
    try:
        ...
        logger.error(
            "user_created_event_publish_failed",
            user_id=str(user.id),
            error=str(e),
            exc_info=True
        )
...
@extend_schema(
    responses={200: CurrentUserSerializer, 401: OpenApiResponse(description='Unauthorized - Invalid or missing token')},
    tags=['Authentication']
)
@api_view(['GET'])
@permission_classes([permissions.IsAuthenticated])
...
@extend_schema(
    responses={200: RefreshTokenResponseSerializer(many=True)},
    tags=['Authentication']
)
@api_view(['GET'])
@permission_classes([permissions.IsAuthenticated])
def list_active_sessions(request):
    ...
    for token in refresh_tokens:
        ...
        session_data = {
            ...
            'is_current': token.id == most_recent_active and not token.is_revoked() and not token.is_expired(),
        }
        sessions.append(session_data)

    serializer = RefreshTokenResponseSerializer(sessions, many=True)
