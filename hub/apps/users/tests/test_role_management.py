...
class RoleManagementTest(TestCase):
    ...
    def test_assign_role(self):
        ...
        data = {"role_id": str(self.provider_role.id), "action": "assign"}

        response = self.client.post(
            f"/api/v1/users/{self.regular_user.id}/roles/", data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        # Verify role was assigned
        self.assertTrue(
            UserRole.objects.filter(user=self.regular_user, role=self.provider_role).exists()
        )
    ...
    def test_remove_role(self):
        ...
        data = {"role_id": str(self.provider_role.id), "action": "remove"}
    ...
    def test_assign_role_cross_tenant(self):
        ...
        other_role, _ = Role.objects.get_or_create(
            tenant=other_tenant, name="DATA_CONSUMER", defaults={"description": "Data Consumer"}
        )

        data = {"role_id": str(other_role.id), "action": "assign"}
