...
class JSONFieldRegressionTest(TestCase):
    ...
    def setUp(self):
        ...
        self.contract = Contract.objects.create(
            ...
            hub_contract_json={
                ...
                "privacy_compliance": {"jurisdictions": ["GDPR"], "contains_personal_data": True},
            },
    ...
    def test_jsonfield_query_regression(self):
        ...
        contracts = qs.filter(hub_contract_json__info__tags__contains=["tag1"])
        self.assertEqual(contracts.count(), 1)

        contracts = qs.filter(hub_contract_json__quality__default_profile_key="great_expectations")
        self.assertEqual(contracts.count(), 1)

        contracts = qs.filter(hub_contract_json__privacy_compliance__contains_personal_data=True)
        self.assertEqual(contracts.count(), 1)
