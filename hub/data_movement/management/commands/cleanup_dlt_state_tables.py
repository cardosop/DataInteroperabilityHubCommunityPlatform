...
class Command(BaseCommand):
    ...
    def handle(self, **options):
        ...
        schemas = list(_DLT_SCHEMAS) if schema_filter == "both" else [schema_filter]
        ...
        for schema_name in schemas:
            ...
            for table_name in tables:
                ...
                if dry_run:
                    self.stdout.write(
                        f"[DRY-RUN] Would drop orphaned table: {schema_name}.{table_name}"
                    )
                    dropped += 1
                    continue
                ...
                if execute:
                    ...
                    self.stdout.write(f"[DROPPED] Orphaned table: {schema_name}.{table_name}")
                    dropped += 1
                    logger.info(
        ...
        if dropped == 0:
            self.stdout.write(self.style.SUCCESS("No orphaned dlt tables found."))
        else:
            action = "Would drop" if dry_run else "Dropped"
            self.stdout.write(self.style.SUCCESS(f"{action} {dropped} orphaned dlt table(s)."))
    ...
    def _active_ingestion_ids(self) -> set[str]:
        ...
        return set(
            ScheduledIngestion.objects.filter(status__in=("ACTIVE", "PAUSED", "ERROR")).values_list(
                "id", flat=True
            )
        )
    ...
    def _active_export_ids(self) -> set[str]:
        ...
        return set(
            ScheduledExport.objects.filter(status__in=("ACTIVE", "PAUSED", "ERROR")).values_list(
                "id", flat=True
            )
        )
