...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=24,
            help="Max hours before a run is considered stuck",
        )
    ...
    def handle(self, **options):
        ...
        if stuck_count == 0:
            self.stdout.write(
                self.style.SUCCESS(f"No stuck runs found (timeout: {timeout_hours}h)")
            )
        else:
    ...
    def _check_ingestion(self, cutoff, dry_run: bool) -> int:
        ...
        for run in stuck:
            self.stdout.write(
                f"  STUCK ingestion: {run.id} (started: {run.started_at}, tenant: {run.tenant_id})"
            )
            if not dry_run:
                run.status = "FAILED"
                run.error_message = (
                    f"Run stuck >{int((timezone.now() - run.started_at).total_seconds() / 3600)}h"
                )
    ...
    def _check_export(self, cutoff, dry_run: bool) -> int:
        ...
        for run in stuck:
            self.stdout.write(
                f"  STUCK export: {run.id} (started: {run.started_at}, tenant: {run.tenant_id})"
            )
            if not dry_run:
                run.status = "FAILED"
                run.error_message = (
