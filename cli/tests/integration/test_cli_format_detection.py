...
def odps_json_file():
    ...
    odps_data = {
        ...
        "product": {
            "details": {
                "en": {"productID": f"odps-product-{uuid.uuid4().hex[:8]}", "name": "ODPS Product"}
            },
            "contract": {
                "spec": {
...
def odcs_json_file():
    ...
    odcs_data = {
        "apiVersion": "3.0.2",
        "kind": "DataContract",
        "metadata": {"name": f"odcs-contract-{uuid.uuid4().hex[:8]}", "version": "1.0.0"},
        "spec": {"schema": {"fields": [{"name": "id", "type": "string"}]}},
    }
...
class TestCLIFormatDetection:
    ...
    def test_auto_detection_odps_vs_odcs_json(self, odps_json_file, odcs_json_file, setup_config):
        ...
        result_odcs = runner.invoke(
            cli, ["contracts", "create", "--file", odcs_json_file, "--format", "json"]
        )
    ...
    def test_auto_detection_odps_vs_odcs_yaml(self, odps_yaml_file, odcs_yaml_file, setup_config):
        ...
        # Test ODCS detection via regular create
        result_odcs = runner.invoke(
            cli, ["contracts", "create", "--file", odcs_yaml_file, "--format", "json"]
        )
    ...
    def test_original_spec_type_display_in_list(self, odps_json_file, odcs_json_file, setup_config):
        ...
        create_odcs = runner.invoke(
            cli, ["contracts", "create", "--file", odcs_json_file, "--output-format", "json"]
        )
    ...
    def test_odps_format_support_in_contract_commands(self, odps_json_file, setup_config):
        ...
        result_get = runner.invoke(cli, ["contracts", "get", contract_id, "--format", "json"])
        ...
        result_lint = runner.invoke(cli, ["contracts", "lint", contract_id, "--format", "json"])
        # May succeed or fail depending on contract validity, but should not crash
        assert result_lint.exit_code == 0

    @pytest.mark.skipif(not _check_api_available(), reason="API service not available")
    def test_format_detection_from_file_extension(
        self, odps_json_file, odps_yaml_file, setup_config
    ):
