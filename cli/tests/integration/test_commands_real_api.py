...
class TestCLIIntegrationRealAPI(LiveServerTestCase):
  ...
  "product": {
    ...
    def test_contracts_create_odps_link_odcs_real_api(self):
        ...
        asset_data = self._create_asset_via_api(
            key='cli-test-asset-for-odps-link',
            name='CLI Test Asset for ODPS Link'
        )
      ...
      "name": "CLI Test ODCS for Link"
        ...
        try:
            ...
            result = self.runner.invoke(cli, [
                'contracts', 'create-odps',
                '--file', temp_file_path,
                '--link-odcs', odcs_contract_data['id']
            ])
            ...
            assert result.exit_code == 0, f"CLI failed with output: {result.output}"
            assert 'ODPS contract created and linked successfully' in result.output or 'id' in result.output
    ...
    "marketplace": {
      ...
      "accessMethods": {
        ...
        try:
            ...
            assert create_result.exit_code == 0, f"Failed to create ODPS contract: {create_result.output}"

            # Extract contract ID from output (look for UUID pattern)
            import re
            contract_id_match = re.search(r'[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}', create_result.output)
