...
class TestBaaSAPIKeysIntegration(LiveServerTestCase):
    ...
    def setUp(self):
        ...
        self.tenant = Tenant.objects.create(
            name=f"BaaS Test Tenant {unique_id}", slug=f"baas-test-tenant-{unique_id}"
        )
    ...
    def test_create_api_key_integration(self):
        ...
        result = self.runner.invoke(
            cli, ["baas", "api-keys", "create", "--name", "Integration Test Key", "--tier", "FREE"]
        )
        ...
        api_key = APIKey.objects.filter(tenant=self.tenant, name="Integration Test Key").first()
    ...
    def test_get_api_key_integration(self):
        ...
        result = self.runner.invoke(cli, ["baas", "api-keys", "get", str(api_key.id)])
    ...
    def test_update_api_key_integration(self):
        ...
        result = self.runner.invoke(
            cli, ["baas", "api-keys", "update", str(api_key.id), "--name", "Updated Name"]
        )
    ...
    def test_revoke_api_key_integration(self):
        ...
        result = self.runner.invoke(cli, ["baas", "api-keys", "revoke", str(api_key.id)])
    ...
    def test_create_api_key_with_expiration_integration(self):
        ...
        result = self.runner.invoke(
            cli,
            ["baas", "api-keys", "create", "--name", "Expiring Key", "--expires-at", future_date],
        )
        ...
        api_key = APIKey.objects.filter(tenant=self.tenant, name="Expiring Key").first()
        assert api_key is not None
    ...
    def test_list_api_keys_with_tier_filter_integration(self):
        ...
        result = self.runner.invoke(cli, ["baas", "api-keys", "list", "--tier", "PRO"])
