...
class TestODPSCompleteWorkflows:
    ...
    def setup_config(self):
        ...
        if not api_key:
            raise ValueError(
                "No API key available. Set DATAHUB_API_KEY or TEST_API_KEY environment variable."
            )
...
"""
    ...
    def _create_asset_via_api(self, key, name, description=None):
        ...
        url = os.environ.get("MESHANT_API_URL", "http://localhost:8000/api/v1") + "/assets/"
        data = {"key": key, "name": name, "visibility": "INTERNAL"}
        if description:
        ...
        try:
            ...
            if result.exit_code != 0:
                ...
                if "workflow" in error_msg.lower() or "Product creation failed" in error_msg:
                    pytest.skip(
                        f"Workflow execution failed. This may indicate a workflow configuration issue. Error: {error_msg}"
                    )
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                assert download_result.exit_code == 0, (
                    f"Failed to download ODPS: {download_result.output}"
                )
    ...
    def test_complete_odps_workflow_link_flow(self, runner):
        ...
        asset_data = self._create_asset_via_api(
            key="odps-link-workflow-asset", name="ODPS Link Workflow Asset"
        )
  ...
  "schema": {
    "fields": [
      {"name": "id", "type": "string", "nullable": false}
        ...
        odcs_contract_data = self._create_odcs_contract_via_api(
            asset_id=asset_data["id"], original_raw=odcs_content
        )
        odcs_id = odcs_contract_data["id"]

        # Step 3: Create ODPS contract (without linking)
        odps_content = self._create_valid_odps_json(
            "link-workflow-product", include_marketplace=False
        )
        ...
        try:
            ...
            assert (
                "linked successfully" in link_result.output.lower()
                or "ODPS contract linked" in link_result.output
            )
            ...
            assert list_links_result.exit_code == 0, (
                f"Failed to list links: {list_links_result.output}"
            )
            ...
            assert (
                "unlinked successfully" in unlink_result.output.lower()
                or "ODPS contract unlinked" in unlink_result.output
            )
            ...
            assert (
                "No links found" in list_links_after_result.output
                or "ODPS Link" not in list_links_after_result.output
            )
        finally:
            if os.path.exists(temp_file_path):
                os.unlink(temp_file_path)
    ...
    def test_complete_odps_workflow_create_with_link_flag(self, runner):
        ...
        asset_data = self._create_asset_via_api(
            key="odps-create-link-asset", name="ODPS Create Link Asset"
        )
  ...
  "schema": {
    "fields": [{"name": "id", "type": "string", "nullable": false}]
        ...
        try:
            result = runner.invoke(
                cli, ["contracts", "create-odps", "--file", temp_file_path, "--link-odcs", odcs_id]
            )
            ...
            assert (
                ...
                or "linked" in result.output.lower()
            )
    ...
    def test_odps_export_yaml_format(self, runner):
        ...
        try:
            ...
            assert (
                "schema:" in export_result.output
                or "product:" in export_result.output
                or "version:" in export_result.output
            )
        finally:
            if os.path.exists(temp_file_path):
                os.unlink(temp_file_path)
    ...
    def test_odps_export_with_version(self, runner):
        ...
        try:
            ...
            assert export_result.exit_code == 0, (
                f"Export with version failed: {export_result.output}"
            )
            export_data = json.loads(export_result.output)
    ...
    def test_odps_download_with_version(self, runner):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                assert download_result.exit_code == 0, (
                    f"Download with version failed: {download_result.output}"
                )
                assert os.path.exists(download_path), "Downloaded file does not exist"
        finally:
            if os.path.exists(temp_file_path):
    ...
    def test_odps_link_existing_contracts(self, runner):
        ...
        # Create asset
        asset_data = self._create_asset_via_api(
            key="odps-link-test-asset", name="ODPS Link Test Asset"
        )
  ...
  "schema": {
    ...
    def test_odps_unlink_contracts(self, runner):
        ...
        asset_data = self._create_asset_via_api(
            key="odps-unlink-test-asset", name="ODPS Unlink Test Asset"
        )
  ...
  "schema": {
    ...
    def test_odps_list_links(self, runner):
        ...
        # Create asset and contracts
        asset_data = self._create_asset_via_api(
            key="odps-list-links-asset", name="ODPS List Links Asset"
        )
  ...
  "schema": {
    "fields": [{"name": "id", "type": "string", "nullable": false}]
        ...
        try:
            ...
            list_result_odps = runner.invoke(cli, ["contracts", "list-links", odps_id])

            assert list_result_odps.exit_code == 0
            # Should show ODCS link or link information
            assert (
                "ODCS Link" in list_result_odps.output
                or odcs_id in list_result_odps.output
                or "Link" in list_result_odps.output
            )
        finally:
            if os.path.exists(temp_file_path):
                os.unlink(temp_file_path)
    ...
    def test_odps_list_links_no_links(self, runner):
        ...
        asset_data = self._create_asset_via_api(
            key="odps-no-links-asset", name="ODPS No Links Asset"
        )
  ...
  "schema": {
    "fields": [{"name": "id", "type": "string", "nullable": false}]
        ...
        assert (
            "No links found" in list_result.output
            or "No ODPS Link" in list_result.output
            or len(list_result.output.strip()) == 0
        )
    ...
    def test_odps_error_invalid_contract_id(self, runner):
        ...
        export_result = runner.invoke(cli, ["contracts", "export", invalid_id, "--format", "odps"])

        assert export_result.exit_code != 0, "Should fail with invalid contract ID"
        assert (
            "error" in export_result.output.lower()
            or "not found" in export_result.output.lower()
            or "invalid" in export_result.output.lower()
        )

        # Test download with invalid ID
        download_result = runner.invoke(
            cli, ["contracts", "download", invalid_id, "--format", "odps"]
        )

        assert download_result.exit_code != 0, "Should fail with invalid contract ID"

        # Test link with invalid IDs
        link_result = runner.invoke(cli, ["contracts", "link-odps", invalid_id, invalid_id])
    ...
    def test_odps_error_missing_required_options(self, runner):
        ...
        try:
            result = runner.invoke(cli, ["contracts", "create-odps", "--file", temp_file_path])

            assert result.exit_code != 0, "Should fail without --extract-odcs or --link-odcs"
            assert (
                "required" in result.output.lower()
                or "extract-odcs" in result.output.lower()
                or "link-odcs" in result.output.lower()
            )
        finally:
            if os.path.exists(temp_file_path):
                os.unlink(temp_file_path)
    ...
    def test_odps_error_invalid_file_path(self, runner):
        ...
        result = runner.invoke(
            cli, ["contracts", "create-odps", "--file", invalid_path, "--extract-odcs"]
        )
    ...
    def test_odps_error_invalid_odps_version(self, runner):
        ...
        try:
            ...
            if export_result.exit_code != 0:
                assert (
                    "version" in export_result.output.lower()
                    or "invalid" in export_result.output.lower()
                    or "error" in export_result.output.lower()
                )
        finally:
            if os.path.exists(temp_file_path):
                os.unlink(temp_file_path)
    ...
    def test_odps_error_linking_nonexistent_contracts(self, runner):
        ...
        link_result = runner.invoke(
            cli, ["contracts", "link-odps", invalid_odcs_id, invalid_odps_id]
        )
    ...
    def test_odps_error_unlinking_without_link(self, runner):
        ...
        asset_data = self._create_asset_via_api(
            key="odps-unlink-error-asset", name="ODPS Unlink Error Asset"
        )
  ...
  "schema": {
    "fields": [{"name": "id", "type": "string", "nullable": false}]
        ...
        if unlink_result.exit_code != 0:
            assert (
                "error" in unlink_result.output.lower()
                or "not found" in unlink_result.output.lower()
                or "link" in unlink_result.output.lower()
            )
    ...
    def test_e2e_complete_odps_workflow(self, runner):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                pricing_result = runner.invoke(cli, ["contracts", "get-pricing", odps_id])

                assert pricing_result.exit_code == 0, f"Step 5a failed: {pricing_result.output}"
                assert (
                    "Pricing Plans" in pricing_result.output
                    or "pricing" in pricing_result.output.lower()
                )

                access_methods_result = runner.invoke(
                    cli, ["contracts", "get-access-methods", odps_id]
                )

                assert access_methods_result.exit_code == 0, (
                    f"Step 5b failed: {access_methods_result.output}"
                )
                assert (
                    "Access Methods" in access_methods_result.output
                    or "access" in access_methods_result.output.lower()
                )
  ...
  "schema": {
    "fields": [
      ...
      {"name": "timestamp", "type": "timestamp", "nullable": false}
            ...
            export_linked_result = runner.invoke(
                cli,
                [
                    "contracts",
                    "export",
            ...
            assert export_linked_result.exit_code == 0, (
                f"Step 9 failed: {export_linked_result.output}"
            )
            ...
            verify_unlink_result = runner.invoke(cli, ["contracts", "list-links", odcs_id])

            assert verify_unlink_result.exit_code == 0, (
                f"Step 11 failed: {verify_unlink_result.output}"
            )
            # Should show no ODPS links
            assert (
                "No links found" in verify_unlink_result.output
                or "ODPS Link" not in verify_unlink_result.output
                or odps_id not in verify_unlink_result.output
            )
        finally:
            if os.path.exists(temp_file_path):
                os.unlink(temp_file_path)
