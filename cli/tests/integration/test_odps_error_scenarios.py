...
class TestODPSErrorScenarios:
    ...
    def test_create_odps_missing_flow_option(self, runner, temp_file):
        ...
        assert (
            isinstance(result.exception, ODPSParameterError)
            or "Must specify either" in result.output
        )
    ...
    def test_create_odps_both_flow_options(self, runner, temp_file):
        ...
        assert result.exit_code != 0
        assert (
            isinstance(result.exception, ODPSParameterError) or "Cannot use both" in result.output
        )
    ...
    def test_create_odps_invalid_version(self, runner, temp_file):
        ...
        result = runner.invoke(
            cli,
            ["contracts", "create-odps", "--file", file_path, "--extract-odcs", "--version", "4"],
        )

        assert result.exit_code != 0
        assert (
            isinstance(result.exception, ODPSParameterError)
            or "Invalid ODPS version format" in result.output
        )
    ...
    def test_create_odps_invalid_contract_id(self, runner, temp_file):
        ...
        result = runner.invoke(
            cli, ["contracts", "create-odps", "--file", file_path, "--link-odcs", "  "]
        )
    ...
    def test_export_invalid_contract_id(self, runner):
        ...
        result = runner.invoke(cli, ["contracts", "export", "", "--format", "odps"])
    ...
    def test_export_invalid_version(self, runner):
        ...
        result = runner.invoke(
            cli, ["contracts", "export", "contract-123", "--format", "odps", "--version", "invalid"]
        )
    ...
    def test_link_odps_invalid_ids(self, runner):
        ...
        result = runner.invoke(cli, ["contracts", "link-odps", "", "odps-123"])
    ...
    def test_unlink_odps_invalid_id(self, runner):
        ...
        result = runner.invoke(cli, ["contracts", "unlink-odps", "  "])
    ...
    def test_list_links_invalid_id(self, runner):
        ...
        result = runner.invoke(cli, ["contracts", "list-links", ""])
    ...
    def test_download_invalid_contract_id(self, runner):
        ...
        result = runner.invoke(cli, ["contracts", "download", "", "--format", "odps"])
    ...
    def test_download_invalid_version(self, runner):
        ...
        result = runner.invoke(
            cli, ["contracts", "download", "contract-123", "--format", "odps", "--version", "4"]
        )
...
class TestODPSErrorMessages:
    ...
    def test_validation_error_has_suggestion(self, runner, temp_file, mock_api_client):
        ...
        file_path, _content = temp_file(".json", '{"invalid": "data"}')
        ...
        mock_api_client.post.side_effect = ClickException(
            f"API error (SCHEMA_VALIDATION_FAILED): {json.dumps(error_response)}"
        )
    ...
    def test_parameter_error_has_suggestion(self, runner, temp_file):
        ...
        result = runner.invoke(
            cli, ["contracts", "create-odps", "--file", file_path, "--version", "invalid"]
        )
        ...
        assert (
            "suggestion" in output_lower
            or "format" in output_lower
            or "major.minor" in output_lower
        )
