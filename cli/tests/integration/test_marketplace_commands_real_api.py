...
class TestMarketplaceCommandsIntegration(LiveServerTestCase):
    ...
    def setUp(self):
        ...
        self.tenant = Tenant.objects.create(
            name="Marketplace CLI Test Tenant",
            slug="marketplace-cli-test-tenant"
        )
    ...
    def test_marketplace_connections_create_list_get_delete(self):
        ...
        assert isinstance(list_data, list) or (isinstance(list_data, dict) and 'results' in list_data)
    ...
    def test_marketplace_mappings_list_get_delete(self):
        ...
        # Clean up
        self.runner.invoke(cli, [
            'marketplace', 'connections', 'delete', connection_id
        ])
    ...
    def test_marketplace_mappings_list_with_filters(self):
        ...
        # Clean up
        self.runner.invoke(cli, [
            'marketplace', 'mappings', 'delete', str(mapping1.id)
        ])
        self.runner.invoke(cli, [
            'marketplace', 'mappings', 'delete', str(mapping2.id)
        ])
        self.runner.invoke(cli, [
            'marketplace', 'connections', 'delete', connection_id
        ])
            ...
            try:
                ...
                if connectors:
                    ...
                    if connector_type:
                        ...
                        result = self.runner.invoke(cli, [
                            'marketplace', 'connectors', 'info', connector_type
                        ])
                        assert result.exit_code == 0
                        assert connector_type in result.output or connector_type.upper() in result.output
            except json.JSONDecodeError:
                ...
                if connectors:
                    ...
                    if connector_type:
                        ...
                        try:
                            ...
                            assert output_json['type'] == connector_type or output_json['type'] == connector_type.upper()
                        except json.JSONDecodeError:
                            # If not JSON, check for error message
    ...
    def test_marketplace_connectors_info_invalid_type(self):
        ...
        result = self.runner.invoke(cli, [
            'marketplace', 'connectors', 'info', 'INVALID_CONNECTOR_TYPE'
        ])
        assert result.exit_code != 0
            ...
            try:
                ...
                if connectors:
                    ...
                    if connector_type:
                        # Try with lowercase
                        result = self.runner.invoke(cli, [
                            'marketplace', 'connectors', 'info', connector_type.lower()
                        ])
