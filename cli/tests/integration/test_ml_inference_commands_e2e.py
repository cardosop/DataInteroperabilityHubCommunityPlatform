...
class TestMLInferenceE2E:
    ...
    def test_complete_inference_workflow(self, runner, api_available, test_model_id):
        ...
        try:
            ...
            if deploy_result.exit_code == 0:
                ...
                assert ('Failed to deploy model' in deploy_result.output or \
    ...
    def test_get_deployment_command_structure(self, runner, api_available):
        ...
        result = runner.invoke(cli, ['ml', 'inference', 'get', test_deployment_id, '--format', 'json'])
        ...
        assert 'Failed to get deployment' in result.output or \
    ...
    def test_undeploy_command_structure(self, runner, api_available):
        ...
        test_deployment_id = 'test-deployment-123'
        result = runner.invoke(cli, ['ml', 'inference', 'undeploy', test_deployment_id, '--format', 'json'])
        # Should fail with not found or auth error, not command parsing error
        assert result.exit_code != 0, "Undeploy should fail for non-existent deployment"
        assert 'Failed to undeploy deployment' in result.output or \
               'Not authenticated' in result.output or \
               'API error' in result.output or \
               'Unknown error' in result.output or \
               'Server error' in result.output or \
    ...
    def test_metrics_command_structure(self, runner, api_available):
        ...
        test_deployment_id = 'test-deployment-123'
        result = runner.invoke(cli, ['ml', 'inference', 'metrics', test_deployment_id, '--format', 'json'])
        # Should fail with not found or auth error, not command parsing error
        assert result.exit_code != 0, "Metrics should fail for non-existent deployment"
        assert 'Failed to get metrics' in result.output or \
               'Not authenticated' in result.output or \
               'API error' in result.output or \
               'Unknown error' in result.output or \
               'Server error' in result.output or \
