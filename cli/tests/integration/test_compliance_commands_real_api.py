...
class TestComplianceCommandsRealAPI(LiveServerTestCase):
    ...
    def setUp(self):
        ...
        self.tenant = Tenant.objects.create(
            name="CLI Compliance Test Tenant", slug="cli-compliance-test-tenant"
        )
        ...
        APIKey.objects.create(
            user=self.user, tenant=self.tenant, name="CLI Compliance Test Key", key_hash=key_hash
        )
    ...
    def _create_asset_via_api(self, key, name):
        ...
        data = {"key": key, "name": name, "status": AssetStatus.DRAFT.value}
        response = requests.post(f"{self.api_base_url}/assets/", json=data, headers=headers)
    ...
    def test_compliance_run_command_with_asset_id(self):
        ...
        if result.exit_code == 0:
            assert (
                "started successfully" in result.output.lower()
                or "Compliance Run ID:" in result.output
            )
    ...
    def test_compliance_get_command(self):
        ...
        compliance_run = ComplianceRun.objects.create(
            tenant=self.tenant, asset_id=asset["id"], job=job, status=ComplianceRunStatus.PENDING
        )

        # Get compliance run via CLI
        result = self.runner.invoke(cli, ["compliance", "get", str(compliance_run.id)])
    ...
    def test_compliance_list_command(self):
        ...
        result = self.runner.invoke(cli, ["compliance", "list", "--limit", "10"])
    ...
    def test_compliance_list_command_with_asset_filter(self):
        ...
        result = self.runner.invoke(
            cli, ["compliance", "list", "--asset-id", asset["id"], "--limit", "10"]
        )
    ...
    def test_compliance_list_command_with_status_filter(self):
        ...
        ComplianceRun.objects.create(
            tenant=self.tenant, asset_id=asset["id"], job=job, status=ComplianceRunStatus.SUCCEEDED
        )
    ...
    def test_compliance_report_command(self):
        ...
        result = self.runner.invoke(
            cli, ["compliance", "report", "--asset-id", asset["id"], "--regulation", "GDPR"]
        )
    ...
    def test_compliance_endpoints_use_standardized_pattern(self):
        ...
        result = self.runner.invoke(
            cli, ["compliance", "run", "--asset-id", asset["id"], "--scan-mode", "internal"]
        )
        ...
        result = self.runner.invoke(cli, ["compliance", "list"])
