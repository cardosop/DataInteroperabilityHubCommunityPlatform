...
class TestMarketplaceCLIComprehensiveValidation(MarketplaceLiveServerTestCase):
    ...
    def test_connections_create_all_marketplace_types(self):
        ...
        for marketplace_type in marketplace_types:
            ...
            assert result.exit_code == 0, (
                f"Failed to create {marketplace_type} connection: {result.output}"
            )
    ...
    def test_connections_create_with_config_file(self):
        ...
        try:
            ...
            assert result.exit_code == 0, (
                f"Failed to create connection with file config: {result.output}"
            )
            output_data = json.loads(result.output)
            assert output_data["name"] == connection_name
            self.created_connections.append(output_data["id"])
    ...
    def test_connections_create_with_is_active_flags(self):
        ...
        result = self.runner.invoke(
            ...
            [
                ...
                "SNOWFLAKE_DATA_MARKETPLACE",
                "--name",
                f"Inactive Connection {unique_id2}",
                "--config",
                config_json,
                "--no-is-active",
                "--format",
    ...
    def test_connections_list_with_all_filters(self):
        ...
        for i in range(3):
            ...
            result = self.runner.invoke(
                ...
                [
                    ...
                    "SNOWFLAKE_DATA_MARKETPLACE",
                    "--name",
                    f"Filter Test Connection {i} {unique_id}",
                    "--config",
                    config_json,
                    "--format",
                    "json",
        ...
        result = self.runner.invoke(
            cli, ["marketplace", "connections", "list", "--is-active", "true", "--format", "json"]
        )
        assert result.exit_code == 0
    ...
    def test_connections_list_output_formats(self):
        ...
        result = self.runner.invoke(
            ...
            [
                ...
                "SNOWFLAKE_DATA_MARKETPLACE",
                "--name",
                f"Format Test Connection {unique_id}",
                "--config",
                config_json,
                "--format",
                "json",
        ...
        # Test table format
        result = self.runner.invoke(
            cli, ["marketplace", "connections", "list", "--format", "table"]
        )
        assert result.exit_code == 0
    ...
    def test_connections_delete_with_verification(self):
        ...
        result = self.runner.invoke(
            ...
            [
                ...
                "SNOWFLAKE_DATA_MARKETPLACE",
                "--name",
                f"Delete Test Connection {unique_id}",
                "--config",
                config_json,
                "--format",
                "json",
        ...
        # Verify deletion
        result = self.runner.invoke(cli, ["marketplace", "connections", "get", connection_id])
        assert result.exit_code != 0
    ...
    def test_connections_error_handling_invalid_id(self):
        ...
        result = self.runner.invoke(
            cli, ["marketplace", "connections", "update", fake_id, "--name", "Test"]
        )
        assert result.exit_code != 0

        # Test delete with invalid ID
        result = self.runner.invoke(cli, ["marketplace", "connections", "delete", fake_id])
        assert result.exit_code != 0
    ...
    def test_connections_error_handling_invalid_config(self):
        ...
        result = self.runner.invoke(
            ...
            [
                ...
                "SNOWFLAKE_DATA_MARKETPLACE",
                "--name",
                f"Invalid Config Test {unique_id}",
                "--config",
                "{invalid json}",
            ],
        )
    ...
    def test_sync_start_all_directions(self):
        ...
        result = self.runner.invoke(
            ...
            [
                ...
                "SNOWFLAKE_DATA_MARKETPLACE",
                "--name",
                f"Sync Test Connection {unique_id}",
                "--config",
                config_json,
                "--format",
                "json",
    ...
    def test_sync_list_with_all_filters(self):
        ...
        result = self.runner.invoke(
            ...
            [
                ...
                "SNOWFLAKE_DATA_MARKETPLACE",
                "--name",
                f"Sync List Test Connection {unique_id}",
                "--config",
                config_json,
                "--format",
                "json",
        ...
        result = self.runner.invoke(
            cli,
            ["marketplace", "sync", "list", "--connection-id", connection_id, "--format", "json"],
        )
        assert result.exit_code == 0

        # Test filter by status
        result = self.runner.invoke(
            cli, ["marketplace", "sync", "list", "--status", "PENDING", "--format", "json"]
        )
        assert result.exit_code == 0

        # Test filter by direction
        result = self.runner.invoke(
            cli, ["marketplace", "sync", "list", "--direction", "PULL", "--format", "json"]
        )
        assert result.exit_code == 0
    ...
    def test_sync_get_with_all_fields(self):
        ...
        result = self.runner.invoke(
            ...
            [
                ...
                "SNOWFLAKE_DATA_MARKETPLACE",
                "--name",
                f"Sync Get Test Connection {unique_id}",
                "--config",
                config_json,
                "--format",
                "json",
    ...
    def test_sync_cancel_workflow(self):
        ...
        result = self.runner.invoke(
            ...
            [
                ...
                "SNOWFLAKE_DATA_MARKETPLACE",
                "--name",
                f"Sync Cancel Test Connection {unique_id}",
                "--config",
                config_json,
                "--format",
                "json",
        ...
        # Cancel sync job
        result = self.runner.invoke(cli, ["marketplace", "sync", "cancel", sync_job_id])
        assert result.exit_code == 0
    ...
    def test_mappings_list_with_all_filters(self):
        ...
        result = self.runner.invoke(
            cli,
            ["marketplace", "mappings", "list", "--asset-id", str(asset.id), "--format", "json"],
        )
        assert result.exit_code == 0
    ...
    def test_mappings_delete_with_verification(self):
        ...
        result = self.runner.invoke(
            ...
            [
                ...
                "SNOWFLAKE_DATA_MARKETPLACE",
                "--name",
                f"Mapping Delete Test Connection {unique_id}",
                "--config",
                config_json,
                "--format",
                "json",
        ...
        # Verify deletion
        result = self.runner.invoke(cli, ["marketplace", "mappings", "get", str(mapping.id)])
        assert result.exit_code != 0
    ...
    def test_connectors_list_output_formats(self):
        ...
        # Test table format
        result = self.runner.invoke(cli, ["marketplace", "connectors", "list", "--format", "table"])
        assert result.exit_code == 0
    ...
    def test_connectors_info_all_fields(self):
        ...
        if result.exit_code == 0:
            try:
                ...
                if connectors:
                    connector_type = connectors[0].get("type") or connectors[0].get(
                        "connector_type"
                    )
                    if connector_type:
    ...
    def test_error_handling_invalid_uuid_format(self):
        ...
        for invalid_id in invalid_ids:
            ...
            result = self.runner.invoke(cli, ["marketplace", "connections", "get", invalid_id])
            assert result.exit_code != 0

            # Test mappings get
            result = self.runner.invoke(cli, ["marketplace", "mappings", "get", invalid_id])
            assert result.exit_code != 0
    ...
    def test_error_handling_missing_required_options(self):
        ...
        result = self.runner.invoke(cli, ["marketplace", "connections", "create"])
        assert result.exit_code != 0

        # Test sync start without required options
        result = self.runner.invoke(cli, ["marketplace", "sync", "start"])
        assert result.exit_code != 0
    ...
    def test_error_handling_invalid_direction_combinations(self):
        ...
        result = self.runner.invoke(
            ...
            [
                ...
                "SNOWFLAKE_DATA_MARKETPLACE",
                "--name",
                f"Direction Test Connection {unique_id}",
                "--config",
                config_json,
                "--format",
                "json",
    ...
    def test_output_format_consistency_table(self):
        ...
        result = self.runner.invoke(
            ...
            [
                ...
                "SNOWFLAKE_DATA_MARKETPLACE",
                "--name",
                f"Format Consistency Test {unique_id}",
                "--config",
                config_json,
                "--format",
                "table",
