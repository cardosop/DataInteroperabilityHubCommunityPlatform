...
class TestMLModelRegistryE2E:
            ...
            if create_result.exit_code == 0:
                ...
                if model_id:
                    ...
                    get_result = runner.invoke(
                        cli, ["ml", "models", "get", model_id, "--format", "json"]
                    )
                    ...
                    versions_result = runner.invoke(
                        cli, ["ml", "models", "versions", model_id, "--format", "json"]
                    )
                    ...
                    assert versions_result.exit_code == 0, (
                        f"Versions failed: {versions_result.output}"
                    )

                    # Step 6: Delete model
                    delete_result = runner.invoke(
                        cli, ["ml", "models", "delete", model_id, "--format", "json"]
                    )
                ...
                assert (
                    "Failed to create model" in create_result.output
                    or "Not authenticated" in create_result.output
                    or "API error" in create_result.output
                    or "not found" in create_result.output.lower()
    ...
    def test_get_model_command_structure(self, runner, api_available):
        ...
        assert (
            "Failed to get model" in result.output
    ...
    def test_update_model_command_structure(self, runner, api_available):
        ...
        test_uuid = str(uuid.uuid4())
        result = runner.invoke(
            cli, ["ml", "models", "update", test_uuid, "--status", "DEPLOYED", "--format", "json"]
        )
        # Should fail with not found or auth error, not command parsing error
        assert result.exit_code != 0, "Update should fail for non-existent model"
        assert (
            "Failed to update model" in result.output
            or "Not authenticated" in result.output
            or "API error" in result.output
            or "not found" in result.output.lower()
        ), f"Unexpected error: {result.output}"
    ...
    def test_delete_model_command_structure(self, runner, api_available):
        ...
        assert (
            "Failed to delete model" in result.output
            or "Not authenticated" in result.output
            or "API error" in result.output
            or "not found" in result.output.lower()
        ), f"Unexpected error: {result.output}"
    ...
    def test_versions_command_structure(self, runner, api_available):
        ...
        assert (
            "Failed to list model versions" in result.output
            or "Not authenticated" in result.output
            or "API error" in result.output
            or "not found" in result.output.lower()
        ), f"Unexpected error: {result.output}"
