...
"""
            ...
            if response.status_code in [200, 201]:
                ...
                if verify_response.status_code == 200:
                    ...
                    if hub_contract:
                        ...
                        product_strategy = x_odps_ext.get('product_strategy') or x_odps_info.get('product_strategy')
                        if product_strategy:
                            return contract_id
    ...
    def test_get_product_strategy_empty_contract(self, setup_config, api_available):
        ...
        try:
            ...
            assert result.exit_code == 0
            assert 'No product strategy found' in result.output or 'product strategy' in result.output.lower()
    ...
    def test_get_product_strategy_odps_4_0_contract(self, setup_config, api_available):
        ...
        try:
            ...
            assert result.exit_code in [0, 1]  # May succeed with empty result or fail with error
            if result.exit_code == 0:
                # If it succeeds, should indicate no product strategy or version issue
                assert 'No product strategy' in result.output or '4.1' in result.output or 'product strategy' in result.output.lower()
            else:
                # If it fails, should indicate version issue
                assert '4.1' in result.output or 'version' in result.output.lower() or 'error' in result.output.lower()
