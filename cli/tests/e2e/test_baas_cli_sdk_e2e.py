...
class TestBaaSCLISDKE2E:
    ...
    async def test_complete_api_key_lifecycle_cli(self, cli_runner):
        ...
        list_result = cli_runner.invoke(cli, [
            'baas', 'api-keys', 'list',
            '--format', 'json'
        ])
        ...
        get_result = cli_runner.invoke(cli, [
            'baas', 'api-keys', 'get',
            api_key_id,
            '--format', 'json'
        ])
        ...
        update_result = cli_runner.invoke(cli, [
            'baas', 'api-keys', 'update',
            api_key_id,
            '--name', f'E2E CLI Updated Key {unique_id}',
            '--format', 'json'
        ])
        ...
        revoke_result = cli_runner.invoke(cli, [
        ...
        get_revoked_result = cli_runner.invoke(cli, [
            'baas', 'api-keys', 'get',
            api_key_id,
            '--format', 'json'
        ])
    ...
    async def test_complete_api_key_lifecycle_sdk(self, baas_api):
        ...
        updated = await baas_api.update_api_key(
            api_key_id,
            name=f"E2E SDK Updated Key {unique_id}"
        )
    ...
    async def test_cli_sdk_consistency(self, cli_runner, baas_api):
        ...
        create_result = cli_runner.invoke(cli, [
            'baas', 'api-keys', 'create',
            '--name', f'CLI-SDK Consistency Key {unique_id}',
            '--tier', 'PRO',
            '--format', 'json'
        ])
        assert create_result.exit_code == 0
        ...
        # Update via SDK
        updated_sdk = await baas_api.update_api_key(
            api_key_id,
            name=f'SDK Updated Consistency Key {unique_id}'
        )
    ...
    async def test_usage_tracking_workflow_cli(self, cli_runner):
        ...
        create_result = cli_runner.invoke(cli, [
            'baas', 'api-keys', 'create',
            '--name', f'Usage Workflow Key {unique_id}',
            '--format', 'json'
        ])
        assert create_result.exit_code == 0
        ...
        # Get usage stats
        stats_result = cli_runner.invoke(cli, [
            'baas', 'usage', 'stats',
        ...
        endpoint_result = cli_runner.invoke(cli, [
    ...
    async def test_usage_tracking_workflow_sdk(self, baas_api):
        ...
        unique_id = uuid.uuid4().hex[:8]
        created = await baas_api.create_api_key(
            name=f"SDK Usage Workflow Key {unique_id}",
            tier="FREE"
        )
        api_key_id = created["id"]
    ...
    async def test_usage_tracking_cli_sdk_consistency(self, cli_runner, baas_api):
        ...
        create_result = cli_runner.invoke(cli, [
            'baas', 'api-keys', 'create',
            '--name', f'Usage Consistency Key {unique_id}',
            '--format', 'json'
        ])
        assert create_result.exit_code == 0
        api_key_id = json.loads(create_result.output)['id']
        ...
        # Get stats via CLI
        cli_stats_result = cli_runner.invoke(cli, [
            'baas', 'usage', 'stats',
            '--api-key-id', api_key_id,
            '--format', 'json'
        ])
        ...
        assert ('total_requests' in cli_stats) == ('total_requests' in sdk_stats or 'success_count' in sdk_stats)
    ...
    async def test_developer_portal_workflow_cli(self, cli_runner):
        ...
        # Get API docs
        docs_result = cli_runner.invoke(cli, [
            'baas', 'docs', 'show',
            '--format', 'json'
        ])
        ...
        # Get OpenAPI schema
        openapi_result = cli_runner.invoke(cli, [
            'baas', 'docs', 'openapi',
            '--format', 'json'
        ])
        ...
        # Get SDK links
        sdks_result = cli_runner.invoke(cli, [
            'baas', 'docs', 'sdks',
            '--format', 'json'
        ])
    ...
    async def test_developer_portal_cli_sdk_consistency(self, cli_runner, baas_api):
        ...
        # Get docs via CLI
        cli_docs_result = cli_runner.invoke(cli, [
            'baas', 'docs', 'show',
            '--format', 'json'
        ])
    ...
    async def test_multi_tier_workflow(self, cli_runner, baas_api):
        ...
        for tier in ['FREE', 'PRO', 'ENTERPRISE']:
            create_result = cli_runner.invoke(cli, [
                'baas', 'api-keys', 'create',
                '--name', f'Multi-Tier Key {tier} {unique_id}',
                '--tier', tier,
                '--format', 'json'
            ])
            assert create_result.exit_code == 0
    ...
    async def test_date_range_workflow(self, cli_runner, baas_api):
        ...
        create_result = cli_runner.invoke(cli, [
            'baas', 'api-keys', 'create',
            '--name', f'Date Range Key {unique_id}',
            '--format', 'json'
        ])
        assert create_result.exit_code == 0
        api_key_id = json.loads(create_result.output)['id']
