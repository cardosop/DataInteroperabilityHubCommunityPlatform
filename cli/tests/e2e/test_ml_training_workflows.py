...
def test_model(api_available, api_key, test_asset):
    ...
    if response and response.status_code == 201:
        ...
        with contextlib.suppress(Exception):
            requests.delete(
                f"http://localhost:8000/api/v1/ml/models/{model_id}/", headers=headers, timeout=10
            )
    else:
        status_code = response.status_code if response else "unknown"
        text = response.text if response else "no response"
...
def test_dataset(api_available, api_key):
    ...
    try:
        verify_response = requests.get(
            f"http://localhost:8000/api/v1/files/{file_id}/", headers=headers, timeout=10
        )
    ...
    if dataset_response.status_code == 201:
        ...
        with contextlib.suppress(Exception):
            requests.delete(
                f"http://localhost:8000/api/v1/datasets/{dataset_id}/", headers=headers, timeout=10
            )
        with contextlib.suppress(Exception):
            requests.delete(
                f"http://localhost:8000/api/v1/files/{file_id}/", headers=headers, timeout=10
...
class TestMLTrainingWorkflowsE2E:
    ...
    def test_complete_training_workflow_submit_list_get_logs_cancel(
        ...
        if submit_result.exit_code != 0:
            ...
            if (
                "ODH" in error_msg
                or "Training Operator" in error_msg
        ...
        try:
            ...
            list_result = runner.invoke(
                cli, ["ml", "training", "list", "--model-id", test_model, "--format", "json"]
            )
            ...
            get_result = runner.invoke(cli, ["ml", "training", "get", job_id, "--format", "json"])
            ...
            # Step 4: Get logs
            logs_result = runner.invoke(cli, ["ml", "training", "logs", job_id, "--format", "json"])
            ...
            cancel_result = runner.invoke(
                cli, ["ml", "training", "cancel", job_id, "--format", "json"]
            )
    ...
    def test_training_workflow_with_file_config(
        ...
        config_data = {"epochs": 5, "batch_size": 16, "learning_rate": 0.0005}
        ...
        try:
            ...
            for _i in range(3):
                get_result = runner.invoke(
                    cli, ["ml", "training", "get", job_id, "--format", "json"]
                )
                ...
                if get_result.exit_code == 0:
                    ...
                    if progress is not None:
                        assert 0.0 <= progress <= 1.0, (
                            f"Progress should be between 0 and 1, got {progress}"
                        )
