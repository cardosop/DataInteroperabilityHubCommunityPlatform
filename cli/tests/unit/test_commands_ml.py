...
class TestMLParameterValidation:
    ...
    def test_validate_uuid_invalid_format(self):
        ...
        assert 'INVALID_UUID_FORMAT' in str(exc_info.value.error_code) or 'Invalid' in str(exc_info.value)

    def test_validate_uuid_empty(self):
        """Test UUID validation with empty string"""
        with pytest.raises(ODHMLModelParameterError) as exc_info:
            validate_uuid('', 'test-id')
        assert 'EMPTY_UUID' in str(exc_info.value.error_code) or 'empty' in str(exc_info.value).lower()

    def test_validate_model_type_valid(self):
        """Test model type validation with valid types"""
        valid_types = ['CLASSIFICATION', 'REGRESSION', 'CLUSTERING', 'NLP', 'COMPUTER_VISION', 'RECOMMENDATION', 'TIME_SERIES', 'ANOMALY_DETECTION', 'OTHER']
    ...
    def test_validate_model_type_invalid(self):
        ...
        assert 'INVALID_MODEL_TYPE' in str(exc_info.value.error_code) or 'Invalid' in str(exc_info.value)
    ...
    def test_validate_model_status_invalid(self):
        ...
        assert 'INVALID_MODEL_STATUS' in str(exc_info.value.error_code) or 'Invalid' in str(exc_info.value)
...
class TestMLCommandArgumentValidation:
    ...
    def test_ml_models_update_with_invalid_status(self, runner):
        ...
        valid_uuid = '123e4567-e89b-12d3-a456-426614174000'
        result = runner.invoke(cli, ['ml', 'models', 'update', valid_uuid, '--status', 'INVALID_STATUS'])
        # Click should validate the choice and show error
        assert result.exit_code != 0
...
class TestMLInferenceParameterValidation:
    ...
    def test_validate_json_invalid(self):
        ...
        from datahub_cli.ml_errors import validate_json, ODHMLModelParameterError
        invalid_json = '{"key": "value"'
        with pytest.raises(ODHMLModelParameterError) as exc_info:
            validate_json(invalid_json, 'test-json')
        assert 'INVALID_JSON_FORMAT' in str(exc_info.value.error_code) or 'Invalid' in str(exc_info.value)
    ...
    def test_validate_json_empty(self):
        ...
        assert 'EMPTY_JSON' in str(exc_info.value.error_code) or 'empty' in str(exc_info.value).lower()
...
class TestMLInferenceCommandArgumentValidation:
    ...
    def test_ml_inference_metrics_with_invalid_format(self, runner):
        ...
        result = runner.invoke(cli, ['ml', 'inference', 'metrics', 'test-id', '--format', 'invalid'])
        # Click should validate the choice and show error
        assert result.exit_code != 0
