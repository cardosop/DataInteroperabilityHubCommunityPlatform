...
class TestAuthManager:
    ...
    def test_refresh_access_token_success(self, mock_post, temp_config_dir):
        ...
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"access_token": "new-access-123"}
        mock_post.return_value = mock_response
...
class TestIsTokenExpired:
    ...
    def _make_jwt(exp_offset_seconds):
        ...
        payload = (
            base64.urlsafe_b64encode(
                json.dumps({"exp": int(time.time() + exp_offset_seconds), "sub": "test"}).encode()
            )
            .rstrip(b"=")
            .decode()
        )
    ...
    def test_payload_not_json_returns_false(self):
        ...
        assert AuthManager._is_token_expired(f"header.{bad_payload}.sig") is False
