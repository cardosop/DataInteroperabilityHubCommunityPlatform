...
class TestNetworkErrors:
    ...
    def test_network_dns_error(self, runner, mock_api_client):
        ...
        mock_api_client.get.side_effect = ClickException("API request failed: Name resolution failed")
    ...
    def test_network_ssl_error(self, runner, mock_api_client):
        ...
        mock_api_client.get.side_effect = ClickException("API request failed: SSL certificate verification failed")
...
class TestAPIErrors:
    ...
    def test_api_400_bad_request(self, runner, mock_api_client):
        ...
        from click import ClickException
        mock_api_client.get.side_effect = ClickException("API error (BAD_REQUEST): Invalid request parameters")
    ...
    def test_api_401_unauthorized(self, runner, mock_api_client):
        ...
        mock_api_client.get.side_effect = ClickException("API error (UNAUTHORIZED): Authentication required")
        
        result = runner.invoke(cli, ['assets', 'list'])
        
        assert result.exit_code != 0
        assert 'UNAUTHORIZED' in result.output or 'Authentication' in result.output or 'Failed to list assets' in result.output
    ...
    def test_api_403_forbidden(self, runner, mock_api_client, temp_file):
        ...
        mock_api_client.post.side_effect = ClickException("API error (FORBIDDEN): Insufficient permissions")
        
        result = runner.invoke(cli, ['contracts', 'create', '--file', file_path])
        
        assert result.exit_code != 0
        assert 'FORBIDDEN' in result.output or 'permissions' in result.output or 'Failed to create contract' in result.output
    
    def test_api_404_not_found(self, runner, mock_api_client):
        """Test handling 404 Not Found errors"""
        mock_api_client.get.side_effect = ClickException("API error (NOT_FOUND): Resource not found")
        
        result = runner.invoke(cli, ['assets', 'get', 'non-existent-id'])
        
        assert result.exit_code != 0
        assert 'NOT_FOUND' in result.output or 'not found' in result.output or 'Failed to get asset' in result.output
    
    def test_api_500_internal_server_error(self, runner, mock_api_client):
        """Test handling 500 Internal Server Error"""
        mock_api_client.post.side_effect = ClickException("API error (INTERNAL_ERROR): Internal server error")
        
        result = runner.invoke(cli, ['contracts', 'validate', 'contract-id'])
        
        assert result.exit_code != 0
        assert 'INTERNAL_ERROR' in result.output or 'server error' in result.output or 'Failed to validate contract' in result.output
    
    def test_api_503_service_unavailable(self, runner, mock_api_client):
        """Test handling 503 Service Unavailable"""
        mock_api_client.get.side_effect = ClickException("API error (SERVICE_UNAVAILABLE): Service temporarily unavailable")
        
        result = runner.invoke(cli, ['lineage', 'contract', 'contract-id'])
        
        assert result.exit_code != 0
        assert 'SERVICE_UNAVAILABLE' in result.output or 'unavailable' in result.output or 'Failed to get contract lineage' in result.output
    ...
    def test_api_error_with_details(self, runner, mock_api_client, temp_file):
        ...
        file_path, content = temp_file('.yaml', 'name: Test Contract')
        mock_api_client.post.side_effect = ClickException("API error (VALIDATION_ERROR): Contract validation failed: Missing required field 'name'")
        
        result = runner.invoke(cli, ['contracts', 'create', '--file', file_path])
        
        assert result.exit_code != 0
        assert 'VALIDATION_ERROR' in result.output or 'validation failed' in result.output or 'Failed to create contract' in result.output
...
class TestInvalidArguments:
    ...
    def test_invalid_option_value(self, runner):
        ...
        assert result.exit_code != 0
        assert 'Invalid value' in result.output or 'invalid-format' in result.output or 'Usage:' in result.output
    
    def test_invalid_choice_value(self, runner):
        """Test handling invalid choice value"""
        result = runner.invoke(cli, ['assets', 'create', '--name', 'Test', '--key', 'test', '--visibility', 'INVALID'])
        
        assert result.exit_code != 0
        assert 'Invalid value' in result.output or 'INVALID' in result.output or 'Usage:' in result.output
    ...
    def test_missing_required_option(self, runner):
        ...
        assert result.exit_code != 0
        assert 'Missing option' in result.output or '--key' in result.output or 'Usage:' in result.output
    
    def test_invalid_file_path(self, runner):
        """Test handling invalid file path"""
        result = runner.invoke(cli, ['contracts', 'create', '--file', '/nonexistent/path/file.yaml'])
        
        assert result.exit_code != 0
        assert 'does not exist' in result.output or 'Invalid value' in result.output or 'Failed to read file' in result.output
...
class TestErrorPropagation:
    ...
    def test_click_exception_propagation(self, runner, mock_api_client):
        ...
        assert 'Custom error message' in result.output or 'Failed to list contracts' in result.output
...
class TestErrorMessages:
    ...
    def test_error_message_contains_context(self, runner, mock_api_client):
        ...
        mock_api_client.get.side_effect = ClickException("API error (NOT_FOUND): Contract with ID 'test-id' not found")
        
        result = runner.invoke(cli, ['contracts', 'get', 'test-id'])
        
        assert result.exit_code != 0
        # Error should mention the resource type and ID
        assert 'test-id' in result.output or 'Contract' in result.output or 'not found' in result.output
    ...
    def test_error_message_includes_command_context(self, runner, mock_api_client):
        ...
        assert 'validate' in result.output.lower() or 'Failed to validate contract' in result.output or 'Validation failed' in result.output
