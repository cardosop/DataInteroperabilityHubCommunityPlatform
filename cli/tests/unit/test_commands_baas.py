...
from datahub_cli.commands import baas
from datahub_cli.api_client import api_client
from datahub_cli.baas_errors import (
    BaaSCLIError,
    BaaSValidationError,
    BaaSNotFoundError,
)
...
class TestAPIKeyCreate:
    ...
    def test_create_api_key_success_table_format(self, runner, mock_api_client):
        ...
        mock_api_client.post.assert_called_once_with(
            'baas/api-keys/',
            json_data={'name': 'Test API Key', 'tier': 'FREE'}
        )
    ...
    def test_create_api_key_with_tier(self, runner, mock_api_client):
        ...
        mock_api_client.post.assert_called_once_with(
            'baas/api-keys/',
            json_data={'name': 'Pro API Key', 'tier': 'PRO'}
        )
...
class TestAPIKeyList:
    ...
    def test_list_api_keys_with_tier_filter(self, runner, mock_api_client):
        ...
        mock_api_client.get.assert_called_once_with(
            'baas/api-keys/',
            params={'limit': 20, 'offset': 0, 'tier': 'PRO'}
        )
...
class TestAPIKeyGet:
    ...
    def test_get_api_key_success_table_format(self, runner, mock_api_client):
        ...
        assert 'api_key' not in result.output.lower() or 'api key value' not in result.output.lower()
    ...
    def test_get_api_key_with_expiration(self, runner, mock_api_client):
        ...
        assert future_date[:10] in result.output or '2027' in result.output or '2026' in result.output
...
class TestAPIKeyUpdate:
    ...
    def test_update_api_key_name(self, runner, mock_api_client):
        ...
        result = runner.invoke(cli, [
            'baas', 'api-keys', 'update', api_key_id,
            '--name', 'Updated Name'
        ])
...
class TestAPIKeyRevoke:
    ...
    def test_revoke_api_key_success_json_format(self, runner, mock_api_client):
        ...
        result = runner.invoke(cli, [
            'baas', 'api-keys', 'revoke', api_key_id,
            '--format', 'json'
        ])
...
class TestUsageStats:
    ...
    def test_usage_stats_invalid_date_range(self, runner):
        ...
        result = runner.invoke(cli, [
            'baas', 'usage', 'stats',
            '--start-date', '2025-12-31T23:59:59Z',
