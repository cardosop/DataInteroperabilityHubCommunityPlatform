...
class TestMLTrainingCommands:
    ...
    def mock_api_client(self):
        """Mock API client"""
        with (
            patch.object(api_client, "post") as mock_post,
    ...
    def test_training_cancel_success(self, runner, mock_api_client):
        ...
        result = runner.invoke(
            cli, ["ml", "training", "cancel", "test-job-123", "--format", "table"]
        )
    ...
    def test_training_logs_success(self, runner, mock_api_client):
        ...
        result = runner.invoke(
            cli, ["ml", "training", "logs", "test-job-123", "--lines", "100", "--format", "table"]
        )
    ...
    def test_training_logs_with_lines(self, runner, mock_api_client):
        ...
        result = runner.invoke(cli, ["ml", "training", "logs", "test-job-123", "--lines", "50"])
    ...
    def test_training_get_api_error(self, runner, mock_api_client):
        ...
        mock_api_client["get"].side_effect = HTTPError("500 Server Error", response=mock_response)
    ...
    def test_training_submit_api_error(self, runner, mock_api_client):
        ...
        mock_response.status_code = 500
        mock_response.text = '{"error": "Internal server error"}'
        mock_api_client["post"].side_effect = HTTPError("500 Server Error", response=mock_response)
