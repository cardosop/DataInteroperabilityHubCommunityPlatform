...
class TestDQCheckExecution:
    ...
    def test_run_dq_check_on_asset(self, runner, authenticated_config):
        ...
        asset_result = runner.invoke(
            cli, ["assets", "create", "--name", "DQ Test Asset", "--key", unique_key("dq-test-")]
        )
        ...
        if "ID:" in asset_result.output:
            ...
            if asset_id:
                ...
                dq_result = runner.invoke(
                    cli, ["dq", "run", "--asset-id", asset_id, "--profile-key", "intake_basic_gx"]
                )

                assert dq_result.exit_code == 0, f"DQ run creation failed: {dq_result.output}"
    ...
    def test_run_dq_check_with_custom_profile(self, runner, authenticated_config):
        ...
        asset_result = runner.invoke(
            cli,
            ["assets", "create", "--name", "Custom Profile Asset", "--key", unique_key("custom-p")],
        )
    ...
    def test_run_dq_check_external_scan_only(self, runner, authenticated_config, temp_file):
        ...
        """
        file_path, _content = temp_file(".csv", "id,name\n1,Test\n2,Sample")
        ...
        if file_id:
            dq_result = runner.invoke(
                cli, ["dq", "run", "--file-id", file_id, "--profile-key", "intake_basic_gx"]
            )
            assert dq_result.exit_code == 0, f"DQ run creation failed: {dq_result.output}"
    ...
    def test_run_dq_check_json_output(self, runner, authenticated_config):
        ...
        asset_result = runner.invoke(
            cli, ["assets", "create", "--name", "JSON DQ Asset", "--key", unique_key("json-dq-")]
        )
        ...
        if asset_id:
            dq_result = runner.invoke(
                cli, ["dq", "run", "--asset-id", asset_id, "--format", "json"]
            )

            assert dq_result.exit_code == 0, f"DQ run (JSON) failed: {dq_result.output}"
...
class TestDQResultReview:
    ...
    def test_get_dq_run_results(self, runner, authenticated_config):
        ...
        result = runner.invoke(cli, ["dq", "get", "00000000-0000-0000-0000-000000000000"])
    ...
    def test_get_dq_run_results_with_checks(self, runner, authenticated_config):
        ...
        # Create asset
        asset_result = runner.invoke(
            cli, ["assets", "create", "--name", "DQ Results Asset", "--key", unique_key("dq-resul")]
        )
        assert asset_result.exit_code == 0, asset_result.output
        ...
        assert run_result.exit_code == 0, f"DQ run creation failed: {run_result.output}"
    ...
    def test_get_dq_run_json_output(self, runner, authenticated_config):
        ...
        asset_result = runner.invoke(
            cli, ["assets", "create", "--name", "JSON Get Asset", "--key", unique_key("json-get")]
        )
        assert asset_result.exit_code == 0, asset_result.output
        ...
        if asset_id:
            ...
            if dq_run_id:
                result = runner.invoke(cli, ["dq", "get", dq_run_id, "--format", "json"])
    ...
    def test_list_dq_runs(self, runner, authenticated_config):
        ...
        assert result.exit_code == 0, f"DQ list failed: {result.output}"
    ...
    def test_list_dq_runs_filtered_by_asset(self, runner, authenticated_config):
        ...
        result = runner.invoke(
            cli, ["dq", "list", "--asset-id", "00000000-0000-0000-0000-000000000000"]
        )
        assert result.exit_code == 0, result.output
    ...
    def test_list_dq_runs_filtered_by_status(self, runner, authenticated_config):
        ...
        result = runner.invoke(cli, ["dq", "list", "--status", "SUCCEEDED"])
        assert result.exit_code == 0, result.output

    def test_get_dq_scorecard(self, runner, authenticated_config):
        """Test getting DQ scorecard for an asset"""
        asset_result = runner.invoke(
            cli, ["assets", "create", "--name", "Scorecard Asset", "--key", unique_key("scorecar")]
        )
        assert asset_result.exit_code == 0, asset_result.output
        ...
        if asset_id:
            scorecard_result = runner.invoke(cli, ["dq", "scorecard", asset_id])
    ...
    def test_get_asset_with_dq_results(self, runner, authenticated_config):
        ...
        asset_result = runner.invoke(
            ...
            [
                ...
                "Asset With DQ",
                "--key",
                unique_key("asset-wi"),  # noqa: PHASE216-STATIC-ID
            ],
        )
        assert asset_result.exit_code == 0, asset_result.output
        ...
        if asset_id:
            get_result = runner.invoke(cli, ["assets", "get", asset_id, "--include", "latest_dq"])
            assert get_result.exit_code == 0, get_result.output

    def test_watch_dq_run(self, runner, authenticated_config):
        """Test watching a non-existent DQ run (should error quickly)."""
        result = runner.invoke(
            cli, ["dq", "watch", "00000000-0000-0000-0000-000000000000", "--timeout", "5"]
        )
...
class TestDQAlerts:
    ...
    def test_list_alerting_rules(self, runner, authenticated_config):
        ...
        result = runner.invoke(cli, ["dq", "alerts", "--list"])
        assert result.exit_code == 0, result.output

    def test_create_alerting_rule(self, runner, authenticated_config):
        """Test creating a DQ alerting rule"""
        asset_result = runner.invoke(
            cli, ["assets", "create", "--name", "Alert Asset", "--key", unique_key("alert-as")]
        )
        assert asset_result.exit_code == 0, asset_result.output
        ...
        if asset_id:
            alert_result = runner.invoke(
                cli, ["dq", "alerts", "--create", "--asset-id", asset_id, "--threshold", "80.0"]
            )
            assert alert_result.exit_code == 0, f"Alert rule creation failed: {alert_result.output}"
    ...
    def test_create_alerting_rule_missing_params(self, runner, authenticated_config):
        ...
        result = runner.invoke(cli, ["dq", "alerts", "--create"])
        assert result.exit_code != 0
    ...
    def test_list_alerting_rules_json_output(self, runner, authenticated_config):
        ...
        result = runner.invoke(cli, ["dq", "alerts", "--list", "--format", "json"])
        assert result.exit_code == 0, result.output
...
class TestDQWorkflows:
    ...
    def test_complete_dq_workflow(self, runner, authenticated_config):
        ...
        asset_result = runner.invoke(
            cli,
            ["assets", "create", "--name", "DQ Workflow Asset", "--key", unique_key("dq-workf")],
        )
        assert asset_result.exit_code == 0, asset_result.output
        ...
        list_result = runner.invoke(cli, ["dq", "list", "--asset-id", asset_id])
