"""
Phase 279.H.public_dsar — Public persona journey tests (CLI).

Tests the Public end-to-end workflow through the CLI.
...
Usage:
  ...
  pytest cli/tests/use_cases/test_public_dsar_journey.py -v
"""
import json
import os
import subprocess
...
@requires_backend
class TestPublicDsarJourney:
    """Public persona — end-to-end CLI journey."""
    ...
    def test_dsar_submit_readiness(self):
        """CLI ``public_dsar submit readiness`` returns valid response."""
        data = _cli("public_dsar", "submit readiness")
