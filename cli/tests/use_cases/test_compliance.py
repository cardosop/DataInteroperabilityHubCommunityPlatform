...
class TestComplianceChecks:
    ...
    def test_run_compliance_check_external_scan_only(
        ...
        try:
            ...
            assert file_id, (
                "Upload succeeded but no file ID in output"
            )
            ...
            assert compliance_result.exit_code == 0, (
                f"Compliance run failed: "
                f"{compliance_result.output}"
            )
    ...
    def test_get_compliance_run_results(self, runner, authenticated_config):
        ...
        if run_id:
            result = runner.invoke(cli, [
                'compliance', 'get', run_id,
            ])
            assert result.exit_code == 0, (
                f"Compliance get failed: {result.output}"
            )
