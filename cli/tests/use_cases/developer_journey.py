...
def test_api_docs_accessible():
    ...
    assert resp.status_code == 200, (
        f"API docs returned {resp.status_code}: {resp.text[:300]}"
    )

    # Docs page should have some content (HTML or JSON)
    assert len(resp.text) > 100, (
        "API docs page returned very little content"
    )
...
def test_openapi_json_valid():
    ...
    assert resp.status_code == 200, (
        f"OpenAPI JSON returned {resp.status_code}: {resp.text[:300]}"
    )
    ...
    assert isinstance(body, dict), (
        f"OpenAPI document should be a dict, got {type(body).__name__}"
    )

    has_version_key = "openapi" in body or "swagger" in body
    assert has_version_key, (
        f"Missing 'openapi' or 'swagger' version key. "
    ...
    assert "title" in body["info"], (
        f"OpenAPI info missing 'title'. Info: {body['info']}"
    )
