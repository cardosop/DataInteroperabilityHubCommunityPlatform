...
def test_upload_endpoint_exists():
    ...
    assert resp.status_code < 500, (
        f"File upload endpoint returned server error {resp.status_code}: "
        f"{resp.text[:300]}"
    )
...
def test_generate_presigned_url():
    ...
    if resp.status_code == 400:
        ...
        if "filename" in error_text or "content_type" in error_text:
            pytest.skip(
                f"Presigned URL payload schema mismatch (400): {resp.text[:300]}"
            )

    assert resp.status_code in (200, 201), (
        f"Presigned URL generation returned {resp.status_code}: "
        f"{resp.text[:500]}"
    )
    ...
    has_url = (
        "url" in body
        or "presigned_url" in body
        or "upload_url" in body
        or "signed_url" in body
    )
    assert has_url, (
        f"Presigned URL response missing URL field. Keys: {list(body.keys())}"
    )
...
def test_file_list():
    ...
    assert resp.status_code == 200, (
        f"/files/ returned {resp.status_code}: {resp.text[:500]}"
    )

    body = resp.json()
    files = _extract_files(body)
    assert isinstance(files, list), (
        f"Expected a list of files, got {type(files).__name__}"
    )
    ...
    if files:
        ...
        assert has_identifier, (
            f"File entry missing identifier field. Keys: {list(f.keys())}"
        )
