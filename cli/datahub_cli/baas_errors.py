...
def parse_baas_api_error_response(error_data: dict[str, Any]) -> BaaSCLIError | None:
    """
    ...
    if error_code:
        ...
        if any(
            ...
            for code in ["VALIDATION", "INVALID", "REQUIRED", "BAD_REQUEST"]
        ):
        ...
        if any(
            code in error_code_upper
            for code in ["AUTHENTICATION", "UNAUTHORIZED", "INVALID_CREDENTIALS"]
        ):
        ...
        if any(
            code in error_code_upper for code in ["AUTHORIZATION", "FORBIDDEN", "PERMISSION_DENIED"]
        ):
        ...
        # Quota errors
        if any(code in error_code_upper for code in ["QUOTA", "LIMIT_EXCEEDED", "RATE_LIMIT"]):
        ...
        # Not found errors
        if any(code in error_code_upper for code in ["NOT_FOUND", "RESOURCE_NOT_FOUND"]):
        ...
        # Conflict errors
        if any(code in error_code_upper for code in ["CONFLICT", "ALREADY_EXISTS", "DUPLICATE"]):
    ...
    return BaaSCLIError(
        message=error_message, error_code=error_code or "BAAS_ERROR", context=context
    )
...
def _get_validation_suggestion(error_code: str, context: dict[str, Any]) -> str:
    ...
    if "INVALID_DATE" in error_code_upper or "EXPIRES_AT" in error_code_upper:
        return (
            "Expiration date must be in the future and in ISO format (e.g., 2025-12-31T23:59:59Z)"
        )
