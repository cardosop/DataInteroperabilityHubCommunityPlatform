...
def parse_api_error_response(error_data: dict[str, Any]) -> ODPSCLIError | None:
    ...
    error_info.get("technical_message") or error_data.get("technical_message")
    ...
    if not isinstance(context_raw, dict):
        ...
        context = context_raw
    error_info.get("recovery_strategy") or error_data.get("recovery_strategy")
    ...
    if error_code:
        ...
        # Validation errors
        if any(
            error_code_upper.startswith(code)
            for code in [
                "VALIDATION",
                "SCHEMA_VALIDATION",
                "REQUIRED_FIELD",
        ...
        if any(
            error_code_upper.startswith(code)
            for code in ["EXPORT", "SERIALIZATION", "FORMAT_NOT_SUPPORTED", "FILE_WRITE"]
        ):
    ...
    return ODPSCLIError(
        message=error_message, error_code=error_code or "ODPS_ERROR", context=context
    )
...
def validate_odps_version(version: str | None) -> None:
    """
...
def validate_file_format(file_path: str, allowed_formats: list[str] | None = None) -> str:
    """
...
def validate_mutually_exclusive_options(
    option1_name: str, option1_value: Any, option2_name: str, option2_value: Any
) -> None:
    """
    ...
    if option1_value and option2_value:
        raise ODPSParameterError(
            ...
            context={"option1": option1_name, "option2": option2_name},
            suggestion=f"Choose either --{option1_name} or --{option2_name}, not both",
        )


def validate_required_option(
    option_name: str, option_value: Any, alternative: str | None = None
) -> None:
    """
...
def _get_linking_suggestion(error_code: str, context: dict[str, Any]) -> str:
    ...
    if "INVALID_LINK" in error_code_upper:
        ...
        if source_id and target_id:
            return (
                f"Check that both contracts exist: ODCS ID '{source_id}' and ODPS ID '{target_id}'"
            )
