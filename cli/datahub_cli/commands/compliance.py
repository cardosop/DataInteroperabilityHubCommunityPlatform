...
def run_compliance_check(
    ...
    try:
        # API endpoint: /api/v1/compliance/compliance-runs/
        result = api_client.post('compliance/compliance-runs/', json_data=data)
        
        if output_format == 'json':
            click.echo(json.dumps(result, indent=2))
        else:
            compliance_run = result.get('compliance_run', {})
            job = result.get('job', {})
            
            click.echo(f"Compliance check started successfully!")
            click.echo(f"Compliance Run ID: {compliance_run.get('id')}")
            click.echo(f"Job ID: {job.get('id')}")
            click.echo(f"Status: {compliance_run.get('status', 'PENDING')}")
...
def get_compliance_run(compliance_run_id: str, output_format: str):
    ...
    try:
        # API endpoint: /api/v1/compliance/compliance-runs/{id}/
        data = api_client.get(f'compliance/compliance-runs/{compliance_run_id}/')
...
def list_compliance_runs(
    ...
    try:
        ...
        # API endpoint: /api/v1/compliance/compliance-runs/
        data = api_client.get('compliance/compliance-runs/', params=params)
...
def generate_compliance_report(asset_id: Optional[str], regulation: str, output_format: str):
    ...
    try:
        ...
        compliance_runs = api_client.get('compliance/compliance-runs/', params={'asset_id': asset_id, 'limit': 1})
