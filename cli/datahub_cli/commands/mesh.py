...
@click.option(
    "--status", type=click.Choice(["ACTIVE", "INACTIVE", "ARCHIVED"]), help="Filter by status"
)
@click.option("--owner-id", help="Filter by owner user ID")
@click.option("--search", help="Search in name and description")
@click.option(
    "--ordering",
    help="Order by field (name, status, created_at, updated_at). Prefix with - for descending",
)
...
def list_domains(
    ...
    try:
        ...
        if isinstance(data, dict) and "results" in data:
            ...
            next_page: int | None = data.get("next")
            previous_page: int | None = data.get("previous")
        ...
        if output_format == "json":
            ...
            for domain_item in results:
                ...
                name = (
                    (domain_item.get("name", "")[:28] + "..")
                    if len(domain_item.get("name", "")) > 30
                    else domain_item.get("name", "N/A")
                )
                status_val = domain_item.get("status", "N/A")
                owner = domain_item.get("owner", "")[:36] if domain_item.get("owner") else "N/A"
                created = (
                    domain_item.get("created_at", "")[:19]
                    if domain_item.get("created_at")
                    else "N/A"
                )
...
@click.option(
    "--status",
    type=click.Choice(["ACTIVE", "INACTIVE", "ARCHIVED"]),
    default="ACTIVE",
    help="Domain status (default: ACTIVE)",
)
@click.option(
    "--format",
    "output_format",
...
def create_domain(
    ...
    try:
        # Build request data
        data = {"name": name.strip(), "status": status}
...
@click.option("--resource-quota", help="Resource quotas as JSON string")
@click.option(
    "--status", type=click.Choice(["ACTIVE", "INACTIVE", "ARCHIVED"]), help="Domain status"
)
@click.option(
    "--format",
    "output_format",
...
@click.option(
    "--include-health-metrics/--no-include-health-metrics",
    default=True,
    help="Include health metrics in response (default: true)",
)
@click.option(
    "--format",
    "output_format",
...
def get_topology(include_health_metrics: bool, output_format: str):
    ...
    try:
        # Build query parameters
        params: dict[str, Any] = {"include_health_metrics": str(include_health_metrics).lower()}
        ...
        if output_format == "json":
            ...
            if not nodes:
                ...
                for node in nodes:
                    ...
                    name = (
                        (node.get("name", "")[:28] + "..")
                        if len(node.get("name", "")) > 30
                        else node.get("name", "N/A")
                    )
            ...
            if not edges:
                ...
                click.echo(
                    f"{'Source Domain ID':<40} {'Target Domain ID':<40} {'Type':<20} {'Weight':<10}"
                )
                click.echo("-" * 110)
                for edge in edges:
...
@click.option(
    "--overrides", help="Policy overrides as JSON string (conditions, effect, priority, etc.)"
)
@click.option(
    "--format",
    "output_format",
...
def apply_policy(domain_id: str, policy_id: str, overrides: str | None, output_format: str):
    """Apply a policy to a data mesh domain"""
    try:
        # Build request data
        data: dict[str, Any] = {"policy_id": policy_id.strip()}
...
@click.option(
    "--status",
    type=click.Choice(["PENDING", "APPLIED", "FAILED", "REVOKED"]),
    help="Filter by policy application status",
)
@click.option("--page", type=int, default=1, help="Page number (default: 1)")
@click.option("--page-size", type=int, default=20, help="Page size (default: 20, max: 100)")
@click.option(
...
def list_policies(
    domain_id: str, status: str | None, page: int, page_size: int, output_format: str
):
    ...
    try:
        ...
        if output_format == "json":
            ...
            # Table format
            click.echo(
                f"{'Policy ID':<40} {'Policy Name':<30} {'Status':<12} {'Applied At':<20} {'Applied By':<40}"
            )
            click.echo("-" * 142)
            for policy_item in results:
                if not isinstance(policy_item, dict):
                    continue
                policy_id = (
                    policy_item.get("policy_id", "")[:36] if policy_item.get("policy_id") else "N/A"
                )
                policy_name = (
                    (policy_item.get("policy_name", "")[:28] + "..")
                    if len(policy_item.get("policy_name", "")) > 30
                    else policy_item.get("policy_name", "N/A")
                )
                status_val = policy_item.get("status", "N/A")
                applied_at = (
                    policy_item.get("applied_at", "")[:19]
                    if policy_item.get("applied_at")
                    else "N/A"
                )
                applied_by = (
                    policy_item.get("applied_by", "")[:36]
                    if policy_item.get("applied_by")
                    else "N/A"
                )
                click.echo(
                    f"{policy_id:<40} {policy_name:<30} {status_val:<12} {applied_at:<20} {applied_by:<40}"
                )
...
def remove_policy(domain_id: str, policy_id: str, reason: str | None, output_format: str):
    ...
    try:
        ...
        result = api_client.delete(
            f"mesh/domains/{domain_id}/policies/{policy_id}/", json_data=data if data else None
        )
...
def check_compliance(domain_id: str, asset_id: str | None, output_format: str):
...
def get_compliance_report(domain_id: str, report_id: str | None, output_format: str):
    ...
    try:
        if report_id:
            ...
            data = api_client.get(
                f"mesh/domains/{domain_id}/compliance/reports/", params={"page_size": 1}
            )
            ...
            if not results:
                ...
                click.echo(
                    "Run 'mesh compliance check <domain-id>' to generate a compliance report."
                )
                return
            ...
            result = api_client.get(
                f"mesh/domains/{domain_id}/compliance/reports/{latest_report_id}/"
            )
            ...
            if violations:
                ...
                for i, violation in enumerate(violations, 1):
                    if isinstance(violation, dict):
                        ...
                        click.echo(
                            f"  {i}. [{violation_type}] {violation_severity}: {violation_desc}"
                        )
                    else:
                        click.echo(f"  {i}. {violation}")
