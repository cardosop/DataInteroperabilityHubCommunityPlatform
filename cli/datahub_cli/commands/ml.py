...
@click.option(
    "--status",
    type=click.Choice(["TRAINING", "TRAINED", "DEPLOYED", "FAILED", "ARCHIVED"]),
...
def list_models(
    ...
    try:
        ...
        if output_format == "json":
            ...
            # Table format
            click.echo(
                f"{'ID':<40} {'Name':<30} {'Version':<15} {'Type':<20} {'Status':<15} {'Asset ID':<40}"
            )
            click.echo("-" * 170)
            for model in results:
                ...
                asset_id_display = (
                    str(model.get("asset_id", ""))[:36] if model.get("asset_id") else "N/A"
                )
                click.echo(
                    f"{model_id:<40} "
                    f"{name:<30} "
    ...
    try:
        ...
        try:
            ...
            if isinstance(e, requests.exceptions.HTTPError) and hasattr(e, "response"):
                response = e.response
                if hasattr(response, "text") and hasattr(response, "status_code"):
                    raise handle_ml_api_error(
                        response.text, response.status_code, f"ml/models/{model_id}/"
                    )
        except (ImportError, AttributeError):
            pass
...
@click.option(
    "--odh-model-name", help="ODH Model Registry model name (validated but not sent to API)"
)
...
def update_model(
    ...
    if not data:
        raise click.ClickException(
            "At least one field must be provided for update (--asset-id, --contract-id, or --status)"
        )
    ...
    try:
        ...
        if output_format == "json":
            ...
            for version in versions:
                ...
                click.echo(f"{version_id:<40} {version_str:<20} {status_str:<15} {created:<25}")
    except click.ClickException:
        raise
    except Exception as e:
        # Try to handle as ML API error if it's a requests exception
...
def deploy_model(model_id: str, config: str | None, output_format: str):
...
@click.option(
    "--input-data", "input_data", required=True, help="Input data (JSON string or file path)"
)
@click.option(
    "--format",
    "output_format",
...
def predict(deployment_id: str, input_data: str, output_format: str):
    ...
    # Build request data
    data = {"deployment_id": deployment_id, "input": input_json}
...
def list_deployments(
    model_id: str | None, status: str | None, limit: int, offset: int, output_format: str
):
    ...
    try:
        ...
        if output_format == "json":
            ...
            # Table format
            click.echo(
                f"{'Deployment ID':<40} {'Model ID':<40} {'Status':<20} {'Replicas':<10} {'Endpoint':<50}"
            )
            click.echo("-" * 160)
            for deployment in results:
                ...
                endpoint = (
                    str(deployment.get("endpoint", ""))[:48]
...
def get_metrics(
    deployment_id: str, start_time: str | None, end_time: str | None, output_format: str
):
...
def submit_training(model_id: str, dataset_id: str, config: str, output_format: str):
    ...
    try:
        ...
        data = {"model_id": model_id, "dataset_id": dataset_id, "config": training_config}
        ...
        try:
            ...
            if isinstance(e, requests.exceptions.HTTPError) and hasattr(e, "response"):
                ...
                if hasattr(response, "text") and hasattr(response, "status_code"):
                    raise handle_ml_api_error(
                        response.text, response.status_code, "ml/training/jobs/"
                    )
        except (ImportError, AttributeError):
            pass
...
def list_training_jobs(
    ...
    try:
        ...
        if output_format == "json":
            ...
            # Table format
            click.echo(
                f"{'Job ID':<40} {'Model ID':<40} {'Status':<15} {'Progress':<10} {'Submitted At':<25}"
            )
            click.echo("-" * 130)
            for job in results:
                ...
                progress = (
                    f"{job.get('progress', 0.0):.1%}" if job.get("progress") is not None else "N/A"
                )
                submitted_at = (
                    str(job.get("submitted_at", ""))[:23] if job.get("submitted_at") else "N/A"
                )
                click.echo(
                    f"{job_id:<40} "
                    f"{model_id_display:<40} "
    ...
    try:
        ...
        try:
            ...
            if isinstance(e, requests.exceptions.HTTPError) and hasattr(e, "response"):
                ...
                if hasattr(response, "text") and hasattr(response, "status_code"):
                    raise handle_ml_api_error(
                        response.text, response.status_code, f"ml/training/jobs/{job_id}/"
                    )
        except (ImportError, AttributeError):
            pass
    ...
    try:
        ...
        try:
            ...
            if isinstance(e, requests.exceptions.HTTPError) and hasattr(e, "response"):
                ...
                if hasattr(response, "text") and hasattr(response, "status_code"):
                    raise handle_ml_api_error(
                        response.text, response.status_code, f"ml/training/jobs/{job_id}/cancel/"
                    )
        except (ImportError, AttributeError):
            pass
...
def get_training_logs(job_id: str, lines: int | None, output_format: str):
    ...
    try:
        ...
        try:
            ...
            if isinstance(e, requests.exceptions.HTTPError) and hasattr(e, "response"):
                ...
                if hasattr(response, "text") and hasattr(response, "status_code"):
                    raise handle_ml_api_error(
                        response.text, response.status_code, f"ml/training/jobs/{job_id}/logs/"
                    )
        except (ImportError, AttributeError):
            pass
...
def deploy_model_serving(model_id: str, endpoint: str | None, output_format: str):
    ...
    try:
        ...
        try:
            ...
            if isinstance(e, requests.exceptions.HTTPError) and hasattr(e, "response"):
                ...
                if hasattr(response, "text") and hasattr(response, "status_code"):
                    raise handle_ml_api_error(
                        response.text, response.status_code, "ml/inference/deployments/"
                    )
        except (ImportError, AttributeError):
            pass
    ...
    try:
        ...
        try:
            ...
            if isinstance(e, requests.exceptions.HTTPError) and hasattr(e, "response"):
                ...
                if hasattr(response, "text") and hasattr(response, "status_code"):
                    raise handle_ml_api_error(
                        response.text, response.status_code, "ml/inference/deployments/predict/"
                    )
        except (ImportError, AttributeError):
            pass
    ...
    try:
        ...
        if output_format == "json":
            ...
            for serving in results:
                ...
                endpoint = (
                    str(serving.get("endpoint", ""))[:48] if serving.get("endpoint") else "N/A"
                )
                click.echo(
                    f"{serving_id:<40} {model_id_display:<40} {status_str:<20} {endpoint:<50}"
                )
    except click.ClickException:
        raise
    except Exception as e:
            ...
            if isinstance(e, requests.exceptions.HTTPError) and hasattr(e, "response"):
                ...
                if hasattr(response, "text") and hasattr(response, "status_code"):
                    raise handle_ml_api_error(
                        ...
                        f"ml/inference/deployments/{serving_id}/",
                    )
        except (ImportError, AttributeError):
            pass
            ...
            if isinstance(e, requests.exceptions.HTTPError) and hasattr(e, "response"):
                ...
                if hasattr(response, "text") and hasattr(response, "status_code"):
                    raise handle_ml_api_error(
                        response.text,
                        response.status_code,
                        f"ml/inference/deployments/{serving_id}/metrics/",
                    )
        except (ImportError, AttributeError):
            pass
...
def create_ab_test(model_id: str, variant_id: str, traffic_split: str, output_format: str):
    ...
    try:
        ...
        raise click.ClickException(
            f"Invalid traffic split format: {e}. Use format 'X:Y' where X+Y=100 (e.g., '50:50')"
        )
    ...
    try:
        ...
        try:
            ...
            if isinstance(e, requests.exceptions.HTTPError) and hasattr(e, "response"):
                ...
                if hasattr(response, "text") and hasattr(response, "status_code"):
                    raise handle_ml_api_error(
                        response.text, response.status_code, "ml/inference/ab-tests/"
                    )
        except (ImportError, AttributeError):
            pass
...
def list_ab_tests(model_id: str | None, output_format: str):
    """List A/B tests"""
    from typing import Any
    ...
    try:
        ...
        if output_format == "json":
            ...
            # Table format
            click.echo(
                f"{'A/B Test ID':<40} {'Base Model ID':<40} {'Variant Model ID':<40} {'Traffic Split':<20} {'Status':<15}"
            )
            click.echo("-" * 155)
...
def get_ab_test(ab_test_id: str, output_format: str):
    ...
    try:
        ...
        if output_format == "json":
            ...
            if isinstance(traffic_split, dict):
                click.echo(
                    f"Traffic Split: {traffic_split.get('base', 0)}:{traffic_split.get('variant', 0)}"
                )
            else:
        ...
        try:
            ...
            if isinstance(e, requests.exceptions.HTTPError) and hasattr(e, "response"):
                ...
                if hasattr(response, "text") and hasattr(response, "status_code"):
                    raise handle_ml_api_error(
                        response.text, response.status_code, f"ml/inference/ab-tests/{ab_test_id}/"
                    )
        except (ImportError, AttributeError):
            pass
...
def deploy_model(
    ...
    try:
        ...
        raise click.ClickException(f"Failed to deploy model: {e}")
...
def undeploy_model(model_id: str, output_format: str = "table"):
    ...
    try:
        ...
        if output_format == "json":
            click.echo(
                json.dumps(
                    result if result else {"status": "undeployed", "model_id": model_id}, indent=2
                )
            )
        else:
        ...
        raise click.ClickException(f"Failed to undeploy model: {e}")
...
def rollback_model(
    ...
    try:
        ...
        if output_format == "json":
            ...
            click.echo(f"Model rolled back to version {target_version}.")
            click.echo(f"Status: {data.get('status')}")
    except click.ClickException:
        raise
    except Exception as e:
        raise click.ClickException(f"Failed to rollback model: {e}")
...
def show_ml_plan(output_format: str):
    ...
    try:
        ...
        raise click.ClickException(f"Failed to get ML plan: {e}")
...
def show_ml_limits(output_format: str):
    ...
    try:
        ...
        ml_limits = {k: v for k, v in limits.items() if "ml" in k}
        ...
        if output_format == "json":
            ...
            if not ml_limits:
                ...
                return
            click.echo(f"{'Limit':<45} {'Value':<15}")
            click.echo("-" * 60)
        ...
        raise click.ClickException(f"Failed to get ML limits: {e}")
...
def marketplace_publish(
    ...
    try:
        ...
        if output_format == "json":
            ...
            click.echo(f"Listing ID: {data.get('listing_id')}")
    except click.ClickException:
        raise
    except Exception as e:
        raise click.ClickException(f"Failed to publish model: {e}")
