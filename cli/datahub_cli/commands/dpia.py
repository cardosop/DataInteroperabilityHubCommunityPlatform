...
@click.option("--risk-level", type=click.Choice(["LOW", "MEDIUM", "HIGH"]),
              default="MEDIUM", help="Initial risk level")
@click.option("--format", "output_format", type=click.Choice(["json", "table"]),
              default="table", help="Output format")
def create(title: str, description: str, data_categories: Optional[str],
           risk_level: str, output_format: str):
...
@click.option("--format", "output_format", type=click.Choice(["json", "table"]),
              default="table", help="Output format")
def list_assessments(filter_status: Optional[str], page: int, page_size: int,
                     output_format: str):
    ...
    try:
        ...
        if output_format == "json":
            ...
            for r in results:
                click.echo(f"  {r.get('id','?')[:8]}… {r.get('status','?'):15s} "
...
def review_queue(page: int, page_size: int, output_format: str):
    ...
    try:
        data = api_client.get(f"{DPIA_PREFIX}/",
...
@click.option("--format", "output_format", type=click.Choice(["json", "table"]),
              default="table", help="Output format")
def trigger_review(dpia_id: str, notes: str, assigned_to: Optional[str],
                   output_format: str):
...
@click.option("--data-categories", prompt=True, default="",
              help="Comma-separated data categories")
@click.option("--risk-level", type=click.Choice(["LOW", "MEDIUM", "HIGH"]),
              prompt="Risk level", default="MEDIUM")
def wizard(title: str, description: str, data_categories: str, risk_level: str):
    ...
    payload = {
        ...
        "risk_level": risk_level, "data_categories": cats,
    }
