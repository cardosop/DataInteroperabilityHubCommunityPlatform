    ...
    try:
        ...
        if output_format == 'json':
            ...
            if models:
                ...
                for model in models:
                    click.echo(f"  - {model.get('namespace', '')}/{model.get('name', '')}/{model.get('model_name', '')}")
            else:
        ...
        if output_format == 'json':
            ...
            if input_fields:
                ...
                for field in input_fields:
                    click.echo(f"  - {field.get('namespace', '')}/{field.get('name', '')}/{field.get('model_name', '')}.{field.get('field', '')}")
            else:
...
@click.option('--format', 'output_format', type=click.Choice(['json', 'table']), default='table', help='Output format')
def get_full_lineage(contract_id: str, max_contract_depth: int, max_model_depth: int, max_field_depth: int, output_format: str):
...
@click.argument('contract_id')
@click.option('--format', type=click.Choice(['json', 'dot', 'mermaid']), default='json', help='Visualization format')
def visualize_lineage(contract_id: str, format: str):
    """Get lineage visualization"""
    try:
        params = {'format': format}
        response = api_client.request('GET', f'contracts/{contract_id}/lineage/visualization/', params=params)
        
        if format in ['dot', 'mermaid']:
            click.echo(response.text if hasattr(response, 'text') else str(response))
        else:
            click.echo(json.dumps(response.json() if hasattr(response, 'json') else response, indent=2))
    except Exception as e:
