...
def list_files(status: Optional[str], limit: int, offset: int, output_format: str):
    """List files"""
...
@click.option('--format', 'output_format', type=click.Choice(['json', 'table']), default='table', help='Output format')
def upload_file(file_path: str, name: Optional[str], output_format: str):
    """Upload a file"""
    # Get file info
...
@click.option('--output', 'output_path', type=click.Path(), help='Output file path (defaults to original filename)')
def download_file(file_id: str, output_path: Optional[str]):
    """Download a file"""
    try:
        # Get file info
...
def delete_file(file_id: str, confirm: bool):
    """Delete a file"""
    if not confirm:
        if not click.confirm(f"Are you sure you want to delete file {file_id}?"):
