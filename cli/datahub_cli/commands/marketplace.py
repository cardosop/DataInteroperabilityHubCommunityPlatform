...
@click.option('--marketplace-type', 'marketplace_type', required=True,
              type=click.Choice([
...
@click.option('--is-active/--no-is-active', 'is_active', default=True, help='Whether connection is active')
@click.option('--format', 'output_format', type=click.Choice(['json', 'table']), default='table', help='Output format')
def create_connection(marketplace_type: str, name: str, config: str, is_active: bool, output_format: str):
...
@click.option('--is-active', 'is_active', type=click.Choice(['true', 'false']), help='Filter by active status')
@click.option('--limit', type=int, default=50, help='Limit number of results (default: 50, max: 100)')
@click.option('--offset', type=int, default=0, help='Offset for pagination')
@click.option('--format', 'output_format', type=click.Choice(['json', 'table']), default='table', help='Output format')
def list_connections(marketplace_type: Optional[str], is_active: Optional[str], limit: int, offset: int, output_format: str):
    ...
    try:
        ...
        if output_format == 'json':
            ...
            # Table format
            click.echo(f"{'ID':<40} {'Name':<30} {'Marketplace Type':<30} {'Active':<10} {'Created':<20}")
            click.echo("-" * 130)
            for conn in results:
...
@click.option('--is-active/--no-is-active', 'is_active', default=None, help='Whether connection is active')
@click.option('--format', 'output_format', type=click.Choice(['json', 'table']), default='table', help='Output format')
def update_connection(connection_id: str, name: Optional[str], config: Optional[str], is_active: Optional[bool], output_format: str):
    ...
    try:
        ...
        result = api_client.patch(f'integrations/marketplace/connections/{connection_id}/', json_data=data)
...
def delete_connection(connection_id: str, output_format: str):
    ...
    try:
        ...
        if output_format == 'json':
            click.echo(json.dumps({'success': True, 'message': 'Connection deleted successfully'}, indent=2))
        else:
...
@click.argument('connection_id')
@click.option('--config', help='Optional test configuration (JSON string or file path). If not provided, uses connection config.')
@click.option('--format', 'output_format', type=click.Choice(['json', 'table']), default='table', help='Output format')
def test_connection(connection_id: str, config: Optional[str], output_format: str):
    ...
    try:
        ...
        result = api_client.post(f'integrations/marketplace/connections/{connection_id}/test/', json_data=data if data else None)
...
@click.option('--direction', required=True, type=click.Choice(['PUSH', 'PULL', 'BIDIRECTIONAL'], case_sensitive=False), help='Sync direction')
...
@click.option('--format', 'output_format', type=click.Choice(['json', 'table']), default='table', help='Output format')
def sync_start(connection_id: str, direction: str, asset_ids: Optional[str], listing_ids: Optional[str], output_format: str):
    """
    ...
    try:
        ...
        if output_format == 'json':
            ...
            # Table format
            click.echo(f"Sync job created successfully!")
            click.echo(f"ID: {data.get('id')}")
...
@click.option('--status', help='Filter by status (PENDING, RUNNING, COMPLETED, FAILED, PARTIAL, CANCELLED)')
@click.option('--direction', type=click.Choice(['PUSH', 'PULL', 'BIDIRECTIONAL'], case_sensitive=False), help='Filter by direction')
@click.option('--limit', type=int, default=20, help='Limit number of results')
@click.option('--offset', type=int, default=0, help='Offset for pagination')
@click.option('--format', 'output_format', type=click.Choice(['json', 'table']), default='table', help='Output format')
def sync_list(connection_id: Optional[str], status: Optional[str], direction: Optional[str], limit: int, offset: int, output_format: str):
    ...
    try:
        ...
        if output_format == 'json':
            ...
            # Table format
            click.echo(f"{'ID':<40} {'Connection':<30} {'Direction':<15} {'Status':<15} {'Progress':<15} {'Created':<25}")
            click.echo("-" * 140)
            for job in results:
                job_id = str(job.get('id', ''))[:36] if job.get('id') else ''
                connection = str(job.get('connection_name', job.get('connection_id', '')))[:28] if job.get('connection_name') or job.get('connection_id') else ''
                direction = str(job.get('direction_display', job.get('direction', '')))[:13] if job.get('direction_display') or job.get('direction') else ''
                status = str(job.get('status_display', job.get('status', '')))[:13] if job.get('status_display') or job.get('status') else ''
    ...
    try:
        def fetch_and_display():
            ...
            if output_format == 'json':
                ...
                # Table format
                click.echo(f"ID: {data.get('id')}")
                click.echo(f"Connection: {data.get('connection_name', data.get('connection_id', 'N/A'))}")
                click.echo(f"Direction: {data.get('direction_display', data.get('direction', 'N/A'))}")
                click.echo(f"Status: {data.get('status_display', data.get('status', 'N/A'))}")
...
def sync_cancel(sync_job_id: str, output_format: str):
    ...
    try:
        ...
        if output_format == 'json':
            ...
            # Table format
            click.echo(f"Sync job cancelled successfully!")
            click.echo(f"ID: {data.get('id')}")
            click.echo(f"Status: {data.get('status_display', data.get('status', 'N/A'))}")
            click.echo(f"Updated: {data.get('updated_at', 'N/A')}")
    except click.ClickException:
    ...
    try:
        ...
        if output_format == 'json':
            ...
            for connector in connectors:
                ...
                click.echo(f"{connector_type:<35} {display_name:<35} {sync_directions:<25} {status:<15}")
    ...
    try:
        ...
        if '404' in error_msg or 'not found' in error_msg:
            raise MarketplaceValidationError(
                ...
                suggestion=f"Use 'datahub marketplace connectors list' to see available connector types"
            )
        raise
    except Exception as e:
        # Check if it's a 404 error by examining the error message
        ...
        raise MarketplaceConnectionError(
            message=f"Failed to get connector information: {str(e)}",
...
@click.option('--offset', type=int, default=0, help='Offset for pagination')
@click.option('--format', 'output_format', type=click.Choice(['json', 'table']), default='table', help='Output format')
def list_mappings(connection_id: Optional[str], asset_id: Optional[str], limit: int, offset: int, output_format: str):
    ...
    try:
        ...
        if output_format == 'json':
            ...
            # Table format
            click.echo(f"{'ID':<40} {'Connection':<30} {'Asset':<30} {'External Listing':<30} {'Last Synced':<20}")
            click.echo("-" * 150)
            for mapping in results:
                ...
                last_synced = str(mapping.get('last_synced_at', ''))[:19] if mapping.get('last_synced_at') else 'N/A'
                click.echo(f"{mapping_id:<40} {connection_name:<30} {asset_name:<30} {external_listing:<30} {last_synced:<20}")
...
def delete_mapping(mapping_id: str, output_format: str):
    ...
    try:
        ...
        if output_format == 'json':
            click.echo(json.dumps({'success': True, 'message': 'Mapping deleted successfully'}, indent=2))
        else:
