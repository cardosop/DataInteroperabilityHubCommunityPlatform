...
@click.option('--profile-key', default='intake_basic_gx',
              help='DQ profile key (default: intake_basic_gx)')
@click.option('--format', 'output_format',
              type=click.Choice(['json', 'table']), default='table',
              help='Output format')
def run_dq(asset_id: Optional[str], dataset_id: Optional[str],
           file_id: Optional[str], profile_key: str, output_format: str):
    """Run a Data Quality check"""
    if not asset_id and not dataset_id and not file_id:
        raise click.ClickException(
            "At least one of --asset-id, --dataset-id, or --file-id "
            "must be provided"
        )
    ...
    try:
        ...
        if output_format == 'json':
            ...
            if isinstance(result, dict) and 'dq_run' in result:
                ...
                job_id = (
                    job_val.get('id') if isinstance(job_val, dict)
                    else job_val
                )
...
def get_dq_run(dq_run_id: str, output_format: str):
    ...
    try:
        ...
        if output_format == 'json':
            ...
            click.echo(
                f"Quality Score: "
                f"{data.get('quality_score', 'N/A')}"
            )
            click.echo(
                f"Profile: {data.get('profile_key', 'N/A')}"
            )
            if data.get('asset'):
...
@click.option('--format', 'output_format',
              type=click.Choice(['json', 'table']), default='table',
              help='Output format')
def list_dq_runs(asset_id: Optional[str], dataset_id: Optional[str],
...
@click.option('--format', 'output_format',
              type=click.Choice(['json', 'table']), default='table',
              help='Output format')
def watch_dq_run(dq_run_id: str, interval: int, timeout: int,
                 output_format: str):
    ...
    try:
        ...
        if output_format == 'json':
            ...
            click.echo(
                ...
                f"{latest.get('quality_score', 'N/A')}"
            )
            click.echo(
                f"Latest Status: "
        ...
        raise click.ClickException(
            f"Failed to get DQ scorecard: {e}"
        )


@dq.command('alerts')
@click.option('--list', 'list_alerts', is_flag=True,
              help='List alerting rules')
@click.option('--create', is_flag=True,
              help='Create alerting rule (interactive)')
...
@click.option('--format', 'output_format',
              type=click.Choice(['json', 'table']), default='table',
              help='Output format')
def manage_alerts(list_alerts: bool, create: bool,
    ...
    if list_alerts:
        try:
            ...
            raise click.ClickException(
                f"Failed to list alerting rules: {e}"
            )
    elif create:
        if not asset_id or threshold is None:
            raise click.ClickException(
                "--asset-id and --threshold are required for "
        ...
        try:
            result = api_client.post(
                'dq/alerting-rules/', json_data=payload,
            )
            ...
            raise click.ClickException(
                f"Failed to create alerting rule: {e}"
            )
    else:
        click.echo(
            "Use --list to list alerting rules "
            "or --create to create one"
        )
        click.echo("Example: datahub dq alerts --list")
        click.echo(
            "Example: datahub dq alerts --create "
            "--asset-id <id> --threshold 80.0"
        )
