...
def list_policies(page: int, page_size: int, output_format: str):
    ...
    try:
        data = api_client.get(f"{AUDIT_PREFIX}/event-retention-policies/",
                              params={"page": page, "page_size": page_size})
        if output_format == "json":
            click.echo(json.dumps(data, indent=2))
        else:
            results = data.get("results", [])
            click.echo(f"Retention policies: {len(results)} (page {page}, total {data.get('count','?')})")
            for r in results:
                click.echo(f"  {r.get('event_type','?'):40s} {r.get('retention_days','?'):5s}d  "
...
def create_policy(event_type: str, retention_days: int, description: str, output_format: str):
    ...
    try:
        data = api_client.post(f"{AUDIT_PREFIX}/event-retention-policies/", data={
            "event_type": event_type, "retention_days": retention_days,
...
def update_policy(policy_id: str, retention_days: int, description: str, output_format: str):
    ...
    try:
        data = api_client.patch(f"{AUDIT_PREFIX}/event-retention-policies/{policy_id}/", data=payload)
        if output_format == "json":
            click.echo(json.dumps(data, indent=2))
        else:
...
def sweep_status(output_format: str):
    ...
    try:
        data = api_client.get(f"{AUDIT_PREFIX}/audit-events/",
                              params={"page_size": 10})
        if output_format == "json":
            click.echo(json.dumps(data, indent=2))
        else:
