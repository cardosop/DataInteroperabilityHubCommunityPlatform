...
@click.option(
    "--status",
    type=click.Choice(["DRAFT", "ACTIVE", "INACTIVE", "ARCHIVED"]),
    help="Filter by status",
)
@click.option(
    "--query-type",
    type=click.Choice(["SQL", "SPARQL", "FEDERATED", "GRAPHQL", "REST"]),
...
@click.option(
    "--ordering",
    help="Order by field (name, query_type, status, version, created_at, updated_at). Prefix with - for descending",
)
...
def list_datasets(
    ...
    try:
        ...
        if output_format == "json":
            ...
            # Table format
            click.echo(
                f"{'ID':<40} {'Name':<30} {'Query Type':<12} {'Status':<12} {'Version':<15} {'Created':<20}"
            )
            click.echo("-" * 149)
            for dataset_item in results:
                ...
                name = (
                    (dataset_item.get("name", "")[:28] + "..")
                    if len(dataset_item.get("name", "")) > 30
                    else dataset_item.get("name", "N/A")
                )
                ...
                created = (
                    dataset_item.get("created_at", "")[:19]
                    if dataset_item.get("created_at")
                    else "N/A"
                )
                click.echo(
                    f"{dataset_id:<40} {name:<30} {query_type_val:<12} {status_val:<12} {version_val:<15} {created:<20}"
                )
...
@click.option(
    ...
    help="Path to dataset definition JSON file (required)",
)
@click.option(
    "--format",
    "output_format",
    ...
    try:
        ...
        if query_type not in valid_query_types:
            raise click.ClickException(
                f"Invalid query_type: {query_type}. Must be one of: {', '.join(valid_query_types)}"
            )
...
@click.option(
    "--query",
    "query_string",
    help="Query string to execute (optional, uses dataset query if not provided)",
)
@click.option("--parameters", help="Query parameters as JSON string")
@click.option(
    "--execution-mode",
    type=click.Choice(["SYNC", "ASYNC", "SCHEDULED", "MANUAL", "AUTOMATED"]),
    help="Execution mode",
)
@click.option(
    "--force-async", is_flag=True, help="Force asynchronous execution even for small queries"
)
@click.option(
    "--timeout-seconds",
    type=int,
    help="Query timeout in seconds (default: 300 for sync, 3600 for async)",
)
@click.option(
    "--format",
    "output_format",
...
def list_queries(
    dataset_id: str | None, status: str | None, page: int, page_size: int, output_format: str
):
    ...
    try:
        ...
        if output_format == "json":
            ...
            for query_item in results:
                ...
                dataset_name = (
                    (query_item.get("virtual_dataset_name", "")[:28] + "..")
                    if len(query_item.get("virtual_dataset_name", "")) > 30
                    else query_item.get("virtual_dataset_name", "N/A")
                )
                status_val = query_item.get("status", "N/A")
                mode_val = query_item.get("execution_mode", "N/A")
                started = (
                    query_item.get("started_at", "")[:19] if query_item.get("started_at") else "N/A"
                )
                click.echo(
                    f"{query_id:<40} {dataset_name:<30} {status_val:<12} {mode_val:<12} {started:<20}"
                )
...
@click.option(
    ...
    "output_file",
    type=click.Path(),
    help="Save result to file (for CSV/Parquet formats)",
)
def get_result(
    execution_id: str,
    output_format: str,
            ...
            if isinstance(result, dict) and "data" in result:
                ...
                if isinstance(parquet_data, str):
                    ...
                    try:
                        ...
                        raise click.ClickException(
                            f"Failed to decode parquet data: {parquet_data[:100]}"
                        )
                else:
                ...
                raise click.ClickException(
                    "Parquet format not supported or result format unexpected"
                )
    except click.ClickException:
        raise
    except Exception as e:
    ...
    try:
        ...
        if output_format == "json":
            ...
            if not nodes:
                ...
                click.echo(
                    f"{'ID':<40} {'Name':<30} {'Query Type':<15} {'Status':<12} {'Health Score':<15}"
                )
                click.echo("-" * 112)
                for node in nodes:
                    ...
                    click.echo(
                        f"{node_id:<40} {name:<30} {query_type:<15} {status:<12} {health_str:<15}"
                    )
            ...
            if not edges:
                ...
                click.echo(
                    f"{'Source Dataset ID':<40} {'Target Dataset ID':<40} {'Type':<20} {'Weight':<10}"
                )
                click.echo("-" * 110)
                for edge in edges:
    ...
    try:
        ...
        if output_format == "json":
            ...
            if not relationships:
                ...
                click.echo(
                    f"{'Related Dataset ID':<40} {'Type':<20} {'Direction':<15} {'Weight':<10}"
                )
                click.echo("-" * 85)
                for rel in relationships:
                    if not isinstance(rel, dict):
                        continue
                    related_id = (
                        str(rel.get("dataset_id", ""))[:36] if rel.get("dataset_id") else "N/A"
                    )
