...
def list_processors(page: int, page_size: int, output_format: str):
    ...
    click.echo("-" * 90)
    for r in results:
        click.echo(f"{str(r.get('id',''))[:36]:<38} {r.get('name',''):<30} {r.get('country_code',''):<8} {(r.get('created_at','') or '')[:10]}")
...
@click.option("--format", "output_format", type=click.Choice(["json", "table"]),
              default="table", help="Output format")
def create_processor(name: str, legal_name: str, country_code: str,
                     website: str, notes: str, output_format: str):
    """Register a new processor"""
    data = api_client.post(f"{BASE}/processors/", json_data={
        "name": name, "legal_name": legal_name,
        "country_code": country_code, "website": website, "notes": notes,
    })
    if output_format == "json":
...
def list_agreements(page: int, page_size: int, output_format: str):
    ...
    data = api_client.get(f"{BASE}/processor-agreements/", params={"page": page, "page_size": page_size})
    results = data.get("results", []) if isinstance(data, dict) else []
    if output_format == "json":
        click.echo(json.dumps(results, indent=2, default=str))
        return
    ...
    click.echo("-" * 100)
    for r in results:
        click.echo(f"{str(r.get('id',''))[:36]:<38} {r.get('agreement_type',''):<8} {r.get('processor_name',''):<25} {r.get('status',''):<12} {str(r.get('effective_from',''))[:10]}")


@agreements.command("create")
@click.option("--processor-id", required=True, help="Processor UUID")
@click.option("--type", "agreement_type", required=True,
...
@click.option("--format", "output_format", type=click.Choice(["json", "table"]),
              default="table", help="Output format")
def create_agreement(processor_id: str, agreement_type: str, document_uri: str,
                     document_hash: str, effective_from: str, expires_on: Optional[str],
                     jurisdiction_region: str, transfer_mechanism: str, output_format: str):
    """Create a new processor agreement"""
    payload: dict = {
        "processor": processor_id, "agreement_type": agreement_type,
        "document_uri": document_uri, "document_hash": document_hash,
...
def list_links(page: int, page_size: int, output_format: str):
    ...
    data = api_client.get(f"{BASE}/asset-processor-links/", params={"page": page, "page_size": page_size})
    results = data.get("results", []) if isinstance(data, dict) else []
    if output_format == "json":
        click.echo(json.dumps(results, indent=2, default=str))
        return
    ...
    click.echo("-" * 116)
    for r in results:
        click.echo(f"{str(r.get('id',''))[:36]:<38} {str(r.get('asset',''))[:36]:<38} {str(r.get('processor',''))[:36]:<38}")
...
def create_link(asset_id: str, processor_id: str, output_format: str):
    ...
    data = api_client.post(f"{BASE}/asset-processor-links/", json_data={
        "asset": asset_id, "processor": processor_id,
    })
    if output_format == "json":
        click.echo(json.dumps(data, indent=2, default=str))
    else:
