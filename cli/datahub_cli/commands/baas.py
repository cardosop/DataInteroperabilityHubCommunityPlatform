...
@click.option(
    "--tier",
    type=click.Choice(["FREE", "PRO", "ENTERPRISE"], case_sensitive=False),
    default="FREE",
    help="API tier (default: FREE)",
)
@click.option(
    "--expires-at", help="Expiration date in ISO format (e.g., 2025-12-31T23:59:59Z). Optional."
)
@click.option(
    "--format",
    "output_format",
...
def create_api_key(name: str, tier: str, expires_at: str | None, output_format: str):
    """
    ...
    data = {"name": name.strip(), "tier": tier.upper()}
    ...
    try:
        ...
        raise BaaSCLIError(
            message=f"Failed to create API key: {e!s}",
...
def list_api_keys(tier: str | None, limit: int, offset: int, output_format: str):
    """List API keys"""
    ...
    try:
        ...
        if output_format == "json":
            ...
            for api_key in results:
                ...
                click.echo(
                    f"{api_key_id:<40} {name:<30} {tier_name:<12} {status:<12} {expires_at:<20}"
                )
        ...
        if (
            ...
            raise handle_baas_api_error(e.response.text, e.response.status_code, "baas/api-keys/")
        raise BaaSCLIError(
            message=f"Failed to list API keys: {e!s}",
...
def get_api_key(api_key_id: str, output_format: str):
    ...
    try:
        ...
        if (
            ...
            raise handle_baas_api_error(
                e.response.text, e.response.status_code, f"baas/api-keys/{api_key_id}/"
            )
        ...
        raise BaaSCLIError(
            message=f"Failed to get API key: {e!s}",
...
@click.option(
    "--tier",
    type=click.Choice(["FREE", "PRO", "ENTERPRISE"], case_sensitive=False),
    help="Update API tier",
)
@click.option(
    "--expires-at", help="Update expiration date in ISO format (e.g., 2025-12-31T23:59:59Z)"
)
@click.option(
    "--format",
    "output_format",
...
def update_api_key(
    api_key_id: str, name: str | None, tier: str | None, expires_at: str | None, output_format: str
):
    """Update an API key"""
    # Validate API key ID
    if not api_key_id or not api_key_id.strip():
    ...
    try:
        ...
        raise BaaSCLIError(
            message=f"Failed to update API key: {e!s}",
...
def revoke_api_key(api_key_id: str, output_format: str):
    ...
    try:
        ...
        if output_format == "json":
            click.echo(
                json.dumps(result if result else {"status": "revoked"}, indent=2, default=str)
            )
        else:
        ...
        raise BaaSCLIError(
            message=f"Failed to revoke API key: {e!s}",
...
def _validate_api_key_id(api_key_id: str | None) -> str | None:
    """
...
def usage_stats(
    api_key_id: str | None, start_date: str | None, end_date: str | None, output_format: str
):
    """
    ...
    try:
        ...
        if (
            ...
            raise handle_baas_api_error(
                e.response.text, e.response.status_code, "baas/usage/stats/"
            )
        raise BaaSCLIError(
            message=f"Failed to get usage statistics: {e!s}",
...
def usage_by_endpoint(
    ...
    try:
        ...
        if output_format == "json":
            ...
            # Table format
            click.echo(
                f"{'Endpoint':<50} {'Method':<10} {'Requests':<12} {'Success':<12} {'Errors':<12} {'Avg Time (ms)':<15}"
            )
            click.echo("-" * 111)
            for item in results:
        ...
        if (
            ...
            raise handle_baas_api_error(
                e.response.text, e.response.status_code, "baas/usage/by-endpoint/"
            )
        raise BaaSCLIError(
            message=f"Failed to get usage by endpoint: {e!s}",
...
def usage_by_tenant(start_date: str | None, end_date: str | None, output_format: str):
    """
    ...
    try:
        ...
        if output_format == "json":
            ...
            # Table format
            click.echo(
                f"{'Tenant ID':<40} {'Tenant Name':<30} {'Requests':<12} {'Success':<12} {'Errors':<12} {'Avg Time (ms)':<15}"
            )
            click.echo("-" * 121)
            for item in results:
        ...
        if (
            ...
            raise handle_baas_api_error(
                e.response.text, e.response.status_code, "baas/usage/by-tenant/"
            )
        raise BaaSCLIError(
            message=f"Failed to get usage by tenant: {e!s}",
...
"""
        ...
        raise BaaSCLIError(
            message=f"Failed to get API documentation: {e!s}",
...
def show_openapi(output_format: str):
    ...
    try:
        if output_format.lower() == "json":
            ...
            try:
                ...
                try:
                    ...
                    raise BaaSCLIError(
                        message=f"Failed to convert OpenAPI schema to YAML: {conv_error!s}",
        ...
        if (
            ...
            raise handle_baas_api_error(
                e.response.text, e.response.status_code, "baas/docs/openapi.json/"
            )
        raise BaaSCLIError(
            message=f"Failed to get OpenAPI schema: {e!s}",
...
def show_sdks(output_format: str):
    ...
    try:
        ...
        if output_format.lower() == "json":
            ...
            # Table format
            click.echo(
                f"{'Language':<20} {'Name':<30} {'Install Command':<40} {'Download URL':<50}"
            )
            click.echo("-" * 140)
            ...
            for language_key, sdk_info in data.items():
                ...
                click.echo(f"{language:<20} {name:<30} {install_cmd:<40} {download_url:<50}")
        ...
        raise BaaSCLIError(
            message=f"Failed to get SDK links: {e!s}",
    ...
    try:
        ...
        if output_format == "json":
            ...
            if not results:
                ...
                return
            click.echo(f"{'ID':<40} {'Name':<25} {'Email':<30}")
            click.echo("-" * 95)
            for c in results:
        ...
        raise click.ClickException(f"Failed to list customers: {e}")
    ...
    try:
        ...
        if output_format == "json":
            ...
            if isinstance(records, list):
                for r in records:
                    click.echo(f"  {r.get('metric', 'N/A')}: {r.get('quantity', 0)}")
            else:
                click.echo(json.dumps(data, indent=2))
    except click.ClickException:
        raise
    except Exception as e:
        raise click.ClickException(f"Failed to get customer usage: {e}")
    ...
    try:
        ...
        if output_format == "json":
            ...
            if not results:
                ...
                return
            click.echo(f"{'ID':<40} {'Period':<15} {'Status':<12} {'Total':<15}")
            click.echo("-" * 82)
            for r in results:
                ...
                click.echo(
                    ...
                    f"{r.get('period', '')!s:<15} "
                    f"{r.get('status', '')!s:<12} "
                    f"{r.get('total_amount', '')!s:<15}"
                )
    except click.ClickException:
        raise
    except Exception as e:
        raise click.ClickException(f"Failed to list billing reports: {e}")
...
def get_billing_report(
    ...
    try:
        ...
        if output_format == "json":
            ...
            click.echo(f"Total: {data.get('total_amount')}")
            click.echo(f"Created: {data.get('created_at')}")
    except click.ClickException:
        raise
    except Exception as e:
        raise click.ClickException(f"Failed to get billing report: {e}")
...
def finalize_billing_report(report_id: str):
    ...
    try:
        api_client.post(
            f"baas/billing-reports/{report_id}/finalize/",
        )
        click.echo(f"Report {report_id} finalized.")
    except click.ClickException:
        raise
    except Exception as e:
        raise click.ClickException(f"Failed to finalize report: {e}")
...
def export_billing_report(
    ...
    try:
        data = api_client.post(
            f"baas/billing-reports/{report_id}/export/",
        ...
        raise click.ClickException(f"Failed to export report: {e}")
...
def send_billing_report(
    ...
    try:
        api_client.post(
            f"baas/billing-reports/{report_id}/send/",
            json_data={"email": email},
        )
        click.echo(f"Report {report_id} sent to {email}.")
    except click.ClickException:
        raise
    except Exception as e:
        raise click.ClickException(f"Failed to send report: {e}")
...
def rotate_api_key(
    ...
    try:
        ...
        if output_format == "json":
            ...
            click.echo(f"New Key: {data.get('api_key')}")
            click.echo(f"Grace Period: {grace_hours}h")
            click.echo("Save the new key — it won't be shown again.")
    except click.ClickException:
        raise
    except Exception as e:
        raise click.ClickException(f"Failed to rotate API key: {e}")
