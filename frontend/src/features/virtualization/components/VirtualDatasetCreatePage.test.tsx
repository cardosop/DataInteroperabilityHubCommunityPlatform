/**
 * VirtualDatasetCreatePage smoke test — Phase 106
 */
...
import { VirtualDatasetCreatePage } from './VirtualDatasetCreatePage';

describe('VirtualDatasetCreatePage', () => {
  ...
  it('renders without crashing', async () => {
    render(<VirtualDatasetCreatePage />, { wrapper: Wrapper });
    // Component should mount without throwing
    expect(document.body.innerHTML.length).toBeGreaterThan(0);
  });

  it('mounts and processes initial render cycle', async () => {
    const { container } = render(<VirtualDatasetCreatePage />, { wrapper: Wrapper });
