/**
 * VirtualDatasetListPage smoke test — Phase 106
 */
...
import { VirtualDatasetListPage } from './VirtualDatasetListPage';

describe('VirtualDatasetListPage', () => {
  ...
  it('renders without crashing', async () => {
    render(<VirtualDatasetListPage />, { wrapper: Wrapper });
    // Component should mount without throwing
    expect(document.body.innerHTML.length).toBeGreaterThan(0);
  });

  it('mounts and processes initial render cycle', async () => {
    const { container } = render(<VirtualDatasetListPage />, { wrapper: Wrapper });
