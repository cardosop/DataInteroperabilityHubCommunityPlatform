/**
 * observabilityService tests — Phase 105
 */
...
describe('observabilityService', () => {
  ...
  it('getMetrics calls GET on correct URL', async () => {
    vi.mocked(mock.get).mockResolvedValue({ data: { results: [] } });
    await observabilityService.getFreshnessDashboard();
  ...
  it('handles network error gracefully', async () => {
    ...
    await expect(observabilityService.getFreshnessDashboard()).rejects.toThrow('Network Error');
  });
