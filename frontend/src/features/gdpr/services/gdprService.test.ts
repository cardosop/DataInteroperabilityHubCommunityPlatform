...
describe('gdprService', () => {
  ...
  it('listExportJobs calls GET on correct URL', async () => {
    vi.mocked(mock.get).mockResolvedValue({ data: { results: [] } });
    await gdprService.listExportJobs();
  ...
  it('handles network error gracefully', async () => {
    ...
    await expect(gdprService.listExportJobs()).rejects.toThrow('Network Error');
  });
