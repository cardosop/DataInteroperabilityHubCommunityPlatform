/**
 * marketplaceMappingService tests — Phase 105
 */
...
import { marketplaceMappingService } from './marketplaceMappingService';

describe('marketplaceMappingService', () => {
  ...
  it('list calls GET on correct URL', async () => {
    ...
    await marketplaceMappingService.list();
    expect(vi.mocked(mock.get)).toHaveBeenCalled();
    const url = vi.mocked(mock.get).mock.calls[0][0] as string;
    expect(url).toContain('integrations/marketplace/mappings');
  });

  it('handles network error gracefully', async () => {
    vi.mocked(mock.get).mockRejectedValue(new Error('Network Error'));
    await expect(marketplaceMappingService.list()).rejects.toThrow('Network Error');
  });
