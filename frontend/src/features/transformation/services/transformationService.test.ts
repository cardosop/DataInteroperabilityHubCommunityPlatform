/**
 * transformationService tests — Phase 105
 */
...
describe('transformationService', () => {
  ...
  it('list calls GET on correct URL', async () => {
    ...
    expect(url).toContain('transformation/pipelines');
  });

  it('handles network error gracefully', async () => {
    vi.mocked(mock.get).mockRejectedValue(new Error('Network Error'));
    await expect(transformationService.list()).rejects.toThrow('Network Error');
  });
