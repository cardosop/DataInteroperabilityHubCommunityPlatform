  ...
  beforeEach(() => {
    ...
    vi.mocked(mock.get).mockResolvedValue({
      data: {
        ...
        name: 'Test Item',
        status: 'ACTIVE',
        // The component template calls ``.toLowerCase()`` on these
        // fields unconditionally — any missing one throws TypeError
        // BEFORE the CanonicalIriCard renders, masking the real
        // assertion. The existing smoke tests pass anyway because
        // they only check ``document.body.innerHTML.length > 0``,
        // which is satisfied by a partial render before the throw.
        visibility: 'PRIVATE',
        dq_status: 'UNKNOWN',
