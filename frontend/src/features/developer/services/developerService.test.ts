/**
 * developerService tests — Phase 105
 */
...
describe('developerService', () => {
  ...
  it('listPlugins calls GET on correct URL', async () => {
    vi.mocked(mock.get).mockResolvedValue({ data: { results: [] } });
    await developerService.listPlugins();
  ...
  it('handles network error gracefully', async () => {
    ...
    await expect(developerService.listPlugins()).rejects.toThrow('Network Error');
  });
