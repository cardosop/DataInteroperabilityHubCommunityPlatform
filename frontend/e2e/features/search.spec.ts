...
test.describe('Feature: Search', () => {
  ...
  test.describe('Success', () => {
    test('search route loads with search input', async ({ page }) => {
      ...
      await loginAndNavigateToRoute(page, testUser, '/search', {
        timeout: 60000,
        contentSelector: '.search-page, .empty-state, h1',
      });
      await assertListPageLoads(page, '.search-page, .empty-state', { timeout: 60000 });
  ...
  test.describe('Failure', () => {
    test('search with special characters does not crash', async ({ page }) => {
      ...
      await page
        .locator('.search-results, .empty-state, .search-page, .error-display')
        .first()
      ...
      const hasContent =
        (await page.locator('.search-page, .search-results, .empty-state').count()) > 0;
  ...
  test.describe('Edge', () => {
    test('search with empty query shows empty state or all results', async ({ page }) => {
      ...
      const hasContent =
        (await page
          .locator(
            '.search-page, .search-results, .empty-state, h1, input[type="search"], input[placeholder*="search" i]'
          )
          .count()) > 0;
