/**
 * E2E Test: ML Models — ML models endpoint smoke test
 *
 * Verifies the ML models endpoint is reachable and does not return 5xx.
 * Gated behind E2E_JOURNEYS env var; skips gracefully when test env is unavailable.
 */
import { test, expect } from '@playwright/test';

test.describe('[ML Models] Journey', () => {
  test.skip(!process.env.E2E_JOURNEYS, 'Set E2E_JOURNEYS=1 to run');

  test('ml models happy path — models list returns non-500', async ({ request }) => {
    ...
    const res = await request.get('/api/v1/ml/models/', { headers });
