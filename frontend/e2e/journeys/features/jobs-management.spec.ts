/**
 * E2E Test: Jobs Management — Jobs endpoint smoke test
 *
 * Verifies the jobs endpoint is reachable and does not return 5xx.
 * Gated behind E2E_JOURNEYS env var; skips gracefully when test env is unavailable.
 */
import { test, expect } from '@playwright/test';

test.describe('[Jobs Management] Journey', () => {
  test.skip(!process.env.E2E_JOURNEYS, 'Set E2E_JOURNEYS=1 to run');

  test('jobs management happy path — jobs list returns non-500', async ({ request }) => {
    ...
    const res = await request.get('/api/v1/jobs/', { headers });
