/**
 * E2E Test: Global Search — Search endpoint smoke test
 *
 * Verifies the search endpoint is reachable and does not return 5xx.
 * Gated behind E2E_JOURNEYS env var; skips gracefully when test env is unavailable.
 */
import { test, expect } from '@playwright/test';

test.describe('[Search Global] Journey', () => {
  test.skip(!process.env.E2E_JOURNEYS, 'Set E2E_JOURNEYS=1 to run');

  test('search global happy path — search query returns non-500', async ({ request }) => {
    ...
    const res = await request.get('/api/v1/search/?q=test', { headers });
