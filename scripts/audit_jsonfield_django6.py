                            ...
                            self.jsonfields.append(
                                {
                                    ...
                                    "keywords": {
                                        kw.arg: kw.value for kw in node.value.keywords if kw.arg
                                    },
                                }
                            )
        self.generic_visit(node)
...
def find_jsonfield_files(root_dir: str) -> list[Path]:
    ...
    for py_file in root.rglob("*.py"):
        ...
        try:
            ...
            if "JSONField" in content and py_file not in files:
    ...
    if "default" in keywords:
        ...
        if isinstance(default_val, ast.Constant) and default_val.value is None:
            recommendations.append(
                "Consider using default=dict for mutable defaults (Django 6 best practice)"
            )
