#!/bin/bash
# Run Tests in Parallel Batches - Optimized for Speed
...
SUMMARY_FILE="${REPORT_DIR}/test_summary_batches_${TIMESTAMP}.txt"
FAILURES_FILE="${REPORT_DIR}/test_failures_batches_${TIMESTAMP}.txt"

# Number of parallel workers per batch
PARALLEL_WORKERS=${PARALLEL_WORKERS:-4}
# Max failures before stopping
MAX_FAILURES=${MAX_FAILURES:-50}

echo -e "${BOLD}${BLUE}========================================${RESET}"
echo -e "${BOLD}${BLUE}Parallel Test Execution (Batched)${RESET}"
echo -e "${BOLD}${BLUE}Workers: ${PARALLEL_WORKERS}, Max Failures: ${MAX_FAILURES}${RESET}"
echo -e "${BOLD}${BLUE}========================================${RESET}"
echo ""
...
# Initialize summary
echo "Parallel Batched Test Execution Summary - $(date)" > "$SUMMARY_FILE"
echo "===============================================" >> "$SUMMARY_FILE"
echo "" >> "$SUMMARY_FILE"
...
echo "Max failures: ${MAX_FAILURES}" >> "$SUMMARY_FILE"
echo "" >> "$SUMMARY_FILE"
...
run_test_suite_parallel() {
    ...
    echo -e "${BOLD}${BLUE}Running ${suite_name} (${PARALLEL_WORKERS} workers)...${RESET}"
    echo "Running ${suite_name} (${PARALLEL_WORKERS} workers)..." >> "$SUMMARY_FILE"
    
    local report_file="${REPORT_DIR}/${suite_name}_batches_${TIMESTAMP}.txt"
    local exit_code=0
    ...
    if [ "$exit_code" -eq 0 ]; then
        ...
        grep -A 3 "FAILED\|ERROR" "$report_file" >> "$FAILURES_FILE" || true
    fi
...
echo -e "${BOLD}${BLUE}Step 6: Run E2E Tests (Sequential - Service Dependencies)${RESET}"
echo -e "${YELLOW}Note: E2E tests run sequentially due to service dependencies${RESET}"
# E2E tests run sequentially to avoid service conflicts
pytest tests/e2e/ -v --tb=short --maxfail=10 > "${REPORT_DIR}/E2E_Tests_batches_${TIMESTAMP}.txt" 2>&1 || OVERALL_STATUS=1
echo ""
...
echo -e "${BOLD}${BLUE}Total Duration: ${TOTAL_DURATION}s (${TOTAL_MINUTES}m ${TOTAL_SECONDS}s)${RESET}"
echo -e "${BOLD}${BLUE}========================================${RESET}"
cat "$SUMMARY_FILE"

echo "" >> "$SUMMARY_FILE"
echo "Total execution time: ${TOTAL_DURATION}s (${TOTAL_MINUTES}m ${TOTAL_SECONDS}s)" >> "$SUMMARY_FILE"
