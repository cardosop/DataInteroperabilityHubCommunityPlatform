...
class GapDetailsDocumenter:
            ...
            if current_section == "Missing Endpoints":
                if line.startswith("#### "):
                    ...
                    if "**Status**:" in line:
                        ...
                        current_gap["performance_requirements"] = line.split(
            ...
            # Enhancements
                ...
                if line.startswith("#### "):
                    ...
                    if "**Priority**:" in line:
                        ...
                        current_gap["enhancement_type"] = line.split("**Enhancement Type**:")[
                            1
                        ].strip()
                    elif "**Impact**:" in line:
                        current_gap["impact"] = line.split("**Impact**:")[1].strip()
    ...
    def map_journeys_and_use_cases(self, gap: dict) -> tuple[list[str], list[str]]:
        ...
        for journey_id, endpoints in self.JOURNEY_API_MAPPING.items():
            if endpoint_key in endpoints and journey_id not in blocked_journeys:
                blocked_journeys.append(journey_id)

        for use_case_id, endpoints in self.USE_CASE_API_MAPPING.items():
            if endpoint_key in endpoints and use_case_id not in blocked_use_cases:
    ...
    def generate_gap_details_report(self, output_path: Path):
        ...
        lines.append("**Task**: 0.3.3 - Document gap details")
        lines.append("")
        lines.append("---")
        lines.append("")
            ...
            for gap_type in ["missing", "incomplete", "enhancement"]:
                ...
                for gap in sorted(type_gaps, key=lambda x: x.endpoint):
                    ...
                    if gap.performance_requirements:
                        lines.append(
                            f"**Performance Requirements**: {gap.performance_requirements}"
                        )
                        lines.append("")
...
def main():
    ...
    parser.add_argument(
        "--output", default="docs/api-audit/gap-details.md", help="Path to output gap details file"
    )
