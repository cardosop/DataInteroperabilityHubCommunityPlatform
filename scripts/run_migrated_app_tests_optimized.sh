#!/bin/bash
# Optimized test execution script for migrated app tests
# - Ensures test database is migrated once
# - Uses SKIP_TEST_MIGRATIONS=1 with --keepdb for faster runs
# - Runs tests in smaller batches with timeouts
...
echo "=========================================="
echo "Running Migrated App Tests (Optimized)"
echo "=========================================="
echo ""

# Ensure test database is set up with migrations
# Run a simple test first to create and migrate the test database if needed
echo "Ensuring test database is set up..."
echo "Running a simple test to create/migrate test database if needed..."
timeout 600 docker compose exec -T api-service python /app/hub/manage.py test hub.apps.tenants.tests.test_tenant_config_job_integration.TenantConfigJobIntegrationTest.test_job_creation_with_platform_defaults --verbosity=0 --keepdb --no-input 2>&1 | grep -E "(Creating test database|Running migrations|OK|FAILED|Using existing)" | head -5 || true
echo "Test database ready."

# Test batches by app (smaller batches to avoid timeouts)
...
for app in "${!TEST_BATCHES[@]}"; do
    ...
    # Use SKIP_TEST_MIGRATIONS=1 for speed (test DB should already be migrated from setup)
    set +e
    OUTPUT=$(timeout 300 docker compose exec -T api-service bash -c "SKIP_TEST_MIGRATIONS=1 python /app/hub/manage.py test $TEST_PATH --verbosity=1 --keepdb --no-input 2>&1") || TEST_EXIT=$?
    set -e
