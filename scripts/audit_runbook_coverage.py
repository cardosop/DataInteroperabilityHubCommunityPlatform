...
def main():
    runbook_count = len(
        [f for f in os.listdir("docs/runbooks") if f.startswith("RB-FLAG-") and f.endswith(".md")]
    )
    ...
    try:
        ...
        claude_flags = 36  # Known count from registry
    ...
    if gap > 0:
        print(
            f"WARNING: {gap} RB-FLAG runbook(s) missing. Create stubs with flag description, default state, retirement date, rollback procedure."
        )
        sys.exit(1)
