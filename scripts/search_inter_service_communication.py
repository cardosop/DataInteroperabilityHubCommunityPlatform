...
class InterServiceCommunicationSearcher:
    ...
    def __init__(self, root_dir: str):
        ...
        self.service_patterns = {
            ...
            "workflow-engine-service": [
                ...
                "localhost:8088",
                ":8088",
                "WORKFLOW_ENGINE_SERVICE_URL",
            ],
            "workflow-registry-service": [
                "workflow-registry-service",
                "localhost:8089",
                ":8089",
                "WORKFLOW_REGISTRY_SERVICE_URL",
            ],
            "event-bus-health-service": [
                "event-bus-health-service",
                "localhost:8090",
                ":8090",
                "EVENT_BUS_HEALTH_SERVICE_URL",
            ],
            "event-schema-registry-service": [
                "event-schema-registry-service",
                "localhost:8091",
                ":8091",
                "EVENT_SCHEMA_REGISTRY_SERVICE_URL",
            ],
        }
    ...
    def identify_target_service(self, line: str, context_lines: list[str]) -> str | None:
    ...
    def extract_endpoint_from_line(self, line: str, context_lines: list[str]) -> str | None:
        ...
        for py_file in worker_files:
            ...
            try:
                ...
                for i, line in enumerate(lines, 1):
                    ...
                    if re.search(
                        r"api-service|localhost:8000|:8000|API_SERVICE_URL", line, re.IGNORECASE
                    ):
                    ...
                    if re.search(
                        r"enqueue|queue\.enqueue|rq\.enqueue|get_queue", line, re.IGNORECASE
                    ):
            ...
            for py_file in external_dir.rglob("*.py"):
                ...
                try:
                    ...
                    for i, line in enumerate(lines, 1):
                        ...
                        if re.search(
                            r"api-service|localhost:8000|:8000|/api/v1|base_url|baseURL",
                            line,
                            re.IGNORECASE,
                        ):
                            target_service = self.identify_target_service(line, lines[i : i + 5])
                    ...
                    for service_name, patterns in self.service_patterns.items():
                        ...
                        for pattern in patterns:
                            if pattern in line and (
                                "http" in line.lower() or "url" in line.lower()
                            ):
                                endpoint = self.extract_endpoint_from_line(line, lines[i : i + 5])
                                method = self._extract_method_from_line(line)
    ...
    def search_service_client_usage(self):
        ...
        for py_file in self.root_dir.rglob("*.py"):
            ...
            try:
                ...
                source_service = self.identify_source_service(
    ...
    def generate_report(self) -> dict[str, Any]:
        ...
        report = {
            ...
            "summary": {
                ...
                "worker_to_api_calls": len(
                    [
                        c
                        for c in self.service_calls
                        if c.source_service == "worker-service"
                        and c.target_service == "api-service"
                    ]
                ),
                "external_to_api_calls": len(
                    [
                        c
                        for c in self.service_calls
                        if c.source_service in ["cli", "sdk"] and c.target_service == "api-service"
                    ]
                ),
                "api_to_external_calls": len(
                    [
                        c
                        for c in self.service_calls
                        if c.source_service == "api-service" and c.target_service != "api-service"
                    ]
                ),
                "services_involved": sorted(
                    list(
                        set(
                            [c.source_service for c in self.service_calls]
                            + [c.target_service for c in self.service_calls]
                        )
                    )
                ),
            },
        ...
        for _key, dep in self.dependencies.items():
    ...
    def run(self) -> dict[str, Any]:
        ...
        print(
            f"Found {len([c for c in self.service_calls if c.source_service == 'worker-service'])} worker service calls"
        )
        ...
        print(
            f"Found {len([c for c in self.service_calls if c.source_service in ['cli', 'sdk']])} external service calls"
        )
        ...
        print(
            f"Found {len([c for c in self.service_calls if c.source_service == 'api-service'])} API service calls"
        )
...
def main():
    ...
    for dep_key, dep_data in list(report["dependencies"].items())[:10]:
        print(
            f"  {dep_key}: {dep_data['call_count']} calls, {len(dep_data['endpoints'])} endpoints"
        )
