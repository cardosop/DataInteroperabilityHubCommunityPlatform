...
def setup_minio_user():
    ...
    try:
        result = subprocess.run(docker_cmd, check=False, capture_output=True, text=True, timeout=30)
        ...
        if result.returncode == 0:
            ...
            policy_cmd = [
                ...
                "hub-net",
                "-e",
                f"MC_HOST_local=http://{root_user}:{root_password}@{minio_host}:{minio_port}",
                "minio/mc:latest",
                "mc",
                "admin",
                "policy",
            ...
            policy_result = subprocess.run(
                policy_cmd, check=False, capture_output=True, text=True, timeout=30
            )
        ...
        # Check if user already exists
            ...
            or "user already exists" in result.stderr.lower()
        ):
        ...
        print(
            "❌ Docker not found. Please install Docker or run this script in an environment with Docker."
        )
        return False
