...
class ODPSLinter:
    ...
    def _lint_odps_content(self, data: dict[str, Any], file_path: Path, format: str) -> bool:
        ...
        has_odps_schema = (
            "schema" in data
            and isinstance(data["schema"], str)
            and "opendataproducts.org/schema" in data["schema"]
        )
        ...
        if not has_odps_schema:
            ...
            is_expected_odps = (
                "valid" in file_path_str
                or "invalid" in file_path_str
                or "marketplace" in file_path_str
                or "multilingual" in file_path_str
            )
            ...
            if is_expected_odps:
                ...
                self.errors.append(
                    (
                        str(file_path),
                        "File does not appear to be an ODPS document (missing schema URL with opendataproducts.org)",
                    )
                )
                is_valid = False
            else:
                ...
                self.warnings.append(
                    (
                        str(file_path),
                        "File does not appear to be an ODPS document (missing schema URL)",
                    )
                )
        ...
        if "schema" in data:
            ...
            if not isinstance(schema, str):
                ...
                self.warnings.append(
                    (
                        str(file_path),
                        f"Schema URL should start with 'https://opendataproducts.org/schema/': {schema}",
                    )
                )
            ...
            # Check product.details
                ...
                if not details:
                    self.errors.append(
                        (
                            str(file_path),
                            "Field 'product.details' must contain at least one language",
                        )
                    )
                    is_valid = False
                else:
                    ...
                    for lang, lang_data in details.items():
                        if not isinstance(lang_data, dict):
                            self.errors.append(
                                (
                                    str(file_path),
                                    f"Field 'product.details.{lang}' must be a dictionary",
                                )
                            )
                            is_valid = False
                        else:
                            ...
                            for field in required_product_fields:
                                if field not in lang_data:
                                    self.errors.append(
                                        (
                                            str(file_path),
                                            f"Missing required field: product.details.{lang}.{field}",
                                        )
                                    )
                                    is_valid = False
        ...
        try:
            ...
            if detected_version == "unknown":
                self.warnings.append(
                    (str(file_path), "Could not detect ODPS version from document")
                )
    ...
    def lint_directory(self, directory: Path, pattern: str = "**/*.{json,yaml,yml}") -> bool:
        ...
        for file_path in directory.glob(pattern):
            if file_path.is_file() and not self.lint_file(file_path):
                all_valid = False
...
Examples:
  ...
  python scripts/lint_odps.py --strict tests/fixtures/odps/
    ...
    parser.add_argument("files", nargs="*", type=Path, help="ODPS files or directories to lint")
    ...
    if not args.files:
        ...
        if default_dir.exists():
            ...
            linter.lint_directory(default_dir)
        else:
