...
def generate_migration_report():
    ...
    if total_count > 0:
        print(f"[WARNING] Found {total_count} contract(s) with original_spec_type='DATACONTRACT_COM'\n")
        
        # Group by tenant
        by_tenant = dcs_contracts.values('tenant__name', 'tenant__id').annotate(
            count=Count('id')
        ).order_by('-count')
        ...
        # Group by status
        by_status = dcs_contracts.values('status').annotate(
            count=Count('id')
        ).order_by('-count')
        ...
        # Group by version
        by_version = dcs_contracts.values('original_spec_version').annotate(
            count=Count('id')
        ).order_by('-count')
...
if __name__ == '__main__':
    try:
        ...
        print(f"\n[ERROR] Failed to generate migration report: {str(e)}")
        import traceback
        traceback.print_exc()
        sys.exit(1)
