...
def setup_minio_bucket():
    ...
    try:
        ...
        endpoint_url = getattr(settings, 'AWS_S3_ENDPOINT_URL', os.environ.get('AWS_S3_ENDPOINT_URL', 'http://localhost:9000'))
        access_key = getattr(settings, 'AWS_ACCESS_KEY_ID', os.environ.get('AWS_ACCESS_KEY_ID', 'minio'))
        secret_key = getattr(settings, 'AWS_SECRET_ACCESS_KEY', os.environ.get('AWS_SECRET_ACCESS_KEY', 'minio123'))
        bucket_name = getattr(settings, 'AWS_STORAGE_BUCKET_NAME', os.environ.get('AWS_STORAGE_BUCKET_NAME', 'hub-files'))
    except Exception:
