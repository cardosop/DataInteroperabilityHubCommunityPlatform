...
def enhance_openapi_spec(file_path: Path) -> bool:
    ...
    try:
        ...
        if not spec or 'openapi' not in spec:
            print(f"  ⚠️  Skipping: Not a valid OpenAPI spec")
            return False
                    ...
                    for method, operation in path_item.items():
                        if method in ['get', 'post', 'put', 'patch', 'delete'] and isinstance(operation, dict):
                            ...
                            if path_key in SECURITY_REQUIREMENTS:
                                ...
                                enhanced |= enhance_operation(operation, path_key, method, reqs, spec)
        ...
        if enhanced:
            ...
            print(f"  ✅ Enhanced with security requirements")
            return True
        else:
            print(f"  ℹ️  No enhancements needed")
            return False
...
def enhance_operation(operation: Dict[str, Any], path: str, method: str, 
                     reqs: Dict[str, Any], spec: Dict[str, Any]) -> bool:
    ...
    if reqs['auth_required']:
        ...
        auth_section += f"Not required (public endpoint)"
    
    if not auth_section in description:
    ...
    if reqs['roles'] or reqs['scopes']:
        ...
        parts = []
        if reqs['roles']:
            roles_str = ', '.join(reqs['roles']) if isinstance(reqs['roles'], list) else reqs['roles']
            parts.append(f"Required role(s): {roles_str}")
        if reqs['scopes']:
            scopes_str = ', '.join(reqs['scopes']) if isinstance(reqs['scopes'], list) else reqs['scopes']
...
def enhance_security_schemes(security_schemes: Dict[str, Any]) -> bool:
    ...
    if 'BearerAuth' not in security_schemes:
        ...
        if 'description' not in security_schemes['BearerAuth'] or len(security_schemes['BearerAuth'].get('description', '')) < 50:
            security_schemes['BearerAuth']['description'] = 'JWT Bearer token authentication. Include token in Authorization header: "Bearer {token}". Tokens expire after 1 hour and can be refreshed using refresh tokens.'
            enhanced = True
    ...
    if 'ApiKeyAuth' not in security_schemes:
        ...
        if 'description' not in security_schemes['ApiKeyAuth'] or len(security_schemes['ApiKeyAuth'].get('description', '')) < 50:
            security_schemes['ApiKeyAuth']['description'] = 'API key authentication. Include key in Authorization header: "ApiKey {key}" or in X-API-Key header. API keys can have scopes to limit access and can be expired or revoked.'
            enhanced = True
...
def main():
    ...
    print(f"Adding security requirements to OpenAPI specs in: {contracts_dir.relative_to(PROJECT_ROOT)}")
    print("=" * 80)
