...
def merge_categorization():
    ...
    if not categorized_file.exists():
        print("Error: Categorized file not found")
        return 1
    ...
    table_header_pattern = re.compile(
        r"(\|\s*Method\s*\|\s*Path\s*\|\s*View\s*\|\s*Action\s*\|\s*Description\s*\|)", re.MULTILINE
    )
    ...
    def update_table(match):
        ...
        new_header = (
            header.rstrip("|")
            + " | Feature Type | Status | Completeness | Schema | Tests | Deprecated |"
        )
        return new_header
