...
class TestFixturesAndFactoriesChecker:
    ...
    def _extract_endpoints_from_dict(self, data: Any, path: str = '') -> List[str]:
        ...
        if isinstance(data, dict):
            ...
            if '/api/v1/' in data or 'localhost:' in data or 'http://' in data or 'https://' in data:
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                return any(keyword in content for keyword in [
                    '@pytest.fixture', '@fixture', 'def fixture', 'fixture_data',
                    'test_fixture', 'sample_data', 'mock_data'
                ])
        except Exception:
            return False
    ...
    def _extract_factory_classes(self, tree: ast.AST, file_path: Path) -> List[Dict[str, Any]]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                is_factory = (
                    'Factory' in node.name or
                    any('factory' in base.id.lower() if isinstance(base, ast.Name) else False
