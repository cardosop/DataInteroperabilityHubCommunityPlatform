...
def profile_asset_create(num_iterations: int = 100) -> dict[str, Any]:
    ...
    for i in range(num_iterations):
        ...
        with query_tracking() as queries, transaction.atomic():
...
def profile_asset_activate(num_iterations: int = 50) -> dict[str, Any]:
    ...
    for i in range(num_iterations):
        ...
        # Measure total time
        asset.refresh_from_db()
        _, exec_time = measure_time(lambda: asset.can_activate())
        execution_times.append(exec_time)
...
def main():
    ...
    parser.add_argument("--endpoint", choices=["create", "activate"], help="Endpoint to profile")
    ...
    if args.all:
        ...
        results_activate = profile_asset_activate(
            args.iterations // 2
        )  # Fewer iterations for activate
