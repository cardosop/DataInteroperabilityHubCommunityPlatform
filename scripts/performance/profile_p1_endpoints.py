...
def profile_marketplace_listings(num_iterations: int = 100) -> dict[str, Any]:
    ...
    if not user:
        user = User.objects.create_user(
            username="testuser", email="test@example.com", password="testpass123", tenant=tenant
        )
    ...
    for _i in range(num_iterations):
        ...
        _, exec_time = measure_time(
            lambda: list(
                Listing.objects.filter(tenant=tenant).select_related("tenant", "asset")[:20]
            )
        )
...
def profile_search_endpoint(num_iterations: int = 100) -> dict[str, Any]:
    ...
    for i in range(num_iterations):
        ...
        with query_tracking() as queries:
            ...
            _results, _total = SearchEngine.search(
                tenant_id=str(tenant.id), query=query, limit=20, offset=0
            )
        ...
        # Measure total time
        _, exec_time = measure_time(
            lambda: SearchEngine.search(tenant_id=str(tenant.id), query=query, limit=20, offset=0)
        )
        execution_times.append(exec_time)
...
def profile_dq_results(num_iterations: int = 50) -> dict[str, Any]:
    ...
    # Create test DQ runs
    dq_runs = []
    for _i in range(min(num_iterations, 10)):  # Create fewer test runs
    ...
    for dq_run in dq_runs:
        with query_tracking() as queries:
            ...
            dq_run = DQRun.objects.select_related("tenant", "asset", "dataset", "job").get(
                id=dq_run.id
            )
            # Access results
        ...
        # Measure total time
        _, exec_time = measure_time(
            lambda: DQRun.objects.select_related("tenant", "asset", "dataset", "job").get(
                id=dq_run.id
            )
        )
        execution_times.append(exec_time)
...
def main():
    ...
    parser.add_argument(
        "--endpoint", choices=["marketplace", "search", "dq"], help="Endpoint to profile"
    )
    parser.add_argument("--all", action="store_true", help="Profile all endpoints")
    parser.add_argument(
