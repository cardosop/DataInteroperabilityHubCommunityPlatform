...
class Gap:
    ...
    endpoint: str
    method: str
    gap_type: str  # missing, incomplete_method, incomplete_params, incomplete_fields, needs_enhancement
    priority: str
    category: str
    description: str
...
class GapAnalyzer:
    ...
    def parse_requirements_matrix(self, file_path: Path) -> Dict[Tuple[str, str], APIEndpoint]:
        ...
        for line in content.split('\n'):
            ...
            if '|' in line and ('GET' in line or 'POST' in line or 'PUT' in line or 'DELETE' in line or 'PATCH' in line):
            ...
            if line.startswith('##### ') and ('POST' in line or 'GET' in line or 'PUT' in line or 'DELETE' in line or 'PATCH' in line):
    ...
    def analyze_gaps(self):
        ...
        for (endpoint, method), required_api in self.required_apis.items():
            if (endpoint, method) in self.current_apis:
                ...
                missing_query_params = set(required_api.query_params) - set(current_api.query_params)
                ...
                if missing_path_params:
                    gap = Gap(
                        ...
                        details={
                            "missing_params": list(missing_path_params)
                        },
                ...
                if required_api.request_schema and not current_api.request_schema:
                    gap = Gap(
                        ...
                        details={
                            "required_schema": required_api.request_schema
                        },
    ...
    def generate_gap_analysis(self, output_path: Path):
        ...
        lines.append(f"**Task**: 0.3.1 - Compare current vs required APIs")
        lines.append("")
        lines.append("---")
        lines.append("")
        ...
        lines.append(f"- **Missing Endpoints**: {len([g for g in self.gaps if g.gap_type == 'missing'])}")
        lines.append(f"- **Incomplete Endpoints**: {len([g for g in self.enhancements if 'incomplete' in g.gap_type])}")
        lines.append(f"- **Endpoints Needing Enhancements**: {len([g for g in self.enhancements if g.gap_type == 'needs_enhancement'])}")
        lines.append("")
        ...
        if missing:
            ...
            for priority in ['P0', 'P1', 'P2', 'P3']:
                ...
                if priority_missing:
                    lines.append(f"### {priority} - {'Critical' if priority == 'P0' else 'High' if priority == 'P1' else 'Medium' if priority == 'P2' else 'Low'} Priority")
                    lines.append("")
        ...
        if enhancements:
            ...
            lines.append("")
            lines.append("Endpoints that exist but need improvements (performance, features, etc.).")
            lines.append("")
    ...
    def export_json(self, output_path: Path):
        ...
        data = {
            ...
            'summary': {
                ...
                'incomplete_endpoints': len([g for g in self.enhancements if 'incomplete' in g.gap_type]),
                'enhancements_needed': len([g for g in self.enhancements if g.gap_type == 'needs_enhancement'])
            },
...
def main():
    ...
    parser.add_argument('--requirements', default='docs/api-audit/api-requirements-matrix-consolidated.md',
    ...
    parser.add_argument('--output', default='docs/api-audit/gap-analysis.md',
