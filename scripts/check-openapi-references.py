...
class Discrepancy:
    ...
    openapi_methods: list[str] | None = None
    actual_methods: list[str] | None = None
...
class URLPatternComparator:
    ...
    def compare(self) -> list[Discrepancy]:
        ...
        for actual_path, endpoints in actual_path_map.items():
            found = False
            for openapi_path in openapi_path_map:
                if self.paths_match(openapi_path, actual_path):
                    found = True
                    break
            ...
            if not found:
                ...
                if any(
                    skip in actual_path
                    for skip in ["openapi.json", "openapi.yaml", "swagger", "redoc"]
                ):
                    continue
                ...
                discrepancies.append(
                    Discrepancy(
                        ...
                        actual_path=actual_path,
                        actual_methods=list(
                            set(m for e in endpoints for m in e.get("methods", ["GET"]))
                        ),
...
class OpenAPISpecChecker:
    ...
    def __init__(self, spec_file: str | None = None, spec_data: dict[str, Any] | None = None):
        if spec_data:
            ...
            for disc_type, discs in sorted(by_type.items()):
                ...
                for disc in discs[:50]:  # Limit to first 50 per type
                    ...
                    if disc.get("openapi_methods"):
                        lines.append(
                            f"  - **OpenAPI Methods**: {', '.join(disc['openapi_methods'])}"
                        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Check OpenAPI spec references against actual URL patterns"
    )
    ...
    if args.spec_file:
        ...
        try:
            with urllib.request.urlopen(args.spec_url, timeout=10) as response:
                content = response.read().decode("utf-8")
                if "yaml" in response.headers.get("Content-Type", "") or args.spec_url.endswith(
                    (".yaml", ".yml")
                ):
                    import yaml
        ...
        try:
            ...
            print(
                f"Error: Could not load OpenAPI spec. Use --spec-file or --spec-url. {e}",
                file=sys.stderr,
            )
            sys.exit(1)
