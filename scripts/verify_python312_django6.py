...
def check_django_middleware() -> bool:
    ...
    try:
        ...
        if django_version.startswith("6."):
            ...
            middleware_files = [
                f
                for f in (base_dir / "hub").rglob("**/middleware.py")
            ...
            for mw_file in middleware_files:
                try:
                    ...
                    if (
                        "MiddlewareMixin" in content
                        and "from django.utils.deprecation import MiddlewareMixin" in content
                    ):
...
def main():
    ...
    if all_passed:
        print_success(
            "🎉 All checks passed! Your environment is ready for Python 3.12+ and Django 6+"
        )
