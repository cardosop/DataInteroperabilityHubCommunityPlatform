                    ...
                    for attr in dir(EndpointCategory):
                        if not attr.startswith("_"):
                            dict_str = dict_str.replace(
                                f"EndpointCategory.{attr}", f'"{getattr(EndpointCategory, attr)}"'
                            )
...
class RateLimitingConfigReviewer:
    ...
    def __init__(self, hub_dir: Path | None = None):
        ...
        self.category_mappings: dict[str, list[tuple[str, str]]] = defaultdict(
            list
    ...
    def map_rate_limiting_to_endpoints(
        self, endpoints: list[dict[str, Any]]
    ) -> list[EndpointRateLimitMapping]:
    ...
    def verify_all_rate_limiting_configs_reviewed(self) -> dict[str, Any]:
        ...
        total_categories = len(
            set(PLATFORM_DEFAULT_LIMITS.keys()) | set(PLATFORM_MAXIMUM_LIMITS.keys())
        )
        ...
        return {
            "status": "success"
            if categories_reviewed == total_categories and rules_reviewed == expected_rules
            else "warning",
    ...
    def verify_rate_limiting_mapping(self) -> dict[str, Any]:
        ...
        for category in set(PLATFORM_DEFAULT_LIMITS.keys()):
            endpoints_in_category = len(
                [m for m in self.endpoint_mappings if m.category == category]
            )
    ...
    def generate_report(self, output_file: Path, format: str = "json"):
        ...
        report = {
            ...
            "rate_limit_rules": {
                ...
                "all": [
                    ...
                    for rule in sorted(
                        self.rate_limit_rules, key=lambda r: (r.category, r.window_seconds)
                    )
                ],
            },
            "endpoint_category_mappings": {
                category: [{"path": path, "method": method} for path, method in endpoints]
            ...
            "endpoint_rate_limit_mappings": [
                ...
                for m in sorted(
                    self.endpoint_mappings, key=lambda m: (m.category, m.endpoint_path, m.method)
                )
            ],
        }
        ...
        if format == "json":
            ...
            with open(output_file, "w", encoding="utf-8") as f:
                ...
                f.write(
                    f"- **Total Rate Limit Rules:** {report['summary']['total_rate_limit_rules']}\n"
                )
                f.write(
                    f"- **Total Endpoint Mappings:** {report['summary']['total_endpoint_mappings']}\n\n"
                )
                ...
                for category, rules in report["rate_limit_rules"]["by_category"].items():
                    ...
                    for rule in rules:
                        f.write(
                            f"| {rule['window']} ({rule['window_seconds']}s) | {rule['default_limit']} | {rule['maximum_limit']} |\n"
                        )
                    f.write("\n")
...
def main():
    ...
    parser.add_argument(
        "--output", default="docs/api-audit/rate-limiting-review.json", help="Output file path"
    )
    parser.add_argument(
    ...
    print(
        f"   Categories reviewed: {configs_verification['categories_reviewed']}/{configs_verification['total_categories']}"
    )
    print(
        f"   Rules reviewed: {configs_verification['rules_reviewed']}/{configs_verification['expected_rules']}"
    )
