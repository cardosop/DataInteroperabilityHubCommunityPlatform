        ...
        try:
            ...
            for config_file in files_to_check:
                ...
                for route in dq_routes:
                    assert route == "/api/v1/dq", f"DQ route should be /api/v1/dq, found: {route}"
                    assert "/dq-runs" not in route, f"DQ route should not include /dq-runs: {route}"
        ...
        def check_service(url: str, timeout: int = 2) -> bool:
            try:
                ...
                return response.status_code in [
                    ...
                    403,
                    404,
                ]  # 404 means route exists but endpoint doesn't
            except Exception:
                return False
