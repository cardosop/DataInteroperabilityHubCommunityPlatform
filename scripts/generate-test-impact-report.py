...
class TestImpactReportGenerator:
    ...
    def _load_test_endpoint_data(self) -> Dict[str, Any]:
        ...
        with open(self.test_impact_file, 'r') as f:
            data = json.load(f)

        self.test_endpoint_data = data
        if self.verbose:
            print(f"Loaded test endpoint data: {data['summary'].get('total_test_files', 0)} test files")
        return data
    ...
    def _load_fixtures_data(self) -> Dict[str, Any]:
        ...
        if self.verbose:
            print(f"Loaded fixtures data: {data['summary'].get('total_fixture_files', 0)} fixture files")
        return data
    ...
    def _load_utilities_data(self) -> Dict[str, Any]:
        ...
        if not self.utilities_file.exists():
            ...
            return {'summary': {}, 'utility_modules': [], 'helper_functions': [], 'endpoint_urls': []}

        with open(self.utilities_file, 'r') as f:
            data = json.load(f)

        self.utilities_data = data
        if self.verbose:
            print(f"Loaded utilities data: {data['summary'].get('total_utility_modules', 0)} utility modules")
        return data
        ...
        for endpoint, refs in references['endpoint_references'].items():
            for ref in refs:
                ...
                if ref['type'] == 'test_file':
                    ...
                    matrix['by_endpoint'][endpoint]['test_types'].add(ref.get('test_type', 'unknown'))
    ...
    def _generate_summary(
        ...
        return {
            ...
            'total_endpoint_references': sum(len(refs) for refs in references['endpoint_references'].values()),
    ...
    def _generate_markdown_report(
        ...
        lines = [
            ...
            '## Executive Summary',
            '',
            f'This report provides a comprehensive analysis of test impact across the codebase, ',
            f'compiling data from test files, fixtures, factories, and utilities.',
            '',
            '### Overall Statistics',
            '',
        ...
        for fixture in references['fixture_files'][:50]:  # Limit to first 50
            ...
            if len(fixture.get('endpoints', [])) > 3:
                endpoints_str += '...'
            lines.append(
                f'| {fixture["file"]} | {fixture["type"]} | {endpoints_str or "None"} |'
            )
        ...
        for util in references['utility_files']:
            ...
            if len(util.get('functions', [])) > 5:
                functions_str += '...'
            lines.append(
                f'| {util["file"]} | {util["type"]} | {functions_str or "None"} |'
            )
        ...
        lines.extend([
            ...
            '',
            f'1. **Test Endpoint Audit** (`test-impact-analysis.json`)',
            ...
            '',
            f'2. **Fixtures and Factories Audit** (`test-fixtures-and-factories-report.json`)',
            ...
            '',
            f'3. **Utilities Audit** (`test-utilities-report.json`)',
