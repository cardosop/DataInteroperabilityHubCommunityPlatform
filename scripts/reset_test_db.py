...
def reset_test_database():
    ...
    try:
        ...
        print("Run tests with: pytest hub/apps/orchestration/workflows/tests/test_contract_creation.py -v")
        
    except psycopg2.OperationalError as e:
        print(f"Could not connect to PostgreSQL: {e}")
        print("Make sure PostgreSQL is running and credentials are correct.")
        print("You can also manually drop the database:")
        print(f'  psql -U {conn_params["user"]} -d postgres -c \'DROP DATABASE IF EXISTS "{test_db_name}";\'')
        sys.exit(1)
