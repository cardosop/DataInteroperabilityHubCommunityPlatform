#!/bin/bash
# Comprehensive test runner for migration validation tests (Task 10.1.21)
...
for task_id in "${!TEST_FILES[@]}"; do
    ...
    # Run test with TESTING=1 to optimize initialization
    if $DOCKER_COMPOSE exec -w /app -e TESTING=1 api-service python -m pytest "$test_file" -v --tb=short --maxfail=5 2>&1 | tee /tmp/test_output_${task_id}.log; then
        PASSED_TESTS+=("$task_id:$test_file")
        echo "✓ PASSED: $task_id"
    else
        EXIT_CODE=$?
        if grep -q "SKIPPED\|skip" /tmp/test_output_${task_id}.log; then
...
if [ ${#FAILED_TESTS[@]} -gt 0 ]; then
    ...
    for test in "${FAILED_TESTS[@]}"; do
        ...
        echo "    See: /tmp/test_output_${task_id}.log"
    done
    echo ""
    exit 1
fi
