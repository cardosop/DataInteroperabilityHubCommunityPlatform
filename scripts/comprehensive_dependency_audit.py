...
class DependencyInfo:
    ...
    pypi_supports: Optional[bool] = None
    pypi_requires_python: Optional[str] = None
...
def check_pypi_python_support(package_name: str) -> Tuple[Optional[bool], Optional[str]]:
...
def test_dependency_installation(package_name: str, version_spec: str, python_exec: str) -> Tuple[bool, Optional[str], bool]:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = subprocess.run(
                [python_exec, '-m', 'pip', 'install', '--dry-run', '--break-system-packages', '--target', tmpdir, package_spec],
                capture_output=True,
                text=True,
                timeout=30
            )
...
def audit_dependency(
    ...
    # Test installation
    can_install, install_info, install_success = test_dependency_installation(name, version_spec, python_exec)
    ...
    tested_version = install_info if install_info and isinstance(install_info, str) and install_info[0].isdigit() else None
...
def audit_requirements_file(requirements_file: Path, python_exec: str, critical_deps: Set[str] = None) -> List[DependencyInfo]:
...
def main():
    ...
    critical_results = {name: all_dependencies.get(name) for name in critical_dependencies if name in all_dependencies}
    ...
    with open(report_file, 'w') as f:
        ...
        for name, result in sorted(critical_results.items()):
            if result:
                status_icon = "✅" if result.status == CompatibilityStatus.COMPATIBLE else "⚠️" if result.status == CompatibilityStatus.WARNING else "❌"
                f.write(f"### {name}\n")
    ...
    if incompatible:
        ...
        print_info("Review incompatible dependencies above and update versions or find alternatives")
        sys.exit(1)
    elif requires_update:
        print_warning(f"All dependencies compatible, but {len(requires_update)} have updates available")
