...
class IntegrationRequirementsDocumenter:
    ...
    def __init__(self):
        ...
        self.infrastructure = {
            ...
            "redis": {"description": "Job queue, caching, and event bus", "required": True},
            "minio": {
                "description": "S3-compatible object storage for files and datasets",
                "required": True,
            },
            "jena-fuseki": {"description": "RDF triple store for semantic data", "required": False},
        }
        ...
        self.endpoint_integrations = {
            ...
            "POST /auth/register/": {
                ...
                "external_services": [
                    ServiceDependency(
                        "email-service",
                        "external",
                        description="Send welcome email (optional)",
                        required=False,
                    )
                ],
                "events": [
            ...
            "POST /scheduled-ingestions/{id}/credentials/test/": {
                "database": [
                    DatabaseRequirement(
                        "ScheduledIngestion", ["read"], "Get scheduled ingestion and credentials"
                    ),
                ],
                "external_services": [
                    ServiceDependency(
            ...
            "POST /ai/natural-language-search/": {
                ...
                "external_services": [
                    ServiceDependency(
                        "llm-service",
                        "external",
                        description="Natural language query understanding",
                        timeout_ms=30000,
                    )
                ],
                "internal_services": [
            ...
            "POST /ai/schema-matching/": {
                ...
                "external_services": [
                    ServiceDependency(
                        "ai-service",
                        "external",
                        description="AI-powered schema matching",
                        timeout_ms=15000,
                    )
                ],
                "events": [
                    EventRequirement(
                        ...
                        {"matching_id": "uuid", "confidence": "float"},
                        "Schema matching completion event",
                    )
                ],
                "infrastructure": ["postgresql", "redis"],
            },
            ...
            "POST /social/ratings/": {
                ...
                "events": [
                    EventRequirement(
                        ...
                        {"rating_id": "uuid", "asset_id": "uuid"},
                        "Rating creation event",
                    )
                ],
                "infrastructure": ["postgresql", "redis"],
            },
            ...
            "GET /marketplace/listings/{id}/preview/": {
                ...
                "internal_services": [
                    ServiceDependency(
                        ...
                        "internal",
                        "/run",
                        "Get data quality metrics for preview",
                        required=False,
                    )
                ],
    ...
    def get_integration_requirements(
        ...
        if key in self.endpoint_integrations:
            ...
            for pattern_key, pattern_config in self.endpoint_integrations.items():
                ...
                if pattern_method == method.upper():
                    ...
                    if endpoint_path == pattern_path_clean or endpoint_path.endswith(
                        pattern_path_clean
                    ):
        ...
        try:
            ...
            for path, path_item in spec["paths"].items():
                ...
                for method in ["get", "post", "put", "patch", "delete"]:
                    ...
                    if (
                        "x-integration-requirements" not in operation
                        or operation["x-integration-requirements"] != serialized
                    ):
    ...
    def _format_integration_requirements(self, requirements: IntegrationRequirements) -> str:
        ...
        if requirements.database_requirements:
            ...
            for db_req in requirements.database_requirements:
                ...
                sections.append(
                    f"- {db_req.model_name} model: {ops} operations - {db_req.description}"
                )
        ...
        if requirements.event_requirements:
            ...
            for event in requirements.event_requirements:
                ...
                sections.append(
                    f"- Publishes `{event.event_type}` event: {event.description} ({required})"
                )
