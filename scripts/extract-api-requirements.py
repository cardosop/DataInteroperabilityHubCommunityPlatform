...
class APIRequirementsExtractor:
    ...
    def _extract_rest_endpoints(self, content: str, spec_name: str) -> List[APIEndpoint]:
        ...
        for pattern, api_type in rest_patterns:
    ...
    def _extract_credential_endpoints(self, content: str, spec_name: str) -> List[APIEndpoint]:
        ...
        credential_endpoints = [
            ...
            ('GET', '/api/v1/scheduled-ingestions/{id}/credentials/validation/', 'Validate credentials'),
        ]
    ...
    def _extract_implicit_endpoints(self, content: str, spec_name: str) -> List[APIEndpoint]:
        ...
        spec_resource_map = {
            'frontend-api-integration': ['credentials', 'api'],
            'frontend-authentication': ['auth', 'login', 'register', 'api-keys', 'sessions', 'sso', 'mfa'],
            'frontend-application': ['assets', 'contracts', 'datasets'],
            'ai-ml-intelligence': ['ai', 'ml', 'search', 'schema-matching', 'recommendations', 'classification'],
            'data-transformation': ['transformations', 'pipelines', 'preview', 'wrangling'],
            'collaboration-social': ['ratings', 'reviews', 'comments', 'communities', 'activity'],
            'advanced-marketplace': ['marketplace', 'preview', 'pricing', 'purchase'],
            'data-mesh': ['domains', 'mesh', 'governance'],
            'scheduled-ingestion': ['scheduled-ingestions', 'credentials'],
            'advanced-governance': ['compliance', 'policies', 'retention', 'gdpr', 'consent'],
            'advanced-observability': ['observability', 'reliability', 'cost', 'performance'],
            'data-virtualization': ['virtual-datasets', 'federated-queries'],
            'integration-ecosystem': ['connectors', 'bi-integration', 'reverse-etl', 'cicd'],
            'developer-experience': ['plugins', 'sdk', 'cli', 'developer-portal'],
        }
    ...
    def generate_markdown_report(self, output_path: Path):
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            f.write("## Overview\n\n")
            f.write("This document contains all API requirements extracted from frontend specification files.\n\n")
            f.write("## Categories\n\n")
