        ...
        if 'dq-service' in content:
            ...
            if 'PathPrefix(`/api/v1/dq`)' not in content:
                ...
                if 'dq-service' in content and 'PathPrefix' in content:
                    ...
                    dq_match = re.search(
                        r'dq-service:.*?rule:\s*"PathPrefix\(`([^`]+)`\)"',
                        content,
                        re.DOTALL
                    )
                    if dq_match:
                    ...
                    if 'http' in rule and 'paths' in rule['http']:
                        for path in rule['http']['paths']:
                            ...
                            if '/dq-runs' in path_value:
                                issues.append({
                                    'type': 'old_dq_pattern_in_ingress',
                                    'path': path_value
                                })
...
def main():
        ...
        if result['exists']:
            if result['issue_count'] > 0:
                ...
                for issue in result['issues'][:3]:
                    print(f"   - {issue.get('type', 'unknown')}: {issue.get('pattern', issue.get('message', 'unknown'))}")
            else:
    ...
    for ingress_file in sorted(ingress_files):
        ...
        if result['exists']:
            if result['issue_count'] > 0:
                print(f"⚠️  {ingress_file.relative_to(project_root)}: {result['issue_count']} issues found")
                ingress_issues_count += result['issue_count']
                all_issues.extend(result['issues'])
                for issue in result['issues'][:3]:
                    print(f"   - {issue.get('type', 'unknown')}: {issue.get('path', issue.get('pattern', 'unknown'))}")
            else:
    ...
    if total_issues == 0:
        ...
        for issue in all_issues[:10]:  # Show first 10
            print(f"  - {issue.get('type', 'unknown')}: {issue.get('pattern', issue.get('path', issue.get('message', 'unknown')))}")
        if total_issues > 10:
