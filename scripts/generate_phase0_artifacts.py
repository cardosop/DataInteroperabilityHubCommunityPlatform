    ...
    with open(ref_path) as f:
        for line in f:
            ...
            # Check for endpoint definitions (GET/POST/PUT/DELETE)
                ...
                m in line for m in ["GET", "POST", "PUT", "DELETE", "PATCH"]
            ):
                endpoint = line.strip()
                if current_domain:
        ...
        for ep in domain_eps:
            ...
            for domain, refs in api_ref.items():
                if any(path.split("/")[-1] in ref for ref in refs):
                    docs_ref = (
                        f"`docs/API_ENDPOINTS_REFERENCE.md#{domain.lower().replace(' ', '-')}`"
                    )
                    break
...
def map_path_to_route(api_path: str) -> str:
    ...
    if path.startswith("assets"):
        return "/assets" + ("/:id" if "{id}" in path else "")
    elif path.startswith("contracts"):
        return "/contracts" + ("/:id" if "{id}" in path else "")
    elif path.startswith("datasets"):
        return "/datasets" + ("/:id" if "{id}" in path else "")
    elif path.startswith("marketplace"):
        return "/marketplace" + ("/:id" if "{id}" in path else "")
        ...
        return "/dq" + ("/:id" if "{id}" in path else "")
    elif path.startswith("compliance"):
        return "/compliance" + ("/:id" if "{id}" in path else "")
    elif path.startswith("mesh"):
        return "/mesh" + ("/:id" if "{id}" in path else "")
    elif path.startswith("virtualization"):
        return "/virtualization" + ("/:id" if "{id}" in path else "")
    elif path.startswith("jobs"):
        return "/jobs" + ("/:id" if "{id}" in path else "")
    elif path.startswith("files"):
        return "/files" + ("/:id" if "{id}" in path else "")
...
def generate_journey_coverage_matrix(openapi: dict[str, Any]) -> str:
    ...
    for journey in journeys[:20]:  # Process first 20 as example
        ...
        if "transformation" in title.lower() or any(
            "transformation" in step.lower() for step in journey.get("steps", [])
        ):
            status = "Red"
        ...
        if any(
            "ai" in step.lower() or "classification" in step.lower() or "anomaly" in step.lower()
    ...
    lines.append(
        "- Status assignments based on OpenAPI verification and transformation removal documentation"
    )
...
def generate_social_endpoints_doc(openapi: dict[str, Any]) -> str:
    ...
    if not any(
        "feed" in ep["path"].lower() or "activity" in ep["path"].lower() for ep in social_endpoints
    ):
