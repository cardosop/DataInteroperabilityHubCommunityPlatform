...
class WebhookPayloadsAndEventsSearcher:
    ...
    def search_webhook_endpoint_references(self):
        ...
        webhook_patterns = [
            r'webhook.*url|url.*webhook',
            r'webhook.*endpoint|endpoint.*webhook',
            r'POST.*webhook|webhook.*POST',
            r'/webhooks/',
            r'webhook.*delivery|delivery.*webhook',
        ]
                ...
                for i, line in enumerate(lines, 1):
                    ...
                    if re.search(r'/api/v1/webhooks', line, re.IGNORECASE):
                        ...
                        if endpoint_match:
                            ...
                            self.endpoint_references.append(WebhookEndpointReference(
                    ...
                    if re.search(r'WebhookDeliveryService|trigger_webhook|deliver_webhook', line):
                        ...
                        for j in range(max(0, i-5), min(len(lines), i+10)):
                            endpoint_match = re.search(r'["\']([^"\']*webhook[^"\']*)["\']', lines[j])
                            if endpoint_match:
                                endpoint = endpoint_match.group(1)
        ...
        for py_file in self.root_dir.rglob('*.py'):
            ...
            try:
                ...
                for i, line in enumerate(lines, 1):
                    ...
                    if re.search(r'payload\s*=|payload\s*:|"payload"|payload\s*\{', line, re.IGNORECASE):
                        ...
                        if event_type:
                            ...
                            if payload_fields:
                                self.payload_structures.append(WebhookPayloadStructure(
                                    file_path=str(py_file.relative_to(self.root_dir)),
                                    line_number=i,
                                    event_type=event_type,
        ...
        for py_file in self.root_dir.rglob('*.py'):
            ...
            try:
                ...
                for i, line in enumerate(lines, 1):
                    ...
                    if re.search(r'event_types\s*=|event_types\s*:|"event_types"', line, re.IGNORECASE):
                        event_types = self._extract_event_types_from_line(line, lines, i)
                        if event_types:
                            url_pattern = self._extract_url_pattern_from_context(lines, i)

                            self.subscription_configs.append(WebhookSubscriptionConfig(
                                file_path=str(py_file.relative_to(self.root_dir)),
                                line_number=i,
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                ...
                if in_event_type_enum:
                    ...
                    event_match = re.search(r'(\w+)\s*=\s*["\']([^"\']+)["\']\s*,\s*["\']([^"\']+)["\']', line)
                    if event_match:
            ...
            for i, line in enumerate(lines, 1):
                if 'get_odps_event_types' in line or 'get_transformation_event_types' in line or 'get_mesh_event_types' in line or 'get_virtualization_event_types' in line:
    ...
    def _extract_event_type_from_context(self, lines: List[str], line_num: int) -> Optional[str]:
        ...
        for i in range(max(0, line_num - 5), min(len(lines), line_num + 10)):
            ...
            event_match = re.search(r'(odps\.|contract\.|asset\.|pipeline\.|mesh\.|virtualization\.)[\w.]+', line)
            if event_match:
                return event_match.group(0)

        return None

    def _extract_event_type_from_function_name(self, lines: List[str], line_num: int) -> Optional[str]:
    ...
    def _extract_payload_fields(self, lines: List[str], line_num: int) -> List[str]:
        ...
        for i in range(max(0, line_num - 10), min(len(lines), line_num + 30)):
            ...
            if field_match:
                ...
                if field_name not in fields and field_name in ['event_type', 'resource_type', 'resource_id', 'timestamp', 'data']:
    ...
    def _extract_payload_example(self, lines: List[str], line_num: int) -> Optional[Dict[str, Any]]:
    ...
    def _extract_event_types_from_line(self, line: str, lines: List[str], line_num: int) -> List[str]:
    ...
    def _extract_url_pattern_from_context(self, lines: List[str], line_num: int) -> Optional[str]:
    ...
    def _is_valid_event_type(self, event_type: str) -> bool:
        ...
        valid_prefixes = (
            'contract', 'asset', 'odps', 'pipeline', 'mesh', 'virtualization',
            'ingestion', 'quality', 'compliance', 'version'
        )
        ...
        for pattern in invalid_patterns:
            if pattern.lower() in event_lower:
                ...
                if not any(event_lower.startswith(prefix) and '.' in event_lower for prefix in valid_prefixes):
                    return False
