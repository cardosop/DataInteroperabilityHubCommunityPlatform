...
def test_jsonfield_operations():
    ...
    contracts = Contract.objects.filter(hub_contract_json__version="2.0.0")
    ...
    nested_data = {"version": "1.0.0", "metadata": {"author": "Test", "tags": ["test", "json"]}}
...
def test_jsonfield_queries():
    ...
    active_contracts = Contract.objects.filter(hub_contract_json__status="active")
    assert active_contracts.count() == 2
    print_success("JSON path query works")

    # Test JSON contains query
    print("Test: JSON contains query...")
    version_contracts = Contract.objects.filter(hub_contract_json__version__startswith="1.")
