...
def test_cutoffIsoFor_subtracts_minutes_from_now() -> None:
    out = _node_eval("m.cutoffIsoFor(new Date('2026-04-25T12:00:00Z'), 60)")
    assert json.loads(out) == "2026-04-25T11:00:00.000Z"


def test_cutoffIsoFor_zero_minutes_returns_now() -> None:
    out = _node_eval("m.cutoffIsoFor(new Date('2026-04-25T12:00:00Z'), 0)")
...
def test_extractCandidates_handles_paginated_payload() -> None:
    ...
    out = _node_eval(f"m.extractCandidates({json.dumps(payload)}, 'e2e-', '2030-01-01T00:00:00Z')")
...
def test_validateArgs_requires_token_unless_dry_run() -> None:
    out = _node_eval("m.validateArgs(m.parseArgs(['node', 'p', '--base=https://x/api/v1']))")
...
def _purge_family(
    ...
    code = (
        PURGE_FAMILY_HARNESS.replace("SCRIPT_PATH", str(SCRIPT))
    ...
    res = subprocess.run(
        ["node", "-e", code], check=False, capture_output=True, text=True, timeout=15
    )
...
def test_purgeFamily_dry_run_lists_candidates_without_mutating() -> None:
    ...
    assert out["fetchCalls"][0]["method"] == "GET"
