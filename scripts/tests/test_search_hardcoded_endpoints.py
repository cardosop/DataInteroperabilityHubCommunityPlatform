...
url = "https://api.example.com/api/v1/auth/login/"
    ...
    def test_categorization_accuracy(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert all(ref.priority == "High" for ref in test_refs), (
                "Test files should be High priority"
            )
            assert all(ref.priority == "High" for ref in cli_refs), (
                "CLI files should be High priority"
            )
...
# Comment with /api/v1/comment/
    ...
    @pytest.mark.skipif(not os.path.exists("tests/"), reason="Tests directory not available")
    ...
    @pytest.mark.skipif(not os.path.exists("cli/"), reason="CLI directory not available")
    ...
    @pytest.mark.skipif(not os.path.exists("docs/"), reason="Docs directory not available")
    ...
    def test_verify_categorization_completeness(self):
        ...
        for ref in references[:100]:  # Check first 100
            ...
            assert ref["category"] in valid_categories, f"Invalid category: {ref['category']}"
