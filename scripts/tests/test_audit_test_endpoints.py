...
class TestTestEndpointAuditor:
    ...
    def test_load_endpoints_from_inventory(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            auditor = TestEndpointAuditor(
                inventory_file=str(inventory_file), project_root=Path(tmpdir)
            )
    ...
    def test_search_test_files(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            auditor = TestEndpointAuditor(
                inventory_file=str(Path(tmpdir) / "inventory.json"), project_root=Path(tmpdir)
            )
            ...
            assert len(integration_tests) >= 1, (
                f"Expected at least 1 integration test, got {len(integration_tests)}"
            )
            ...
            assert len(regression_tests) >= 1, (
                f"Expected at least 1 regression test, got {len(regression_tests)}"
            )
    ...
    def test_extract_endpoint_references(self):
        ...
        auditor = TestEndpointAuditor(inventory_file="dummy.json", project_root=Path("/tmp"))
