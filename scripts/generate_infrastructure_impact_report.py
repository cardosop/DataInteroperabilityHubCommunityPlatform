...
class InfrastructureImpactReporter:
    ...
    def load_review_reports(self) -> dict[str, Any]:
        ...
        reports = {"cicd": {}, "monitoring": {}, "rate_limiting": {}, "gateway": {}}

        # Load CI/CD review
        if self.cicd_review.exists():
            try:
                with open(self.cicd_review) as f:
                    reports["cicd"] = json.load(f)
                print(
                    f"  Loaded CI/CD review: {len(reports['cicd'].get('workflows', []))} workflows"
                )
            except Exception as e:
        ...
        if self.monitoring_review.exists():
            try:
                with open(self.monitoring_review) as f:
                    reports["monitoring"] = json.load(f)
                print(
                    f"  Loaded monitoring review: {reports['monitoring'].get('summary', {}).get('endpoints_found', 0)} endpoints"
                )
            except Exception as e:
        ...
        if self.rate_limiting_review.exists():
            try:
                with open(self.rate_limiting_review) as f:
                    reports["rate_limiting"] = json.load(f)
                print(
                    f"  Loaded rate limiting review: {reports['rate_limiting'].get('summary', {}).get('total_categories', 0)} categories"
                )
            except Exception as e:
        ...
        if self.gateway_review.exists():
            try:
                with open(self.gateway_review) as f:
                    reports["gateway"] = json.load(f)
                print(
                    f"  Loaded gateway review: {reports['gateway'].get('summary', {}).get('routing_rules', 0)} routing rules"
                )
            except Exception as e:
    ...
    def compile_infrastructure_components_list(self) -> dict[str, Any]:
        ...
        components_list["configuration_references"] = {
            "cicd_workflows": len(
                self.infrastructure_components.get("cicd", {}).get("workflows", [])
            ),
            "monitoring_endpoints": self.infrastructure_components.get("monitoring", {})
            .get("summary", {})
            .get("endpoints_found", 0),
            "rate_limiting_categories": self.infrastructure_components.get("rate_limiting", {})
            .get("summary", {})
            .get("total_categories", 0),
            "gateway_routing_rules": self.infrastructure_components.get("gateway", {})
            .get("summary", {})
            .get("routing_rules", 0),
        }

        print("  Compiled components list:")
        ...
        print(
            f"    - Kubernetes resources: {sum(components_list['kubernetes_resources'].values())}"
        )
    ...
    def generate_report(self) -> dict[str, Any]:
        ...
        report = {
            ...
            "summary": {
                "docker_compose_services": self.infrastructure_components.get(
                    "docker_compose", {}
                ).get("total_count", 0),
                "kubernetes_components": self.infrastructure_components.get("kubernetes", {}).get(
                    "total_count", 0
                ),
                "infrastructure_services": len(
                    self.infrastructure_components.get("docker_compose", {}).get(
                        "infrastructure", []
                    )
                ),
                "application_services": len(
                    self.infrastructure_components.get("docker_compose", {}).get("application", [])
                ),
                "monitoring_services": len(
                    self.infrastructure_components.get("docker_compose", {}).get("monitoring", [])
                ),
                "gateway_services": len(
                    self.infrastructure_components.get("docker_compose", {}).get("gateway", [])
                ),
                "orchestration_services": len(
                    self.infrastructure_components.get("docker_compose", {}).get(
                        "orchestration", []
                    )
                ),
            ...
            "monitoring": {
                ...
                "prometheus_scrape_configs": len(
                    self.infrastructure_components.get("monitoring", {})
                    .get("prometheus", {})
                    .get("scrape_configs", [])
                ),
            },
            "gateway": {
                ...
                "routing_rules_count": self.infrastructure_components.get("gateway", {})
                .get("summary", {})
                .get("routing_rules", 0),
                "rate_limiting_rules_count": self.infrastructure_components.get("gateway", {})
                .get("summary", {})
                .get("rate_limiting_rules", 0),
            },
            "rate_limiting": {
                "summary": self.infrastructure_components.get("rate_limiting", {}).get(
                    "summary", {}
                ),
                "categories_count": self.infrastructure_components.get("rate_limiting", {})
                .get("summary", {})
                .get("total_categories", 0),
            },
            "cicd": {
                "summary": self.infrastructure_components.get("cicd", {}).get("summary", {}),
                "workflows_count": self.infrastructure_components.get("cicd", {})
                .get("summary", {})
                .get("total_workflows", 0),
            },
...
"""
        ...
        markdown_content += (
            f"- **Persistent Volume Claims**: {k8s_resources['persistent_volume_claims']}\n"
        )
        ...
        markdown_content += (
            f"- **Endpoints Monitored**: {monitoring_summary.get('endpoints_found', 0)}\n"
        )
        markdown_content += f"- **Prometheus Scrape Configs**: {report.get('monitoring', {}).get('prometheus_scrape_configs', 0)}\n"
        markdown_content += (
            f"- **Grafana Queries**: {monitoring_summary.get('grafana_queries_reviewed', 0)}\n"
        )
        ...
        markdown_content += (
            f"- **Rate Limiting Rules**: {gateway_summary.get('rate_limiting_rules', 0)}\n"
        )
        markdown_content += (
            f"- **Authentication Rules**: {gateway_summary.get('authentication_rules', 0)}\n"
        )
        markdown_content += (
            f"- **Mapped Endpoints**: {gateway_summary.get('mapped_endpoints', 0)}\n\n"
        )

        markdown_content += "## Rate Limiting\n\n"
        rate_limiting_summary = report.get("rate_limiting", {}).get("summary", {})
        markdown_content += (
            f"- **Categories**: {rate_limiting_summary.get('total_categories', 0)}\n"
        )
        markdown_content += (
            f"- **Rate Limit Rules**: {rate_limiting_summary.get('total_rate_limit_rules', 0)}\n"
        )
        ...
        markdown_content += (
            f"- **Endpoint References**: {cicd_summary.get('total_endpoint_references', 0)}\n"
        )
        markdown_content += (
            f"- **Workflow Types**: {len(cicd_summary.get('workflows_by_type', {}))}\n\n"
        )
