...
def check_github_workflows() -> tuple[bool, list[str]]:
    ...
    for workflow_path in workflows:
        ...
        if re.search(r'python-version:\s*[\'"]3\.1[2-9]', content) or re.search(
            r"python-version:\s*\${{.*matrix\.python-version", content
        ):
...
def check_pyproject_toml() -> tuple[bool, list[str]]:
    ...
    for file_path in files:
        ...
        if (
            re.search(r'requires-python = ">=3\.12"', content)
            or re.search(r'target-version = [\'"]py312', content)
            or re.search(r'python_version = "3\.12"', content)
        ):
            print_success(f"{file_path}: Configured for Python 3.12+")
        elif (
            re.search(r'requires-python = ">=3\.(9|10|11)"', content)
            or re.search(r'target-version = [\'"]py(39|310|311)', content)
            or re.search(r'python_version = "3\.(9|10|11)"', content)
        ):
    ...
    for req_file in requirements_files:
        ...
        try:
            ...
            # Check if it's just an externally-managed-environment error
                ...
                print_info(
                    "  This is expected on some systems. Use a virtual environment for actual installation."
                )
                print_success(
                    f"{req_file}: Dependencies appear compatible (requires venv for actual install)"
                )
            else:
