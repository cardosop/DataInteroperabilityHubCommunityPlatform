...
def format_effort_estimate(estimate: Dict[str, Any]) -> str:
    ...
    lines.append(f"**Total Effort**: {estimate['total_hours']:.1f} hours ({estimate['total_days']:.1f} days)")
    lines.append("")
    lines.append("**Effort Breakdown**:")
    lines.append(f"- **Development**: {estimate['development_hours']:.1f} hours ({estimate['development_hours']/8:.1f} days)")
    ...
    if estimate['breakdown']['service_integration'] > 0:
        lines.append(f"  - Service integration: {estimate['breakdown']['service_integration']:.1f}h")
    if estimate['breakdown']['workflow_integration'] > 0:
        lines.append(f"  - Workflow integration: {estimate['breakdown']['workflow_integration']:.1f}h")
    lines.append(f"- **Testing**: {estimate['testing_hours']:.1f} hours ({estimate['testing_hours']/8:.1f} days)")
    ...
    lines.append(f"- **Documentation**: {estimate['documentation_hours']:.1f} hours ({estimate['documentation_hours']/8:.1f} days)")
    lines.append(f"  - API documentation: 0.5h (OpenAPI spec exists)")
...
def find_endpoint_in_backlog(backlog_content: str, endpoint: str) -> List[Tuple[int, str]]:
    ...
    endpoint_pattern = re.escape(endpoint.replace('{id}', '{id}'))
...
def update_backlog_with_estimates():
    ...
    print(f"✅ Updated backlog with detailed effort estimates")
