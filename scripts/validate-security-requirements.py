...
def validate_security_requirements(file_path: Path) -> tuple[bool, List[str]]:
    ...
    try:
        ...
        if not spec or 'openapi' not in spec:
            errors.append(f"Not a valid OpenAPI spec")
            return False, errors
                ...
                if isinstance(path_item, dict):
                    for method, operation in path_item.items():
                        if method in ['get', 'post', 'put', 'patch', 'delete'] and isinstance(operation, dict):
                            ...
                            if 'Authentication' not in description and 'authentication' not in description.lower():
                                warnings.append(f"{method.upper()} {path}: Missing authentication documentation in description")
                            
                            # Check for rate limiting documentation
                            if 'Rate Limiting' not in description and 'rate limit' not in description.lower():
                                warnings.append(f"{method.upper()} {path}: Missing rate limiting documentation in description")
                            
                            # Check for input validation documentation
                            if 'Input Validation' not in description and 'input validation' not in description.lower():
                                warnings.append(f"{method.upper()} {path}: Missing input validation documentation in description")
                            ...
                            if 'security' not in operation:
                                ...
                                if 'Required' in description and 'Authentication' in description:
                                    warnings.append(f"{method.upper()} {path}: Missing security array but authentication is required")
                            ...
                            if 'responses' in operation and '429' in operation['responses']:
                                ...
                                if 'headers' not in response_429:
                                    warnings.append(f"{method.upper()} {path}: 429 response missing rate limit headers")
                                else:
                                    ...
                                    for header in required_headers:
                                        if header not in headers:
                                            warnings.append(f"{method.upper()} {path}: 429 response missing {header} header")
...
def main():
    ...
    print(f"Validating security requirements in OpenAPI specs")
    print("=" * 80)
    ...
    for spec_file in sorted(spec_files):
        ...
        is_valid, issues = validate_security_requirements(spec_file)
        ...
        if not errors and not warnings:
            print(f"  ✅ All security requirements validated")
            validated_count += 1
    
    print("\n" + "=" * 80)
    print(f"Validation Summary:")
    print(f"  ✅ Validated: {validated_count}/{len([f for f in spec_files if f.name != 'README.md'])} files")
