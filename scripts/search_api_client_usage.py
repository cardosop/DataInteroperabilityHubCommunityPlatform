...
class APIClientUsageSearcher:
        ...
        for py_file in self.root_dir.rglob("*.py"):
            ...
            try:
                ...
                for i, line in enumerate(lines, 1):
                    ...
                    client_method_match = re.search(
                        r"(\w+)\.(get|post|put|patch|delete|request)\s*\(", line
                    )
                    if client_method_match:
                        ...
                        if (
                            client_var in ["client", "api_client", "http_client", "sdk_client"]
                            or client_var.endswith("Client")
                            or client_var.endswith("API")
                        ):
                            endpoint = self.extract_endpoint_from_line(line, lines[i : i + 5])
                            if endpoint:
                                self.api_calls.append(
                    ...
                    async_match = re.search(
                        r"await\s+(\w+)\.(get|post|put|patch|delete|request)\s*\(", line
                    )
                    if async_match:
        ...
        for api_file in sdk_python_dir.glob("*.py"):
            ...
            try:
                ...
                for i, line in enumerate(lines, 1):
                                ...
                                self.client_usages.append(
                                    ClientUsage(
                                        ...
                                        context=self._get_context(
                                            lines, method_start_line, context_lines=10
                                        ),
                                    )
                                )
                    ...
                    if current_method and re.search(
                        r"await\s+self\.client\.(get|post|put|patch|delete)\s*\(", line
                    ):
                        # Extract endpoint from this line or next few lines
                        endpoint = self._extract_endpoint_from_client_call(
    ...
    def _extract_endpoint_from_method(self, method_body: str) -> str | None:
    ...
    def _extract_endpoint_from_client_call(self, line: str, next_lines: list[str]) -> str | None:
