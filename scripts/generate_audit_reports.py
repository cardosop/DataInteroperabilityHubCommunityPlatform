...
def generate_markdown_inventory(data):
    ...
    lines.append(f'- Naming Inconsistencies: {len(data["issues"].get("naming_inconsistencies", []))}\n')
...
def generate_proposed_fixes_markdown(data):
    ...
    lines.append(f'- Naming Issues to Fix: {len(data["issues"].get("naming_inconsistencies", []))}\n')
