...
class ODPSSecurityScanner:
    ...
    def scan_file(self, file_path: Path) -> bool:
        ...
        if not isinstance(data, dict):
            self.vulnerabilities.append(
                (str(file_path), "ERROR", "ODPS document must be a dictionary")
            )
            return False
        ...
        if (
            ...
            or "opendataproducts.org/schema" not in data.get("schema", "")
        ):
    ...
    def scan_directory(self, directory: Path, pattern: str = "**/*.{json,yaml,yml}") -> bool:
        ...
        for file_path in directory.glob(pattern):
            if file_path.is_file() and not self.scan_file(file_path):
                all_secure = False
    ...
    def print_results(self) -> None:
        ...
        if self.vulnerabilities:
            ...
            if critical:
                ...
                for file_path, _severity, issue in critical:
                    print(f"  {file_path}: {issue}")

            if high:
                print("\n⚠️  HIGH Severity Issues:")
                for file_path, _severity, issue in high:
                    print(f"  {file_path}: {issue}")

            if medium:
                print("\n⚠️  MEDIUM Severity Issues:")
                for file_path, _severity, issue in medium:
                    print(f"  {file_path}: {issue}")

            if errors:
                print("\n❌ Errors:")
                for file_path, _severity, issue in errors:
                    print(f"  {file_path}: {issue}")
...
Examples:
  ...
  python scripts/scan_odps_security.py --strict tests/fixtures/odps/
    ...
    parser.add_argument("files", nargs="*", type=Path, help="ODPS files or directories to scan")
    ...
    if not args.files:
        ...
        if default_dir.exists():
            ...
            scanner.scan_directory(default_dir)
        else:
