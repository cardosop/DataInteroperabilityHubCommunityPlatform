...
class DocumentationImpactReportGenerator:
    ...
    def __init__(
        self,
        audit_reports_dir: str = "docs/api-audit",
        base_path: str = "."
    ):
    ...
    def compile_documentation_references(self) -> Dict[str, Any]:
        ...
        if 'documentation_endpoint_audit' in self.audit_reports:
            ...
            references['statistics']['total_documentation_files'] = summary.get('total_documentation_files', 0)
            references['statistics']['total_endpoint_references'] = summary.get('total_references', 0)
            ...
            for endpoint_data in doc_audit.get('endpoints', []):
                ...
                if normalized_path not in references['endpoints']:
                    ...
                    references['endpoints'][normalized_path]['documentation_files'].update(doc_files)
        ...
        if 'code_examples_audit' in self.audit_reports:
            ...
            references['statistics']['code_example_endpoints_found'] = summary.get('total_endpoints_found', 0)
        ...
        if 'postman_collections_audit' in self.audit_reports:
            ...
            references['statistics']['total_postman_collections'] = summary.get('total_collections', 0)
            references['statistics']['valid_postman_collections'] = summary.get('valid_collections', 0)
            references['statistics']['invalid_postman_collections'] = summary.get('invalid_collections', 0)
        ...
        if 'inventory_review' in self.audit_reports:
            ...
            references['statistics']['inventory_endpoints'] = summary.get('total_endpoints_in_inventory', 0)
            references['statistics']['codebase_endpoints'] = summary.get('total_endpoints_in_codebase', 0)
        ...
        for endpoint_path, endpoint_info in references['endpoints'].items():
            ...
            endpoint_info['documentation_files'] = sorted(list(endpoint_info['documentation_files']))
    ...
    def _write_report_header(self, f):
        ...
        f.write("This report compiles documentation references from all audit reports and identifies "
    ...
    def _write_summary(self, f, references: Dict[str, Any], updates: Dict[str, Any]):
        ...
        f.write(f"- **Invalid Postman Collections:** {len(updates.get('invalid_collections', []))}\n")
        f.write(f"- **Endpoints with Multiple Documentation Files:** {len(updates.get('endpoints_with_multiple_docs', []))}\n")
    ...
    def _write_endpoints(self, f, references: Dict[str, Any], updates: Dict[str, Any]):
        ...
        if endpoints:
            ...
            if multiple_docs:
                ...
                f.write("These endpoints have documentation in multiple files and may need consolidation:\n\n")
    ...
    def _write_code_examples(self, f, references: Dict[str, Any], updates: Dict[str, Any]):
        ...
        if invalid_examples:
            ...
            for file_path, examples in list(files_with_issues.items())[:10]:  # Limit to first 10 files
                f.write(f"#### `{file_path}`\n\n")
    ...
    def _write_recommendations(self, f, updates: Dict[str, Any]):
        ...
        if recommendations:
            ...
            for rec in recommendations:
                ...
                f.write(f"### {priority_emoji} **{priority.upper()} Priority**: {category.replace('_', ' ').title()}\n\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate documentation impact analysis report"
    )
    parser.add_argument(
        '--reports-dir',
    ...
    try:
        generator = DocumentationImpactReportGenerator(
            audit_reports_dir=args.reports_dir,
