...
class ErrorResponse:
    ...
    error_code: str
    error_message: str
    example_details: Optional[Dict] = None
...
class OpenAPIErrorResponseEnhancer:
    ...
    def get_error_example(self, error: ErrorResponse, endpoint_name: Optional[str] = None) -> Dict:
        ...
        if endpoint_name and endpoint_name in self.endpoint_specific_errors:
            specific_errors = self.endpoint_specific_errors[endpoint_name].get(error.status_code, [])
            if specific_errors:
    ...
    def determine_required_errors(self, method: str, path: str, requires_auth: bool = True) -> List[int]:
                ...
                for method in ['get', 'post', 'put', 'patch', 'delete', 'head', 'options']:
                    ...
                    for status_code in required_errors:
                        ...
                        if status_str not in operation['responses']:
                            ...
                            if endpoint_name and endpoint_name in self.endpoint_specific_errors:
                                specific_errors = self.endpoint_specific_errors[endpoint_name].get(status_code, [])
                                for idx, (code, message) in enumerate(specific_errors):
                                    ...
                                    response_def["content"]["application/json"]["examples"][f"example_{idx+1}"] = {
                                        "summary": f"{code} example",
                                        "value": specific_example
                                    }
                            ...
                            if 'content' in existing and 'application/json' in existing['content']:
                                ...
                                if 'default' not in existing['content']['application/json']['examples']:
                                    default_example = self.get_error_example(error, endpoint_name)
                                    existing['content']['application/json']['examples']['default'] = {
                                        "summary": f"{error.error_code} example",
                                        "value": default_example
                                    }
                                    modified = True
    ...
    def _extract_endpoint_name(self, path: str) -> Optional[str]:
