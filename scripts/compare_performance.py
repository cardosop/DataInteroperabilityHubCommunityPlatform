...
class PerformanceComparator:
    ...
    def compare_metrics(self, baseline_metrics: Dict[str, float], current_metrics: Dict[str, float], 
                       category: str, threshold: float = 0.1) -> Dict[str, Any]:
    ...
    def generate_report(self) -> str:
        ...
        report.append("=" * 80)
        report.append(f"Baseline: {self.comparison['baseline_version']} ({self.comparison['baseline_timestamp']})")
        report.append(f"Current:  {self.comparison['current_version']} ({self.comparison['current_timestamp']})")
        report.append("")
        ...
        if self.comparison['improvements']:
            ...
            report.append("-" * 80)
            for imp in sorted(self.comparison['improvements'], 
                            key=lambda x: x.get('improvement_percent', 0), reverse=True)[:10]:
                report.append(f"  {imp['category']}.{imp['metric']}:")
                report.append(f"    Improvement: {imp.get('improvement_percent', 0):.2f}%")
                report.append(f"    Baseline: {imp['baseline']:.3f}ms → Current: {imp['current']:.3f}ms")
            report.append("")
        else:
        ...
        if self.comparison['regressions']:
            ...
            report.append("-" * 80)
            for reg in sorted(self.comparison['regressions'],
                            key=lambda x: x.get('regression_percent', 0), reverse=True)[:10]:
                report.append(f"  {reg['category']}.{reg['metric']}:")
                report.append(f"    Regression: {reg.get('regression_percent', 0):.2f}%")
                report.append(f"    Baseline: {reg['baseline']:.3f}ms → Current: {reg['current']:.3f}ms")
            report.append("")
        else:
...
def main():
    ...
    parser.add_argument('--baseline', required=True,
                       help='Baseline performance file (e.g., Django 4.2)')
    parser.add_argument('--current', required=True,
                       help='Current performance file (e.g., Django 6)')
