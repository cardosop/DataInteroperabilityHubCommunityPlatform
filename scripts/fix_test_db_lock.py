...
def fix_test_database_lock():
    ...
    try:
        ...
        if (
            "staging" in db_config.get("NAME", "").lower()
            or "staging" in db_config.get("USER", "").lower()
        ):
        ...
        drop_query = sql.SQL("DROP DATABASE IF EXISTS {}").format(sql.Identifier(test_db_name))
        ...
        try:
            ...
            result1 = subprocess.run(cmd1, check=False, capture_output=True, text=True, timeout=10)
            if result1.returncode == 0:
                print("✓ Terminated connections via Docker")
            else:
            ...
            result2 = subprocess.run(cmd2, check=False, capture_output=True, text=True, timeout=10)
            if result2.returncode == 0:
                print("✓ Dropped test database via Docker")
        ...
        print("  Option 1 - Via Docker:")
        print(
            f"    docker exec {docker_container} psql -U {conn_params['user']} -d postgres -c \"SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '{test_db_name}' AND pid <> pg_backend_pid();\""
        )
        print(
            f"    docker exec {docker_container} psql -U {conn_params['user']} -d postgres -c 'DROP DATABASE IF EXISTS \"{test_db_name}\";'"
        )
        print("  Option 2 - Direct connection:")
        print(
            f"    psql -h {conn_params['host']} -p {conn_params['port']} -U {conn_params['user']} -d postgres"
        )
        print(
            f"    Then run: SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '{test_db_name}' AND pid <> pg_backend_pid();"
        )
