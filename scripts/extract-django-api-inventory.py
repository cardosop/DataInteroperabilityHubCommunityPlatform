...
class DjangoAPIExtractor:
    ...
    def _extract_urlpatterns(self, content: str, file_path: Path) -> List[APIEndpoint]:
        ...
        for match in router_pattern.finditer(content):
            ...
            rest_endpoints = self._generate_rest_endpoints_with_prefix(url_prefix, prefix, viewset, file_path)
    ...
    def _resolve_view_class(self, view: str, file_path: Path) -> Optional[Path]:
    ...
    def _extract_methods_from_class(self, view_file: Path) -> List[str]:
        ...
        try:
            ...
            action_pattern = re.compile(r'@action\s*\([^)]*methods\s*=\s*\[([^\]]+)\]', re.MULTILINE)
    ...
    def _generate_rest_endpoints(self, prefix: str, viewset: str, file_path: Path) -> List[APIEndpoint]:
        ...
        if 'api' in str(file_path) or any(app in str(file_path) for app in ['assets', 'contracts', 'datasets', 'jobs', 'marketplace', 'scheduled_ingestion', 'users', 'audit', 'files', 'compliance', 'dq', 'governance', 'search', 'webhooks']):
    ...
    def _determine_url_prefix(self, urls_file: Path, router_prefix: str) -> str:
        ...
        if 'apps/api' in str(urls_file) or any(app in str(urls_file) for app in ['assets', 'contracts', 'datasets', 'jobs', 'marketplace', 'scheduled_ingestion', 'users', 'audit', 'files', 'compliance', 'dq', 'governance', 'search', 'webhooks']):
            ...
            if parent_urls.exists():
                ...
                pattern = re.compile(rf"path\s*\(['\"]([^'\"]+)['\"]\s*,\s*include\s*\(['\"]hub\.apps\.{app_name}\.urls", re.MULTILINE)
    ...
    def _generate_rest_endpoints_with_prefix(self, url_prefix: str, router_prefix: str, viewset: str, file_path: Path) -> List[APIEndpoint]:
                ...
                for match in action_pattern.finditer(content):
                    ...
                    if base_path:
                        ...
                        for method in methods:
                            ...
                            if not any(e.path == endpoint.path and e.method == endpoint.method for e in self.endpoints):
                                self.endpoints.append(endpoint)
                                print(f"Found custom action: {method.upper()} {action_path} ({action_name})")
    ...
    def _find_viewset_base_path(self, viewset_name: str, views_file: Path) -> Optional[str]:
        ...
        for urls_file in urls_files:
            try:
                ...
                pattern = re.compile(rf'router\.register\s*\([^,]+,\s*{viewset_name}[^)]*\)', re.MULTILINE)
                match = pattern.search(content)
                if match:
                    # Extract the prefix
                    prefix_match = re.search(r'router\.register\s*\(["\']([^"\']+)["\']', match.group(0))
                    if prefix_match:
    ...
    def generate_inventory(self, output_path: Path):
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"**Source:** Django URL patterns and ViewSets\n\n")
            ...
            for endpoint in sorted(unique_endpoints, key=lambda e: (e.path, e.method)):
                ...
                f.write(f"| {endpoint.method} | `{endpoint.path}` | `{view_class}` | `{file_path}` |\n")
