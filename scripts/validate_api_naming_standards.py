...
class APINamingStandardsValidator:
    ...
    def validate_pattern_consistency(self, endpoint: dict[str, Any]) -> list[ValidationError]:
        ...
        matches_pattern = (
            self.COLLECTION_PATTERN.match(path)
            or self.DETAIL_PATTERN.match(path)
            or self.ACTION_PATTERN.match(path)
            or self.SUB_RESOURCE_PATTERN.match(path)
        )
    ...
    def generate_report(self, result: ValidationResult, output_file: Path | None = None) -> dict:
...
def main():
    """Main execution"""
    parser = argparse.ArgumentParser(description="Validate API endpoints against naming standards")
    parser.add_argument(
        "--strict", action="store_true", help="Treat pattern consistency violations as errors"
    )
    parser.add_argument(
        "--output",
