...
class APICategorizer:
    ...
    def _determine_source_file(self, path: str, view_class: str | None) -> str | None:
    ...
    def _categorize_endpoint(self, endpoint_key: str, endpoint_info: dict) -> APICategory | None:
    ...
    def _determine_status(self, path: str, view_class: str | None, source_file: str | None) -> str:
    ...
    def _determine_completeness(
        self, path: str, view_class: str | None, source_file: str | None, method: str
    ) -> str:
        ...
        if view_class and "ViewSet" in view_class:
            ...
            if view_file and view_file.exists():
                try:
                    ...
                    if action and action not in [
                        ...
                        if re.search(action_pattern, content, re.MULTILINE):
                            ...
                            if method_match:
                                ...
                                if method_body and method_body not in [
                                    "pass",
                                    "...",
                    ...
                    elif ("{id}" in path or "{pk}" in path) and (
                        (method == "GET" and "def retrieve" in content)
                        or (method == "PUT" and "def update" in content)
                        or (method == "PATCH" and "def partial_update" in content)
                        or (method == "DELETE" and "def destroy" in content)
                    ):
                        return "Complete"
    ...
    def _extract_action_from_path(self, path: str, method: str) -> str | None:
    ...
    def _check_schema(self, path: str, source_file: str | None) -> bool:
    ...
    def _check_tests(self, path: str, source_file: str | None) -> bool:
            ...
            for test_file in test_files:
                if test_file.exists():
                    try:
                        ...
                        if (
                            path_segment in content
                            or path in content
                            or "test_" in content.lower()
                            or "APIClient" in content
                            or "client.get" in content
                            or "client.post" in content
                        ):
                            return True
                    except Exception:
                        continue
    ...
    def _check_deprecated(self, path: str, view_class: str | None, source_file: str | None) -> bool:
        ...
        if source_file:
            ...
            if file_path.exists():
                ...
                if re.search(
                    r"@deprecated|deprecated\s*=\s*True|#\s*deprecated", content, re.IGNORECASE
                ):
                    return True
    ...
    def _find_view_file(self, source_file: str, view_class: str) -> Path | None:
            ...
            if view_class in content:
                ...
                if class_pattern.search(content):
                    ...
                    if class_match:
                        ...
                        class_body = content[
                            start : start + (next_class.start() if next_class else len(content))
                        ]
    ...
    def _get_implemented_methods(
        self, path: str, view_class: str | None, source_file: str | None
    ) -> set[str]:
    ...
    def generate_categorized_inventory(self, output_path: Path):
        ...
        self.inventory_file.read_text(encoding="utf-8")
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            f.write(f"- **Total Endpoints:** {stats['total']}\n")
            f.write(
                f"- **With Schema:** {stats['with_schema']} ({stats['with_schema'] * 100 // stats['total'] if stats['total'] > 0 else 0}%)\n"
            )
            f.write(
                f"- **With Tests:** {stats['with_tests']} ({stats['with_tests'] * 100 // stats['total'] if stats['total'] > 0 else 0}%)\n"
            )
            ...
            for endpoint_key in sorted(categories.keys()):
                ...
                f.write(
                    f"| {method} | `{path}` | {category.feature_type} | {category.status} | {completeness_short} | {schema_mark} | {tests_mark} | {deprecated_mark} |\n"
                )
