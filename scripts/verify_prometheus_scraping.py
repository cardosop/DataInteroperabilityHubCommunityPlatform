...
def check_metrics_endpoint(api_url: str, timeout: int = 30) -> Tuple[bool, str, Optional[str]]:
    """
    ...
    try:
        ...
        return False, f"Error checking metrics endpoint: {str(e)}", None


def verify_metric_names(metrics_content: str, required_metrics: List[str]) -> Tuple[bool, List[str]]:
    """
...
def verify_prometheus_scraping(prometheus_url: str, api_url: str, timeout: int = 60) -> Tuple[bool, str]:
    """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('status') == 'success':
                ...
                if len(result) > 0:
                    return True, f"Prometheus successfully scraped metrics ({len(result)} series found)"
                else:
        ...
        return False, f"Error checking Prometheus: {str(e)}"
...
def main():
    parser = argparse.ArgumentParser(description='Verify Prometheus scraping of OpenTelemetry metrics')
    ...
    parser.add_argument('--required-metrics', nargs='+',
    ...
    if metrics_content:
        ...
        if args.prometheus_url:
            ...
            success, message = verify_prometheus_scraping(args.prometheus_url, args.url, args.timeout)
            if success:
                print(f"   ✓ {message}")
            else:
                print(f"   ✗ {message}")
