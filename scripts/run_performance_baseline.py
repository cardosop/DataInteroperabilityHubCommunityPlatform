...
class PerformanceBaselineCollector:
    ...
    def measure_api_response_times(self) -> Dict[str, float]:
        ...
        for endpoint, name in endpoints:
            ...
            if times:
                ...
                metrics[f'{name}_p95'] = sorted(times)[int(len(times) * 0.95)] if len(times) > 1 else times[0]
                metrics[f'{name}_p99'] = sorted(times)[int(len(times) * 0.99)] if len(times) > 1 else times[0]
    ...
    def measure_job_queue_processing_times(self) -> Dict[str, float]:
        ...
        for i in range(10):
            job = Job.objects.create(
                ...
                resource_type='asset',
                resource_id=str(Asset.objects.create(
                    tenant=self.tenant,
                    name=f"Test Asset {i}",
                    status='DRAFT'
                ).id)
    ...
    def collect_all_metrics(self):
        ...
        self.results['metrics']['middleware_execution_times'] = self.measure_middleware_execution_times()
        self.results['metrics']['job_queue_processing_times'] = self.measure_job_queue_processing_times()
        self.results['metrics']['file_storage_operations_times'] = self.measure_file_storage_operations_times()
...
def main():
    ...
    parser.add_argument('--django-version', default=None,
                       help='Django version (default: current version)')
