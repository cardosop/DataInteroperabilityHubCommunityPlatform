...
class ErrorResponseValidator:
    ...
    def __init__(self):
        ...
        self.required_schema_fields = ["code", "message", "http_status", "request_id", "timestamp"]
    ...
    def _validate_error_schema(self, schema: dict, results: dict) -> bool:
        ...
        if missing_fields:
            results["errors"].append(
                f"ErrorResponse schema missing fields: {', '.join(missing_fields)}"
            )
            return False
    ...
    def validate_all_specs(self, contracts_dir: Path) -> dict:
        ...
        for spec_file in yaml_files:
            ...
            results["summary"]["total_error_responses"] += len(
                spec_results["error_responses_found"]
            )
