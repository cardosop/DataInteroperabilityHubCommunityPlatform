...
def validate_dashboard():
    ...
    dashboard_path = (
        project_root / "monitoring" / "grafana" / "dashboards" / "odps-marketplace-integration.json"
    )
    ...
    pricing_sqls = [
        title
        for title, sql in all_sqls
        if "pricing" in sql.lower() or "pricing_plans" in sql.lower()
    ]
    access_sqls = [
        title for title, sql in all_sqls if "access_method" in sql.lower() or "accessMethod" in sql
    ]
