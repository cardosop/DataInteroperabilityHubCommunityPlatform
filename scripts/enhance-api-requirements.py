...
class EnhancedAPIExtractor:
        ...
        if 'credential' in content.lower():
            credential_endpoints = [
                ('GET', '/api/v1/scheduled-ingestions/{id}/credentials/', 
                 'Get masked credentials for scheduled ingestion', ['id']),
                ('POST', '/api/v1/scheduled-ingestions/{id}/credentials/test/', 
                 'Test credential connection', ['id']),
                ('PUT', '/api/v1/scheduled-ingestions/{id}/credentials/', 
                 'Update credentials for scheduled ingestion', ['id']),
                ('POST', '/api/v1/scheduled-ingestions/{id}/credentials/rotate/', 
                 'Rotate credentials with test-before-switch', ['id']),
                ('GET', '/api/v1/scheduled-ingestions/{id}/credentials/validation/', 
    ...
    def _extract_from_scenarios(self, content: str, spec_name: str):
        ...
        for resource_name, base_path in resources.items():
            if resource_name in content_lower or resource_name.replace('-', ' ') in content_lower:
                # List endpoint
                self._add_endpoint('GET', base_path, f'List {resource_name}s', 
                                 spec_name, query_params=['page', 'page_size', 'search', 'filter', 'sort'])
                
                # Create endpoint
                self._add_endpoint('POST', base_path, f'Create {resource_name}', 
                                 spec_name)
                
                # Detail endpoints
                detail_path = f"{base_path}/{{id}}"
                self._add_endpoint('GET', detail_path, f'Get {resource_name} by ID', 
                                 spec_name, path_params=['id'])
                self._add_endpoint('PUT', detail_path, f'Update {resource_name}', 
                                 spec_name, path_params=['id'])
                self._add_endpoint('PATCH', detail_path, f'Partially update {resource_name}', 
                                 spec_name, path_params=['id'])
                self._add_endpoint('DELETE', detail_path, f'Delete {resource_name}', 
                                 spec_name, path_params=['id'])
        
        # Special endpoints
        if 'search' in content_lower or 'query' in content_lower:
            self._add_endpoint('GET', '/api/v1/search', 'Search assets and contracts',
                             spec_name, query_params=['q', 'type', 'domain', 'page', 'page_size'])
        
        if 'upload' in content_lower:
            self._add_endpoint('POST', '/api/v1/datasets/upload', 'Upload dataset file',
                             spec_name)
        
        if 'validate' in content_lower and 'contract' in content_lower:
            self._add_endpoint('POST', '/api/v1/contracts/{id}/validate', 'Validate contract',
                             spec_name, path_params=['id'])
        
        if 'publish' in content_lower and 'contract' in content_lower:
            self._add_endpoint('POST', '/api/v1/contracts/{id}/publish', 'Publish contract to marketplace',
                             spec_name, path_params=['id'])
        
        # Authentication endpoints
        if 'login' in content_lower or 'authentication' in content_lower:
            self._add_endpoint('POST', '/api/v1/auth/login', 'User login',
                             spec_name, auth_required=False)
            self._add_endpoint('POST', '/api/v1/auth/register', 'User registration',
                             spec_name, auth_required=False)
            self._add_endpoint('POST', '/api/v1/auth/logout', 'User logout',
                             spec_name)
            self._add_endpoint('POST', '/api/v1/auth/refresh', 'Refresh JWT token',
                             spec_name)
        
        if 'api key' in content_lower or 'api-key' in content_lower:
            self._add_endpoint('GET', '/api/v1/api-keys', 'List API keys',
                             spec_name)
            self._add_endpoint('POST', '/api/v1/api-keys', 'Create API key',
                             spec_name)
            self._add_endpoint('DELETE', '/api/v1/api-keys/{id}', 'Revoke API key',
                             spec_name, path_params=['id'])
        
        # Natural language search
        if 'natural language' in content_lower or 'nlp' in content_lower:
            self._add_endpoint('POST', '/api/v1/ai/search', 'Natural language search',
                             spec_name, query_params=['query'])
        
        # Schema matching
        if 'schema matching' in content_lower or 'schema-matching' in content_lower:
            self._add_endpoint('POST', '/api/v1/ai/schema-matching', 'AI schema matching',
                             spec_name)
        
        # Recommendations
        if 'recommendation' in content_lower:
            self._add_endpoint('GET', '/api/v1/ai/recommendations', 'Get asset recommendations',
                             spec_name, query_params=['asset_id', 'limit'])
        
        # Transformation pipeline
        if 'pipeline' in content_lower and 'execute' in content_lower:
            self._add_endpoint('POST', '/api/v1/pipelines/{id}/execute', 'Execute transformation pipeline',
                             spec_name, path_params=['id'])
            self._add_endpoint('POST', '/api/v1/pipelines/{id}/preview', 'Preview pipeline results',
                             spec_name, path_params=['id'])
    
    def _add_endpoint(self, method: str, path: str, description: str, 
                     spec_name: str, query_params: List[str] = None,
                     path_params: List[str] = None, auth_required: bool = True,
    ...
    def generate_comprehensive_report(self, output_path: Path):
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            f.write("## Overview\n\n")
            f.write("This document contains comprehensive API requirements extracted from frontend specification files.\n")
            f.write("Each endpoint includes request/response schemas, query parameters, path parameters, and error responses.\n\n")
            ...
            for category, endpoints in sorted(results['categories'].items()):
                ...
                for endpoint in sorted(endpoints, key=lambda e: (e.method or '', e.path)):
                    ...
                    if endpoint.rate_limited:
                        f.write(f"**Rate Limited:** Yes\n\n")
                    
                    f.write("---\n\n")
            
            # Summary section
            f.write("## Summary\n\n")
            f.write(f"### Endpoint Statistics\n\n")
            ...
            for category, endpoints in sorted(results['categories'].items(), key=lambda x: -len(x[1])):
