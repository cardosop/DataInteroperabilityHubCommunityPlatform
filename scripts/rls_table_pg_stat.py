#!/usr/bin/env python3
"""Capture and compare pg_stat_statements snapshots for table RLS rollouts."""
...
def _read_stats(database_url: str, table: str, limit: int) -> list[QueryStat]:
    sql = """
        SELECT
...
def build_parser() -> argparse.ArgumentParser:
    ...
    snap = sub.add_parser(
        "snapshot",
        help="capture top query stats for a table",
    )
    snap.add_argument(
        "--database-url",
        default=os.getenv("DATABASE_URL_ADMIN") or os.getenv("DATABASE_URL"),
    )
    snap.add_argument("--table", required=True)
