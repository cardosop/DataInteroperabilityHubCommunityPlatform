...
class OpenAPISpecValidator:
    ...
    def validate_with_openapi_spec_validator(self, file_path: Path) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            errors.append("openapi_spec_validator not installed. Install with: pip install openapi-spec-validator")
            return False, errors
    ...
    def validate_with_spectral(self, file_path: Path) -> Tuple[bool, List[str], List[str]]:
        ...
        try:
            ...
            if result.returncode != 0:
                warnings.append("npx not found. Spectral validation skipped. Install Node.js to enable Spectral validation.")
            ...
            errors.append(f"Spectral validation error: {str(e)}")
            return False, errors, warnings
                        ...
                        if method in ['get', 'post', 'put', 'patch', 'delete', 'head', 'options']:
                            ...
                            if 'responses' in operation:
                                ...
                                if not isinstance(responses, dict):
                                    errors.append(f"Operation {method.upper()} {path} 'responses' must be an object")
                                elif len(responses) == 0:
                                    errors.append(f"Operation {method.upper()} {path} must have at least one response")
    ...
    def validate_spec_file(self, file_path: Path) -> Tuple[bool, List[str], List[str]]:
        ...
        # Validate structure
        is_valid, structure_errors = self.validate_structure(spec, file_path)
        errors.extend(structure_errors)
        
        # Validate with openapi_spec_validator (optional)
        try:
            is_valid_validator, validator_errors = self.validate_with_openapi_spec_validator(file_path)
            ...
            warnings.append(f"openapi_spec_validator validation skipped: {str(e)}")
        
        # Validate with Spectral
        try:
            is_valid_spectral, spectral_errors, spectral_warnings = self.validate_with_spectral(file_path)
            if not is_valid_spectral:
                errors.extend(spectral_errors)
            warnings.extend(spectral_warnings)
        except Exception as e:
            warnings.append(f"Spectral validation skipped: {str(e)}")
    ...
    def validate_all_specs(self, specs_dir: Path) -> Dict[str, Any]:
        ...
        for spec_file in sorted(spec_files):
            ...
            results['warnings'].extend([
                {'file': str(spec_file.relative_to(PROJECT_ROOT)), 'warning': w}
                for w in warnings
            ])
