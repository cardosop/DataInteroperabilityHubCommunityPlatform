...
class ComplianceEndpointVerifier:
                ...
                for line_num, line in enumerate(lines, 1):
                    ...
                    for pattern in self.OLD_PATTERNS:
                        if re.search(pattern, line, re.IGNORECASE):
                            ...
                            if not re.search(r'/api/v1/|http://|https://|post\(|get\(|put\(|delete\(|\.post\(|\.get\(|\.put\(|\.delete\(', line, re.IGNORECASE):
                                continue
                            # Skip if this is clearly a metrics reference (not an API endpoint)
                            if self.is_metrics_reference(line) and not re.search(r'/api/v1/', line, re.IGNORECASE):
                                continue
                            ...
                            if re.search(r'db_table\s*=|related_name\s*=|def\s+\w*compliance|compliance_runs\s*=|compliance_runs\.get\(|compliance_runs\s*\[|compliance_runs\s*\{|isinstance\(compliance_runs', line, re.IGNORECASE):
                                continue
                            # Skip test assertions that check old URLs don't resolve (they're testing the right thing)
                            if re.search(r'resolve\(.*compliance.*compliance-runs|test.*old.*endpoint|test.*old.*url|test.*old.*pattern|endpoint.*not.*accessible|does.*not.*resolve', line, re.IGNORECASE):
                                continue
                            # Skip test cases that verify old endpoints return 404 (they're intentionally using old URLs)
                            if re.search(r'client\.get\(.*compliance.*compliance-runs|client\.post\(.*compliance.*compliance-runs', line, re.IGNORECASE):
                                # Check if it's in a test that expects 404
                                context_lines = self._get_context(file_path, line_num, context_lines=5)
                                if re.search(r'404|NOT_FOUND|does.*not.*resolve|not.*accessible', context_lines, re.IGNORECASE):
                                    continue
    ...
    def verify_consumer(self, consumer_name: str, consumer_type: str,
                       file_path: str) -> ConsumerVerification:
        ...
        file_refs = [ref for ref in self.references
                    if ref.file_path == file_path]

        old_patterns = [ref.endpoint_pattern for ref in file_refs
                       if ref.pattern_type == 'old']
        new_patterns = [ref.endpoint_pattern for ref in file_refs
                      if ref.pattern_type == 'new']
    ...
    def verify_all_consumers(self) -> List[ConsumerVerification]:
        ...
        verifications.append(self.verify_consumer(
        ...
        for sdk_file in sdk_files:
            verifications.append(self.verify_consumer(
                f'JavaScript SDK - {Path(sdk_file).name}',
                'sdk',
                sdk_file
            ))
    ...
    def generate_report(self) -> Dict:
        ...
        consumers_needs_update = len([v for v in self.consumer_verifications if v.status == 'needs_update'])
        consumers_not_found = len([v for v in self.consumer_verifications if v.status == 'not_found'])
        ...
        report = {
            ...
            'old_patterns': [
                ...
                for ref in self.references if ref.pattern_type == 'old'
            ],
            'consumers': {
                consumer_type: [
                    {
    ...
    def print_report(self, report: Dict):
        ...
        for consumer_type, consumers in report['consumers'].items():
            ...
            for consumer in consumers:
                status_icon = "✅" if consumer['status'] == 'verified' else "❌" if consumer['status'] == 'needs_update' else "⚠️"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Verify compliance endpoint usage across codebase'
    )
    parser.add_argument(
        '--root-dir',
        type=str,
