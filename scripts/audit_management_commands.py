...
def find_commands():
    cmds = []
    for root, _dirs, files in os.walk("hub/apps"):
        if root.endswith("/management/commands"):
            for f in files:
                if f.endswith(".py") and not f.startswith("__"):
                    ...
                    has_audit = bool(
                        re.search(
                            r"create_audit_event|_audit_plan_mutation|log_tenant_operation", content
                        )
                    )
...
def main():
    ...
    print(
        f"Management commands: {len(cmds)} total, {covered} audit-covered, {len(cmds) - covered} uncovered"
    )
