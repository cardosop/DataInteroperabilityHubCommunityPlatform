...
BLOCKING_DATE = datetime(2026, 6, 20, tzinfo=UTC)  # 30 days from now
...
def _git_modified_files(repo_root: Path, since_days: int | None) -> list[str]:
    ...
    return sorted(
        set(
            line.strip() for line in result.stdout.splitlines() if line.strip().endswith(".spec.ts")
        )
    )
...
def _find_spec_files(repo_root: Path) -> list[str]:
    ...
    if not features_dir.is_dir():
        return []
    return sorted(f.name for f in features_dir.glob("*.spec.ts") if f.name not in EXEMPT_FILES)
...
def run_lint(
    ...
    if json_output:
        ...
        if missing:
            ...
            print("  Import from '../fixtures/verifyBackend'.")
