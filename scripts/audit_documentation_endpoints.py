...
class DocumentationEndpointAuditor:
    ...
    def __init__(
        self, base_path: str = ".", docs_dir: str | None = None, runbooks_dir: str | None = None
    ):
        ...
        self.endpoint_patterns = {
            ...
            # GraphQL endpoints
            "graphql": re.compile(
                r'(?:^|\s|`|"|\'|/)(/graphql[^\s`"\'\)]*)', re.IGNORECASE | re.MULTILINE
            ),
            # WebSocket endpoints
        ...
        for api_file in api_files:
            try:
                ...
                file_refs = self._extract_endpoints_from_content(
                    content, str(api_file.relative_to(self.base_path)), "api_reference"
                )
    ...
    def _extract_from_api_audit(self) -> list[EndpointReference]:
        ...
        for audit_file in audit_files:
            try:
                ...
                file_refs = self._extract_endpoints_from_content(
                    content, str(audit_file.relative_to(self.base_path)), "api_audit"
                )
                references.extend(file_refs)
        ...
        if examples_dir.exists():
            for spec_file in examples_dir.glob("*.yaml"):
                try:
                    ...
                    print(
                        f"   ✓ {spec_file.name}: {len([r for r in references if r.source_file == str(spec_file.relative_to(self.base_path))])} endpoints"
                    )
                except Exception as e:
        ...
        for ref_file in reference_files:
            if ref_file.exists():
                try:
                    ...
                    file_refs = self._extract_endpoints_from_content(
                        content, str(ref_file.relative_to(self.base_path)), "api_reference"
                    )
                    references.extend(file_refs)
        ...
        for guide_file in guide_files:
            if guide_file.exists():
                try:
                    ...
                    file_refs = self._extract_endpoints_from_content(
                        content, str(guide_file.relative_to(self.base_path)), "developer_guide"
                    )
                    references.extend(file_refs)
    ...
    def _extract_from_runbooks(self) -> list[EndpointReference]:
        ...
        for runbook_file in runbook_files:
            try:
                ...
                file_refs = self._extract_endpoints_from_content(
                    content, str(runbook_file.relative_to(self.base_path)), "runbook"
                )
                references.extend(file_refs)
    ...
    def _extract_endpoints_from_content(
        self, content: str, source_file: str, doc_type: str
        ...
        for match in self.endpoint_patterns["rest"].finditer(content):
            ...
            normalized_endpoints_with_methods = [
                self._normalize_endpoint_path(r.endpoint_path) for r in references if r.method
            ]
    ...
    def verify_all_documentation_found(self) -> dict[str, Any]:
        ...
        total_files = (
            ...
            + len(list((self.docs_dir / "api-audit").glob("*.md")))
            if (self.docs_dir / "api-audit").exists()
            else 0 + len(list(self.runbooks_dir.glob("*.md")))
    ...
    def verify_documentation_mapping(self) -> dict[str, Any]:
        ...
        endpoints_with_docs = len(
            [m for m in self.endpoint_mappings.values() if len(m.documentation_files) > 0]
        )
        endpoints_with_multiple_docs = len(
            [m for m in self.endpoint_mappings.values() if len(m.documentation_files) > 1]
        )
    ...
    def generate_report(self, output_file: str, format: str = "json"):
        ...
        summary = {
            ...
            "total_documentation_files": len(
                set(ref.source_file for ref in self.endpoint_references)
            ),
...
def main():
    """Main execution"""
    parser = argparse.ArgumentParser(description="Audit documentation files for endpoint URLs")
    parser.add_argument(
        "--output",
    ...
    parser.add_argument("--docs-dir", help="Documentation directory (default: {base_path}/docs)")
    parser.add_argument("--runbooks-dir", help="Runbooks directory (default: {base_path}/runbooks)")

    args = parser.parse_args()

    auditor = DocumentationEndpointAuditor(
        base_path=args.base_path, docs_dir=args.docs_dir, runbooks_dir=args.runbooks_dir
    )
    ...
    print(
        f"   Endpoints with multiple docs: {mapping_verification['endpoints_with_multiple_docs']}"
    )
