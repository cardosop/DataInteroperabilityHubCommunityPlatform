    ...
    for path in ("/health/", "/healthz/", "/health"):
        ...
        if resp.status_code != 404:
            assert resp.status_code == 200, (
                f"Health endpoint {path} returned {resp.status_code}: {resp.text[:500]}"
            )
            return
    ...
    for path in ("/metrics/", "/observability/metrics/", "/metrics"):
        ...
        if resp.status_code != 404:
            assert resp.status_code in (200, 204), (
                f"Metrics endpoint {path} returned {resp.status_code}: {resp.text[:500]}"
            )
            return
...
def test_health_returns_ok():
    ...
    if isinstance(body, dict):
        ...
        assert any(keyword in body_str for keyword in ("ok", "healthy", "up", "pass", "alive")), (
