"""
Phase 279.H.8 — SDK TA persona journey mirror tests.

SDK-side equivalents of the CLI journey tests in
``cli/tests/use_cases/test_ta_operations_journey.py``.  Requires ``DATAHUB_BASE_URL``
...
Usage:
  ...
  pytest sdk/python/tests/use_cases/test_ta_operations_journey.py -v
"""
import os
...
@requires_backend
class TestSDKTAJourney:
    """SDK TA persona journey."""
    ...
    async def test_notifications_list(self):
        """SDK notifications API returns valid response."""
        from datahub_interoperability import DataHubClient, DataHubClientConfig
        from datahub_interoperability.notifications import NotificationAPI
        config = DataHubClientConfig(base_url=DATAHUB_BASE_URL, api_token=_api_token())
        async with DataHubClient(config) as client:
            api = NotificationAPI(client)
            data = await api.list_notifications()
            assert data is not None

    async def test_tenants_usage(self):
        """SDK tenants usage returns valid response."""
        from datahub_interoperability import DataHubClient, DataHubClientConfig
        config = DataHubClientConfig(base_url=DATAHUB_BASE_URL, api_token=_api_token())
        async with DataHubClient(config) as client:
            data = await client.tenants.get_usage()
            assert data is not None
