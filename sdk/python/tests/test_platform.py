...
class TestPlatformAPI:
    ...
    async def test_list_tenants(self, api):
        ...
        api.client.get.assert_called_once_with("platform/tenants/", params={"page": 1, "page_size": 25})
        assert result == {"results": []}

    @pytest.mark.asyncio
    async def test_create_tenant(self, api):
        api.client.post.return_value = {"id": "t1", "slug": "test"}
        result = await api.create_tenant({"slug": "test", "display_name": "T", "admin_email": "a@b.com"})
        api.client.post.assert_called_once_with("platform/tenants/", data={"slug": "test", "display_name": "T", "admin_email": "a@b.com"})
        assert result["id"] == "t1"
