...
def test_dry_run_flag_audit():
    """Audit whether --dry-run is supported by the CLI root group.

    TODO: When --dry-run is added to the CLI, this test should verify:
    1. `datahub --dry-run assets create ...` prints what WOULD be done
    2. No actual API call is made
    3. Exit code is 0
    """
    result = subprocess.run(
        [sys.executable, "-m", "datahub_cli", "--help"],
    ...
    if "--dry-run" in result.stdout:
        ...
        pytest.skip(
            "CLI does not have --dry-run flag yet — "
            "TODO: add --dry-run to applicable write commands"
        )
