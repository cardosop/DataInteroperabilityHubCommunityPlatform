...
async def test_apply_policy_integration(real_client):
    ...
    created_domain = await real_client.mesh.create_domain(
        name=domain_name, description="Test domain for policy application", status="ACTIVE"
    )
    ...
    try:
        # Apply policy
        result = await real_client.mesh.apply_policy(created_domain["id"], policy_id)
...
async def test_apply_policy_with_overrides_integration(real_client):
    ...
    created_domain = await real_client.mesh.create_domain(
        name=domain_name, description="Test domain for policy overrides", status="ACTIVE"
    )
    ...
    try:
        ...
        result = await real_client.mesh.apply_policy(
            created_domain["id"], policy_id, overrides=overrides
        )
...
async def test_list_policies_integration(real_client):
    ...
    created_domain = await real_client.mesh.create_domain(
        name=domain_name, description="Test domain for listing policies", status="ACTIVE"
    )
...
async def test_list_policies_with_filters_integration(real_client):
    ...
    created_domain = await real_client.mesh.create_domain(
        name=domain_name, description="Test domain for policy filters", status="ACTIVE"
    )

    try:
        # Filter by status
        result = await real_client.mesh.list_policies(created_domain["id"], status="APPLIED")
        ...
        # Test pagination
        paginated = await real_client.mesh.list_policies(created_domain["id"], page=1, page_size=10)
...
async def test_remove_policy_integration(real_client):
    ...
    created_domain = await real_client.mesh.create_domain(
        name=domain_name, description="Test domain for policy removal", status="ACTIVE"
    )
    ...
    try:
        # Apply policy first
        applied = await real_client.mesh.apply_policy(created_domain["id"], policy_id)

        assert applied is not None

        # Remove policy
        result = await real_client.mesh.remove_policy(created_domain["id"], policy_id)
