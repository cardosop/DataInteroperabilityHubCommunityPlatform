"""
Phase 216.X.8 — local SDK duplicate of ``hub/apps/api/mvp_mode.py::_parse_bool_env``.

The SDK test suite cannot import Django at runtime (the SDK is shipped
standalone) so it duplicates the canonical environment-bool parser
byte-for-byte. A drift test (``sdk/python/tests/test_parse_bool_env_drift.py``)
