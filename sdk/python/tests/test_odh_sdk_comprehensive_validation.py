...
def api_key():
    ...
    key = get_api_key()
    if not key:
        pytest.skip("API key not available. Set TEST_API_KEY or DATAHUB_API_KEY environment variable, or ensure Docker Compose services are running.")
    return key
...
class TestODHIntegrationAPIComprehensive:
    ...
    async def test_update_model(self, odh_api, test_model):
        ...
        updated = await odh_api.update_model(
            test_model,
            status="TRAINED"
        )
    ...
    async def test_link_model_to_asset(self, odh_api, test_model, test_asset, client):
        ...
        try:
            new_asset = await client.post("/assets/", {
    ...
    async def test_link_model_to_dataset(self, odh_api, test_model, test_dataset):
        ...
        try:
            link = await odh_api.link_model_to_dataset(
                test_model,
                test_dataset,
                role="TRAINING"
            )
...
class TestTrainingAPIComprehensive:
    ...
    @pytest.mark.asyncio
    async def test_submit_training_job_error_handling_invalid_uuid(self, training_api, test_dataset):
    ...
    @pytest.mark.asyncio
    async def test_submit_training_job_error_handling_invalid_config(self, training_api, test_model, test_dataset):
...
class TestInferenceAPIComprehensive:
    ...
    async def test_deploy_model(self, inference_api, test_model, odh_inference_scheduler_available):
        ...
        try:
            ...
            error_str = str(e).lower()
            if any(keyword in error_str for keyword in ["odh", "inference-scheduler", "circuit breaker", "service unavailable", "connection refused"]):
                pytest.skip(f"ODH Inference Scheduler service unavailable: {e}")
            raise

    @pytest.mark.asyncio
    async def test_deploy_model_without_config(self, inference_api, test_model, odh_inference_scheduler_available):
    ...
    @pytest.mark.asyncio
    async def test_list_deployments_with_filters(self, inference_api, test_model, odh_inference_scheduler_available):
