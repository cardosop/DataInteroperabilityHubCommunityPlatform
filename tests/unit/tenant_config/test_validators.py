    ...
    def test_valid_profile_keys(self):
        ...
        for profile in VALID_DQ_PROFILES:
            try:
                ...
                self.fail(
                    f"validate_dq_profile raised ValidationError for valid profile: {profile}"
                )
    ...
    def test_valid_regimes(self):
        ...
        for regime in VALID_COMPLIANCE_REGIMES:
            try:
                ...
                self.fail(
                    f"validate_compliance_regimes raised ValidationError for valid regime: {regime}"
                )
...
class ValidateRateLimitsTest(TestCase):
    ...
    def test_valid_rate_limits_for_each_category(self):
        ...
        for category in PLATFORM_MAX_RATE_LIMITS.keys():
            ...
            try:
                ...
                self.fail(
                    f"validate_rate_limits raised ValidationError for valid category {category}"
                )
    ...
    def test_rate_limits_exceeding_platform_maximums(self):
        ...
        with self.assertRaises(ValidationError) as cm:
            validate_rate_limits(
                {
                    "dq_runs": {
                        "sustained_per_min": PLATFORM_MAX_RATE_LIMITS["dq_runs"][
                            "sustained_per_min"
                        ]
                        + 1
                    }
        ...
        with self.assertRaises(ValidationError) as cm:
            validate_rate_limits(
                {"dq_runs": {"daily_cap": PLATFORM_MAX_RATE_LIMITS["dq_runs"]["daily_cap"] + 1}}
            )
        error_message = str(cm.exception)
        self.assertIn("exceeds platform maximum", error_message)
    ...
    def test_rate_limits_below_platform_maximums(self):
        ...
        try:
            ...
            self.fail(
                "validate_rate_limits raised ValidationError for limits below platform maximum"
            )
    ...
    def test_daily_cap_for_category_that_doesnt_support_it(self):
        ...
        with self.assertRaises(ValidationError) as cm:
            validate_rate_limits({"file_uploads": {"daily_cap": 1000}})
...
class GetPlatformDefaultsTest(TestCase):
    ...
    def test_platform_defaults_rate_limits_structure(self):
        ...
        for category in expected_categories:
            self.assertIn(
                category, rate_limits, f"Platform defaults rate_limits missing category: {category}"
            )
