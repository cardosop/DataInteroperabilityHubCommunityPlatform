...
from hub.apps.rate_limiting.utils import TimeWindow, get_rate_limit_info, sliding_window_check
...
class SlidingWindowAlgorithmTest(TestCase):
    ...
    def test_sliding_window_allows_within_limit(self, mock_from_url, mock_get_pool, mock_cache):
        ...
        allowed, count, reset_time = sliding_window_check(
            key="test-key", limit=10, window=60, current_time=150.0
        )
    ...
    def test_sliding_window_ttl_expiration(self, mock_from_url, mock_get_pool, mock_cache):
        ...
        sliding_window_check(key="test-key", limit=10, window=60, current_time=200.0)
    ...
    @patch("hub.apps.core.redis_pools.get_redis_cache_pool")
    @patch("redis.from_url")
    def test_sliding_window_redis_unavailable_fails_open(
        self, mock_from_url, mock_get_pool, mock_cache
    ):
        ...
        allowed, count, reset_time = sliding_window_check(
            key="test-key", limit=10, window=60, current_time=100.0
        )
    ...
    @patch("hub.apps.core.redis_pools.get_redis_cache_pool")
    @patch("redis.from_url")
    def test_sliding_window_cache_hit_prevents_redis_call(
        self, mock_from_url, mock_get_pool, mock_cache
    ):
        ...
        allowed, count, _reset_time = sliding_window_check(
            key="test-key",
            limit=10,
            window=60,
    ...
    def test_sliding_window_caches_result(self, mock_from_url, mock_get_pool, mock_cache):
        ...
        cache_key, _cache_value, ttl = mock_cache.set.call_args[0]
...
class MultipleTimeWindowsTest(TestCase):
    ...
    @patch("hub.apps.core.redis_pools.get_redis_cache_pool")
    @patch("redis.from_url")
    def test_window_sliding_prevents_boundary_bursts(
        self, mock_from_url, mock_get_pool, mock_cache
    ):
        ...
        _allowed1, _count1, _reset1 = sliding_window_check(
            key="test-key", limit=10, window=10, current_time=100.0
        )

        # Second request just after window (should still be limited)
        mock_client.zcard.side_effect = [10, 11]  # Still at limit
        _allowed2, _count2, _reset2 = sliding_window_check(
            key="test-key", limit=10, window=10, current_time=110.0
        )
...
class GetRateLimitInfoTest(TestCase):
    ...
    def test_get_rate_limit_info(self, mock_from_url, mock_get_pool):
        ...
        info = get_rate_limit_info(key="test-key", window=60, current_time=150.0)
    ...
    def test_get_rate_limit_info_removes_expired(self, mock_from_url, mock_get_pool):
        ...
        get_rate_limit_info(key="test-key", window=60, current_time=200.0)
    ...
    def test_get_rate_limit_info_redis_unavailable(self, mock_from_url, mock_get_pool):
        ...
        info = get_rate_limit_info(key="test-key", window=60, current_time=100.0)
