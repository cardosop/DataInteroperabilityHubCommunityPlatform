...
class RateLimitingEdgeCaseTest(TestCase):
    ...
    def test_rate_limit_window_boundary(self):
        ...
        # First request
        allowed1, _count1, _reset1 = sliding_window_check(key, limit, window, current_time)
        ...
        # Request at boundary
        allowed2, _count2, _reset2 = sliding_window_check(key, limit, window, boundary_time)
    ...
    def test_rate_limit_concurrent_requests(self):
        ...
        def make_request(index):
            allowed, count, _reset = sliding_window_check(key, limit, TimeWindow.BURST)
    ...
    def test_rate_limit_multiple_windows(self):
        ...
        # Check all windows
        burst_allowed, _burst_count, _burst_reset = sliding_window_check(
            key_burst, 10, TimeWindow.BURST
        )
        sustained_allowed, _sustained_count, _sustained_reset = sliding_window_check(
            key_sustained, 100, TimeWindow.SUSTAINED
        )
        daily_allowed, _daily_count, _daily_reset = sliding_window_check(
    ...
    def test_rate_limit_expired_entries_removal(self):
        ...
        # Make request
        current_time = time.time()
        _allowed1, _count1, _reset1 = sliding_window_check(key, limit, window, current_time)

        # Make request in the future (past window)
        future_time = current_time + window + 1
        allowed2, count2, _reset2 = sliding_window_check(key, limit, window, future_time)
    ...
    def test_rate_limit_per_endpoint_category(self):
        ...
        allowed1, _count1, _reset1 = sliding_window_check(key1, 10, TimeWindow.BURST)
        allowed2, _count2, _reset2 = sliding_window_check(key2, 10, TimeWindow.BURST)
    ...
    def test_rate_limit_reset_time_calculation(self):
        ...
        # Make request
        _allowed, _count, reset_time = sliding_window_check(key, limit, window)
    ...
    def test_rate_limit_cache_behavior(self):
        ...
        allowed1, _count1, _reset1 = sliding_window_check(key, limit, window)

        # Make second request quickly (should use cache)
        time.sleep(0.5)  # noqa: sleep-needed  # INTENTIONAL: test-specific delay  # Within cache TTL
        allowed2, _count2, _reset2 = sliding_window_check(key, limit, window)
    ...
    def test_rate_limit_remaining_calculation(self):
        ...
        # Make requests
        for _i in range(5):
            _allowed, count, _reset = sliding_window_check(key, limit, window)
