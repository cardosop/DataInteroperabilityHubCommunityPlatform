...
class TestTransformationExecutionMethodsIntegration(SDKTestBase):
    ...
    async def test_execute_pipeline_success(self):
        ...
        if not getattr(self, 'storage_available', True):
            pytest.skip(f"Storage not available: {getattr(self, 'storage_error', 'Unknown error')}")
        ...
        async with DataHubClient(self.sdk_config) as client:
            ...
            from hub.apps.transformation.models import PipelineExecution, ExecutionMode
    ...
    async def test_execute_pipeline_with_idempotency_key(self):
        ...
        async with DataHubClient(self.sdk_config) as client:
            ...
            assert execution.idempotency_key == idempotency_key or execution.idempotency_key == str(execution.id), \
    ...
    async def test_list_executions_success(self):
        ...
        from hub.apps.transformation.models import PipelineExecution, ExecutionMode, ExecutionStatus
    ...
    async def test_list_executions_with_status_filter(self):
        ...
        running_execution = await sync_to_async(PipelineExecution.objects.create)(
            pipeline=self.pipeline,
            asset=self.asset,
            execution_mode=ExecutionMode.ASYNC,
            status=ExecutionStatus.RUNNING,
        ...
        completed_execution = await sync_to_async(PipelineExecution.objects.create)(
            pipeline=self.pipeline,
            asset=self.asset,
            execution_mode=ExecutionMode.ASYNC,
