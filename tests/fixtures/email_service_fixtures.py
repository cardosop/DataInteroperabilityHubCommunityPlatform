...
class WorkerServiceFixtures:
    ...
    def get_worker_config() -> Dict[str, Any]:
        ...
        return {
            ...
            'WORKER_MAX_CONCURRENCY_PER_TENANT': int(os.getenv('TEST_WORKER_MAX_CONCURRENCY_PER_TENANT', '2')),
            'WORKER_RESERVED_SLOTS_RATIO': float(os.getenv('TEST_WORKER_RESERVED_SLOTS_RATIO', '0.5')),
            'WORKER_STARVATION_THRESHOLD_SECONDS': int(os.getenv('TEST_WORKER_STARVATION_THRESHOLD_SECONDS', '300')),
            'REDIS_URL': os.getenv('TEST_REDIS_URL', 'redis://localhost:6379/1'),  # Use DB 1 for tests
        }
