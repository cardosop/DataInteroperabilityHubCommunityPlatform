...
class CICDPipelineComprehensiveValidationBase(TestCase):
    ...
    def setUpClass(cls):
        ...
        if not cls.workflows_dir.exists():
            raise FileNotFoundError(
                f"Workflows directory not found: {cls.workflows_dir}"
            )
        ...
        cls.workflow_files = [
            f for f in cls.workflow_files
            if f.name not in ["README.md", "CI_CD_SUMMARY.md"]
        ]
    ...
    def _get_workflow(self, workflow_name: str) -> Optional[Dict[str, Any]]:
...
class CICDPipelineExecutionTest(CICDPipelineComprehensiveValidationBase):
    ...
    def test_pipeline_execution_on_spec_changes(self):
        ...
        if openapi_workflow:
            ...
            if isinstance(push_config, dict):
                ...
                spec_paths = [p for p in paths if 'api' in p.lower() or 'spec' in p.lower() or 'openapi' in p.lower()]
                # If paths are specified, should include spec-related paths
                if paths:
                    self.assertGreater(len(spec_paths), 0,
                                      "OpenAPI workflow should trigger on spec file changes")
    ...
    def test_pipeline_execution_failure_handling(self):
        ...
        self.assertGreater(workflows_with_failure_handling, 0,
                          "At least some workflows should have failure handling")
...
class PipelineFailureHandlingTest(CICDPipelineComprehensiveValidationBase):
    ...
    def test_pipeline_failure_detection(self):
        ...
        self.assertGreater(workflows_with_failure_detection, 0,
                          "Workflows should detect failures")
    ...
    def test_pipeline_failure_notification(self):
        ...
        for wf_name, wf_data in self.workflows.items():
            ...
            for step in steps:
                ...
                if ('notify' in step_name or 'notification' in step_name or
                    'slack' in step_name or 'email' in step_name):
                    if 'failure()' in step_if or 'always()' in step_if:
        ...
        if deploy_workflow:
            ...
            has_notification = any(
                'notify' in s.get('name', '').lower() or
                'notification' in s.get('name', '').lower()
                for s in steps
            )
            # Deploy workflow should have notification steps
            self.assertTrue(has_notification or workflows_with_notifications > 0,
    ...
    def test_pipeline_failure_rollback(self):
        ...
        if deploy_workflow:
            ...
            self.assertTrue(has_rollback or self._has_rollback(deploy_workflow),
    ...
    def test_pipeline_failure_reporting(self):
        ...
        if ci_workflow:
            ...
            has_failure_reporting = any(
                self._has_artifact_upload(s) and
                ('always()' in s.get('if', '') or 'failure()' in s.get('if', ''))
                for s in steps
            )
            self.assertTrue(has_failure_reporting,
                          "CI workflow should report failures via artifacts")
...
class PipelineArtifactValidationTest(CICDPipelineComprehensiveValidationBase):
    ...
    def test_pipeline_artifact_generation(self):
        ...
        self.assertGreater(workflows_with_artifacts, 0,
                          "Workflows should generate artifacts")
        ...
        if ci_workflow:
            ...
            has_test_artifacts = any(
                self._has_artifact_upload(s) and
                ('test' in s.get('name', '').lower() or 'result' in s.get('name', '').lower())
                for s in steps
            )
            self.assertTrue(has_test_artifacts,
                          "CI workflow should upload test result artifacts")
    ...
    def test_pipeline_artifact_validation(self):
        ...
        self.assertGreater(len(artifacts_found), 0,
                          "Workflows should have artifact uploads configured")

        # Validate artifact names are meaningful
        for artifact in artifacts_found:
            self.assertIsNotNone(artifact.get('workflow'),
                               "Artifact should be associated with a workflow")
        ...
        for wf_name, wf_data in self.workflows.items():
            ...
            for step in steps:
                if self._has_artifact_upload(step):
                    ...
                    if 'retention-days' in step_content.lower() or 'retention' in step_content.lower():
    ...
    def test_pipeline_artifact_retrieval(self):
        ...
        if test_results_workflow:
            ...
            has_download_related = any(
                'download' in s.get('name', '').lower() or
                'artifact' in s.get('name', '').lower() or
                'github-script' in s.get('uses', '').lower()
                for s in steps
            )
            self.assertTrue(has_download or has_download_related or workflows_with_downloads > 0,
                          "Test results reporting workflow should download artifacts")
    ...
    def test_pipeline_artifact_security(self):
        ...
        self.assertGreater(artifacts_checked, 0,
                          "Should validate artifact security")
    ...
    def test_workflow_yaml_syntax_validation(self):
        ...
        self.assertEqual(len(invalid_workflows), 0,
                         f"All workflows should have valid YAML syntax. "
                         f"Invalid: {invalid_workflows}")
        ...
        for wf_name, wf_data in self.workflows.items():
            ...
            for field in required_fields:
                ...
                self.assertTrue(has_field,
                            f"Workflow {wf_name} should have '{field}' field")

            # Check jobs have steps
            jobs = workflow_data.get('jobs', {})
            self.assertGreater(len(jobs), 0,
                             f"Workflow {wf_name} should have at least one job")

            for job_name, job_data in jobs.items():
                self.assertIn('steps', job_data,
                            f"Job {job_name} in {wf_name} should have 'steps'")
    ...
    def test_workflow_reviewer_integration(self):
        ...
        if self.workflow_reviewer:
            ...
            self.assertGreater(len(self.workflow_reviewer.workflows), 0,
                             "Workflow reviewer should load workflows")

            # Verify endpoint extraction works
            verification = self.workflow_reviewer.verify_workflow_reference_extraction()
            self.assertTrue(verification.get('extraction_working', False),
