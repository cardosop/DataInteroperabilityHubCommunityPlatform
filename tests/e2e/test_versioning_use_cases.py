        ...
        if isinstance(versions_data, list):
            ...
            if len(versions) >= 2:
                self.assertGreaterEqual(
                    int(versions[0].get('version', 0)),
                    int(versions[1].get('version', 0))
                )
...
class VersionComparisonUseCasesTest(E2ETestBase):
    ...
    def test_compare_versions_via_api_success(self):
        ...
        field_changes = [c for c in changes if isinstance(c, dict) and c.get('field_name') == 'email']
    ...
    def test_view_schema_diff_success(self):
        ...
        comparison = VersionComparisonService.compare_versions(
            old_version=self.dataset_v1,
            new_version=self.dataset_v2,
    ...
    def test_analyze_version_impact_success(self):
        ...
        impact = analyzer.analyze_impact(
            dataset_id=str(self.dataset_v2.id),
...
class TimeTravelUseCasesTest(E2ETestBase):
    ...
    def setUp(self):
        ...
        self.dataset_v2 = Dataset(
            ...
            asset=self.asset,
            file=self.file2,
            schema_json={"fields": [{"name": "id", "data_type": "integer"}, {"name": "name", "data_type": "string"}]},
            row_count=150,
            format="CSV",
            version=2,
            created_by=self.user
    ...
    def test_get_snapshots_for_dataset_success(self):
        ...
        snapshot1 = TimeTravelQuery.create_snapshot(
            dataset=self.dataset_v1,
            snapshot_type="FULL"
        )
        snapshot2 = TimeTravelQuery.create_snapshot(
            dataset=self.dataset_v2,
            snapshot_type="SCHEMA_ONLY"
        )
