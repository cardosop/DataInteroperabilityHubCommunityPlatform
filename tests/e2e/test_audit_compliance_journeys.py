...
class ComplianceOfficerTests(E2ETestBase):
    ...
    def test_view_compliance_report_success(self):
        ...
        response = self.client.get(f"/api/v1/compliance/runs/{self.compliance_run_id}/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_filter_compliance_runs_by_asset(self):
        ...
        response = self.client.get("/api/v1/compliance/runs/", {"asset_id": str(self.asset_id)})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_filter_compliance_runs_by_status(self):
        ...
        response = self.client.get(
            "/api/v1/compliance/runs/", {"status": ComplianceRunStatus.SUCCEEDED}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        if results:
            for run in results:
                ...
                self.assertIn(
                    run_status, [ComplianceRunStatus.SUCCEEDED, ComplianceRunStatus.PENDING]
                )
...
class ComplianceOfficerWorkflowTests(E2ETestBase):
    ...
    def test_review_compliance_for_asset(self):
        ...
        response = self.client.get("/api/v1/compliance/runs/", {"asset_id": str(asset_id)})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
