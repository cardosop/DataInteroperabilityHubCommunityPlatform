...
def api_key_for_cli(test_user_and_tenant):
    ...
    api_key = APIKey.objects.create(
        user=user, name="CLI E2E Test Key", tenant=tenant, key_hash=key_hash
    )
...
class CLIJourneyTest:
    ...
    def test_cli_authentication_with_api_key(self, runner, temp_config_dir, api_key_for_cli):
        ...
        assert str(global_config_file) == str(config_file), (
            f"Global config using wrong file. Expected {config_file}, got {global_config_file}"
        )
        ...
        assert file_api_key == plaintext_key, (
            f"API key not in file. Expected {plaintext_key}, got {file_api_key}. File content: {file_content}"
        )

        # Reload global config to ensure it reads from the patched file
        global_config._load()
        global_api_key = global_config.get_api_key()
        assert global_api_key == plaintext_key, (
            f"Global config API key not set. Expected {plaintext_key}, got {global_api_key}. Config file path: {global_config._get_config_file()}"
        )
        ...
        assert str(config_file_path) == str(config_file), (
            f"Config instance using wrong file path. Expected {config_file}, got {config_file_path}"
        )
        ...
        assert file_content_after.get("api_key") == plaintext_key, (
            f"API key not in file after reload. Expected {plaintext_key}, got {file_content_after.get('api_key')}. File content: {file_content_after}"
        )
        ...
        assert api_key == plaintext_key, (
            f"API key not set correctly in file. Expected {plaintext_key}, got {api_key}. Config file: {config_file}, File exists: {config_file.exists()}, Config file path used: {config_file_path}, Config dict: {config._config}, File content: {file_content_after}"
        )
        ...
        assert is_authenticated is True, (
            f"ensure_authenticated returned False. API key from config: {config.get_api_key()}, API key from auth_manager: {auth_manager.config.get_api_key()}, Config file: {config_file}, Config dict: {config._config}"
        )
    ...
    def test_cli_asset_management_journey(
        ...
        _user, _tenant = test_user_and_tenant
    ...
    def test_cli_file_management_journey(
        self, runner, temp_config_dir, api_key_for_cli, test_user_and_tenant
    ):
    ...
    def test_cli_job_management_journey(
        self, runner, temp_config_dir, api_key_for_cli, test_user_and_tenant
    ):
    ...
    def test_cli_logout_clears_tokens(self, runner, temp_config_dir):
        ...
        assert config.get_api_key() == "api-key", (
            f"API key should remain after logout. Got: {config.get_api_key()}"
        )
...
"""
    ...
    def test_cli_contract_validate_journey(
        self, runner, temp_config_dir, api_key_for_cli, test_user_and_tenant
    ):
    ...
    def test_cli_assets_update_journey(
        ...
        _user, tenant = test_user_and_tenant

        # Create asset via API first
        asset = Asset.objects.create(
            tenant=tenant, name="CLI Test Asset", key="cli-test-asset", status=AssetStatus.DRAFT
        )
