...
class DQServiceE2ETest(E2ETestBase):
    ...
    def test_dq_run_execution_updates_status(self):
        ...
        self.assertNotEqual(
            dq_run.status, DQRunStatus.PENDING,
        ...
        if dq_run.status == DQRunStatus.SUCCEEDED:
            self.assertIsNotNone(dq_run.overall_status,
                                 "Succeeded DQ run must have overall_status")
            self.assertIsNotNone(dq_run.quality_score,
                                 "Succeeded DQ run must have quality_score")
    ...
    def test_dq_run_updates_asset_dq_status(self):
        ...
        asset_before = Asset.objects.get(id=asset_id)
        dq_status_before = asset_before.dq_status
        ...
        if dq_run.status == DQRunStatus.SUCCEEDED:
            ...
            self.assertIsNotNone(dq_run.completed_at,
                                 "Failed DQ run should have completed_at timestamp")
    ...
    def test_dq_run_result_structure(self):
        ...
        if dq_run.status == DQRunStatus.SUCCEEDED:
            ...
            self.assertIsInstance(dq_run.checks_json, list,
                                 "checks_json should be a list of check results")
        else:
            # If FAILED, verify there is a completed_at and a linked job with error
            self.assertIsNotNone(dq_run.completed_at,
                                 "Failed DQ run should have completed_at")
            job = Job.objects.filter(type=JobType.DQ_RUN, resource_id=dq_run_id).first()
            self.assertIsNotNone(job)
    ...
    def test_dq_run_error_handling(self):
        ...
        if response.status_code in [status.HTTP_400_BAD_REQUEST, status.HTTP_500_INTERNAL_SERVER_ERROR]:
            ...
            self.assertTrue(
                any(kw in error_msg for kw in ['empty', 'no headers', 'schema inference', 'no data']),
