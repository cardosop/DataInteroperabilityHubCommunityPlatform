...
class APIDocumentationE2ETest(E2ETestBase):
    ...
    def test_openapi_schema_includes_all_endpoints(self):
        ...
        self.assertTrue(
            any("/auth/login" in key for key in path_keys),
            f"Auth login not found in {path_keys[:5]}",
        )
        self.assertTrue(
            any("/tenants" in key for key in path_keys), f"Tenants not found in {path_keys[:5]}"
        )
        self.assertTrue(
            any("/users" in key for key in path_keys), f"Users not found in {path_keys[:5]}"
        )
        self.assertTrue(
            any("/assets" in key for key in path_keys), f"Assets not found in {path_keys[:5]}"
        )
        self.assertTrue(
            any("/contracts" in key for key in path_keys), f"Contracts not found in {path_keys[:5]}"
        )
        self.assertTrue(
            any("/marketplace/listings" in key for key in path_keys),
            f"Marketplace listings not found in {path_keys[:5]}",
        )
