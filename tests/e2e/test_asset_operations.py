...
class AssetOperationsE2ETest(E2ETestBase):
    ...
    def test_activate_asset_success(self):
        ...
        file_id = self.init_file_upload(name='activate_test.csv', content_type='text/csv', size=len(test_content))
        ...
        if response.status_code != status.HTTP_200_OK:
            ...
            # Log for debugging
            print(f"Activation failed: {response.status_code}, error: {blockers}, details: {error_details}")
    ...
    def test_activate_asset_without_contract_fails(self):
        ...
        self.assertTrue(
            'contract' in error_msg or 
            'contract' in str(details).lower() or
            'requirements not met' in error_msg,
    ...
    def test_attach_dataset_to_asset(self):
        ...
        test_content = b'col1,col2\nval1,val2'
        content_hash = hashlib.sha256(test_content).hexdigest()
        file_id = self.init_file_upload(name='attach_test.csv', content_type='text/csv', size=len(test_content))
        self.complete_file_upload(file_id, content_sha256=content_hash, test_content=test_content)
        dataset_id = self.create_dataset(file_id, asset_id)
