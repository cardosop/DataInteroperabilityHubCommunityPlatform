...
class ExternalResourceSemanticDiscoveryE2ETest(E2ETestBase):
    ...
    def test_semantic_discovery_of_federated_assets_with_external_resources(self):
        ...
        discovery_result = client.query_sparql(
            query=discovery_query, output_format="json", tenant_id=str(self.tenant.id)
        )
        ...
        metadata_result = client.query_sparql(
            query=metadata_query, output_format="json", tenant_id=str(self.tenant.id)
        )
        # Handle circuit breaker failures gracefully
        ...
        filter_result = client.query_sparql(
            query=filter_query, output_format="json", tenant_id=str(self.tenant.id)
        )
        # Handle circuit breaker failures gracefully
