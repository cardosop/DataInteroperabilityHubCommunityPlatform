...
from hub.apps.scheduled_ingestion.models import (
    ScheduledIngestion, ScheduledIngestionStatus, ScheduledIngestionRunStatus
)
...
class TestJOURNEYDE001ProgrammaticContractFirst(E2ETestBase):
    ...
    def test_happy_path_programmatic_contract_first(self):
        ...
        if asset.status != AssetStatus.ACTIVE:
            ...
            self.assertIn(activate_response.status_code, [
                status.HTTP_200_OK,
                status.HTTP_202_ACCEPTED,
                status.HTTP_400_BAD_REQUEST,  # Activation requirements not met
                status.HTTP_409_CONFLICT  # Version mismatch
            ])
    ...
    def test_error_invalid_contract_format(self):
        ...
        self.assertIn(response.status_code, [
            ...
            status.HTTP_201_CREATED  # Created but marked as invalid
        ])
    ...
    def test_error_schema_inference_failure(self):
        ...
        self.assertIn(dataset_response.status_code, [
...
class TestJOURNEYDE002ExternalComplianceScan(E2ETestBase):
    ...
    def test_happy_path_external_compliance_scan(self):
        ...
        self.assertIn(scan_response.status_code, [
            status.HTTP_201_CREATED,
            status.HTTP_202_ACCEPTED,
        ...
        if scan_response.status_code in [status.HTTP_201_CREATED, status.HTTP_202_ACCEPTED]:
            ...
            report_response = client.get(
                f'/api/v1/compliance-runs/{compliance_run_id}/',
                format='json'
            )
...
class TestJOURNEYDE004MonitorIngestionJobs(E2ETestBase):
    ...
    def test_list_scheduled_ingestions(self):
        ...
        if created_count == 0:
            pytest.skip("Scheduled ingestion endpoint not available (405 Method Not Allowed)")
        ...
        ingestions = list_response.data.get('results', list_response.data) if isinstance(list_response.data, dict) else list_response.data
    ...
    def test_get_scheduled_ingestion_details(self):
        ...
        if detail_response.status_code == status.HTTP_404_NOT_FOUND:
            # Try alternative path
            detail_response = client.get(
                f'/api/v1/scheduled-ingestions/scheduled-ingestions/{ingestion_id}/',
                format='json'
            )
    ...
    def test_list_ingestion_runs(self):
        ...
        runs_response = client.get(
            f'/api/v1/scheduled-ingestions/{ingestion_id}/runs/',
            format='json'
        )
        ...
        runs = runs_response.data.get('results', runs_response.data) if isinstance(runs_response.data, dict) else runs_response.data
    ...
    def test_trigger_ingestion_manually(self):
        ...
        self.assertIn(trigger_response.status_code, [
            status.HTTP_200_OK,
            status.HTTP_202_ACCEPTED,
...
class TestJOURNEYDE005CICDIntegration(E2ETestBase):
    ...
    def test_cicd_contract_validation(self):
        ...
        if hasattr(contract, 'validation_status'):
            validation_passed = contract.validation_status in [
...
class TestJOURNEYDE006SchemaEvolution(E2ETestBase):
    ...
    def test_schema_version_creation(self):
        ...
        version_response = client.post(
            f'/api/v1/datasets/datasets/{dataset_id}/versions/',
            {
        ...
        self.assertIn(version_response.status_code, [
            status.HTTP_201_CREATED,
            status.HTTP_202_ACCEPTED,
            status.HTTP_404_NOT_FOUND
        ])
    ...
    def test_schema_comparison(self):
        ...
        if versions_response.status_code == status.HTTP_200_OK:
            versions = versions_response.data.get('results', versions_response.data) if isinstance(versions_response.data, dict) else versions_response.data
            if len(versions) >= 2:
                # Compare versions
                compare_response = client.get(
                    f'/api/v1/datasets/datasets/{dataset_id}/versions/compare/',
                    {
                        'version1': versions[0]['id'],
                        'version2': versions[1]['id']
                    },
                    format='json'
                )
                # May return comparison or 404
                self.assertIn(compare_response.status_code, [
                    status.HTTP_200_OK,
                    status.HTTP_404_NOT_FOUND
                ])
    ...
    def test_schema_impact_analysis(self):
        ...
        impact_response = client.post(
            f'/api/v1/contracts/contracts/{contract_id}/analyze-impact/',
            {
                'proposed_changes': {
        ...
        self.assertIn(impact_response.status_code, [
            status.HTTP_200_OK,
            status.HTTP_202_ACCEPTED,
            status.HTTP_404_NOT_FOUND
        ])
