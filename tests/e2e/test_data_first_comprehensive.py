...
class DataFirstFlowSuccessTests(E2ETestBase):
    ...
    def test_complete_data_first_journey_happy_path(self):
        ...
        asset_id = self.create_asset(
            key="customer-orders", name="Customer Orders", description="Customer order data"
        )

        # Step 2: Upload file (use default size from helper)
        file_id = self.init_file_upload(name="orders.csv", content_type="text/csv")
        ...
        self.assertIn(
            "validation_status", validate_response, "Response must include validation_status"
        )
        ...
        self.assertIn(
            validate_response["validation_status"],
            ["VALID", "WARNING_ONLY", "INVALID"],
    ...
    def test_data_first_with_json_file(self):
        ...
        file_id = self.init_file_upload(
            name="data.json", content_type="application/json", size=len(json_content)
        )
...
class DataFirstFlowFailureTests(E2ETestBase):
    ...
    def test_dq_failure_blocks_activation(self):
        ...
        if dq_run.status == DQRunStatus.FAILED:
            ...
            self.assertIn(
                "blocked", activate_response.data.get("error", {}).get("message", "").lower()
            )
...
class DataFirstFlowEdgeCasesTests(E2ETestBase):
    ...
    def test_empty_file_handling(self):
        ...
        response = self.complete_file_upload(
            file_id, content_sha256=content_hash, test_content=empty_content
        )
        ...
        if response.status_code == status.HTTP_201_CREATED:
            ...
            self.assertTrue(
                "empty" in error_msg
                or "no headers" in error_msg
                or "schema inference" in error_msg
            ...
            self.fail(
                f"Unexpected status code {response.status_code} for empty file: {response.data if hasattr(response, 'data') else 'N/A'}"
            )

    def test_very_large_file_handling(self):
        """Test handling of very large file (boundary condition)"""
        self.create_asset(key="large-file", name="Large File")
        ...
        response = self.client.post(
            "/api/v1/files/init/",
            {"name": "large.csv", "content_type": "text/csv", "size": large_size},
            format="json",
        )
    ...
    def test_malformed_csv_handling(self):
        ...
        try:
            ...
            self.assertIn(
                "format",
                str(e).lower() + str(type(e).__name__).lower(),
        ...
        if response.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR:
            ...
            if "name resolution" in error_msg.lower() or "temporary failure" in error_msg.lower():
                pytest.skip(  # noqa: skip-in-body — runtime service dependency
                    "DataContract service DNS resolution failed (service may not be accessible at configured URL)"
                )
...
class DataFirstFlowSchemaInferenceTests(E2ETestBase):
    ...
    def test_schema_inference_with_nested_json(self):
        ...
        file_id = self.init_file_upload(
            name="nested.json", content_type="application/json", size=len(nested_json_content)
        )
