...
class HealthCheckE2ETest(E2ETestBase):
    ...
    def test_semantic_service_health(self):
        ...
        if not check_service_health(semantic_url, timeout=5):
            pytest.skip(f"Semantic service not available at {semantic_url}")  # noqa: skip-in-body — runtime service dependency
        ...
        self.assertIn(
            "fuseki", data, "Healthy semantic service response must include 'fuseki' status"
        )
    ...
    def test_datacontract_service_health(self):
        ...
        if not check_service_health(datacontract_url, timeout=5):
            pytest.skip(f"DataContract service not available at {datacontract_url}")  # noqa: skip-in-body — runtime service dependency
    ...
    def test_compliance_service_health(self):
        ...
        if not check_service_health(compliance_url, timeout=5):
            pytest.skip(f"Compliance service not available at {compliance_url}")  # noqa: skip-in-body — runtime service dependency
    ...
    def test_dq_service_health(self):
        ...
        if not check_service_health(dq_url, timeout=5):
            pytest.skip(f"DQ service not available at {dq_url}")  # noqa: skip-in-body — runtime service dependency
