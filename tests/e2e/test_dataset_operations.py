...
class DatasetOperationsE2ETest(E2ETestBase):
    ...
    def test_create_dataset_success(self):
        ...
        file_id = self.init_file_upload(
            name="dataset_test.csv", content_type="text/csv", size=len(test_content)
        )
    ...
    def test_create_dataset_with_json_format(self):
        ...
        file_id = self.init_file_upload(
            name="dataset_test.json", content_type="application/json", size=len(json_content)
        )
    ...
    def test_list_datasets_with_filters(self):
        ...
        file_id1 = self.init_file_upload(
            name="dataset1.csv", content_type="text/csv", size=len(test_content1)
        )
        self.complete_file_upload(
            file_id1, content_sha256=content_hash1, test_content=test_content1
        )
        ...
        file_id2 = self.init_file_upload(
            name="dataset2.csv", content_type="text/csv", size=len(test_content2)
        )
        self.complete_file_upload(
            file_id2, content_sha256=content_hash2, test_content=test_content2
        )
    ...
    def test_get_dataset_details(self):
        ...
        content_hash = hashlib.sha256(test_content).hexdigest()
        file_id = self.init_file_upload(
            name="get_test.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id, content_sha256=content_hash, test_content=test_content)
        dataset_id = self.create_dataset(file_id, asset_id)
    ...
    def test_dataset_versioning(self):
        ...
        content_hash1 = hashlib.sha256(test_content1).hexdigest()
        file_id1 = self.init_file_upload(
            name="v1.csv", content_type="text/csv", size=len(test_content1)
        )
        self.complete_file_upload(
            file_id1, content_sha256=content_hash1, test_content=test_content1
        )
        ...
        content_hash2 = hashlib.sha256(test_content2).hexdigest()
        file_id2 = self.init_file_upload(
            name="v2.csv", content_type="text/csv", size=len(test_content2)
        )
        self.complete_file_upload(
            file_id2, content_sha256=content_hash2, test_content=test_content2
        )
    ...
    def test_dataset_schema_inference(self):
        ...
        content_hash = hashlib.sha256(test_content).hexdigest()
        file_id = self.init_file_upload(
            name="schema_test.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id, content_sha256=content_hash, test_content=test_content)
        dataset_id = self.create_dataset(file_id, asset_id)
    ...
    def test_dataset_sample_data_extraction(self):
        ...
        content_hash = hashlib.sha256(test_content).hexdigest()
        file_id = self.init_file_upload(
            name="sample_test.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id, content_sha256=content_hash, test_content=test_content)
        dataset_id = self.create_dataset(file_id, asset_id)
    ...
    def test_dataset_row_count_estimation(self):
        ...
        content_hash = hashlib.sha256(test_content).hexdigest()
        file_id = self.init_file_upload(
            name="rowcount_test.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id, content_sha256=content_hash, test_content=test_content)
        dataset_id = self.create_dataset(file_id, asset_id)
    ...
    def test_multiple_datasets_per_asset(self):
        ...
        for i in range(3):
            ...
            content_hash = hashlib.sha256(test_content).hexdigest()
            file_id = self.init_file_upload(
                name=f"dataset_{i}.csv", content_type="text/csv", size=len(test_content)
            )
            self.complete_file_upload(
        ...
        self.assertEqual(
            dataset_versions, [1, 2, 3], "Versions should be created in sequential order"
        )
