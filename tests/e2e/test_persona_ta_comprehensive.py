...
class JourneyTA001OnboardNewUserTests(E2ETestBase):
    ...
    def test_assign_role_to_user(self):
        ...
        user = User.objects.create_user(
            email="norole@test-tenant.com", tenant=self.tenant, status=UserStatus.ACTIVE
        )

        # Assign role
        response = self.client.post(
            f"/api/v1/users/{user.id}/roles/",
            {"role_id": str(self.data_provider_role.id), "action": "assign"},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_remove_role_from_user(self):
        ...
        user = User.objects.create_user(
            email="withrole@test-tenant.com", tenant=self.tenant, status=UserStatus.ACTIVE
        )
        ...
        response = self.client.post(
            f"/api/v1/users/{user.id}/roles/",
            {"role_id": str(self.data_provider_role.id), "action": "remove"},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_user_details(self):
        ...
        # Create user
        user = User.objects.create_user(
            email="toupdate@test-tenant.com", tenant=self.tenant, status=UserStatus.ACTIVE
        )

        # Update user
        response = self.client.patch(
            f"/api/v1/users/{user.id}/", {"display_name": "Updated Name"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_disable_user(self):
        ...
        user = User.objects.create_user(
            email="todisable@test-tenant.com", tenant=self.tenant, status=UserStatus.ACTIVE
        )
    ...
    def test_list_users_with_filters(self):
        ...
        User.objects.create_user(
            email="active@test-tenant.com", tenant=self.tenant, status=UserStatus.ACTIVE
        )
        User.objects.create_user(
            email="invited@test-tenant.com", tenant=self.tenant, status=UserStatus.INVITED
        )
    ...
    def test_error_duplicate_email(self):
        ...
        # Create first user
        User.objects.create_user(
            email="duplicate@test-tenant.com", tenant=self.tenant, status=UserStatus.ACTIVE
        )

        # Try to create duplicate
        response = self.client.post(
            "/api/v1/users/",
            {"email": "duplicate@test-tenant.com", "display_name": "Duplicate User"},
            format="json",
        )
    ...
    def test_error_invite_existing_user(self):
        ...
        # Create user first
        User.objects.create_user(
            email="existing@test-tenant.com", tenant=self.tenant, status=UserStatus.ACTIVE
        )

        # Try to invite existing user
        response = self.client.post(
            "/api/v1/users/invite/",
            {"email": "existing@test-tenant.com", "display_name": "Existing User"},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class JourneyTA002ConfigureTenantSettingsTests(E2ETestBase):
    ...
    def test_update_compliance_regimes(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            set((get_response_data(response) or {})["allowed_compliance_regimes"]),
            {"GDPR", "HIPAA", "SOX"},
        )
        self.assertEqual(
            set((get_response_data(response) or {})["default_compliance_regimes"]),
            {"GDPR", "HIPAA"},
        )
    ...
    def test_update_rate_limits(self):
        ...
        """
        rate_limits = {
            "dq_runs": {"burst_per_10s": 25, "sustained_per_min": 70, "daily_cap": 12000},
            "file_uploads": {"burst_per_10s": 15, "sustained_per_min": 40},
        }

        response = self.client.patch(
            f"/api/v1/tenants/{self.tenant.id}/config/", {"rate_limits": rate_limits}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_job_concurrency_limits(self):
        ...
        response = self.client.patch(
            f"/api/v1/tenants/{self.tenant.id}/config/",
            {"max_job_concurrency": 10, "max_queued_jobs": 100},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class JourneyTA003MonitorTenantUsageTests(E2ETestBase):
    ...
    def test_list_tenant_users_count(self):
        ...
        for i in range(5):
            User.objects.create_user(
                email=f"user{i}@test-tenant.com", tenant=self.tenant, status=UserStatus.ACTIVE
            )
...
class TenantAdminUseCasesTests(E2ETestBase):
    ...
    def test_complete_user_management_workflow(self):
        ...
        # Update user
        response = self.client.patch(
            f"/api/v1/users/{user_id}/", {"display_name": "Updated Workflow User"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.post(
            ...
            {"role_id": str(self.data_consumer_role.id), "action": "assign"},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class TenantAdminErrorScenariosTests(E2ETestBase):
    ...
    def test_error_create_user_missing_email(self):
        ...
        """
        response = self.client.post(
            "/api/v1/users/", {"display_name": "No Email User"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_error_assign_nonexistent_role(self):
        ...
        """
        user = User.objects.create_user(
            email="testuser@test-tenant.com", tenant=self.tenant, status=UserStatus.ACTIVE
        )

        fake_role_id = str(uuid.uuid4())
        response = self.client.post(
            f"/api/v1/users/{user.id}/roles/",
            {"role_id": fake_role_id, "action": "assign"},
            format="json",
        )
    ...
    def test_error_update_config_without_permission(self):
        ...
        regular_user = User.objects.create_user(
            email="regular@test-tenant.com", tenant=self.tenant, status=UserStatus.ACTIVE
        )
    ...
    def test_error_delete_self(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn(
            "cannot delete themselves", (get_response_data(response) or {}).get("error", "").lower()
        )
