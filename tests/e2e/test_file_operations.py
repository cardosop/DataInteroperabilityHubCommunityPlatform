...
class FileOperationsE2ETest(E2ETestBase):
    ...
    def test_simple_file_upload_success(self):
        ...
        file_id = self.init_file_upload(
            name="test_simple.csv", content_type="text/csv", size=len(test_content)
        )
        ...
        self.verify_file_in_s3(
            file_id, expected_content=test_content, expected_size=len(test_content)
        )
    ...
    def test_chunked_file_upload_success(self):
        ...
        file_id = self.init_file_upload(
            name="test_large.csv", content_type="text/csv", size=len(test_content)
        )
    ...
    def test_file_upload_with_presigned_url(self):
        ...
        response = self.client.post(
            "/api/v1/files/init/",
            {"name": "presigned_test.csv", "content_type": "text/csv", "size": len(test_content)},
            format="json",
        )
    ...
    def test_file_download_success(self):
        ...
        # Upload file first
        file_id = self.init_file_upload(
            name="download_test.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id, content_sha256=content_hash, test_content=test_content)
    ...
    def test_file_download_deleted_file_fails(self):
        ...
        # Upload file
        file_id = self.init_file_upload(
            name="delete_test.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id, content_sha256=content_hash, test_content=test_content)
        ...
        # 404 (not found), or 410 (gone)  # noqa: broad-status-codes
    ...
    def test_delete_file_success(self):
        ...
        # Verify audit log created
        self.verify_audit_log(
            action="FILE_DELETED", resource_type="FILE", resource_id=file_id, result="SUCCESS"
        )
    ...
    def test_file_upload_with_invalid_hash_fails(self):
        ...
        # Initialize upload
        file_id = self.init_file_upload(
            name="invalid_hash.csv", content_type="text/csv", size=len(test_content)
        )

        # Try to complete with wrong hash
        response = self.client.post(
            f"/api/v1/files/{file_id}/complete/", {"content_sha256": wrong_hash}, format="json"
        )
    ...
    def test_file_upload_with_size_mismatch_fails(self):
        ...
        response = self.client.post(
            "/api/v1/files/init/",
            {"name": "size_mismatch.csv", "content_type": "text/csv", "size": declared_size},
            format="json",
        )
        ...
        content_hash = hashlib.sha256(test_content).hexdigest()
        response = self.complete_file_upload(
            file_id, content_sha256=content_hash, test_content=test_content
        )
    ...
    def test_file_upload_unsupported_format(self):
        ...
        response = self.client.post(
            "/api/v1/files/init/",
            {"name": "test.xyz", "content_type": "application/xyz", "size": 100},
            format="json",
        )
    ...
    def test_file_upload_concurrent_uploads(self):
        ...
        file_id1 = self.init_file_upload(
            name="concurrent1.csv", content_type="text/csv", size=len(test_content1)
        )
        file_id2 = self.init_file_upload(
            name="concurrent2.csv", content_type="text/csv", size=len(test_content2)
        )
    ...
    def test_file_status_transitions(self):
        ...
        file_id = self.init_file_upload(
            name="status_test.csv", content_type="text/csv", size=len(test_content)
        )
    ...
    def test_get_file_metadata(self):
        ...
        # Upload file
        file_id = self.init_file_upload(
            name="metadata_test.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id, content_sha256=content_hash, test_content=test_content)
    ...
    def test_file_upload_verifies_s3_storage(self):
        ...
        # Upload file
        file_id = self.init_file_upload(
            name="s3_test.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id, content_sha256=content_hash, test_content=test_content)
