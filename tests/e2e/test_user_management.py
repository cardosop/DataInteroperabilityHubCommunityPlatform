...
class UserManagementE2ETest(E2ETestBase):
    ...
    def setUp(self):
        ...
        self.tenant_admin_role, _created = Role.objects.get_or_create(
            tenant=self.tenant,
            name="TENANT_ADMIN",
    ...
    def test_create_user_with_roles(self):
        ...
        data_consumer_role, _ = Role.objects.get_or_create(
            tenant=self.tenant, name="DATA_CONSUMER", defaults={"description": "Data consumer role"}
        )
        ...
        self.assertEqual(
            (get_response_data(response) or {})["status"], UserStatus.ACTIVE
        )  # No invitation
        ...
        self.verify_audit_log(
            action="USER_CREATED", resource_type="USER", resource_id=user.id, result="SUCCESS"
        )
    ...
    def test_update_user_display_name(self):
        ...
        response = self.client.patch(
            f"/api/v1/users/{test_user.id}/", {"display_name": "Updated Name"}, format="json"
        )
        ...
        # Verify audit log created
        self.verify_audit_log(
            action="USER_UPDATED", resource_type="USER", resource_id=test_user.id, result="SUCCESS"
        )
    ...
    def test_update_user_roles_increments_token_version(self):
        ...
        response = self.client.post(
            ...
            {"role_id": str(data_provider_role.id), "action": "assign"},
            format="json",
        )
    ...
    def test_invite_user(self):
        ...
        response = self.client.post(
            "/api/v1/users/invite/",
            {"email": "invitee@example.com", "display_name": "Invitee User"},
            format="json",
        )
        ...
        # Verify audit log created
        self.verify_audit_log(
            action="USER_INVITED", resource_type="USER", resource_id=user.id, result="SUCCESS"
        )
    ...
    def test_delete_user_without_resources(self):
        ...
        audit_event = (
            AuditEvent.objects.filter(
                action="USER_DELETED", resource_type="USER", tenant=self.tenant
            )
            .order_by("-timestamp")
            .first()
        )
    ...
    def test_delete_user_with_resources_soft_delete(self):
        ...
        Asset.objects.create(
            tenant=self.tenant, key="test-asset", name="Test Asset", created_by=test_user
        )
    ...
    def test_delete_self_fails(self):
        ...
        self.assertIn(
            "cannot delete themselves", (get_response_data(response) or {})["error"].lower()
        )
    ...
    def test_create_user_duplicate_email_fails(self):
        ...
        User.objects.create_user(
            email="duplicate@example.com", password="testpass123", tenant=self.tenant
        )
    ...
    def test_create_user_invalid_email_fails(self):
        ...
        response = self.client.post(
            "/api/v1/users/",
            {"email": "invalid-email", "display_name": "Invalid User", "send_invitation": False},
            format="json",
        )
    ...
    def test_update_user_status(self):
        ...
        response = self.client.patch(
            f"/api/v1/users/{test_user.id}/", {"status": UserStatus.DISABLED}, format="json"
        )
