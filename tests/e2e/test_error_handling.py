...
class ErrorHandlingE2ETest(E2ETestBase):
    ...
    def test_validation_error_format(self):
        ...
        if 'error' in (get_response_data(response) or {}):
            ...
            self.assertIn(
                'VALIDATION', error.get('code', '').upper()
            )
    ...
    def test_forbidden_error_format(self):
        ...
        asset_id = self.create_asset(
            key='forbidden-test', name='Forbidden Test'
        )
        ...
        if 'field_errors' in details:
            ...
            for field_error in field_errors:
                ...
                self.assertIsInstance(
                    field_error['field'], str
                )
                self.assertGreater(
                    len(field_error['field']), 0
                )
    ...
    def test_error_http_status_mapping(self):
        ...
        test_cases = [
            ...
            (
                '/api/v1/assets/', 'GET',
                status.HTTP_401_UNAUTHORIZED, 'AUTH',
            ),
        ]
