...
class JourneyDEV001BuildCustomIntegrationTests(E2ETestBase):
    ...
    def test_authenticate_with_api_key(self):
        ...
        response = self.client.post(
            "/api/v1/auth/api-keys/", {"name": "Test API Key", "scopes": []}, format="json"
        )
            ...
            "original_raw": json.dumps(
                {
                    ...
                    "info": {"name": "Test Contract API", "version": "1.0.0"},
                    "schema": {
                        "fields": [
                            {"name": "id", "type": "string"},
    ...
    def test_error_invalid_request_data(self):
        ...
        response = self.client.post(
            "/api/v1/assets/", {"description": "Missing required fields"}, format="json"
        )
...
class JourneyDEV004SetUpWebhooksTests(E2ETestBase):
    ...
    def test_test_webhook_delivery(self):
        ...
        response = self.client.post(
            f"/api/v1/webhooks/webhooks/{webhook.id}/test/", {}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        self.assertGreaterEqual(
            deliveries.count(), 1, "Expected at least 1 webhook delivery after test trigger"
        )
    ...
    def test_webhook_signature_verification(self):
        ...
        expected_signature = hmac.new(
            b"test-secret-key", test_payload.encode("utf-8"), hashlib.sha256
        ).hexdigest()
    ...
    def test_error_invalid_webhook_url(self):
        ...
        response = self.client.post(
            "/api/v1/webhooks/webhooks/",
            {"url": "not-a-valid-url", "event_types": ["asset.created"], "secret": "secret-key"},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class ExternalDeveloperErrorScenariosTests(E2ETestBase):
    ...
    def test_error_webhook_delivery_failure(self):
        ...
        Asset.objects.create(
            tenant=self.tenant, key="trigger-asset", name="Trigger Asset", status=AssetStatus.ACTIVE
        )
