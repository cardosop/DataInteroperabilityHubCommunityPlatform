...
class ContractMigrationE2ETest(E2ETestBase):
    ...
    def test_on_write_migration_strategy(self):
        ...
        # Note: This depends on implementation - may need to check after normalization
    ...
    def test_on_read_migration_strategy(self):
        ...
        if not contract.hub_contract_json:
            ...
            hub_contract, spec_type, spec_version, norm_status, norm_errors, norm_warnings = normalize_contract(
                raw_contract=contract.original_raw,
                format=contract.original_format.lower()
            )
            if hub_contract:
        ...
        if response.status_code == status.HTTP_400_BAD_REQUEST:
            ...
            elif 'already at target version' in str(error_msg).lower() or 'not needed' in str(error_msg).lower():
        ...
        if response.status_code in [status.HTTP_404_NOT_FOUND, status.HTTP_500_INTERNAL_SERVER_ERROR]:
    ...
    def test_background_migration_strategy(self):
        ...
        if response.status_code == status.HTTP_400_BAD_REQUEST:
            ...
            self.assertIn('contract', response.data)
            return
        elif response.status_code in [status.HTTP_404_NOT_FOUND, status.HTTP_500_INTERNAL_SERVER_ERROR]:
            pytest.skip(f"Migrate endpoint not available or service error: {response.status_code}")
        else:
            ...
            try:
                ...
                job.refresh_from_db()
                self.assertIn(job.status, [JobStatus.PENDING, JobStatus.RUNNING, JobStatus.COMPLETED, JobStatus.FAILED])
    ...
    def test_migration_version_conflicts(self):
        ...
        contract.save(update_fields=['hub_contract_version', 'hub_contract_json', 'normalization_status'])
    ...
    def test_migration_with_invalid_contract_fails(self):
        ...
        contract_id = self.create_contract(
            asset_id,
            original_raw='{"invalid": "contract", "cannot": "migrate"}'
        )
    ...
    def test_migration_preserves_original_contract(self):
        ...
        # Verify original contract preserved
        contract = Contract.objects.get(id=contract_id)
        contract.refresh_from_db()
