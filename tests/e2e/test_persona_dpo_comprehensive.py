...
class JourneyDPO001DataFirstOnboardingTests(E2ETestBase):
        ...
        for i in range(max_retries):
            try:
                self.verify_audit_log(
                    action="FILE_UPLOAD_COMPLETED", resource_type="FILE", resource_id=file_id
                )
                break
    ...
    def test_error_scenario_invalid_contract(self):
        ...
        try:
            contract_id = self.create_contract(
                asset_id, original_raw='{"id": "invalid", "invalid": "contract"}'
            )
            ...
            self.assertTrue(
                "name" in error_msg
                or "fields" in error_msg
                or "invalid" in error_msg
                or "normali" in error_msg
...
class JourneyDPO002MarketplacePublicationTests(E2ETestBase):
    ...
    def test_error_scenario_eligibility_failure_tenant_not_verified(self):
        ...
        if response.status_code == status.HTTP_403_FORBIDDEN:
            ...
            self.assertTrue(
                "KYC_VERIFICATION_REQUIRED" in str(error_data.get("code", ""))
                or "kyc" in error_str
                or "verified" in error_str
                or "subscription" in error_str,
...
class JourneyDPO003UpdateAssetContractTests(E2ETestBase):
    ...
    def test_update_asset_contract_happy_path(self):
        ...
        while wait_time < max_wait and (
            not contract.hub_contract_json
            or contract.hub_contract_json.get("info", {}).get("name") != "Updated Contract"
        ):
        ...
        if contract.hub_contract_json and contract.hub_contract_json.get("info"):
            ...
            self.assertEqual(
                contract.hub_contract_json.get("info", {}).get("name"), "Updated Contract"
            )

        # Verify audit log
        self.verify_audit_log(
            action="CONTRACT_UPDATED", resource_type="CONTRACT", resource_id=contract_id
        )

    def test_update_contract_requires_revalidation(self):
        """Test that updating contract requires re-validation"""
        asset_id = self.create_asset(
            key="revalidate-contract-asset", name="Revalidate Contract Asset"
        )
    ...
    def test_update_contract_error_asset_not_found(self):
        ...
        self.assertNotEqual(
            str(updated_contract.asset_id or ""),
            fake_asset_id,
            "Non-existent asset_id should not be applied",
        )
    ...
    def test_update_contract_performance_targets(self):
        ...
        self.assertLess(
            update_duration, 5.0, f"Contract update took {update_duration:.2f}s, expected < 5s"
        )
        ...
        self.assertLess(
            validation_duration,
            30.0,
            f"Contract validation took {validation_duration:.2f}s, expected < 30s",
        )

        # Total duration should be < 3 minutes
        total_duration = update_duration + validation_duration
        self.assertLess(
            total_duration, 180.0, f"Total duration took {total_duration:.2f}s, expected < 180s"
        )
...
class JourneyDPO004MonitorAssetQualityTests(E2ETestBase):
    ...
    def test_monitor_asset_quality_happy_path(self):
        ...
        file_id = self.init_file_upload(
            name="quality.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id, test_content=test_content)
        dataset_id = self.create_dataset(file_id, asset_id)
    ...
    def test_get_asset_dq_history(self):
        ...
        test_content = b"id,name\n1,Test\n2,Data"
        file_id = self.init_file_upload(
            name="history.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id, test_content=test_content)
        dataset_id = self.create_dataset(file_id, asset_id)

        # Run multiple DQ checks
        self.run_dq_check(file_id=file_id, dataset_id=dataset_id, asset_id=asset_id)
        time.sleep(1)  # noqa: sleep-needed  # Small delay  # INTENTIONAL: test-specific timing
        self.run_dq_check(file_id=file_id, dataset_id=dataset_id, asset_id=asset_id)

        # Get DQ runs for asset
        response = self.client.get("/api/v1/dq/runs/", {"asset_id": str(asset_id)}, format="json")
    ...
    def test_monitor_asset_quality_performance_targets(self):
        ...
        test_content = b"id,name,value\n1,Test,100\n2,Data,200"
        file_id = self.init_file_upload(
            name="perf-quality.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id, test_content=test_content)
        dataset_id = self.create_dataset(file_id, asset_id)
        ...
        self.assertLess(
            retrieval_duration,
            5.0,
            f"DQ run retrieval took {retrieval_duration:.2f}s, expected < 5s",
        )
        ...
        if health_response.status_code == status.HTTP_200_OK:
            ...
            self.assertLess(
                analysis_duration, 10.0, f"Analysis took {analysis_duration:.2f}s, expected < 10s"
            )

        # Total duration should be < 1 minute
        total_duration = retrieval_duration + analysis_duration
        self.assertLess(
            total_duration, 60.0, f"Total duration took {total_duration:.2f}s, expected < 60s"
        )
    ...
    def test_monitor_asset_quality_complete_steps_verification(self):
        ...
        test_content = b"id,name,value\n1,Test,100\n2,Data,200"
        file_id = self.init_file_upload(
            name="complete-quality.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id, test_content=test_content)
        dataset_id = self.create_dataset(file_id, asset_id)
...
class JourneyDPO005ManageAssetVersionsTests(E2ETestBase):
    ...
    def test_create_new_dataset_version(self):
        ...
        file_id_v1 = self.init_file_upload(
            name="v1.csv", content_type="text/csv", size=len(test_content_v1)
        )
        self.complete_file_upload(file_id_v1, test_content=test_content_v1)
        self.create_dataset(file_id_v1, asset_id)

        # Create new version with updated data
        test_content_v2 = b"id,name,value\n1,Version2,100\n2,Data2,200"
        file_id_v2 = self.init_file_upload(
            name="v2.csv", content_type="text/csv", size=len(test_content_v2)
        )
    ...
    def test_get_version_history(self):
        ...
        for i in range(3):
            ...
            file_id = self.init_file_upload(
                name=f"v{i}.csv", content_type="text/csv", size=len(test_content)
            )
            self.complete_file_upload(file_id, test_content=test_content)
            self.create_dataset(file_id, asset_id)

        # Get datasets for asset (versions)
        response = self.client.get("/api/v1/datasets/", {"asset_id": str(asset_id)}, format="json")
...
class JourneyDPO006RetireAssetTests(E2ETestBase):
    ...
    def test_retire_asset_happy_path(self):
        ...
        test_content = b"id,name\n1,Test"
        file_id = self.init_file_upload(
            name="retire.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id, test_content=test_content)
        self.create_dataset(file_id, asset_id)
    ...
    def test_retire_asset_unpublishes_marketplace_listings(self):
        ...
        test_content = b"id,name\n1,Test"
        file_id = self.init_file_upload(
            name="retire-listing.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id, test_content=test_content)
        self.create_dataset(file_id, asset_id)
        ...
        if listing_response.status_code == status.HTTP_201_CREATED:
            ...
            self.assertEqual(
                ...
                status.HTTP_200_OK,
                f"Publish failed: {get_response_data(publish_resp)}",
            )
            listing = Listing.objects.get(id=listing_id)
            ...
            self.assertEqual(
                ...
                "Listing must be PUBLISHED before retire test",
            )
