...
def ml_serving_available(api_available):
    ...
    if not _check_ml_serving_available():
        pytest.skip(
            "ML serving endpoint not responsive at http://localhost:8000/api/v1/ml/serving/"
        )
    return True
...
def test_model_id(api_available, e2e_api_key, django_db_blocker):
    ...
    try:
        ...
        asset_response = requests.get(
            "http://localhost:8000/api/v1/assets/", headers=headers, params={"limit": 1}, timeout=10
        )
        ...
        if not asset_id:
            ...
            asset_create_response = requests.post(
                "http://localhost:8000/api/v1/assets/", json=asset_data, headers=headers, timeout=15
            )
            if asset_create_response.status_code in [200, 201]:
                asset_id = asset_create_response.json().get("id")
  # noqa: skip-in-body — runtime service dependency
        if not asset_id:
        ...
        create_response = requests.post(
            "http://localhost:8000/api/v1/ml/models/", json=model_data, headers=headers, timeout=15
        )
        ...
        if create_response.status_code == 400 and "limit exceeded" in create_response.text.lower():
            ...
            if retry_response.status_code == 200:
                ...
                retry_results = (
                    retry_data.get("results", []) if isinstance(retry_data, dict) else retry_data
                )
                if retry_results:
...
def sdk_client(api_available, e2e_api_key, django_db_blocker):
    ...
    try:
        ...
        if probe.status_code in (401, 403):
            pytest.skip(f"SDK auth not available: API returned {probe.status_code}")
...
class TestMLServingCLIWorkflow:
    ...
    def test_complete_serving_workflow_cli(self, runner, ml_serving_available, test_model_id):
        ...
        get_result = runner.invoke(cli, ["ml", "serving", "get", serving_id, "--format", "json"])
        ...
        metrics_result = runner.invoke(
            cli, ["ml", "serving", "metrics", serving_id, "--format", "json"]
        )
        ...
        undeploy_result = runner.invoke(
            cli, ["ml", "serving", "undeploy", serving_id, "--format", "json"]
        )
    ...
    def test_ab_test_workflow_cli(self, runner, ml_serving_available, test_model_id):
        ...
        get_result = runner.invoke(
            cli, ["ml", "serving", "ab-test", "get", ab_test_id, "--format", "json"]
        )
...
class TestMLServingSDKWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_complete_serving_workflow_sdk(
        self, ml_serving_available, sdk_model_serving_api, sdk_test_model_id
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_ab_test_workflow_sdk(
        self, ml_serving_available, sdk_model_serving_api, sdk_test_model_id
    ):
        """
...
class TestMLServingCLISDKConsistency:
    ...
    def test_deploy_consistency(
        self, runner, ml_serving_available, sdk_test_model_id, sdk_model_serving_api
    ):
        """Test that CLI and SDK produce consistent results for deploy"""
        # Deploy via CLI
        cli_result = runner.invoke(
            cli, ["ml", "serving", "deploy", "--model-id", sdk_test_model_id, "--format", "json"]
        )
    ...
    def test_list_consistency(self, runner, ml_serving_available, sdk_model_serving_api):
        ...
        # List via CLI
        cli_result = runner.invoke(cli, ["ml", "serving", "list", "--format", "json"])
    ...
    def test_metrics_consistency(
        ...
        # Get metrics via CLI
        cli_metrics_result = runner.invoke(
            cli, ["ml", "serving", "metrics", serving_id, "--format", "json"]
        )
