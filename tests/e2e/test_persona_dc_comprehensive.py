...
class JourneyDC001DiscoverAndPurchaseTests(E2ETestBase):
    ...
    def test_discover_and_purchase_free_auto_approve_asset(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertGreater(
            len(response.data.get("results", [])), 0, "Should have at least one listing"
        )
        ...
        self.assertEqual(
            ...
            f"Listing should be published, but status is {listing.status}",
        )
        ...
        self.verify_audit_log(
            action="ORDER_CREATED_AUTO_APPROVED", resource_type="ORDER", resource_id=order_id
        )
    ...
    def test_discover_and_request_approval_asset(self):
        ...
        our_entitlement = next(
            (
                ...
                if str(e.get("order", "")) == order_id_str
                or str(e.get("asset", "")) == asset_id_str
            ),
            None,
        )
        self.assertIsNotNone(
            our_entitlement,
            f"Entitlement should be created after approval. Found {len(entitlements)} entitlements",
        )
    ...
    def test_discover_error_scenarios(self):
        ...
        response = self.client.post(
            "/api/v1/marketplace/orders/", {"listing_id": fake_id}, format="json"
        )
...
class JourneyDC003DownloadPurchasedDataTests(E2ETestBase):
    ...
    def test_download_purchased_data_happy_path(self):
        ...
        asset_id = self.create_asset(
            key="download-asset", name="Download Asset", description="Asset for download test"
        )
        ...
        self.assertIsNotNone(
            entitlement_from_order, "FREE_AUTO_APPROVE order response must include an entitlement"
        )
    ...
    def test_download_without_entitlement_fails(self):
        ...
        self.create_asset(key="no-access-asset", name="No Access Asset")
...
class JourneyDC004ExploreAssetLineageTests(E2ETestBase):
    ...
    def test_explore_asset_lineage_happy_path(self):
        ...
        asset_id = self.create_asset(
            key="lineage-asset", name="Lineage Asset", description="Asset with lineage"
        )
...
class JourneyDC005ReviewAssetQualityTests(E2ETestBase):
    ...
    def test_review_asset_quality_happy_path(self):
        ...
        asset_id = self.create_asset(
            key="quality-asset", name="Quality Asset", description="Asset with quality metrics"
        )
