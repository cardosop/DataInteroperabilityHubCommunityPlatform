...
class DataFirstE2ETest(TestCase):
    ...
    def test_complete_data_first_journey(self):
        ...
        file_init_response = self.client.post(
            "/api/v1/files/init/",
            {"name": "orders.csv", "content_type": "text/csv", "size": file_size},
            format="json",
        )
        ...
        if compliance_run.job_id:
            try:
                ...
                logging.getLogger(__name__).warning(
                    "Inline job execution failed (expected if service unavailable): %s", exc
                )
        ...
        self.assertIn(
            validate_data.get("validation_status"),
            ["VALID", "INVALID"],
        ...
        normalize_response = self.client.post(
            f"/api/v1/contracts/{contract_id}/normalize/", {}, format="json"
        )
