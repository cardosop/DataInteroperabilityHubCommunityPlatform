...
class RateLimitingE2ETest(E2ETestBase):
    ...
    def test_rate_limit_headers_present(self):
        ...
        self.assertTrue(
            has_header,
            "At least one rate limit header should be present in the response",
        )
    ...
    def test_rate_limit_enforcement(self):
        ...
        self.assertTrue(
            rate_limited,
            "Rate limiting should trigger after many rapid requests",
        )
    ...
    def test_rate_limit_exceeded_response(self):
        ...
        for _i in range(20):
            ...
            time.sleep(0.01)  # noqa: sleep-needed — INTENTIONAL

        self.assertIsNotNone(
            rate_limited_response,
            "Should trigger rate limit after rapid requests",
        )
    ...
    def test_rate_limit_headers_consistency(self):
        ...
        self.assertGreater(
            len(rate_limit_headers),
            0,
            "Rate limit headers should be present in at least one response",
        )
        unique_limits = set(rate_limit_headers)
        self.assertEqual(
            len(unique_limits),
            1,
            "Rate limit should be the same across all requests",
        )
    ...
    def test_rate_limit_per_endpoint(self):
        ...
        self.assertGreater(
            len(limits),
            0,
            "At least one endpoint should have rate limit headers",
        )
