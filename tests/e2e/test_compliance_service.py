...
class ComplianceServiceE2ETest(E2ETestBase):
    ...
    def test_compliance_run_execution_updates_status(self):
        ...
        asset_id = self.create_asset(
            key="compliance-execution-test", name="Compliance Execution Test"
        )
        test_content = b"col1,col2\nval1,val2\nval3,val4"
        content_hash = hashlib.sha256(test_content).hexdigest()
        file_id = self.init_file_upload(
            name="compliance_execution_test.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id, content_sha256=content_hash, test_content=test_content)
        dataset_id = self.create_dataset(file_id, asset_id)
        ...
        if compliance_run.status == ComplianceRunStatus.SUCCEEDED:
            self.assertIsNotNone(
                compliance_run.overall_status, "Succeeded compliance run must have overall_status"
            )
            self.assertIsNotNone(
                compliance_run.risk_level, "Succeeded compliance run must have risk_level"
            )
    ...
    def test_compliance_run_updates_asset_compliance_status(self):
        ...
        content_hash = hashlib.sha256(test_content).hexdigest()
        file_id = self.init_file_upload(
            name="compliance_status_test.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id, content_sha256=content_hash, test_content=test_content)
        dataset_id = self.create_dataset(file_id, asset_id)
        ...
        if compliance_run.status == ComplianceRunStatus.SUCCEEDED:
            ...
            self.assertIsNotNone(
                compliance_run.completed_at, "Failed compliance run should have completed_at"
            )
    ...
    def test_compliance_run_with_pii_detection(self):
        ...
        content_hash = hashlib.sha256(test_content).hexdigest()
        file_id = self.init_file_upload(
            name="pii_test.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id, content_sha256=content_hash, test_content=test_content)
        dataset_id = self.create_dataset(file_id, asset_id)
        ...
        if compliance_run.status == ComplianceRunStatus.SUCCEEDED:
            self.assertIsNotNone(
                compliance_run.detected_categories_json,
    ...
    def test_compliance_run_fail_closed_behavior(self):
        ...
        content_hash = hashlib.sha256(test_content).hexdigest()
        file_id = self.init_file_upload(
            name="fail_closed_test.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id, content_sha256=content_hash, test_content=test_content)
        dataset_id = self.create_dataset(file_id, asset_id)
    ...
    def test_compliance_run_with_regulations(self):
        ...
        content_hash = hashlib.sha256(test_content).hexdigest()
        file_id = self.init_file_upload(
            name="regulations_test.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id, content_sha256=content_hash, test_content=test_content)
        dataset_id = self.create_dataset(file_id, asset_id)
    ...
    def test_list_compliance_runs_with_filters(self):
        ...
        file_id1 = self.init_file_upload(
            name="compliance_list1.csv", content_type="text/csv", size=len(test_content)
        )
        ...
        file_id2 = self.init_file_upload(
            name="compliance_list2.csv", content_type="text/csv", size=len(test_content)
        )
    ...
    def test_get_compliance_run_details(self):
        ...
        content_hash = hashlib.sha256(test_content).hexdigest()
        file_id = self.init_file_upload(
            name="compliance_details_test.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id, content_sha256=content_hash, test_content=test_content)
        dataset_id = self.create_dataset(file_id, asset_id)
        compliance_run_id = self.run_compliance_check(file_id, dataset_id, asset_id)
    ...
    def test_compliance_run_result_structure(self):
        ...
        content_hash = hashlib.sha256(test_content).hexdigest()
        file_id = self.init_file_upload(
            name="compliance_result_test.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id, content_sha256=content_hash, test_content=test_content)
        dataset_id = self.create_dataset(file_id, asset_id)
    ...
    def test_compliance_run_risk_level_calculation(self):
        ...
        content_hash = hashlib.sha256(test_content).hexdigest()
        file_id = self.init_file_upload(
            name="risk_level_test.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id, content_sha256=content_hash, test_content=test_content)
        dataset_id = self.create_dataset(file_id, asset_id)
