    ...
    def test_prometheus_alerts_endpoint(self):
        ...
        if not self._check_prometheus_available():
            try:
                ...
                pass  # noqa: skip-in-body — runtime service dependency
...
class DistributedTracingE2ETest(TestCase):
    ...
    def test_trace_correlation_across_services(self):
        ...
        self.assertIsNotNone(
            tid1,
            "First response must include X-Trace-Id",
        )
        self.assertIsNotNone(
            tid2,
            "Second response must include X-Trace-Id",
        )
...
class LogCorrelationE2ETest(TestCase):
    ...
    def test_log_correlation_structured_format(self):
        ...
        processor_names = [
            getattr(p, "__name__", type(p).__name__) for p in config.get("processors", [])
        ]
        self.assertIn(
            "merge_contextvars",
            processor_names,
...
class MonitoringEdgeCasesE2ETest(TestCase):
    ...
    def test_metrics_endpoint_high_load(self):
        ...
        self.assertLess(
            metrics_duration,
            2.0,
            f"Metrics endpoint took {metrics_duration:.2f}s, expected < 2s",
        )
        ...
        self.assertLess(
            load_duration,
            30.0,
            f"100 health requests took {load_duration:.2f}s, expected < 30s",
        )
    ...
    def test_tracing_with_disabled_setting(self):
        ...
        with override_settings(OPENTELEMETRY_ENABLED=False):
            ...
            self.assertIsNone(
                tracer,
                "get_tracer must return None when OPENTELEMETRY_ENABLED=False",
            )
    ...
    def test_metrics_middleware_normalizes_routes(self):
        ...
        normalized = middleware._normalize_route(
            "/api/v1/contracts/123e4567-e89b-12d3-a456-426614174000/",
        )
        self.assertEqual(
            normalized,
            "/api/v1/contracts/{id}/",
