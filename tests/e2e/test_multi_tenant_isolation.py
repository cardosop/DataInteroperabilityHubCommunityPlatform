...
class MultiTenantIsolationE2ETest(E2ETestBase):
    ...
    def setUp(self):
        ...
        self.other_user = User.objects.create_user(
            email=f"other-{_suffix}@example.com", password="testpass123", tenant=self.other_tenant
        )
    ...
    def test_dataset_tenant_isolation(self):
        ...
        file_id1 = self.init_file_upload(
            name="dataset_iso1.csv", content_type="text/csv", size=len(test_content)
        )
    ...
    def test_file_tenant_isolation(self):
        ...
        content_hash = hashlib.sha256(test_content).hexdigest()
        file_id1 = self.init_file_upload(
            name="file_iso1.csv", content_type="text/csv", size=len(test_content)
        )
        self.complete_file_upload(file_id1, content_sha256=content_hash, test_content=test_content)
        ...
        self.assertNotEqual(
            response.status_code, status.HTTP_200_OK, "Cross-tenant file download must be denied"
        )
    ...
    def test_cross_tenant_access_with_entitlement(self):
        ...
        entitlement = Entitlement.objects.create(
            tenant=self.other_tenant, asset=asset, listing=listing, status=EntitlementStatus.ACTIVE
        )
