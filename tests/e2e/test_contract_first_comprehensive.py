...
class ContractFirstFlowSuccessTests(E2ETestBase):
    ...
    def test_complete_contract_first_journey_happy_path(self):
        ...
        # Step 1: Create asset
        asset_id = self.create_asset(key="product-catalog", name="Product Catalog")
        ...
        self.run_compliance_check(file_id, dataset_id, asset_id)
        self.run_dq_check(file_id, dataset_id, asset_id)
    ...
    def test_contract_first_with_yaml_format(self):
        ...
        if (
            isinstance(validate_response, dict)
            and validate_response.get("status_code") == status.HTTP_500_INTERNAL_SERVER_ERROR
        ):
    ...
    def test_schema_mismatch_warning(self):
        ...
        if contract.normalization_status in [
            ...
            self.assertIsNotNone(
                contract.hub_contract_json, "Contract was normalized but hub_contract_json is None"
            )
...
class ContractFirstFlowSchemaReconciliationTests(E2ETestBase):
    ...
    def test_schema_reconciliation_with_extra_fields(self):
        ...
        self.assertGreater(
            len(dataset.schema_json.get("fields", [])), 0, "Schema should have at least one field"
        )
...
class ContractFirstFlowEdgeCasesTests(E2ETestBase):
    ...
    def test_contract_with_empty_schema(self):
        ...
        contract_id = self.create_contract(
            asset_id, original_raw='{"id": "test", "schema": {"fields": []}}'
        )
    ...
    def test_contract_with_complex_nested_schema(self):
        ...
        contract_id = self.create_contract(asset_id, original_raw=nested_schema)
    ...
    def test_contract_normalization_failure_handling(self):
        ...
        contract_id = self.create_contract(
            asset_id, original_raw='{"invalid": "structure", "cannot": "normalize"}'
        )
