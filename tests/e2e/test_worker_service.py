...
class WorkerServiceE2ETest(TestCase):
    ...
    def _check_worker_service_available(self):
        ...
        try:
            ...
            if response.status_code != 200:
                pytest.skip(
                    f"Worker service not available at {self.worker_url} (status: {response.status_code})"
                )
    ...
    def test_job_processing_with_real_worker(self):
        ...
        self.assertIn(
            job.status,
            [JobStatus.COMPLETED, JobStatus.FAILED, JobStatus.PENDING, JobStatus.RUNNING],
        )
    ...
    def test_priority_queue_processing(self):
        ...
        time.sleep(10)  # noqa: sleep-needed  # INTENTIONAL: e2e/integration test polling real services
    ...
    @pytest.mark.timeout(
        300
    )  # 5 min: DQ_RUN retry backoff starts at 60s, so observing a retry needs ≥90s
