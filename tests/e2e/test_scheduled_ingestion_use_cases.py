...
class ScheduledIngestionExecutionUseCasesTest(E2ETestBase):
    ...
    def test_trigger_ingestion_manually_success(self):
        ...
        response = self.client.post(
            f'/api/v1/scheduled-ingestions/{self.ingestion.id}/trigger/',
            {},
            format='json'
        )
        
        # May return 200, 202, 503 (Prefect unavailable), or 500
        self.assertIn(response.status_code, [
            status.HTTP_200_OK,
    ...
    def test_list_ingestion_runs_success(self):
        ...
        response = self.client.get(
            f'/api/v1/scheduled-ingestions/{self.ingestion.id}/runs/'
        )
    ...
    def test_get_ingestion_run_details_success(self):
        ...
        response = self.client.get(
            f'/api/v1/scheduled-ingestions/runs/{run.id}/'
        )
...
class ScheduledIngestionManagementUseCasesTest(E2ETestBase):
    ...
    def test_delete_ingestion_success(self):
        ...
        if response.status_code == status.HTTP_204_NO_CONTENT:
            ...
            self.assertFalse(
                ScheduledIngestion.objects.filter(id=ingestion_id).exists()
            )
    ...
    def test_filter_ingestions_by_status_success(self):
        ...
        if isinstance(results, list):
            ...
            active_ingestions = [item for item in results if isinstance(item, dict) and item.get('status') == ScheduledIngestionStatus.ACTIVE]
            self.assertGreater(len(active_ingestions), 0, "Should have at least one ACTIVE ingestion")
            
            # If status filter is working, verify no PAUSED ingestions
            paused_ingestions = [item for item in results if isinstance(item, dict) and item.get('status') == ScheduledIngestionStatus.PAUSED]
            if len(paused_ingestions) == 0:
                ...
                self.assertIn(str(self.ingestion.id), [item.get('id') for item in active_ingestions])
