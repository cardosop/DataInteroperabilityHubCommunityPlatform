...
class TestAPINamingStandardsCompleteness(unittest.TestCase):
    ...
    def test_has_table_of_contents(self):
        ...
        self.assertIn(
            "## Table of Contents", self.standards_content, "Document should have table of contents"
        )

    def test_has_overview_section(self):
        """Test that document has overview section"""
        self.assertIn(
            "## Overview", self.standards_content, "Document should have overview section"
        )
    ...
    def test_has_no_duplication_rule(self):
        ...
        self.assertIn(
            "MUST NOT appear multiple times",
            self.standards_content,
    ...
    def test_has_consistent_patterns(self):
        ...
        self.assertIn(
            "Collections", self.standards_content, "Document should define collections pattern"
        )
        self.assertIn("Detail", self.standards_content, "Document should define detail pattern")
        self.assertIn("Actions", self.standards_content, "Document should define actions pattern")
        self.assertIn(
            "Sub-resources", self.standards_content, "Document should define sub-resources pattern"
        )
    ...
    def test_has_resource_naming_conventions(self):
        ...
        self.assertIn(
            "kebab-case", self.standards_content, "Document should specify kebab-case convention"
        )
    ...
    def test_has_examples_section(self):
        ...
        self.assertIn(
            "## Examples", self.standards_content, "Document should have examples section"
        )
        self.assertIn("✅ Correct", self.standards_content, "Document should have correct examples")
        self.assertIn(
            "❌ Incorrect", self.standards_content, "Document should have incorrect examples"
        )
...
class TestAPINamingStandardsExamples(unittest.TestCase):
    ...
    def test_correct_examples_format(self):
        ...
        correct_section = self.standards_content.split("### ✅ Correct Examples")[1].split(
            "### ❌ Incorrect Examples"
        )[0]

        # Check for collection examples
        self.assertIn("/api/v1/assets/", correct_section, "Should have correct collection example")
        self.assertIn(
            "/api/v1/contracts/", correct_section, "Should have correct collection example"
        )

        # Check for detail examples
        self.assertIn("/api/v1/assets/{id}/", correct_section, "Should have correct detail example")
    ...
    def test_incorrect_examples_format(self):
        ...
        self.assertIn(
            "/api/v1/assets/assets/", incorrect_section, "Should have duplication violation example"
        )
        self.assertIn(
        ...
        self.assertIn(
            "/api/v1/asset/", incorrect_section, "Should have singular form violation example"
        )

        # Check for case violations
        self.assertIn("dataContracts", incorrect_section, "Should have case violation example")
        self.assertIn("data_contracts", incorrect_section, "Should have case violation example")
    ...
    def test_examples_have_explanations(self):
        ...
        has_collection_explanation = (
            "list" in correct_lower or "collection" in correct_lower or "# list" in correct_section
        )
        self.assertTrue(has_collection_explanation, "Should explain collection endpoints")

        # Check for detail explanations (flexible - can be "get", "retrieve", "get asset", etc.)
        has_detail_explanation = (
            "get" in correct_lower
            or "retrieve" in correct_lower
            or "# get" in correct_section
            or "# retrieve" in correct_section
        )
        self.assertTrue(has_detail_explanation, "Should explain detail endpoints")
        ...
        has_duplication_explanation = (
            "appears twice" in incorrect_lower
            or "duplicate" in incorrect_lower
            or "duplication" in incorrect_lower
        )
        self.assertTrue(has_duplication_explanation, "Should explain duplication violations")

        # Check for correction suggestions
        has_correction = (
            "should be" in incorrect_lower
            or "should" in incorrect_lower
            or "must" in incorrect_lower
        )
        self.assertTrue(has_correction, "Should explain what should be used instead")

    def test_examples_match_patterns(self):
        """Test that correct examples match the defined patterns"""
        # Extract pattern definitions
        patterns_section = self.standards_content.split("## Consistent Patterns")[1].split(
            "## Resource Naming Conventions"
        )[0]
        ...
        if "/api/v1/{resource}/" in patterns_section:
            ...
            for example in collection_examples[:5]:  # Check first 5
                self.assertTrue(
                    example.endswith("/"), f"Collection example {example} should end with /"
                )
                self.assertNotIn(
                    "{id}", example, f"Collection example {example} should not have {{id}}"
                )
        ...
        if "/api/v1/{resource}/{id}/" in patterns_section:
            ...
            for example in detail_examples[:5]:  # Check first 5
                self.assertIn("{id}", example, f"Detail example {example} should have {{id}}")
                self.assertTrue(
                    example.endswith("/"), f"Detail example {example} should end with /"
                )
    ...
    def test_migration_examples_are_complete(self):
        ...
        self.assertIn("Before", migration_section, "Should have 'Before' examples")
        self.assertIn("After", migration_section, "Should have 'After' examples")

        # Check for code examples
        self.assertIn("```python", migration_section, "Should have Python code examples")

        # Check for duplication fix example
        self.assertIn(
            "assets/assets",
            migration_section.lower() or "duplication",
            "Should have duplication fix example",
        )

        # Check for singular form fix example
        self.assertIn("asset", migration_section.lower(), "Should have singular form fix example")
        self.assertIn("assets", migration_section.lower(), "Should have plural form fix example")
...
class TestAPINamingStandardsValidationRules(unittest.TestCase):
    ...
    def test_validation_checklist_exists(self):
        ...
        self.assertIn(
            "Validation Checklist", validation_section, "Should have validation checklist"
        )

        # Check for specific validation checks
        self.assertIn(
            "No Duplication Check", validation_section, "Should have no duplication check"
        )
        self.assertIn("Plural Form Check", validation_section, "Should have plural form check")
        self.assertIn("Kebab-Case Check", validation_section, "Should have kebab-case check")
        self.assertIn(
            "Pattern Consistency Check", validation_section, "Should have pattern consistency check"
        )
    ...
    def test_validation_regex_patterns_exist(self):
        ...
        self.assertIn(
            "COLLECTION_PATTERN", validation_section, "Should have collection pattern regex"
        )
        self.assertIn("DETAIL_PATTERN", validation_section, "Should have detail pattern regex")
        self.assertIn("ACTION_PATTERN", validation_section, "Should have action pattern regex")
        self.assertIn(
            "SUB_RESOURCE_PATTERN", validation_section, "Should have sub-resource pattern regex"
        )
    ...
    def test_validation_regex_patterns_are_valid(self):
        ...
        validation_section.split("```python")[1].split("```")[0]
        ...
        try:
            ...
            self.assertTrue(
                collection_pattern.match("/api/v1/assets/"),
                "Collection pattern should match correct example",
            )
            self.assertTrue(
                detail_pattern.match("/api/v1/assets/{id}/"),
                "Detail pattern should match correct example",
            )
            self.assertTrue(
                action_pattern.match("/api/v1/assets/{id}/activate/"),
                "Action pattern should match correct example",
            )
            self.assertTrue(
                sub_resource_pattern.match("/api/v1/assets/{id}/datasets/"),
