...
class TestTerraformStagingProdDiff:
    ...
    def test_cluster_name_different(self, staging_main, prod_main):
        ...
        assert "prod" in prod_name.lower(), (
            f"Prod cluster name should contain 'prod': {prod_name}"
        )
    ...
    def test_nat_gateway_count_staging_1_prod_3(self, staging_main, prod_main):
        ...
        assert prod_nat == "3", (
            f"Prod NAT GW count should be 3 (HA), got {prod_nat}"
        )
    ...
    def test_eks_public_endpoint(self, staging_main, prod_main):
        ...
        assert staging_pe == "true", (
            f"Staging public_endpoint should be true, got {staging_pe}"
        )
    ...
    def test_staging_single_node_group_prod_four(self, staging_main, prod_main):
        ...
        assert prod_ngs == 4, (
            f"Prod should have 4 node groups, found {prod_ngs} instance_types"
        )
    ...
    def test_staging_instance_type_t3_prod_m6i(self, staging_main, prod_main):
        ...
        """
        assert "t3.large" in staging_main, (
            "Staging should use t3.large instances"
        )
        assert "m6i.xlarge" in prod_main, (
            "Prod should use m6i.xlarge for app nodes"
        )
        assert "m6i.large" in prod_main, (
            "Prod should use m6i.large for worker nodes"
        )
    ...
    def test_staging_desired_1_prod_desired_3(self, staging_main, prod_main):
        ...
        assert "desired_size   = 3" in prod_main, (
            "Prod app-ng should have desired_size=3"
        )

    def test_prod_system_ng_has_taint(self, prod_main):
        """Prod system-ng must have CriticalAddonsOnly taint."""
        assert "CriticalAddonsOnly" in prod_main, (
            "Prod system-ng must taint for CriticalAddonsOnly"
        )
    ...
    def test_rds_instance_class_diff(self, staging_main, prod_main):
        ...
        assert "t4g" in staging_class, (
            f"Staging RDS should be t4g (burstable), got {staging_class}"
        )
        assert "r7g" in prod_class, (
            f"Prod RDS should be r7g (provisioned), got {prod_class}"
        )
    ...
    def test_rds_multi_az_diff(self, staging_main, prod_main):
        ...
        assert staging_maz == "false", (
            f"Staging multi_az should be false, got {staging_maz}"
        )
        assert prod_maz == "true", (
            f"Prod multi_az should be true, got {prod_maz}"
        )

    def test_rds_backup_retention_diff(self, staging_main, prod_main):
        """Staging: 7 days; prod: 14 days."""
        staging_br = _extract_module_param(
            staging_main, "rds", "backup_retention_days"
        )
        prod_br = _extract_module_param(prod_main, "rds", "backup_retention_days")
        assert staging_br == "7", (
            f"Staging backup retention should be 7, got {staging_br}"
        )
        assert prod_br == "14", (
            f"Prod backup retention should be 14, got {prod_br}"
        )

    def test_rds_performance_insights_diff(self, staging_main, prod_main):
        """Staging: no PI; prod: PI enabled."""
        staging_pi = _extract_module_param(
            staging_main, "rds", "enable_performance_insights"
        )
        ...
        assert prod_pi == "true", (
            f"Prod performance_insights should be true, got {prod_pi}"
        )
    ...
    def test_elasticache_node_type_diff(self, staging_main, prod_main):
        ...
        assert "t4g" in staging_type, (
            f"Staging ElastiCache should be t4g, got {staging_type}"
        )
        assert "r7g" in prod_type, (
            f"Prod ElastiCache should be r7g, got {prod_type}"
        )
    ...
    def test_elasticache_multi_az_diff(self, staging_main, prod_main):
        ...
        assert prod_maz == "true", (
            f"Prod ElastiCache multi_az should be true, got {prod_maz}"
        )
    ...
    def test_s3_versioning_diff(self, staging_main, prod_main):
        ...
        assert staging_ver == "false", (
            f"Staging S3 versioning should be false, got {staging_ver}"
        )
        assert prod_ver == "true", (
            f"Prod S3 versioning should be true, got {prod_ver}"
        )
    ...
    def test_staging_has_vpc_endpoints_prod_does_not(self, staging_main, prod_main):
        ...
        assert "enable_vpc_endpoints" in staging_main, (
            "Staging should have enable_vpc_endpoints"
        )
    ...
    def test_staging_has_ecr_pull_through_cache(self, staging_main, prod_main):
        ...
        assert "module \"ecr\"" in staging_main, (
            "Staging should have ECR pull-through cache module"
        )
    ...
    def test_staging_has_route53_dns_automation(self, staging_main, prod_main):
        ...
        assert "aws_route53_zone" in staging_main, (
            "Staging should have Route53 DNS automation"
        )
    ...
    def test_both_have_required_modules(self, staging_main, prod_main):
        ...
        for mod in required_modules:
            assert f'module "{mod}"' in staging_main, (
                f"Staging missing module: {mod}"
            )
            assert f'module "{mod}"' in prod_main, (
                f"Prod missing module: {mod}"
            )
    ...
    def test_aws_provider_version_same(self, staging_main, prod_main):
        ...
        if staging_ver and prod_ver:
            assert staging_ver.group(1) == prod_ver.group(1), (
                f"AWS provider versions differ"
            )
...
class TestTerraformNoUnexpectedDrift:
    ...
    def test_production_is_not_a_copy_of_staging_with_env_swapped(self,
                                                                  staging_main,
                                                                  prod_main):
    ...
    def test_no_mystery_parameters_in_prod_not_in_staging(self, staging_main, prod_main):
        ...
        KNOWN_PROD_ONLY = {
            "source", "version",  # provider config
            "default_tags",       # provider config
            "system-ng", "app-ng", "worker-ng", "gpu-ng",  # prod-only node groups
        }
