...
class TestRateLimitMiddlewareDefinition:
    ...
    def test_rate_limit_middleware_exists_in_template(self, routes_tmpl):
        ...
        assert "rate-limit" in middlewares, (
            "rate-limit middleware missing from routes.yml.tmpl"
        )
    ...
    def test_rate_limit_middleware_exists_in_static(self, routes_yml):
        ...
        assert "rate-limit" in middlewares, (
            "rate-limit middleware missing from routes.yml"
        )
    ...
    def test_rate_limit_middleware_exists_in_k8s(self, k8s_routes):
        ...
        assert "rate-limit" in middlewares, (
            "rate-limit middleware missing from K8s ConfigMap"
        )

    @pytest.mark.parametrize("config_name,loader", [
    ...
    def test_rate_limit_average_is_100(self, request, config_name, loader):
        ...
        assert average == 100, (
            f"[{config_name}] rate-limit average must be 100, got {average}"
        )
    ...
    def test_rate_limit_burst_is_50(self, request, config_name, loader):
        ...
        assert burst == 50, (
            f"[{config_name}] rate-limit burst must be 50, got {burst}"
        )
    ...
    def test_rate_limit_uses_source_criterion_ip_strategy(self, request,
                                                          config_name, loader):
        ...
        assert source_criterion, (
            f"[{config_name}] rate-limit must have sourceCriterion"
        )
        ...
        assert depth is not None, (
            f"[{config_name}] ipStrategy must specify depth"
        )
...
class TestRateLimitAppliedToRouters:
    ...
    def test_all_production_routers_have_rate_limit(self, request,
                                                     config_name, loader):
        ...
        for router_name in self.ROUTERS_REQUIRING_RATE_LIMIT:
            ...
            if router is None:
                ...
                if loader == "k8s_routes":
                    continue
                pytest.fail(
                    f"[{config_name}] Required router '{router_name}' not found"
                )
    ...
    def test_rate_limit_before_other_non_auth_middlewares(self, request,
                                                           config_name, loader):
...
class TestRateLimitCrossFileConsistency:
    ...
    def test_average_consistent_across_all_files(self, routes_tmpl, routes_yml,
                                                  k8s_routes):
        """Average must be 100 in all config files."""
        for name, routes in [("tmpl", routes_tmpl), ("yml", routes_yml),
                              ("k8s", k8s_routes)]:
            rl = routes.get("http", {}).get("middlewares", {}).get("rate-limit", {})
            avg = rl.get("rateLimit", {}).get("average")
            assert avg == 100, f"[{name}] average={avg}, expected 100"

    def test_burst_consistent_across_all_files(self, routes_tmpl, routes_yml,
                                                k8s_routes):
    ...
    def test_ip_strategy_depth_consistent(self, routes_tmpl, routes_yml,
                                           k8s_routes):
        ...
        assert len(unique_depths) == 1, (
            f"ipStrategy depth differs across files: {depths}"
        )
...
class TestRateLimitRawFileChecks:
    ...
    def test_rate_limit_not_commented_out_in_template(self):
        ...
        assert re.search(r'^\s+rate-limit:\s*$', content, re.MULTILINE), (
            "rate-limit middleware appears to be commented out or missing "
            "in routes.yml.tmpl"
        )
