...
class TestCircuitBreakerThresholds:
    ...
    def test_all_success_thresholds_positive(self, thresholds):
        ...
        for name, t in thresholds.items():
            assert t.success_threshold >= 1, (
                f"{name}: success_threshold={t.success_threshold} < 1"
            )
        ...
        for name, t in thresholds.items():
            if name in external_services:
                assert t.timeout_seconds >= 300, (
                    f"{name}: external service must have timeout >= 300s, "
                    f"got {t.timeout_seconds}s"
                )
    ...
    def test_all_thresholds_have_rationale(self, thresholds):
        ...
        for name, t in thresholds.items():
            ...
            assert len(t.rationale) > 20, (
                f"{name}: rationale too short ({len(t.rationale)} chars)"
            )
    ...
    def test_all_thresholds_have_exercised_by_journey(self, thresholds):
        ...
        for name, t in thresholds.items():
            assert t.exercised_by_journey, (
                f"{name}: missing exercised_by_journey"
            )

    def test_key_services_defined(self, thresholds):
        """Critical services must have configured thresholds."""
        required = {
            "compliance-service", "dq-service", "semantic-service",
            "search-service", "webhook-delivery", "stripe_api",
            "email_provider", "file-upload-s3", "asset-service",
            "contract-service", "marketplace-service", "governance-service",
        }
    ...
    def test_k6_journey_references_are_valid(self, thresholds):
        ...
        VALID_JOURNEYS = {
            "login", "asset-crud", "contract-workflow", "marketplace",
            "search", "sparql", "file-upload", "compliance-scan",
            "governance", "webhook", "unknown",
        }
        for name, t in thresholds.items():
    ...
    def test_validate_threshold_catches_low_failure_threshold(self):
        from hub.apps.core.resilience.circuit_breaker_thresholds import (
            CircuitBreakerThreshold, validate_threshold,
        )
        t = CircuitBreakerThreshold(
            service_name="test", failure_threshold=1,
    ...
    def test_validate_threshold_catches_low_timeout(self):
        ...
        t = CircuitBreakerThreshold(
            service_name="test", failure_threshold=5,
            timeout_seconds=10, success_threshold=2,
        )
        violations = validate_threshold(t)
        assert len(violations) >= 1
    ...
    def test_validate_threshold_catches_zero_success_threshold(self):
        ...
        t = CircuitBreakerThreshold(
            service_name="test", failure_threshold=5,
            timeout_seconds=60, success_threshold=0,
        )
        violations = validate_threshold(t)
        assert len(violations) >= 1
    ...
    def test_default_threshold_is_safe(self):
        ...
        from hub.apps.core.resilience.circuit_breaker_thresholds import (
            validate_threshold, get_default_threshold,
        )
