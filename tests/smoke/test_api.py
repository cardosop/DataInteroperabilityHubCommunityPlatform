...
class TestAPIFullStackSmoke:
    ...
    def test_user_credentials(self):
        return {
            "email": os.environ.get("SMOKE_TEST_EMAIL", f"smoke-test-{os.urandom(4).hex()}@meshant.test"),
    ...
    def test_02_register(self, session, test_user_credentials):
        ...
        r = _api("/api/v1/auth/register/", method="post", session=session, json={
            "email": test_user_credentials["email"],
            "password": test_user_credentials["password"],
            "name": test_user_credentials["name"],
        })
        # Registration may return 201 (created) or 400 (already exists from prior run).
        assert r.status_code in (201, 400), f"Unexpected status: {r.status_code} body={r.text[:200]}"

    def test_03_login(self, session, test_user_credentials):
        """Login and obtain auth token."""
        r = _api("/api/v1/auth/login/", method="post", session=session, json={
            "email": test_user_credentials["email"],
            "password": test_user_credentials["password"],
        })
        if r.status_code == 404:
            pytest.skip("Auth endpoint not available (MVP-gated?)")
    ...
    def test_04_profile(self, session):
        ...
        if r.status_code == 404:
            pytest.skip("Profile endpoint not available (MVP-gated?)")
        assert r.status_code == 200, f"Profile failed: {r.status_code}"

    def test_05_create_asset(self, session):
        """Create a test asset."""
        r = _api("/api/v1/assets/", method="post", session=session, json={
            "name": f"smoke-test-asset-{os.urandom(4).hex()}",
            "description": "Created by full-stack smoke test (312.15.1)",
            "asset_type": "dataset",
        })
        if r.status_code == 404:
            pytest.skip("Assets endpoint not available (MVP-gated?)")
        assert r.status_code in (200, 201), f"Asset creation failed: {r.status_code} body={r.text[:200]}"
    ...
    def test_07_delete_asset(self, session):
        ...
        data = r.json()
        results = data if isinstance(data, list) else data.get("results", data.get("data", []))
        smoke_assets = [a for a in results if isinstance(a, dict) and "smoke-test-asset" in str(a.get("name", ""))]
        if not smoke_assets:
            pytest.skip("No smoke test assets to delete")
