    ...
    def test_create_sync_delete_lifecycle(
        ...
        try:
            ...
            if resp.status_code == 404:
                pytest.skip(
                    f"Scheduled ingestion endpoint not found at "
                    f"{_INGESTIONS_PATH} — check routing."
                )
            ...
            print(
                f"\n  Created ingestion {ingestion_id} "
                f"(sync_status={initial_sync})"
            )
            ...
            sync_status = synced_data.get(
                "deployment_sync_status", "unknown"
            )

            if sync_status != "SYNCED":
                # Attempt explicit re-sync via action endpoint
                sync_url = (
                    f"{base_url}/api/v1/scheduled-ingestions/"
                    f"{ingestion_id}/sync/"
                )
                sync_resp = authenticated_session.post(
                    sync_url, timeout=timeout
                )
                print(
                    f"  Sync status={sync_status!r} — "
                    f"re-sync via /sync/ → {sync_resp.status_code}"
                )
            ...
            prefect_deployment_id = synced_data.get(
                "prefect_deployment_id"
            )
            print(
                f"  SYNCED — "
                f"prefect_deployment_id={prefect_deployment_id}"
            )
            ...
            del_url = (
                f"{base_url}"
                f"{_INGESTION_DETAIL_TPL.format(id=ingestion_id)}"
            )
            del_resp = authenticated_session.delete(
                del_url, timeout=timeout
            )
            ...
            if prefect_deployment_id:
                ...
                while time.monotonic() < deadline:
                    check_resp = authenticated_session.get(
                        f"{base_url}"
                        f"{_INGESTION_DETAIL_TPL.format(id=cleaned_id)}",
                        timeout=timeout,
                    )
                ...
                print(
                    "  Prefect deployment confirmed removed "
                    "after ingestion deletion."
                )
            ...
            if ingestion_id:
                try:
                    ...
                    print(
                        f"\n  Cleanup warning: could not delete "
                        f"{ingestion_id}: {exc}"
                    )
...
class TestOrphanDeploymentPurge:
    ...
    def test_orphan_deployment_purge(
        ...
        try:
            ...
            if resp.status_code == 404:
                pytest.skip(
                    "Scheduled ingestion endpoint not available."
                )
            assert resp.status_code in (200, 201, 207), (
                f"Create failed ({resp.status_code}): {resp.text[:500]}"
            )
            ...
            if not container:
                ...
                _exclude = {
                    "gateway", "pgbouncer", "exporter",
                    "monitor", "nginx", "traefik",
                }
            ...
            find_mgmt = subprocess.run(
                ["docker", "exec", container,
                 "find", "/app", "-name", "manage.py",
                 "-path", "*/hub/*", "-maxdepth", "3"],
                capture_output=True, text=True, timeout=10,
            )
            manage_py = find_mgmt.stdout.strip().split("\n")[0] \
                if find_mgmt.stdout.strip() else "manage.py"
            ...
            if check_resp.status_code == 404:
                print(
                    "  Record hard-deleted by purge — "
                    "orphan cleanup confirmed."
                )
                ...
                if not remaining_dep:
                    ...
                    assert status == "DELETED", (
                        f"Expected status=DELETED after soft-delete, "
                        f"got {status!r}"
                    )
                    print(
