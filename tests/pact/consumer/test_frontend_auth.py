...
class TestFrontendAuthContract:
    ...
    def test_login_success(self, auth_pact):
        auth_pact.given("valid credentials").upon_receiving(
        ...
        ).with_request("POST", "/api/v1/auth/login/", body={
            ...
            "user": {"id": Term(r"^[0-9a-f-]+$", "user-uuid"), "email": Like("user@meshant.com")},
        })

        import requests
        result = requests.post("http://localhost:1236/api/v1/auth/login/", json={
            "email": "user@meshant.com", "password": "password123",
        })
    ...
    def test_login_invalid(self, auth_pact):
        auth_pact.given("invalid credentials").upon_receiving(
            ...
            "email": Like("user@meshant.com"), "password": Like("wrong"),
        }).will_respond_with(401, body={
            "error": {"code": "INVALID_CREDENTIALS", "message": Like("Invalid email or password")},
        })

        import requests
        result = requests.post("http://localhost:1236/api/v1/auth/login/", json={
            "email": "user@meshant.com", "password": "wrong",
        })
        assert result.status_code == 401

    def test_register(self, auth_pact):
        auth_pact.given("registration enabled").upon_receiving(
            "register new user"
        ).with_request("POST", "/api/v1/auth/register/", body={
        ...
        import requests
        result = requests.post("http://localhost:1236/api/v1/auth/register/", json={
            "email": "new@meshant.com", "password": "SecurePass1!", "name": "New User",
        })
