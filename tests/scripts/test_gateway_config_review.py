...
"""
        ...
        try:
            ...
            self.assertEqual(
                result['routers']['contract-service']['rule'],
                "PathPrefix(`/api/v1/contracts`)"
            )
                            ...
                            'paths': [
                                {
                                    'path': '/',
                                    'backend': {
                                        'service': {'name': 'test-service', 'port': {'number': 8000}}
                                    }
                                }
                            ]
                        }
...
class TestGatewayConfigReviewerIntegration(unittest.TestCase):
    ...
    def test_find_actual_traefik_configs(self):
        ...
        self.assertIn(expected_docker_compose, config_paths,
                     f"Should find Docker Compose Traefik config: {expected_docker_compose}")
        self.assertIn(expected_k8s, config_paths,
                     f"Should find Kubernetes Traefik config: {expected_k8s}")
    ...
    def test_map_actual_gateway_rules_to_endpoints(self):
        ...
        self.assertGreater(len(mappings['traefik_mappings']), 0,
                          "Should have Traefik route mappings")
        self.assertGreater(len(mappings['ingress_mappings']), 0,
                          "Should have ingress rule mappings")

        # Verify endpoint count
        self.assertGreater(mappings['total_endpoints'], 0,
                          "Should map to at least some endpoints")
    ...
    def test_traefik_routers_have_valid_services(self):
        ...
        self.assertEqual(len(missing_services), 0,
                        f"Routers reference undefined services: {missing_services}")
    ...
    def test_ingress_rules_have_valid_structure(self):
        ...
        for rule in review['rules']:
            ...
            self.assertIsInstance(rule.get('service_port'), int,
    ...
    def test_traefik_middlewares_are_defined(self):
        ...
        self.assertEqual(len(missing_middlewares), 0,
    ...
    def test_traefik_configs_match_docker_compose(self):
        ...
        self.assertGreater(len(matching_services), 0,
