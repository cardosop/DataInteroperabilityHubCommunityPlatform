...
class TestFixturesAndFactoriesChecker(unittest.TestCase):
    ...
    def test_report_exists(self):
        ...
        self.assertTrue(self.report_path.exists(), f"Report not found at {self.report_path}")
    ...
    def test_summary_values(self):
        ...
        self.assertGreater(
            summary["total_fixture_files"], 0, "Should have at least one fixture file"
        )
        self.assertGreaterEqual(
            summary["total_factory_classes"], 0, "Factory classes should be non-negative"
        )
        self.assertGreaterEqual(
            summary["total_endpoint_urls"], 0, "Endpoint URLs should be non-negative"
        )
    ...
    def test_fixture_files_structure(self):
        ...
        for fixture in fixture_files:
            ...
            self.assertTrue(file_path.exists(), f"Fixture file should exist: {fixture['path']}")
    ...
    def test_factory_classes_structure(self):
        ...
        for factory in factory_classes:
            ...
            self.assertTrue(
                file_path.exists(), f"Factory file should exist: {factory['file_path']}"
            )
    ...
    def test_endpoint_urls_structure(self):
        ...
        for endpoint_info in endpoint_urls:
            ...
            is_valid_endpoint = (
                ...
                or "http://" in endpoint
                or "https://" in endpoint
                or ".com" in endpoint
                or ".org" in endpoint
                or ".example" in endpoint
                or endpoint.startswith("/")
            )
            self.assertTrue(
                is_valid_endpoint, f"Endpoint should look like a URL or path: {endpoint}"
            )
    ...
    def test_fixture_endpoints_structure(self):
        ...
        for file_path, endpoints in fixture_endpoints.items():
            ...
            self.assertGreater(
                len(endpoints), 0, f"Should have at least one endpoint for {file_path}"
            )
    ...
    def test_fixture_files_by_type(self):
        ...
        for file_type, count in manual_count.items():
            self.assertEqual(
                summary_by_type.get(file_type, 0), count, f"Count for {file_type} should match"
            )
    ...
    def test_known_fixture_files_exist(self):
        ...
        known_fixtures = ["tests/fixtures", "tests/fixtures/odps", "tests/conftest.py"]

        # At least some known fixtures should be found
        found_known = sum(
            1 for known in known_fixtures if any(known in path for path in fixture_paths)
        )
        self.assertGreater(found_known, 0, "Should find at least some known fixture files")
    ...
    def test_known_factory_files_exist(self):
        ...
        self.assertGreater(found_known, 0, "Should find at least some known factory files")
    ...
    def test_endpoints_in_fixtures(self):
        ...
        self.assertGreater(
            len(fixture_endpoints), 0, "Should have at least some fixtures with endpoints"
        )
    ...
    def test_fixture_file_sizes(self):
        ...
        for fixture in fixture_files:
            size = fixture["size"]
            self.assertGreaterEqual(size, 0, f"File size should be non-negative: {fixture['path']}")
...
class TestFixturesAndFactoriesCheckerIntegration(unittest.TestCase):
    ...
    def test_script_runs_successfully(self):
        ...
        self.assertEqual(
            result.returncode, 0, f"Script should run successfully. Error: {result.stderr}"
        )

        # Verify report was generated
        report_path = (
            project_root / "docs" / "api-audit" / "test-fixtures-and-factories-report.json"
        )
        self.assertTrue(report_path.exists(), "Report should be generated after running script")
    ...
    def test_fixture_files_are_accessible(self):
        ...
        for fixture in fixture_files[:10]:  # Test first 10
            file_path = project_root / fixture["path"]
            self.assertTrue(
                file_path.exists(), f"Fixture file should be accessible: {fixture['path']}"
            )
    ...
    def test_factory_files_are_accessible(self):
        ...
        for file_path_str in list(factory_files)[:10]:  # Test first 10
            ...
            self.assertTrue(
                file_path.exists(), f"Factory file should be accessible: {file_path_str}"
            )
    ...
    def test_endpoints_are_valid_patterns(self):
        ...
        endpoint_urls = report["endpoint_urls"]
        valid_patterns = ["/api/v1/", "localhost:", "http://", "https://", "api.example.com"]

        for endpoint_info in endpoint_urls:
            endpoint = endpoint_info["endpoint"]
            # At least one pattern should match
            matches = any(pattern in endpoint for pattern in valid_patterns)
            self.assertTrue(matches, f"Endpoint should match at least one pattern: {endpoint}")
    ...
    def test_fixture_files_have_reasonable_sizes(self):
        ...
        for fixture in fixture_files:
            size = fixture["size"]
            self.assertGreater(size, 0, f"Fixture file should not be empty: {fixture['path']}")
            self.assertLess(
                size,
    ...
    def test_factory_classes_have_methods(self):
        ...
        self.assertGreater(
            len(factories_with_methods), 0, "Should have at least some factory classes with methods"
        )
    ...
    def test_report_completeness(self):
        ...
        self.assertGreater(len(report["fixture_files"]), 0, "Should have at least one fixture file")
        self.assertGreaterEqual(
            len(report["factory_classes"]), 0, "Should have factory classes (can be 0)"
        )
        self.assertGreaterEqual(
            len(report["endpoint_urls"]), 0, "Should have endpoint URLs (can be 0)"
        )
    ...
    def test_factory_files_in_expected_locations(self):
        ...
        factories_in_expected = sum(
            1 for file_path in factory_files if any(loc in file_path for loc in expected_locations)
        )

        self.assertGreater(
            factories_in_expected, 0, "Should have at least some factories in expected locations"
        )
