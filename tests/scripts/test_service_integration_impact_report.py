...
class TestServiceIntegrationImpactReport(unittest.TestCase):
    ...
    def test_summary_values(self):
        ...
        self.assertGreaterEqual(
            summary["total_integrations"], 0, "Integrations should be non-negative"
        )
        self.assertGreaterEqual(
            summary["total_service_calls"], 0, "Service calls should be non-negative"
        )
    ...
    def test_service_integrations_structure(self):
        ...
        self.assertGreater(
            len(service_integrations), 0, "Should have at least one service integration"
        )

        for _service_name, service_data in service_integrations.items():
        ...
        for service in summary["services"]:
            ...
            for target_service in summary["services"]:
                ...
                self.assertIsInstance(
                    matrix[service][target_service], int, "Matrix value should be integer"
                )
    ...
    def test_integration_matrix_values(self):
        ...
        for source, targets in matrix.items():
            for target, count in targets.items():
                self.assertGreaterEqual(
                    count, 0, f"Matrix value should be non-negative: {source} -> {target}"
                )
    ...
    def test_outbound_integrations_structure(self):
        ...
        for _service_name, service_data in service_integrations.items():
            ...
            for _target, details in outbound.items():
    ...
    def test_inbound_integrations_structure(self):
        ...
        for _service_name, service_data in service_integrations.items():
            ...
            for _source, details in inbound.items():
                self.assertIn("call_count", details)
                self.assertIn("endpoints", details)
                self.assertIn("client_types", details)
                self.assertIsInstance(details["call_count"], int)
        ...
        for _service_name, service_data in service_integrations.items():
            ...
            for route in gateway_routes:
                ...
                self.assertIn(
                    ...
                    "Route type should be traefik or kubernetes-ingress",
                )
    ...
    def test_call_count_consistency(self):
        ...
        for service_name, service_data in service_integrations.items():
            # Sum outbound calls
            outbound_total = sum(
                details.get("call_count", 0) for details in service_data["outbound"].values()
            )
    ...
    def test_report_completeness(self):
        ...
        self.assertTrue(
            has_inter_service_data or has_gateway_data or has_service_client_data,
    ...
    def test_markdown_report_exists(self):
        ...
        self.assertTrue(md_path.exists(), f"Markdown report should exist at {md_path}")
    ...
    def test_markdown_report_content(self):
        ...
        if md_path.exists():
            ...
            self.assertIn(
                "# Service Integration Impact Report", content, "Markdown report should have title"
            )
...
class TestServiceIntegrationImpactReportIntegration(unittest.TestCase):
    ...
    def test_api_service_has_integrations(self):
        ...
        self.assertIn(
            "api-service", service_integrations, "api-service should be in service integrations"
        )
        ...
        self.assertGreater(
            len(api_service["outbound"]), 0, "api-service should have outbound integrations"
        )

        # api-service should have inbound integrations (from CLI/SDK)
        self.assertGreater(
            len(api_service["inbound"]), 0, "api-service should have inbound integrations"
        )
    ...
    def test_service_clients_are_tracked(self):
        ...
        services_with_clients = [
            name for name, data in service_integrations.items() if data.get("service_clients")
        ]

        # Should have at least some services with clients
        self.assertGreater(
            len(services_with_clients), 0, "Should have at least one service with service clients"
        )
    ...
    def test_gateway_routes_are_tracked(self):
        ...
        services_with_routes = [
            name for name, data in service_integrations.items() if data.get("gateway_routes")
        ]

        # Should have at least some services with gateway routes
        self.assertGreater(
            len(services_with_routes), 0, "Should have at least one service with gateway routes"
        )
    ...
    def test_integration_matrix_has_expected_services(self):
        ...
        for service in expected_services:
            if service in summary["services"]:
                self.assertIn(
                    service, matrix, f"Expected service {service} should be in integration matrix"
                )
    ...
    def test_report_regeneration(self):
        ...
        self.assertTrue(
            self.report_path.exists(), "Report should be generated after running script"
        )
    ...
    def test_data_sources_are_loaded(self):
        ...
        self.assertGreater(len(existing_sources), 0, "At least one data source file should exist")
    ...
    def test_endpoints_are_valid(self):
        ...
        for service_name, service_data in service_integrations.items():
            ...
            for endpoint in endpoints:
                self.assertIsInstance(
                    endpoint, str, f"Endpoint should be string for {service_name}"
                )
                # Endpoints can be empty strings, but we'll check they're not None
                self.assertIsNotNone(endpoint, f"Endpoint should not be None for {service_name}")
    ...
    def test_summary_statistics_accuracy(self):
        ...
        actual_integrations = sum(
            len(data.get("outbound", {})) for data in service_integrations.values()
        )
        self.assertEqual(
        ...
        sum(data.get("call_count", 0) for data in service_integrations.values())
        # Note: This might be double-counted (outbound + inbound), so we'll check it's reasonable
        self.assertGreaterEqual(
            summary["total_service_calls"], 0, "Total service calls should be non-negative"
        )
