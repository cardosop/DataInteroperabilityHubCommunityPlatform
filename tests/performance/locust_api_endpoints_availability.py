...
if LOCUST_AVAILABLE:
    ...
    class APIEndpointsAvailabilityUser(FastHttpUser):
        ...
        def on_start(self):
            ...
            self.created_resources = {"assets": [], "contracts": [], "files": [], "datasets": []}
        ...
        def test_get_contract_detail(self):
            ...
            if self.created_resources["contracts"]:
                ...
                self._test_endpoint(
                    "GET", f"/api/v1/contracts/{contract_id}", name="GET /contracts/{id}"
                )
            else:
        ...
        def test_create_asset(self):
            ...
            response = self._test_endpoint(
                "POST", "/api/v1/assets", json=asset_data, name="POST /assets"
            )
        ...
        def test_update_asset(self):
            ...
            if self.created_resources["assets"]:
                asset_id = random.choice(self.created_resources["assets"])
                update_data = {"description": f"Updated at {time.time()}"}
                self._test_endpoint(
                    "PATCH",
        ...
        def test_create_contract(self):
            ...
            contract_data = {
                ...
                "spec": {
                    ...
                    "info": {"title": "Performance Test Contract", "version": "1.0.0"},
                },
            }
            response = self._test_endpoint(
                "POST", "/api/v1/contracts", json=contract_data, name="POST /contracts"
            )
            if response and response.status_code == 201:
                data = response.json()
                if "id" in data:
        ...
        def test_create_dq_run(self):
            ...
            dq_data = {"profile_key": "intake_basic", "engine": "great_expectations"}
            self._test_endpoint(
                "POST", "/api/v1/dq-runs", json=dq_data, name="POST /dq-runs", allow_errors=True
            )
        ...
        def test_init_file_upload(self):
            ...
            response = self._test_endpoint(
                "POST", "/api/v1/files/init", json=file_data, name="POST /files/init"
            )
            if response and response.status_code == 201:
                data = response.json()
                if "id" in data:
            ...
            try:
                if method == "GET":
                    with self.client.get(
                        path, headers=self.headers, name=endpoint_name, catch_response=True
                    ) as response:
