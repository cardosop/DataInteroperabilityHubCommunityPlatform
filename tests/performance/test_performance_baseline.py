...
class DatabaseQueryBaselineTest(PerformanceBaselineTest):
    ...
    def setUp(self):
        ...
        self.asset = Asset.objects.create(
            tenant=self.tenant, key="baseline-asset", name="Baseline Asset"
        )
    ...
    def test_simple_query_baseline(self):
        ...
        stats = self.measure_multiple(
            lambda: Contract.objects.filter(tenant=self.tenant).count(), iterations=20
        )
    ...
    def test_jsonfield_query_baseline(self):
        ...
        self.assertLess(
            stats["max"], 0.2, f"JSONField query max time too high: {stats['max']:.3f}s"
        )
    ...
    def test_jsonfield_nested_query_baseline(self):
        ...
        self.assertLess(
            stats["mean"], 0.15, f"Nested JSONField query too slow: {stats['mean']:.3f}s"
        )
        self.assertLess(
            stats["max"], 0.3, f"Nested JSONField query max time too high: {stats['max']:.3f}s"
        )
    ...
    def test_join_query_baseline(self):
        ...
        stats = self.measure_multiple(
            lambda: list(
                Contract.objects.select_related("asset", "tenant").filter(tenant=self.tenant)[:10]
            ),
            iterations=20,
        )
    ...
    def setUp(self):
        ...
        for i in range(20):
            Contract.objects.create(
                ...
                original_raw="{}",
                hub_contract_version="1.0.0",
                hub_contract_json={"info": {"tags": [f"tag{i % 5}"]}},
                created_by=self.user,
            )

    def test_health_endpoint_baseline(self):
        """Establish baseline for health endpoint."""
        stats = self.measure_multiple(lambda: self.client.get("/health/"), iterations=20)

        # Health endpoint should be very fast
        self.assertLess(stats["mean"], 0.05, f"Health endpoint too slow: {stats['mean']:.3f}s")
        self.assertLess(
            stats["max"], 0.1, f"Health endpoint max time too high: {stats['max']:.3f}s"
        )

    def test_contract_list_endpoint_baseline(self):
        """Establish baseline for contract list endpoint."""
        stats = self.measure_multiple(lambda: self.client.get("/api/v1/contracts/"), iterations=20)

        # Contract list endpoint should be reasonably fast
        self.assertLess(
            stats["mean"], 0.5, f"Contract list endpoint too slow: {stats['mean']:.3f}s"
        )
        self.assertLess(
            stats["max"], 1.0, f"Contract list endpoint max time too high: {stats['max']:.3f}s"
        )

    def test_contract_filter_endpoint_baseline(self):
        """Establish baseline for contract filter endpoint."""
        stats = self.measure_multiple(
            lambda: self.client.get("/api/v1/contracts/", {"tag": "tag1"}), iterations=20
        )

        # Contract filter endpoint should be reasonably fast
        self.assertLess(
            stats["mean"], 0.6, f"Contract filter endpoint too slow: {stats['mean']:.3f}s"
        )
        self.assertLess(
            stats["max"], 1.2, f"Contract filter endpoint max time too high: {stats['max']:.3f}s"
        )
...
class MiddlewareBaselineTest(PerformanceBaselineTest):
    ...
    def test_middleware_chain_baseline(self):
        ...
        stats = self.measure_multiple(lambda: self.client.get("/api/v1/assets/"), iterations=20)

        # Middleware chain should be reasonably fast
        self.assertLess(stats["mean"], 0.5, f"Middleware chain too slow: {stats['mean']:.3f}s")
        self.assertLess(
            stats["max"], 1.0, f"Middleware chain max time too high: {stats['max']:.3f}s"
        )
...
class JSONFieldGINIndexBaselineTest(PerformanceBaselineTest):
    ...
    def setUp(self):
        ...
        self.asset = Asset.objects.create(
            tenant=self.tenant, key="gin-baseline-asset", name="GIN Baseline Asset"
        )
        ...
        for i in range(100):
            Contract.objects.create(
                ...
                hub_contract_version="1.0.0",
                hub_contract_json={
                    "info": {"tags": [f"tag{i % 20}", f"tag{(i + 1) % 20}"]},
                    "quality": {
                        "default_profile_key": "great_expectations" if i % 2 == 0 else "soda"
                    },
                    "privacy_compliance": {
    ...
    def test_gin_index_array_contains_baseline(self):
        ...
        self.assertLess(
            stats["mean"], 0.05, f"GIN index array contains too slow: {stats['mean']:.3f}s"
        )
        self.assertLess(
            stats["max"], 0.1, f"GIN index array contains max time too high: {stats['max']:.3f}s"
        )
    ...
    def test_gin_index_nested_key_baseline(self):
        ...
        self.assertLess(
            stats["max"], 0.15, f"GIN index nested key max time too high: {stats['max']:.3f}s"
        )
    ...
    def test_gin_index_complex_filter_baseline(self):
        ...
        self.assertLess(
            stats["mean"], 0.1, f"GIN index complex filter too slow: {stats['mean']:.3f}s"
        )
        self.assertLess(
            stats["max"], 0.2, f"GIN index complex filter max time too high: {stats['max']:.3f}s"
        )
