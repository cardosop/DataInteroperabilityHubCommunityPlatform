...
class JSONFieldQueryPerformanceTest(PerformanceTest):
    ...
    def setUp(self):
        ...
        from hub.apps.assets.models import Asset
        from hub.apps.contracts.models import ContractStatus, OriginalFormat, OriginalSpecType

        asset = Asset.objects.create(
            tenant=self.tenant, key="json-perf-asset", name="JSON Perf Asset"
        )
        ...
        for i in range(10):
            Contract.objects.create(
                ...
                hub_contract_json={
                    ...
                    "metadata": {"author": f"Author {i}", "tags": [f"tag{i}", f"tag{i + 1}"]},
                },
            )
    ...
    def test_jsonfield_query_performance(self):
        ...
        def run_query():
            return Contract.objects.filter(hub_contract_json__status="active").count()
    ...
    def test_jsonfield_gin_index_usage(self):
        ...
        query = Contract.objects.filter(hub_contract_json__metadata__tags__contains=["tag1"])
        ...
        with connection.cursor() as cursor:
            ...
            uses_index = (
                ...
                or "hub_contract_json" in plan_str.lower()
            )

            self.assertTrue(uses_index, f"GIN index not used in query plan: {plan_str}")
...
class MiddlewarePerformanceTest(PerformanceTest):
    ...
    def test_middleware_chain_performance(self):
        ...
        self.assertLess(
            elapsed,
            1.0,
            f"Middleware chain too slow: {elapsed:.3f}s (target: <500ms in production)",
        )
...
class JobQueuePerformanceTest(PerformanceTest):
    ...
    def setUp(self):
        ...
        self.asset = Asset.objects.create(
            tenant=self.tenant, key="perf-asset", name="Performance Asset"
        )
    ...
    def test_job_bulk_creation_performance(self):
        ...
        self.assertLess(
            avg_time_per_job, 0.05, f"Average time per job too high: {avg_time_per_job:.3f}s"
        )
...
class FileStoragePerformanceTest(PerformanceTest):
    ...
    def test_file_init_upload_performance(self):
        ...
        def init_upload():
            ...
            response = self.client.post(
                "/api/v1/files/init/",
                {"name": "perf-upload.csv", "content_type": "text/csv", "size": 2048},
                format="json",
            )
            return response
    ...
    def test_file_bulk_creation_performance(self):
        ...
        self.assertLess(
            avg_time_per_file, 0.05, f"Average time per file too high: {avg_time_per_file:.3f}s"
        )
    ...
    def test_file_metadata_query_performance(self):
        ...
        def query_file_metadata():
            return list(
                File.objects.filter(tenant=self.tenant, metadata_json__upload_method="browser")
            )
