...
class MetricsOverheadPerformanceTest(TestCase):
    ...
    def test_metrics_endpoint_performance(self):
        ...
        if latencies:
            ...
            self.assertLess(
                p95_latency,
                500.0,
                f"Metrics endpoint P95 latency is {p95_latency:.2f}ms (avg: {avg_latency:.2f}ms), should be <500ms",
            )
    ...
    def test_metric_recording_overhead(self):
        ...
        self.assertLess(
            p95_time,
            100.0,
            f"Metric recording P95 time is {p95_time:.2f}μs (avg: {avg_time:.2f}μs), should be <100μs",
        )
    ...
    def test_metrics_collection_under_load(self):
        ...
        for i in range(10000):
            http_requests_total.labels(
                method="GET", route=f"/test{i % 10}/", status_class="2xx"
            ).inc()
            jobs_started_total.labels(type="DQ_RUN", tenant_id=tenant_id).inc()
        end_time = time.perf_counter()
        ...
        self.assertGreater(
            ops_per_second,
            10000,
            f"Metrics collection throughput is {ops_per_second:.0f} ops/s, should be >10,000 ops/s",
        )
    ...
    def test_metrics_endpoint_under_load(self):
        ...
        self.assertGreater(
            requests_per_second,
            10,
            f"Metrics endpoint throughput is {requests_per_second:.0f} req/s, should be >10 req/s",
        )
...
class MetricsMemoryPerformanceTest(TestCase):
    ...
    def test_metrics_memory_usage(self):
        ...
        self.assertLess(
            object_increase,
            10000,
            f"Memory increase is {object_increase} objects, should be <10,000 for 1000 metrics",
        )
    ...
    def test_metrics_no_memory_leak(self):
        ...
        for _iteration in range(10):
            for i in range(100):
                http_requests_total.labels(
                    method="GET", route=f"/test{i}/", status_class="2xx"
                ).inc()
                jobs_started_total.labels(type="DQ_RUN", tenant_id=tenant_id).inc()
        ...
        self.assertIsNotNone(
            True
        )  # Operation completed without raising  # If we get here without OOM, test passes
