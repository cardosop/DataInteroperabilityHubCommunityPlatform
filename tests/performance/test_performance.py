...
from hub.apps.rate_limiting.utils import sliding_window_check, generate_rate_limit_key, TimeWindow
...
class RateLimitPerformanceTest(TestCase):
    ...
    def test_rate_limit_check_latency_p95(self):
        ...
        self.assertLess(p95_latency, 20.0, f"P95 latency is {p95_latency:.2f}ms (avg: {avg_latency:.2f}ms), target: <5ms in production")
...
class JobProcessingPerformanceTest(TestCase):
    ...
    def test_job_processing_throughput(self):
        ...
        self.assertGreater(throughput, 1.0, f"Throughput is {throughput:.2f} jobs/sec, should be >1")
        ...
        try:
            ...
            if semantic_resource:
                ...
                try:
                    ...
                    self.assertLess(duration, 1.0, f"SPARQL query took {duration:.2f}s, should be <1s")
                except Exception:
...
class MetricsCollectionOverheadTest(TestCase):
    ...
    def test_metrics_endpoint_performance(self):
        ...
        if latencies:
            ...
            # Should be <100ms
            self.assertLess(p95_latency, 100.0, f"P95 latency is {p95_latency:.2f}ms, should be <100ms")
...
class TracingSamplingImpactTest(TestCase):
    ...
    def test_tracing_sampling_impact(self):
        ...
        overhead_ratio = (end_time_with_tracing - end_time_without_tracing) / end_time_without_tracing if end_time_without_tracing > 0 else 0
        
        # Should be <5% overhead (tracing is sampled)
        self.assertLess(overhead_ratio, 0.05, f"Tracing overhead is {overhead_ratio*100:.2f}%, should be <5%")
    ...
    def test_tracing_sampling_rate_impact(self):
        ...
        self.assertTrue(0.0 <= expected_rate <= 1.0, f"Sampling rate should be between 0 and 1, got {expected_rate}")
