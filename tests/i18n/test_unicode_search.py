...
class UnicodeSearchTest(TestCase):
    ...
    def test_search_chinese_asset_by_name(self):
        Asset.objects.create(tenant=self.tenant, key=f"cn-{uuid.uuid4().hex[:6]}", name="数据质量检查", status=AssetStatus.DRAFT)
    ...
    def test_search_emoji_asset(self):
        Asset.objects.create(tenant=self.tenant, key=f"em-{uuid.uuid4().hex[:6]}", name="🚀 Launch Data", status=AssetStatus.DRAFT)
        results = Asset.objects.filter(name__icontains="🚀")
        self.assertEqual(results.count(), 1)

    def test_search_accented_asset(self):
        Asset.objects.create(tenant=self.tenant, key=f"ac-{uuid.uuid4().hex[:6]}", name="café data", status=AssetStatus.DRAFT)
        results = Asset.objects.filter(name__icontains="café")
        self.assertEqual(results.count(), 1)

    def test_search_special_chars(self):
        Asset.objects.create(tenant=self.tenant, key=f"sp-{uuid.uuid4().hex[:6]}", name="O'Brien Dataset", status=AssetStatus.DRAFT)
