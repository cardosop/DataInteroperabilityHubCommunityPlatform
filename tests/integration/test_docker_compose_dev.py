...
class TestDockerComposeDev:
    ...
    def test_dev_volumes_defined(self, dev_compose_config):
        """Test that development-specific volumes are defined."""
    ...
    def test_dev_environment_variables(self, dev_compose_config):
        ...
        # Services that should have ENVIRONMENT=development
            ...
            for vol in volumes:
                if isinstance(vol, str) and ':' in vol:
                    ...
                    if vol_name and not vol_name.startswith('.') and not vol_name.startswith('/'):
                        assert 'dev' in vol_name or vol_name in ['pgdata-dev', 'redis-data-dev', 
                                                                 'minio-data-dev', 'fuseki-data-dev',
                                                                 'prometheus-data-dev', 'grafana-data-dev',
                                                                 'alertmanager-data-dev', 'prefect-server-data-dev',
                                                                 'prefect-db-data-dev', 'traefik-letsencrypt-dev'], \
    ...
    def test_dev_networks_use_dev_network(self, dev_compose_config):
        ...
        for service_name, service_config in services.items():
            ...
            if isinstance(networks, list):
                assert 'hub-net-dev' in networks, \
        ...
        for service_name in application_services_with_code:
            if service_name in services:
                ...
                code_volume_mounted = any(
                    ':/app' in str(vol) or 
                    ':/code' in str(vol) or
                    (isinstance(vol, str) and vol.startswith('.:'))
                    for vol in volumes
                )
                
                assert code_volume_mounted, \
