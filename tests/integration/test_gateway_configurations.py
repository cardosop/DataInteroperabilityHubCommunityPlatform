...
class TestTraefikConfiguration:
    ...
    def setup(self):
        ...
        self.routes_file = (
            self.project_root / "infrastructure" / "traefik" / "dynamic" / "routes.yml"
        )
        self.k8s_configmap = (
            self.project_root / "k8s" / "api-gateway" / "traefik" / "configmap.yaml"
        )
    ...
    def test_compliance_service_route_configured(self):
        ...
        if not self.routes_file.exists():
            pytest.skip("Traefik routes file not found")  # noqa: skip-in-body — runtime service dependency
    ...
    def test_dq_service_route_configured(self):
        ...
        assert "/api/v1/dq/dq-runs" not in content, "Should not use old dq-runs pattern"
    ...
    def test_k8s_configmap_configured(self):
        ...
        if not self.k8s_configmap.exists():
            pytest.skip("K8s ConfigMap not found")  # noqa: skip-in-body — runtime service dependency
...
class TestKubernetesIngressRules:
    ...
    def test_compliance_service_ingress(self):
        ...
        if not compliance_ingress or not compliance_ingress.exists():
            pytest.skip("Compliance service ingress not found")  # noqa: skip-in-body — runtime service dependency
...
class TestGatewayRouting:
    ...
    def test_compliance_endpoint_routing(self):
        ...
        try:
            ...
            response = requests.get(f"{self.api_url}/api/v1/compliance/runs/", timeout=5)
    ...
    def test_dq_endpoint_routing(self):
        ...
        try:
            ...
            response = requests.get(f"{self.api_url}/api/v1/dq/runs/", timeout=5)
    ...
    def test_old_patterns_not_routable(self):
        ...
        try:
            ...
            response = requests.get(f"{self.api_url}/api/v1/compliance/compliance-runs/", timeout=5)
            ...
            response = requests.get(f"{self.api_url}/api/v1/dq/dq-runs/", timeout=5)
...
class TestGatewayVerificationScript:
    ...
    def test_verification_script_runs(self):
        ...
        assert (
            "All API gateway configurations use standardized endpoint patterns" in result.stdout
