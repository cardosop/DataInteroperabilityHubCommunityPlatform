...
try:
    from hub.apps.core.events.kafka_poc import KafkaEventBus, KAFKA_AVAILABLE
except ImportError:
    KAFKA_AVAILABLE = False
    KafkaEventBus = None

try:
    from hub.apps.core.events.rabbitmq_poc import RabbitMQEventBus, RABBITMQ_AVAILABLE
except ImportError:
...
class KafkaPOCTest(TestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            producer = self.kafka_bus._get_producer()
...
class PerformanceComparisonTest(TestCase):
    ...
    def test_publish_throughput_comparison(self):
        ...
        for impl, metrics in results.items():
            print(f"{impl.upper()}: {metrics['throughput']:.2f} events/sec ({metrics['time']:.3f}s)")
    ...
    def test_publish_latency_comparison(self):
        ...
        for impl, metrics in results.items():
            print(f"{impl.upper()}: avg={metrics['avg']:.2f}ms, min={metrics['min']:.2f}ms, max={metrics['max']:.2f}ms, p95={metrics['p95']:.2f}ms")
