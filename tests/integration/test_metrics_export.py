...
class MetricsExportTest(TestCase):
    ...
    def test_api_service_metrics_endpoint(self):
        ...
        # Make a request to generate HTTP metrics via middleware
        ...
        if response.status_code == 200:
            ...
            # Verify common metrics are present (HTTP metrics generated by middleware)
    ...
    def test_metrics_include_job_metrics(self):
        ...
        # Record job metrics first (OpenTelemetry only exports metrics that have been used)
    ...
    def test_metrics_include_service_metrics(self):
        ...
        # Record service metrics first (OpenTelemetry only exports metrics that have been used)
        from hub.apps.observability.otel_metrics import (
            dq_runs_total,
        ...
        tenant_id = str(self.tenant.id)
        dq_runs_total.labels(status='success', engine='great_expectations', tenant_id=tenant_id).inc()
        compliance_runs_total.labels(status='success', risk_level='low', tenant_id=tenant_id).inc()
        contract_validations_total.labels(status='valid', spec_type='ODCS', tenant_id=tenant_id).inc()
