...
class TestDockerComposeStaging:
    ...
    def test_staging_compose_file_exists(self, staging_compose_file):
        ...
        assert staging_compose_file.exists(), f"docker-compose.staging.yml not found at {staging_compose_file}"
    ...
    def test_staging_volumes_defined(self, staging_compose_config):
        """Test that staging-specific volumes are defined."""
    ...
    def test_staging_environment_variables(self, staging_compose_config):
        ...
        # Services that should have ENVIRONMENT=staging
        ...
        for service_name in application_services:
            ...
            # DEBUG should be False for staging
            if 'DEBUG' in environment:
                ...
                if '${DEBUG' not in debug_value:
                    assert debug_value.lower() in ['false', '0', 'no'], \
    ...
    def test_staging_volumes_use_staging_suffix(self, staging_compose_config):
        """Test that volumes use staging-specific names."""
        services = staging_compose_config.get('services', {})
            ...
            for vol in volumes:
                if isinstance(vol, str) and ':' in vol:
                    ...
                    if vol_name and not vol_name.startswith('.') and not vol_name.startswith('/'):
                        assert 'staging' in vol_name or vol_name in ['pgdata-staging', 'redis-data-staging', 
                                                                      'minio-data-staging', 'fuseki-data-staging',
                                                                      'prometheus-data-staging', 'grafana-data-staging',
                                                                      'alertmanager-data-staging', 'prefect-server-data-staging',
                                                                      'prefect-db-data-staging', 'traefik-letsencrypt-staging'], \
    ...
    def test_staging_resource_limits(self, staging_compose_config):
        ...
        # Infrastructure services may not have resource limits
        infrastructure_services = {'postgres', 'redis', 'minio', 'fuseki', 'prometheus', 'grafana', 'jaeger', 'alertmanager', 'traefik'}
