...
class TestApiLogic:
    ...
    def test_api_endpoint_responds(self):
        """Verify the DQ compliance runs API endpoint returns a valid response."""
        client = APIClient()

        tenant = Tenant.objects.create(
            name=f"DQ API Test {uuid.uuid4().hex[:8]}",
            slug=f"dq-api-{uuid.uuid4().hex[:8]}",
            status="ACTIVE",
            kyc_status="UNVERIFIED",
        )
        user = User.objects.create_user(
            email=f"dq-api-{uuid.uuid4().hex[:8]}@example.com",
            password="testpass123",
            tenant=tenant,
            status=UserStatus.ACTIVE,
        )
        # Assign a role that grants access to DQ endpoints
        ...
        assert response.status_code == status.HTTP_200_OK, (
            f"DQ runs endpoint returned {response.status_code}, expected 200. "
