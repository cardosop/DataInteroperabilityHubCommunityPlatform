...
class TestDQRunCreateAPI(TestCase):
    ...
    def setUp(self):
        ...
        self.dataset = DatasetFactory.create_dataset(
            tenant=self.tenant, asset=self.asset, file=self.file, created_by=self.user
        )
    ...
    def test_create_dq_run_with_profile_key(self):
        ...
        data = {"asset_id": str(self.asset.id), "profile_key": "intake_basic_gx"}
    ...
    def test_create_dq_run_invalid_profile_key(self):
        ...
        data = {"asset_id": str(self.asset.id), "profile_key": "invalid_profile"}
    ...
    def test_create_dq_run_user_without_tenant(self):
        ...
        # Should return 400, 403 (tenant suspended/no subscription), or 404  # noqa: broad-status-codes
...
class TestDQRunRetrieveAPI(TestCase):
    ...
    def setUp(self):
        ...
        self.tenant_a = TenantFactory.create_tenant(
            name=f"Tenant A {uuid.uuid4().hex[:8]}", slug="tenant-a", status=TenantStatus.ACTIVE
        )

        self.tenant_b = TenantFactory.create_tenant(
            name=f"Tenant B {uuid.uuid4().hex[:8]}", slug="tenant-b", status=TenantStatus.ACTIVE
        )
        ...
        self.asset_a = AssetFactory.create_asset(
            tenant=self.tenant_a, created_by=self.user_a, status=AssetStatus.ACTIVE
        )

        self.asset_b = AssetFactory.create_asset(
            tenant=self.tenant_b, created_by=self.user_b, status=AssetStatus.ACTIVE
        )
...
class TestDQRunUpdateAPI(TestCase):
    ...
    def setUp(self):
        ...
        # Create DQ run
        job = JobFactory.create_job(
            tenant=self.tenant, type=JobType.DQ_RUN, status=JobStatus.PENDING, created_by=self.user
        )
    ...
    def test_partial_update_dq_run_success(self):
        ...
        response = self.client.patch(f"/api/v1/dq/runs/{self.dq_run.id}/", {}, format="json")
    ...
    def test_full_update_dq_run_success(self):
        ...
        response = self.client.put(
            f"/api/v1/dq/runs/{self.dq_run.id}/",
            {"asset": str(self.asset.id), "profile_key": "intake_basic_gx"},
            format="json",
        )
    ...
    def test_update_dq_run_tenant_isolation(self):
        ...
        other_asset = AssetFactory.create_asset(
            tenant=other_tenant, created_by=other_user, status=AssetStatus.ACTIVE
        )
        ...
        response = self.client.patch(f"/api/v1/dq/runs/{other_dq_run.id}/", {}, format="json")
    ...
    def test_update_dq_run_not_found(self):
        ...
        fake_id = uuid.uuid4()
        response = self.client.patch(f"/api/v1/dq/runs/{fake_id}/", {}, format="json")
...
class TestDQScorecardsAPI(TestCase):
    ...
    def test_get_executive_dashboard_success(self):
        ...
        if response.status_code == status.HTTP_404_NOT_FOUND:
            ...
            dashboard = DQScorecardService.get_executive_dashboard(
                tenant_id=str(self.tenant.id), days=30
            )
    ...
    def test_get_asset_scorecard_success(self):
        ...
        response = self.client.get(
            "/api/v1/dq/scorecards/", {"type": "asset", "asset_id": str(self.asset.id)}
        )
        ...
        if response.status_code == status.HTTP_404_NOT_FOUND:
            ...
            scorecard = DQScorecardService.get_asset_scorecard(
                asset_id=str(self.asset.id), tenant_id=str(self.tenant.id), days=30
            )
    ...
    def test_get_scorecard_drill_down(self):
        ...
        response = self.client.get(
            "/api/v1/dq/scorecards/",
            {"type": "drill_down", "asset_id": str(self.asset.id), "days": 30},
        )
        ...
        if response.status_code == status.HTTP_404_NOT_FOUND:
            ...
            drill_down = DQScorecardService.drill_down(
                tenant_id=str(self.tenant.id), asset_id=str(self.asset.id), days=30
            )
    ...
    def test_get_scorecards_with_date_range(self):
        ...
        response = self.client.get("/api/v1/dq/scorecards/", {"type": "executive", "days": 7})
        ...
        if response.status_code == status.HTTP_200_OK:
            ...
            dashboard = DQScorecardService.get_executive_dashboard(
                tenant_id=str(self.tenant.id), days=7
            )
    ...
    def test_get_scorecards_empty_data(self):
        ...
        if response.status_code == status.HTTP_200_OK:
            ...
            dashboard = DQScorecardService.get_executive_dashboard(
                tenant_id=str(empty_tenant.id), days=30
            )
...
class TestDQRunIntegration(TestCase):
    ...
    def test_create_and_retrieve_dq_run_workflow(self):
        ...
        # Create DQ run
        create_data = {"asset_id": str(self.asset.id)}
...
class TestDQRunEdgeCases(TestCase):
    ...
    def test_create_dq_run_with_empty_profile_key(self):
        ...
        data = {"asset_id": str(asset.id), "profile_key": ""}
