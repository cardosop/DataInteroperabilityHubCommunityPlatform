...
class FileUploadTest(TransactionTestCase):
    ...
    def test_file_upload_size_limits_browser(self):
        ...
        if init_response.status_code == 500:
            error_msg = str(init_response.data) if hasattr(init_response, 'data') else str(init_response.content)
        ...
        self.assertTrue('exceeds' in error_text or 'size' in error_text or 'limit' in error_text,
    ...
    def test_file_upload_validation_invalid_type(self):
        ...
        if 'error' in init_response.data:
            ...
            error_text = str(init_response.data).lower()
        self.assertTrue('not allowed' in error_text or 'type' in error_text or 'exe' in error_text,
    ...
    def test_file_upload_validation_no_extension(self):
        ...
        if 'error' in init_response.data:
            ...
            error_text = str(init_response.data).lower()
        self.assertTrue('extension' in error_text or 'must have' in error_text,
    ...
    def test_file_upload_progress_tracking_multipart(self):
        ...
        if init_response.status_code == 500:
            ...
            if 'InvalidAccessKeyId' in error_msg or 'S3' in error_msg or 'MinIO' in error_msg:
                self.skipTest(f"S3/MinIO connection failed: {error_msg[:200]}. "
    ...
    def test_file_upload_error_handling_missing_tenant(self):
        ...
        user_no_tenant = User.objects.create_user(
            email=f"notenant_{uuid.uuid4().hex[:8]}@example.com",
            password="testpass123"
        )
        ...
        self.assertIn('tenant', error_text,
                     f"Expected error about tenant, got: {init_response.data}")
...
class FileDownloadTest(TransactionTestCase):
    ...
    def test_file_download_error_handling_deleted_file(self):
        ...
        if response.status_code == status.HTTP_400_BAD_REQUEST:
            ...
            self.assertTrue('not available' in error_text or 'status' in error_text or 'deleted' in error_text,
...
class FileStorageTest(TransactionTestCase):
    ...
    def test_file_storage_versioning(self):
        ...
        storage_path_v1 = self.storage_client.save_file(
            tenant_id=str(self.tenant.id),
            file_id=file_id,
            file_content=BytesIO(file_content_v1)
        )
        
        # Save version 2 (same file_id, different path for versioning)
        file_content_v2 = b"Version 2 content"
        storage_path_v2 = f"{storage_path_v1}.v2"
        self.storage_client.save_file(
            tenant_id=str(self.tenant.id),
            file_id=file_id,
            file_content=BytesIO(file_content_v2)
        )
    ...
    def test_odps_file_upload_and_create_contract(self):
        ...
        if contract_response.status_code in [status.HTTP_201_CREATED, status.HTTP_200_OK]:
            ...
            if isinstance(response_data, dict):
                ...
                odps_contract_id = response_data.get('odps_contract', {}).get('id') if isinstance(response_data.get('odps_contract'), dict) else None
                ...
                if odps_contract_id:
                    ...
                    odcs_contract_id = response_data.get('odcs_contract', {}).get('id') if isinstance(response_data.get('odcs_contract'), dict) else None
                    if odcs_contract_id:
