...
class TestSearchAPI(TestCase):
    ...
    def test_search_pagination_offset(self):
        ...
        self.client.force_authenticate(user=self.user_a)
        response_page1 = self.client.get('/api/v1/search/search/', {
            'q': 'data',
            'limit': 1,
            'offset': 0
        })
        response_page2 = self.client.get('/api/v1/search/search/', {
            'q': 'data',
            'limit': 1,
            'offset': 1
        })
        ...
        if len(response_page1.data['results']) > 0 and len(response_page2.data['results']) > 0:
            self.assertNotEqual(
                response_page1.data['results'][0]['id'],
                response_page2.data['results'][0]['id']
            )
