...
class RESTAPIEndpointsTest(TestCase):
    ...
    def test_assets_endpoints(self):
        ...
        if response.status_code != 404:
            ...
            # 400 (Bad Request) is valid for validation errors
            self.assertIn(create_response.status_code, [201, 400, 404, 405])
    ...
    def test_files_endpoints(self):
        ...
        if response.status_code != 404:
            ...
            self.assertIn(init_response.status_code, [200, 201, 400, 404])
...
class AuthorizationChecksTest(TestCase):
    ...
    def test_tenant_isolation(self):
        ...
        if response.status_code == 200:
            ...
            assets = response.data.get('results', response.data if isinstance(response.data, list) else [])
            for asset in assets:
