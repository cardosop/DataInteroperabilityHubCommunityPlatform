...
class TestRunbookEndpointReferences:
    ...
    def test_runbooks_no_old_patterns(self):
        ...
        for file_path in runbook_files:
            ...
            for pattern in old_patterns:
                ...
                for match in matches:
                    ...
                    is_deprecated = any(marker in line_content.lower() for marker in [
                        'deprecated', 'do not use', 'old pattern', 'should return 404',
        ...
        assert len(issues) == 0, (
            ...
            "\n".join([f"  {issue['file']}:{issue['line']} - {issue['pattern']}\n    Context: {issue['context']}" for issue in issues])
        )
    ...
    def test_troubleshooting_guide_has_endpoint_section(self):
        ...
        assert '/api/v1/dq/runs/' in content, (
            "Troubleshooting guide should reference DQ endpoints"
        )
    ...
    def test_runbook_endpoints_accessible(self):
        """Test that endpoints referenced in runbooks are accessible (if services running)"""
        ...
        try:
            response = requests.get(
                f"{self.api_base_url}/api/v1/dq/runs/",
                timeout=5
            )
            # Should return 401 (unauthorized) or 200 (if public), not 404
    ...
    def test_old_patterns_not_accessible(self):
        """Test that old endpoint patterns are not accessible (if services running)"""
        if not self.services_available:
            pytest.skip("API services not available")
        ...
        try:
            response = requests.get(
                f"{self.api_base_url}/api/v1/dq/dq-runs/",
                timeout=5,
                allow_redirects=False
            )
            # Old patterns should return 404 (not found) or redirect
