...
class JobQueueDashboardsTest(TestCase):
    ...
    def test_odps_dashboard_structure(self):
        ...
        self.assertGreater(len(dashboard_obj['panels']), 0, "Dashboard must have at least one panel")
    ...
    def test_odps_dashboard_queries_use_correct_metrics(self):
        ...
        for metric in expected_metrics:
            self.assertIn(
                metric,
                queries_str,
                f"ODPS dashboard should use metric '{metric}'"
            )
    ...
    def test_odps_dashboard_filters_odps_jobs(self):
        ...
        self.assertTrue(
            has_odps_filter,
            "ODPS dashboard should filter for ODPS job types"
        )
    ...
    def test_worker_health_dashboard_queries_use_correct_metrics(self):
        ...
        for metric in expected_metrics:
            self.assertIn(
                metric,
                queries_str,
                f"Worker health dashboard should use metric '{metric}'"
            )
        ...
        for dashboard_path in dashboards:
            ...
            for panel in panels:
                if 'gridPos' in panel:
                    ...
                    self.assertGreaterEqual(grid_pos['x'], 0, "Panel x position should be non-negative")
                    self.assertGreaterEqual(grid_pos['y'], 0, "Panel y position should be non-negative")
