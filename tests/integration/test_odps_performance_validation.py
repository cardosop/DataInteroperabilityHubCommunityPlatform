...
SEMANTIC_MAPPING_TARGET_MS_REALISTIC = (
    10000  # More realistic target based on actual performance (10s)
)
...
class TestODPSIngestionPerformance:
    ...
    def test_odps_ingestion_performance_complex(self, sample_odps_product, authenticated_client):
        ...
        if "marketplace" not in product["product"]:
            product["product"]["marketplace"] = {"pricingPlans": []}
    ...
    def test_odps_ingestion_performance_multiple_products(
        self, sample_odps_product, authenticated_client
    ):
        ...
        for i in range(5):
            ...
            product["product"]["details"]["en"]["productID"] = (
                f"perf-batch-{i}-{uuid.uuid4().hex[:8]}"
            )
...
class TestRefResolutionPerformance:
    ...
    def test_ref_resolution_performance_external(self, sample_odps_product, authenticated_client):
        ...
        if response.status_code not in [200, 201]:
            ...
            pytest.skip(  # noqa: skip-in-body — runtime service dependency
                f"External ref resolution failed (expected for some URLs): {_response_body(response)}"
            )
    ...
    def test_ref_resolution_performance_multiple_refs(
        self, sample_odps_product, authenticated_client
    ):
...
class TestExportGenerationPerformance:
    ...
    def test_export_generation_performance_odps(self, sample_odps_product, authenticated_client):
        ...
        export_response = client.get(f"/api/v1/contracts/{contract_id}/export/?format=odps")
    ...
    def test_export_generation_performance_odcs(self, sample_odps_product, authenticated_client):
        ...
        export_response = client.get(f"/api/v1/contracts/{contract_id}/export/?format=odcs")
        export_time_ms = (time.time() - start_time) * 1000
...
class TestSemanticMappingPerformance:
    ...
    def test_semantic_mapping_performance_basic(self, sample_odps_product):
        ...
        try:
            ...
            assert response.status_code == 200, f"Semantic mapping failed: {response.text}"
    ...
    def test_semantic_mapping_performance_complex(self, sample_odps_product):
        ...
        product["product"]["marketplace"]["pricingPlans"].extend(
            [{"planID": f"plan-{i}", "name": f"Plan {i}", "price": 10.0 * i} for i in range(10)]
        )
        ...
        try:
            ...
            assert response.status_code == 200, f"Complex semantic mapping failed: {response.text}"
    ...
    def test_semantic_mapping_performance_batch(self, sample_odps_product):
        ...
        try:
            ...
            json={"product": product, "product_uuid": product_uuid},
                ...
                assert response.status_code == 200, f"Batch mapping {i} failed: {response.text}"
...
class TestPerformanceComprehensive:
    ...
    def test_end_to_end_performance(self, sample_odps_product, authenticated_client):
        ...
        if mapping_time is not None:
            total_target = (
                ODPS_INGESTION_TARGET_MS
                + int(SEMANTIC_MAPPING_TARGET_MS_REALISTIC * 1.5)
                + EXPORT_GENERATION_TARGET_MS
            )
        else:
