...
class TestDeveloperGuidesAccuracy:
    ...
    def test_verification_script_exists(self):
        ...
        assert self.verify_script.exists(), (
            f"Verification script should exist: {self.verify_script}"
        )
        assert self.verify_script.is_file(), (
            f"Verification script should be a file: {self.verify_script}"
        )
    ...
    def test_verification_script_runs_successfully(self):
        ...
        assert (
            ...
            or "standardized" in result.stdout.lower()
    ...
    def test_no_old_patterns_in_documentation(self):
        ...
        assert (
            "compliance-runs" not in result.stdout.lower()
            or "issues found: 0" in result.stdout.lower()
    ...
    def test_key_developer_guides_exist(self):
        ...
        for guide_name in key_guides:
            ...
            assert guide_path.stat().st_size > 0, (
                f"Developer guide should not be empty: {guide_path}"
            )
...
class TestCodeExamplesWork:
    ...
    def test_code_examples_are_valid(self):
        ...
        assert (
            "syntax_error" not in result.stdout.lower()
            or "issues found: 0" in result.stdout.lower()
...
class TestIntegrationGuides:
    ...
    def test_odps_integration_guide_uses_standardized_patterns(self):
        ...
        if not odps_guide.exists():
            pytest.skip("ODPS_INTEGRATION_GUIDE.md not found")  # noqa: skip-in-body — runtime service dependency
...
class TestMigrationGuide:
    ...
    def test_migration_guide_exists(self):
        ...
        assert self.migration_guide.exists(), (
            f"Migration guide should exist: {self.migration_guide}"
        )
        assert self.migration_guide.is_file(), (
            f"Migration guide should be a file: {self.migration_guide}"
        )
    ...
    def test_migration_guide_has_old_patterns(self):
        ...
        assert "/compliance-runs/" in content, (
            "Migration guide should document old compliance pattern"
        )
    ...
    def test_migration_guide_has_new_patterns(self):
        ...
        assert "/api/v1/compliance/runs/" in content, (
            "Migration guide should document new compliance pattern"
        )
