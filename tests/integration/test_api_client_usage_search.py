...
class TestAPIClientUsageSearch:
    ...
    def test_client_types_detected(self):
        ...
        assert len(found_types) > 0, f"Expected client types not found. Found: {list(client_types.keys())}"
    ...
    def test_http_methods_detected(self):
        ...
        assert len(found_methods) > 0, f"Expected HTTP methods not found. Found: {list(methods.keys())}"

    def test_endpoint_mapping_structure(self):
        """Test that endpoint mappings have correct structure"""
        report = self.report
        for endpoint, mapping in list(report['endpoint_mappings'].items())[:10]:
    ...
    def test_api_v1_endpoints_found(self):
        ...
        report = self.report
        api_v1_endpoints = [
            ep for ep in report['endpoint_mappings'].keys()
            if '/api/v1/' in ep
        ]
    ...
    def test_sdk_client_usages_found(self):
        ...
        sdk_usages = [
            usage for usage in report.get('client_usages', [])
    ...
    def test_endpoint_diversity(self):
        ...
        found_services = [
            keyword for keyword in service_keywords
            if any(keyword in ep for ep in endpoints)
        ]
        assert len(found_services) >= 2, f"Expected endpoints from multiple services. Found: {found_services}"
