...
class TestInterServiceCommunicationSearch:
    ...
    def test_report_exists(self):
        ...
        assert self.report_path.exists(), f"Inter-service communication report not found at {self.report_path}"
    ...
    def test_api_service_dependencies(self):
        ...
        report = self.report
        api_deps = [
            dep for dep_key, dep in report['dependencies'].items()
        ...
        external_services = ['dq-service', 'compliance-service', 'semantic-service', 'datacontract-service']
        found_external = [
            dep['target_service'] for dep in api_deps
            if dep['target_service'] in external_services
        ]
        assert len(found_external) > 0, f"Expected API dependencies to external services. Found: {found_external}"
    ...
    def test_service_client_types(self):
        ...
        expected_types = ['httpx', 'requests', 'ComplianceServiceClient', 'DQServiceClient', 'SemanticServiceClient']
        found_types = [ct for ct in expected_types if any(ct in client_type for client_type in client_types)]
        assert len(found_types) > 0, f"Expected service client types not found. Found: {list(client_types)}"

    def test_endpoint_diversity(self):
        """Test that diverse endpoints are found"""
        report = self.report
        endpoints = set(call['endpoint'] for call in report['service_calls'] if call['endpoint'] != 'unknown')

        # Should have endpoints from different services
        endpoint_keywords = ['health', 'scan', 'run', 'map', 'validate', 'normalize', 'sparql']
        found_keywords = [
            keyword for keyword in endpoint_keywords
    ...
    def test_comprehensive_coverage(self):
        ...
        found_areas = [
            ...
            if any(area in path for path in file_paths)
        ]
    ...
    def test_service_communication_patterns(self):
        ...
        queue_calls = [c for c in report['service_calls'] if c['method'] == 'ENQUEUE' or 'queue' in c['endpoint'].lower()]
    ...
    def test_external_service_communication(self):
        ...
        report = self.report
        external_calls = [
            call for call in report['service_calls']
