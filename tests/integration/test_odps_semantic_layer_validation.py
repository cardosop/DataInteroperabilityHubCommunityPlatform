...
def _map_product_to_rdf(
    ...
    product_uuid: str,
    odcs_contract_uuid: Optional[str] = None,
...
def _execute_sparql_query(
    ...
    if response.status_code in [500, 503]:
        try:
            error_detail = response.json().get("detail", "") if response.status_code == 503 else response.text
        except:
    ...
    def test_complete_odps_to_rdf_mapping(self, complete_odps_product):
        ...
        if result["semantic_status"] == "OK":
            ...
            try:
                ...
                pytest.skip(f"Fuseki query verification failed (mapping succeeded): {e}")
    ...
    def test_odps_mapping_with_marketplace_components(self, complete_odps_product):
        ...
        if result["semantic_status"] == "OK":
            ...
            try:
                ...
                pytest.skip("Fuseki not available - skipping marketplace component verification")
...
class TestSPARQLQueries:
    ...
    def test_query_products_by_name(self, complete_odps_product):
        ...
        if mapping_result.get("semantic_status") != "OK":
            ...
            pytest.skip("Fuseki not available - skipping query tests but mapping verified")

        # Test query by name
        query = ODPSProductQueryBuilder.query_products_by_name(
            name_pattern="Comprehensive Validation",
            limit=10
        )
        bindings = _execute_sparql_query(query)
        ...
        product_found = any(
            "validation-test-product-001" in b.get("productUri", {}).get("value", "") or
    ...
    def test_query_products_by_description(self, complete_odps_product):
        ...
        if mapping_result.get("semantic_status") != "OK":
            ...
            pytest.skip("Fuseki not available - skipping query tests")

        # Test query by description
        query = ODPSProductQueryBuilder.query_products_by_description(
            description_pattern="comprehensive product",
            limit=10
        )
        bindings = _execute_sparql_query(query)
    ...
    def test_query_products_by_pricing_plan(self, complete_odps_product):
        ...
        plan_found = any(
            "Premium" in b.get("planName", {}).get("value", "")
            for b in bindings
        )
    ...
    def test_query_products_by_access_method(self, complete_odps_product):
        ...
        method_found = any(
            "REST API" in b.get("methodType", {}).get("value", "")
            for b in bindings
        )
    ...
    def test_query_products_by_product_strategy(self, complete_odps_product):
        ...
        objective_found = any(
            "adoption" in b.get("objective", {}).get("value", "").lower()
            for b in bindings
        )
    ...
    def test_query_empty_results(self, complete_odps_product):
        ...
        query = ODPSProductQueryBuilder.query_products_by_name(
            name_pattern="NonExistentProductName12345",
            limit=10
        )
        bindings = _execute_sparql_query(query)
...
class TestMultilingualSupport:
    ...
    def test_multilingual_rdf_mapping(self, complete_odps_product):
        ...
        if mapping_result.get("semantic_status") == "OK":
            ...
            try:
                ...
                pytest.skip("Fuseki not available - skipping multilingual structure verification")
    ...
    def test_multilingual_query_english(self, complete_odps_product):
        ...
        if mapping_result.get("semantic_status") != "OK":
            assert mapping_result["triples_count"] > 0
            pytest.skip("Fuseki not available - skipping multilingual query tests")
    ...
    def test_multilingual_query_french(self, complete_odps_product):
        ...
        # Test French query
        query = ODPSProductQueryBuilder.query_products_by_name(
            name_pattern="Validation Complet",
            language="fr",
            limit=10
        )
        bindings = _execute_sparql_query(query)
    ...
    def test_multilingual_query_german(self, complete_odps_product):
        ...
        # Test German query
        query = ODPSProductQueryBuilder.query_products_by_name(
            name_pattern="Validierungs-Testprodukt",
            language="de",
            limit=10
        )
        bindings = _execute_sparql_query(query)
    ...
    def test_multilingual_query_spanish(self, complete_odps_product):
        ...
        # Test Spanish query
        query = ODPSProductQueryBuilder.query_products_by_name(
            name_pattern="Validación Integral",
            language="es",
            limit=10
        )
        bindings = _execute_sparql_query(query)
...
class TestProductContractLinking:
    ...
    def test_product_contract_linking_bidirectional(self, complete_odps_product):
        ...
        mapping_result = _map_product_to_rdf(
            complete_odps_product,
            product_uuid,
            odcs_contract_uuid=odcs_contract_uuid
        )

        # Only test queries if both mappings were successful
        if mapping_result.get("semantic_status") != "OK" or contract_result.get("semantic_status") != "OK":
    ...
    def test_product_contract_linking_specific_contract(self, complete_odps_product):
        ...
        if mapping_result.get("semantic_status") != "OK" or contract_result.get("semantic_status") != "OK":
            ...
            pytest.skip("Fuseki not available - skipping linking query tests")
    ...
    def test_product_contract_linking_specific_product(self, complete_odps_product):
        ...
        query = ODPSProductQueryBuilder.query_odcs_contracts_linked_to_products(
            product_id="validation-test-product-001",
            limit=10
        )
        bindings = _execute_sparql_query(query)
...
class TestComprehensiveIntegration:
    ...
    def test_complete_workflow_integration(self, complete_odps_product):
        ...
        if mapping_result.get("semantic_status") != "OK":
            assert mapping_result["triples_count"] > 0
            pytest.skip("Fuseki not available - skipping query workflow tests but mapping verified")
        ...
        if contract_result.get("semantic_status") == "OK":
            ...
            link_found = any(
                product_uuid in b.get("product", {}).get("value", "") and
                odcs_contract_uuid in b.get("odcsContract", {}).get("value", "")
                for b in bindings
            )
    ...
    def test_multiple_products_multilingual_queries(self, complete_odps_product):
        ...
        if mapping_result.get("semantic_status") != "OK":
            pytest.skip("Fuseki not available - skipping multi-product query tests")
        ...
        assert len(bindings) >= 1, f"Should find at least one mapped product (found {len(bindings)})"
