...
class TestConsumerImpactReport:
    ...
    def report_data(self, report_data_path):
        ...
        if not report_data_path.exists():
            pytest.skip(
                "Consumer impact report not found. Run scripts/generate_consumer_impact_report.py first."
            )
    ...
    def test_impact_matrix_structure(self, report_data):
        ...
        for endpoint, info in impact_matrix.items():
            ...
            assert isinstance(info["impact_score"], (int, float)), (
                f"impact_score must be numeric for {endpoint}"
            )
            ...
            assert isinstance(info["total_consumers"], int), (
                f"total_consumers must be integer for {endpoint}"
            )
            assert info["total_consumers"] >= 0, f"total_consumers must be >= 0 for {endpoint}"
            assert isinstance(info["consumer_types"], dict), (
                f"consumer_types must be dict for {endpoint}"
            )
    ...
    def test_priority_classification_accuracy(self, report_data):
        ...
        for endpoint, info in impact_matrix.items():
            ...
            if priority == "CRITICAL":
                ...
                assert 20 <= score < 50, (
                    f"HIGH priority endpoint {endpoint} has score {score} not in [20, 50)"
                )
            elif priority == "MEDIUM":
                assert 10 <= score < 20, (
                    f"MEDIUM priority endpoint {endpoint} has score {score} not in [10, 20)"
                )
            elif priority == "LOW":
        ...
        for endpoint, info in impact_matrix.items():
            ...
            for consumer_id in info["consumers"]:
                assert consumer_id in consumers, (
                    f"Consumer {consumer_id} not found in consumers dict"
                )
            ...
            for consumer_id in info["consumers"]:
                if consumer_id in consumers:
                    ...
                    if consumer_refs:
                        ...
                        actual_type_counts[consumer_type] = actual_type_counts.get(
                            consumer_type, 0
                        ) + len(consumer_refs)
        ...
        for endpoint, info in list(impact_matrix.items())[:10]:  # Test first 10 endpoints
            ...
            for consumer_id in info["consumers"]:
                if consumer_id in consumers:
                    ...
                    if consumer_refs:
                        consumer_type = consumer_refs[0].get("type", "unknown")
                        type_counts[consumer_type] = type_counts.get(consumer_type, 0) + len(
                            consumer_refs
                        )
            ...
            variance = (
                abs(actual_score - expected_score) / max(expected_score, 1)
                if expected_score > 0
                else 0
            )
    ...
    def test_consumer_type_diversity(self, report_data):
        ...
        all_types = set()
        for _endpoint, info in impact_matrix.items():
    ...
    def test_top_endpoints_have_high_scores(self, report_data):
        ...
        sorted_endpoints = sorted(
            impact_matrix.items(), key=lambda x: x[1]["impact_score"], reverse=True
        )

        # Top 10 endpoints should have impact scores > 0
        for endpoint, info in sorted_endpoints[:10]:
            assert info["impact_score"] > 0, f"Top endpoint {endpoint} has impact score 0"
