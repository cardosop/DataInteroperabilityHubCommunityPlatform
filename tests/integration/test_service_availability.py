...
class ServiceAvailabilityTests(TestCase):
    ...
    def test_check_dq_service_availability(self):
        ...
        is_available, error_msg = check_service_availability(
            service_name="dq-service", service_url=dq_url, health_path="/health", timeout=5
        )
    ...
    def test_check_datacontract_service_availability(self):
        ...
        datacontract_url = getattr(
            settings, "DATACONTRACT_CLI_SERVICE_URL", "http://localhost:8080"
        )
    ...
    def test_check_all_services(self):
        ...
        # Verify each result
        for _service_name, (is_available, error_msg) in results.items():
            self.assertIsInstance(is_available, bool)
            if not is_available:
                self.assertIsNotNone(error_msg)
    ...
    def test_get_all_service_configs(self):
        ...
        for _service_name, config in configs.items():
