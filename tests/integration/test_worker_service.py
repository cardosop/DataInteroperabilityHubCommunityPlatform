...
from tests.factories import JobFactory, TenantConfigFactory, TenantFactory
...
class WorkerServiceIntegrationTest(TestCase):
    ...
    def test_job_creation_with_tenant_limits(self):
        ...
        TenantConfigFactory.create_tenant_config(
            tenant=self.tenant, max_job_concurrency=2, max_queued_jobs=3
        )
    ...
    def test_job_creation_with_queued_limits(self):
        ...
        # Create tenant config with low limits
        TenantConfigFactory.create_tenant_config(
            tenant=self.tenant, max_job_concurrency=10, max_queued_jobs=2
        )
    ...
    def test_concurrency_limit_enforcement_at_job_creation(self):
        ...
        # Create tenant config with low limits
        TenantConfigFactory.create_tenant_config(
            tenant=self.tenant, max_job_concurrency=1, max_queued_jobs=5
        )
    ...
    def test_concurrency_limit_tenant_isolation(self):
        ...
        TenantConfigFactory.create_tenant_config(
            tenant=tenant1, max_job_concurrency=1, max_queued_jobs=3
        )
        TenantConfigFactory.create_tenant_config(
            tenant=tenant2, max_job_concurrency=2, max_queued_jobs=5
        )
    ...
    def test_queue_selection_integration(self):
        ...
        for job_type, expected_queue in job_types_and_queues:
            ...
            self.assertEqual(
                queue, expected_queue, f"Job type {job_type} should use queue {expected_queue}"
            )
