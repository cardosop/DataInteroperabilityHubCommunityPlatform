...
class SecurityAuditLogAPIIntegrationTest(TestCase):
    ...
    def test_filter_by_event_type(self):
        ...
        self.assertEqual(response.data['results'][0]['event_type'], SecurityEventType.EXTERNAL_REF_FETCH.value)
    ...
    def test_filter_by_multiple_criteria(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data['results']), 1)
        self.assertEqual(response.data['results'][0]['event_type'], SecurityEventType.CACHE_HIT.value)
    ...
    def test_serializer_includes_all_fields(self):
        ...
        expected_fields = [
            'id', 'event_type', 'timestamp', 'severity', 'ref_type', 'ref_path',
            'resolved_path', 'rate_limit_level', 'cache_operation', 'cache_key',
            'eviction_reason', 'violation_type', 'attempted_path', 'attempted_url',
            'description', 'metadata_json', 'request_id', 'ip_address', 'user_agent',
            'tenant_id', 'tenant_name', 'user_id', 'user_email', 'contract_id',
        ]
