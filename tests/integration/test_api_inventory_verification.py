...
class TestAPIInventoryCompleteness(TestCase):
    ...
    def setUp(self):
        ...
        self.assertIsNotNone(self.inventory_path.exists(),
    ...
    def test_inventory_file_exists(self):
        ...
        self.assertTrue(self.inventory_path.exists(),
                       f"Inventory file should exist: {self.inventory_path}")
        self.assertGreater(self.inventory_path.stat().st_size, 0,
                          "Inventory file should not be empty")
    ...
    def test_inventory_parses_correctly(self):
        ...
        self.assertIsInstance(endpoints_by_app, dict,
                            "Inventory should parse to dictionary")
        self.assertGreater(len(endpoints_by_app), 0,
                          "Inventory should contain at least one app")

        # Verify structure
        for app_name, endpoints in endpoints_by_app.items():
            self.assertIsInstance(endpoints, list,
                                f"Endpoints for {app_name} should be a list")
            for endpoint in endpoints:
                self.assertIn('method', endpoint,
                            f"Endpoint should have 'method' field")
                self.assertIn('path', endpoint,
                            f"Endpoint should have 'path' field")
                self.assertIsInstance(endpoint['method'], str,
                                    f"Method should be a string")
                self.assertIsInstance(endpoint['path'], str,
                                    f"Path should be a string")
    ...
    def test_inventory_has_minimum_endpoints(self):
        ...
        self.assertGreaterEqual(total_endpoints, 100,
    ...
    def test_inventory_has_expected_apps(self):
        ...
        self.assertEqual(len(missing_apps), 0,
                        f"Inventory missing expected apps: {', '.join(missing_apps)}")
    ...
    def test_inventory_completeness_verification(self):
        ...
        if not passed:
            self.fail(f"Inventory completeness issues:\n" + "\n".join(issues))
...
class TestAPIInventoryAccuracy(TestCase):
    ...
    def test_compliance_endpoints_accuracy(self):
        ...
        self.assertEqual(len(missing), 0,
                        f"Missing expected compliance endpoints:\n" + "\n".join(missing))
    ...
    def test_compliance_endpoints_verification(self):
        ...
        if not passed:
            self.fail(f"Compliance endpoints verification failed:\n" + "\n".join(issues))
    ...
    def test_dq_endpoints_verification(self):
        ...
        if not passed:
            self.fail(f"DQ endpoints verification failed:\n" + "\n".join(issues))
...
class TestAPIInventoryStandardizedPatterns(TestCase):
    ...
    def test_compliance_uses_runs_pattern(self):
        ...
        self.assertGreater(len(standardized), 0,
                          "Compliance endpoints should use standardized '/runs/' pattern")
        ...
        self.assertEqual(len(non_standardized), 0,
...
class TestAPIInventoryVerificationScript(TestCase):
    ...
    def test_verification_script_runs(self):
        ...
        self.assertTrue(script_path.exists(),
                       f"Verification script should exist: {script_path}")
        ...
        self.assertIn("Verifying API inventory", result.stdout,
                     "Script should output verification messages")
    ...
    def test_verification_script_checks_compliance(self):
        ...
        self.assertIn("compliance", result.stdout.lower(),
                     "Script should check compliance endpoints")
    ...
    def test_verification_script_checks_patterns(self):
        ...
        self.assertIn("standardized", result.stdout.lower(),
                     "Script should verify standardized patterns")
...
class TestAPIInventoryAgainstActualEndpoints(TestCase):
    ...
    def test_inventory_matches_actual_endpoints(self):
        ...
        compliance_inventory = {
            f"{e['method']} {e['path']}"
            for e in self.inventory_endpoints.get('compliance', [])
        }

        compliance_actual = {
            ep for ep in self.actual_endpoints
            if '/compliance/runs/' in ep or '/compliance/compliance-runs/' in ep
        }

        # Should have compliance endpoints
        self.assertGreater(len(compliance_inventory), 0,
                          "Inventory should contain compliance endpoints")

        # Check that standardized patterns are used
        standardized_inventory = {ep for ep in compliance_inventory if '/runs/' in ep}
        self.assertGreater(len(standardized_inventory), 0,
                          "Inventory should use standardized '/runs/' pattern")
