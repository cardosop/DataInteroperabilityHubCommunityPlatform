...
class OpenTelemetryMetricsIntegrationTest(TestCase):
    ...
    def test_all_counter_metrics_exported(self):
        ...
        file_uploads_total.labels(status='success', file_type='csv', tenant_id=str(self.tenant.id)).inc()
        contract_validations_total.labels(status='valid', spec_type='ODCS', tenant_id=str(self.tenant.id)).inc()
        contract_migrations_total.labels(
            source_version='1.0', target_version='2.0', strategy='auto', status='success', tenant_id=str(self.tenant.id)
        ).inc()
        ...
        if response.status_code == 200:
            ...
            counter_metrics = [
                'http_requests_total', 'http_errors_total', 'jobs_started_total',
                'jobs_completed_total', 'jobs_failed_total', 'dq_runs_total',
                'compliance_runs_total', 'cache_hits_total', 'cache_misses_total',
                'file_uploads_total', 'contract_validations_total', 'contract_migrations_total'
            ]
            
            for metric in counter_metrics:
                # Check for metric name (may be prefixed by OpenTelemetry)
                found = any(metric in line or metric.replace('_total', '') in line for line in content.split('\n'))
    ...
    def test_all_histogram_metrics_exported(self):
        ...
        if response.status_code == 200:
            ...
            histogram_metrics = [
                'http_request_duration_seconds', 'job_duration_seconds',
                'db_query_duration_seconds', 'file_upload_size_bytes'
            ]
    ...
    def test_all_gauge_metrics_exported(self):
        ...
        if response.status_code == 200:
            ...
            gauge_metrics = [
                'db_connections_active', 'job_queue_length',
                'tenant_running_jobs', 'tenant_queued_jobs',
                'asset_dq_status_count', 'asset_compliance_status_count'
            ]
        ...
        if response.status_code == 200:
            ...
            for line in lines[:10]:  # Check first 10 metric lines
                if line.strip():
                    ...
                    parts = line.split()
                    self.assertGreaterEqual(len(parts), 2, f"Metric line should have name and value: {line}")
    ...
    def test_histogram_buckets_exported(self):
        ...
        http_request_duration_seconds.labels(method='GET', route='/test/', status_class='2xx').observe(0.1)
        http_request_duration_seconds.labels(method='GET', route='/test/', status_class='2xx').observe(0.5)
        http_request_duration_seconds.labels(method='GET', route='/test/', status_class='2xx').observe(1.0)
    ...
    def test_metric_labels_correct_format(self):
        ...
        if response.status_code == 200:
            ...
            label_pattern = r'\{[^}]+\}'
            lines_with_labels = [line for line in content.split('\n') if re.search(label_pattern, line)]
