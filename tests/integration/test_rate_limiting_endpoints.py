...
from hub.apps.rate_limiting.utils import get_endpoint_category, EndpointCategory, TimeWindow
...
class TestRateLimitingConfiguration(TestCase):
    ...
    def test_rate_limits_reasonable(self):
        ...
        categories = [EndpointCategory.COMPLIANCE_RUN, EndpointCategory.DQ_RUN, EndpointCategory.CATALOG_READ]
        ...
        for category in categories:
            for window in windows:
                ...
                self.assertGreaterEqual(maximum, default,
                                      f"Maximum should be >= default for {category}/{window}")
                self.assertGreater(default, 0, f"Default limit should be > 0 for {category}/{window}")
                self.assertGreater(maximum, 0, f"Maximum limit should be > 0 for {category}/{window}")
        ...
        for path, method in standardized_paths:
            ...
            if method == 'POST':
                if '/compliance' in path:
                    ...
                    self.assertEqual(category, EndpointCategory.DQ_RUN,
                                   f"POST {path} should be DQ_RUN")
            elif method == 'GET':
                self.assertEqual(category, EndpointCategory.CATALOG_READ,
                               f"GET {path} should be CATALOG_READ")
