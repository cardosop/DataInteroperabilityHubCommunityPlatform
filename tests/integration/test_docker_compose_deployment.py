...
class DockerComposeManager:
    ...
    def _run_command(self, command: List[str], check: bool = True, timeout: int = 300) -> subprocess.CompletedProcess:
    ...
    def start_services(self, services: Optional[List[str]] = None, wait: bool = True) -> None:
        """Start Docker Compose services."""
        if self.services_started:
            return
        
        # First, try to stop any existing containers with conflicting names
        # This handles the case where containers exist from previous test runs
        try:
    ...
    def stop_services(self, services: Optional[List[str]] = None) -> None:
        ...
        try:
            ...
            try:
                self._run_command(["down", "-v", "--timeout", "10", "--remove-orphans"], check=False, timeout=30)
            except Exception:
                pass
...
class TestDockerComposeHealthChecks:
    ...
    # Health endpoint mappings: (service_name, port, endpoint_path)
    HEALTH_ENDPOINTS = [
        ('workflow-engine-service', 8088, '/healthz'),
        ('workflow-engine-service', 8088, '/ready'),
    ...
    def started_services(self, docker_compose_manager, infrastructure_services, application_services):
...
class TestDockerComposeServiceCommunication:
    ...
    def test_services_can_reach_each_other(
        ...
        if workflow_status and postgres_status:
            ...
            workflow_health = requests.get('http://localhost:8088/ready', timeout=5)
    ...
    def test_event_bus_redis_connection(
        ...
        """Test that event bus service can connect to Redis."""
        ...
        if event_bus_status and redis_status:
            assert event_bus_status.get('State') == 'running'
            assert redis_status.get('State') == 'running'
    ...
    def test_service_discovery_works(
        self, docker_compose_manager, started_services
    ):
        ...
        if registry_status and engine_status:
            ...
            engine_health = requests.get('http://localhost:8088/healthz', timeout=5)
            
            assert registry_health.status_code == 200
            assert engine_health.status_code == 200
    
    def test_api_service_can_reach_backend_services(
        self, docker_compose_manager, started_services
    ):
...
class TestDockerComposeServiceDependencies:
    ...
    def test_workflow_engine_depends_on_postgres(
        ...
        workflow_config = docker_compose_config.get('services', {}).get('workflow-engine-service', {})
    ...
    def test_workflow_engine_depends_on_redis(
        ...
        """Test that workflow-engine-service depends on redis."""
        workflow_config = docker_compose_config.get('services', {}).get('workflow-engine-service', {})
        depends_on = workflow_config.get('depends_on', {})
    ...
    def test_event_bus_depends_on_postgres_and_redis(
        ...
        """Test that event-bus-health-service depends on postgres and redis."""
        event_bus_config = docker_compose_config.get('services', {}).get('event-bus-health-service', {})
        depends_on = event_bus_config.get('depends_on', {})
        
        assert 'postgres' in depends_on or 'postgres' in (depends_on if isinstance(depends_on, list) else []), \
            "event-bus-health-service must depend on postgres"
        assert 'redis' in depends_on or 'redis' in (depends_on if isinstance(depends_on, list) else []), \
            "event-bus-health-service must depend on redis"
    ...
    def test_api_service_depends_on_infrastructure(
        ...
        for dep in required_deps:
            assert dep in depends_on or dep in (depends_on if isinstance(depends_on, list) else []), \
                f"api-service must depend on {dep}"
    ...
    def test_services_fail_without_dependencies(
        ...
        # Try to start workflow-engine (should fail or wait)
        try:
            docker_compose_manager.start_services(['workflow-engine-service'], wait=False)
            # If it starts, check logs for errors
            time.sleep(5)
            logs = docker_compose_manager.get_service_logs('workflow-engine-service')
            # Service should show connection errors
            assert 'error' in logs.lower() or 'failed' in logs.lower() or 'unable' in logs.lower(), \
                "Service should show errors when dependencies are unavailable"
        except Exception:
            # Expected - service may fail to start
            pass
        finally:
            # Restore postgres
            docker_compose_manager.start_services(['postgres'], wait=True)
...
def pytest_collection_modifyitems(config, items):
    ...
    skip_runtime = pytest.mark.skip(reason="Docker Compose runtime tests require services to be started. Set PYTEST_DOCKER_COMPOSE_RUNTIME=1 to run them.")
