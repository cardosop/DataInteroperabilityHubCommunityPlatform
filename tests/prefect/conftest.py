...
def prefect_server_available(prefect_server_health: bool) -> bool:
    ...
    if not prefect_server_health:
        pytest.skip("Prefect Server is not available. Start with: docker-compose -f docker-compose.test.yml up -d prefect-server")
    return True


@pytest.fixture(scope="function")
def prefect_api_key(prefect_server_available: bool) -> Optional[str]:
    """
...
@pytest.fixture(scope="function")
def prefect_work_pool(prefect_server_available: bool, prefect_api_key: Optional[str]) -> str:
    """
