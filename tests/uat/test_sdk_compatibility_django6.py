...
@pytest.mark.skip(reason="Python SDK not installed")
    ...
    def test_python_sdk_client_initialization(self):
        ...
        try:
            ...
            config = DataHubClientConfig(
                base_url="http://localhost:8000/api/v1", api_token="test-token"
            )
...
@pytest.mark.skip(reason="Python SDK not installed")
    def test_python_sdk_functionality(self):
        ...
        try:
            from datahub_interoperability import DataHubClient, DataHubClientConfig
            from datahub_interoperability.errors import NotFoundError, ValidationError
...
class JavaScriptSDKCompatibilityTest(TestCase):
    ...
    def test_javascript_sdk_exists(self):
        ...
        if sdk_js_path.exists():
            ...
            pytest.skip("JavaScript SDK not found")  # noqa: skip-in-body — runtime service dependency
    ...
    def test_javascript_sdk_package_json(self):
        ...
        package_json_path = (
            Path(__file__).parent.parent.parent / "sdk" / "javascript" / "package.json"
        )
        ...
        if package_json_path.exists():
            ...
            pytest.skip("JavaScript SDK package.json not found")  # noqa: skip-in-body — runtime service dependency
...
@pytest.mark.skip(reason="CLI tool not available")
    def test_cli_tool_importable(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["datahub", "--help"], check=False, capture_output=True, text=True, timeout=5
            )
...
@pytest.mark.skip(reason="CLI tool not available")
    def test_cli_tool_functionality(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["datahub", "--version"], check=False, capture_output=True, text=True, timeout=5
            )
...
class SDKFunctionalityMaintainedTest(TestCase):
    ...
    def test_sdk_crud_operations(self):
        ...
        # Create
        response = self.client.post(
            "/api/v1/assets/", {"key": "sdk-test", "name": "SDK Test"}, format="json"
        )
