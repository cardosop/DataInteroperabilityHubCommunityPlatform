...
class RandomServiceFailureTest(TestCase):
    ...
    def test_compliance_down_does_not_affect_dq(self):
        ...
        compliance_cb = CircuitBreaker(
            service_name="compliance-chaos-1", failure_threshold=3,
            timeout_seconds=10, success_threshold=1, redis_client=None,
        )
        dq_cb = CircuitBreaker(
            service_name="dq-chaos-1", failure_threshold=3,
            timeout_seconds=10, success_threshold=1, redis_client=None,
        )
    ...
    def test_dq_down_does_not_affect_semantic(self):
        ...
        dq_cb = CircuitBreaker(
            service_name="dq-chaos-2", failure_threshold=3,
            timeout_seconds=10, success_threshold=1, redis_client=None,
        )
        semantic_cb = CircuitBreaker(
            service_name="semantic-chaos-1", failure_threshold=3,
            timeout_seconds=10, success_threshold=1, redis_client=None,
        )
    ...
    def test_circuit_breaker_reset_recovers(self):
        ...
        cb = CircuitBreaker(
            service_name="reset-chaos", failure_threshold=2,
            timeout_seconds=0, success_threshold=1, redis_client=None,
        )
