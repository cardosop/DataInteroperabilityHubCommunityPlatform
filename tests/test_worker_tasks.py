...
class TestScheduledExportWorkerTasks(TestCase):
    ...
    def setUp(self):
        ...
        self.tenant = Tenant.objects.create(
            name="se-worker", slug="se-worker", status="ACTIVE",
        )
    ...
    def test_apply_side_effects_happy_path(self):
        ...
        from hub.apps.scheduled_export.models import (
            ScheduledExport, ScheduledExportRun, ScheduledExportStatus,
        )
        export = ScheduledExport.objects.create(
            tenant=self.tenant, name="test-export",
            export_config={}, schedule="0 6 * * *",
        )
        run = ScheduledExportRun.objects.create(
            scheduled_export=export, status=ScheduledExportStatus.RUNNING,
        )
    ...
    def test_apply_side_effects_idempotent(self):
        ...
        export = ScheduledExport.objects.create(
            tenant=self.tenant, name="test-export-2",
            export_config={}, schedule="0 6 * * *",
        )
        run = ScheduledExportRun.objects.create(
            scheduled_export=export, status=ScheduledExportStatus.RUNNING,
    ...
    def test_apply_side_effects_invalid_status_handled(self):
        ...
        export = ScheduledExport.objects.create(
            tenant=self.tenant, name="test-export-3",
            export_config={}, schedule="0 6 * * *",
        )
        run = ScheduledExportRun.objects.create(
            scheduled_export=export, status=ScheduledExportStatus.COMPLETED,
        )
        from hub.apps.scheduled_export.worker_run_lifecycle import (
            apply_run_completion_side_effects,
        )
...
class TestScheduledIngestionWorkerTasks(TestCase):
    ...
    def setUp(self):
        from hub.apps.tenants.models import Tenant
        self.tenant = Tenant.objects.create(
            name="si-worker", slug="si-worker", status="ACTIVE",
        )
    ...
    def test_start_run_happy_path(self):
        ...
        ingestion = ScheduledIngestion.objects.create(
            tenant=self.tenant, name="test-si",
            source_config={}, schedule="0 0 * * *",
        )
        run = ScheduledIngestionRun.objects.create(
            ingestion=ingestion, status="PENDING",
        )
        from hub.data_movement.worker_run_lifecycle import start_run
        result = start_run(run, job_id="job-1",
                           tenant_id=str(self.tenant.id), direction="ingestion")
    ...
    def test_worker_run_lifecycle_idempotent(self):
        ...
        ingestion = ScheduledIngestion.objects.create(
            tenant=self.tenant, name="test-si-2",
            source_config={}, schedule="0 0 * * *",
        )
        run = ScheduledIngestionRun.objects.create(
            ingestion=ingestion, status="PENDING",
        ...
        from hub.data_movement.worker_run_lifecycle import start_run
        start_run(run, job_id="job-2", tenant_id=str(self.tenant.id),
                  direction="ingestion")
...
class TestFilesWorkerTasks(TestCase):
    ...
    def setUp(self):
        from hub.apps.tenants.models import Tenant
        self.tenant = Tenant.objects.create(
            name="file-worker", slug="file-worker", status="ACTIVE",
        )
...
class TestBillingWorkerTasks(TestCase):
    ...
    def setUp(self):
        from hub.apps.tenants.models import Tenant
        self.tenant = Tenant.objects.create(
            name="bill-worker", slug="bill-worker", status="ACTIVE",
        )
...
class TestContractsWorkerTasks(TestCase):
    ...
    def setUp(self):
        from hub.apps.tenants.models import Tenant
        self.tenant = Tenant.objects.create(
            name="ct-worker", slug="ct-worker", status="ACTIVE",
        )
...
class TestDataMovementWorkerTasks(TestCase):
    ...
    def setUp(self):
        from hub.apps.tenants.models import Tenant
        self.tenant = Tenant.objects.create(
            name="dm-worker", slug="dm-worker", status="ACTIVE",
        )
    ...
    def test_start_run_exports_audit_event(self):
        ...
        ingestion = ScheduledIngestion.objects.create(
            tenant=self.tenant, name="dm-test",
            source_config={}, schedule="0 0 * * *",
        )
        run = ScheduledIngestionRun.objects.create(
            ingestion=ingestion, status="PENDING",
        ...
        from hub.data_movement.worker_run_lifecycle import start_run
        result = start_run(run, job_id="dm-job-1",
                           tenant_id=str(self.tenant.id), direction="ingestion")
...
class TestWorkerTaskCrossCutting(TestCase):
    ...
    def test_start_run_with_empty_job_id(self):
        ...
        ingestion = ScheduledIngestion.objects.create(
            tenant=self.tenant(), name="empty-job-test",
            source_config={}, schedule="0 0 * * *",
        )
        run = ScheduledIngestionRun.objects.create(
            ingestion=ingestion, status="PENDING",
        ...
        from hub.data_movement.worker_run_lifecycle import start_run
        result = start_run(run, job_id="", tenant_id=str(self.tenant().id),
                           direction="ingestion")
        assert "status" in result
    ...
    def _create_tenant(self, suffix):
        ...
        return Tenant.objects.create(
            name=f"xk-{suffix}", slug=f"xk-{suffix}", status="ACTIVE",
        )
