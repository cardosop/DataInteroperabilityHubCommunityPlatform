...
class FileUploadTest(FileStorageRegressionTest):
    ...
    def test_file_upload_init(self):
        ...
        response = self.client.post(
            "/api/v1/files/init/",
            {"name": "test-upload.csv", "content_type": "text/csv", "size": 1024},
            format="json",
        )
        ...
        if response.status_code == status.HTTP_201_CREATED:
            ...
            self.assertIsNotNone(
                file_id,
                f"Response should have 'file_id' or 'id'. Got: {list(response.data.keys())}",
            )
    ...
    def test_file_upload_complete(self):
        ...
        init_response = self.client.post(
            "/api/v1/files/init/",
            {"name": "complete-upload.csv", "content_type": "text/csv", "size": 2048},
            format="json",
        )
        ...
        if init_response.status_code == status.HTTP_201_CREATED:
            ...
            self.assertIsNotNone(
                file_id,
                f"Response should have 'file_id' or 'id'. Got: {list(init_response.data.keys())}",
            )

            # Complete upload
            complete_response = self.client.post(
                f"/api/v1/files/{file_id}/complete/", {}, format="json"
            )
            self.assertLess(
    ...
    def test_file_upload_with_ingestion_mode(self):
        ...
        init_response = self.client.post(
            "/api/v1/files/init/",
            {"name": "ingestion-upload.csv", "content_type": "text/csv", "size": 4096},
            format="json",
        )
...
class FileDeletionTest(FileStorageRegressionTest):
    ...
    def test_file_deletion(self):
        ...
        if response.status_code == status.HTTP_204_NO_CONTENT:
            ...
            self.assertEqual(
                file.status, FileStatus.DELETED, "File should be soft-deleted (status=DELETED)"
            )
...
class FileMetadataTest(FileStorageRegressionTest):
    ...
    def test_file_metadata_update(self):
        ...
        response = self.client.patch(
            f"/api/v1/files/{file.id}/", {"name": "updated-metadata-test.csv"}, format="json"
        )
