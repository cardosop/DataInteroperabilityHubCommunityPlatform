...
class IntegrationRegressionTest(TestCase):
    ...
    def setUp(self):
        ...
        self.asset = Asset.objects.create(
            tenant=self.tenant, key="integration-asset", name="Integration Asset"
        )
        ...
        self.contract = Contract.objects.create(
            ...
            hub_contract_json={
                ...
                "info": {"title": "Integration Contract"},
            },
        )
...
class ComplianceServiceIntegrationTest(IntegrationRegressionTest):
    ...
    def test_compliance_run_list(self):
        ...
        ComplianceRun.objects.create(
            tenant=self.tenant, asset=self.asset, job=job1, status=ComplianceRunStatus.PENDING
        )
        ComplianceRun.objects.create(
            tenant=self.tenant, asset=self.asset, job=job2, status=ComplianceRunStatus.RUNNING
        )
    ...
    def test_compliance_scan_modes(self):
        ...
        response = self.client.post(
            "/api/v1/compliance/runs/",
            {"asset_id": str(self.asset.id), "scan_mode": "external"},
            format="json",
        )
        self.assertIn(response.status_code, [status.HTTP_201_CREATED, status.HTTP_400_BAD_REQUEST])
...
class SemanticServiceIntegrationTest(IntegrationRegressionTest):
    ...
    def test_semantic_resource_creation(self):
        ...
        semantic_resource = SemanticResource.objects.filter(
            resource_type=ResourceType.ASSET, resource_id=str(self.asset.id)
        ).first()
        ...
        if semantic_resource:
            ...
            response = self.client.get(
                f"/api/v1/semantic/semantic-resources/{semantic_resource.id}/"
            )
    ...
    def test_sparql_query(self):
        ...
        response = self.client.post("/api/v1/semantic/sparql", {"query": query}, format="json")
    ...
    def test_resolve_uri(self):
        ...
        self.assertIn(
            response.status_code,
            [status.HTTP_200_OK, status.HTTP_404_NOT_FOUND, status.HTTP_503_SERVICE_UNAVAILABLE],
        )
...
class CrossServiceIntegrationTest(IntegrationRegressionTest):
    ...
    def test_dq_and_compliance_integration(self):
        ...
        self.assertIn(
            dq_response.status_code, [status.HTTP_201_CREATED, status.HTTP_400_BAD_REQUEST]
        )
        self.assertIn(
            compliance_response.status_code, [status.HTTP_201_CREATED, status.HTTP_400_BAD_REQUEST]
        )
    ...
    def test_contract_semantic_integration(self):
        ...
        semantic_resource = SemanticResource.objects.filter(
            resource_type=ResourceType.CONTRACT, resource_id=str(self.contract.id)
        ).first()
