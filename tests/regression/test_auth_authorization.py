...
class JWTAuthenticationTest(AuthenticationRegressionTest):
    ...
    def test_jwt_login(self):
        ...
        has_refresh = "refresh_token" in response.data or response.cookies.get("refresh_token")
        self.assertTrue(
            has_refresh,
...
class APIKeyAuthenticationTest(AuthenticationRegressionTest):
    ...
    def test_api_key_authentication(self):
        ...
        response = self.client.get("/api/v1/assets/", HTTP_AUTHORIZATION="ApiKey test-api-key-123")
        self.assertIn(response.status_code, [status.HTTP_200_OK, status.HTTP_403_FORBIDDEN])
...
class AuthorizationTest(AuthenticationRegressionTest):
    ...
    def test_tenant_isolation(self):
        ...
        asset = Asset.objects.create(
            tenant=self.tenant, key="isolated-asset", name="Isolated Asset"
        )
