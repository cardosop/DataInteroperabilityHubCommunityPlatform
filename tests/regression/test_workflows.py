...
class ContractCreationWorkflowTest(WorkflowRegressionTest):
    ...
    def test_contract_creation_workflow(self):
        ...
        self.assertIn(
            contract_response.status_code, [status.HTTP_201_CREATED, status.HTTP_400_BAD_REQUEST]
        )
        ...
        if contract_response.status_code == status.HTTP_201_CREATED:
            ...
            asset_field = retrieve_response.data.get("asset") or retrieve_response.data.get(
                "asset_id"
            )
    ...
    def test_contract_validation_workflow(self):
        ...
        asset_response = self.client.post(
            "/api/v1/assets/",
            {"key": "validation-asset", "name": "Validation Asset"},
            format="json",
        )
        self.assertEqual(asset_response.status_code, status.HTTP_201_CREATED)
        asset_id = asset_response.data["id"]
...
class AssetOnboardingWorkflowTest(WorkflowRegressionTest):
    ...
    def test_data_first_onboarding_workflow(self):
        ...
        file_init_response = self.client.post(
            "/api/v1/files/init/",
            {"name": "test-data.csv", "content_type": "text/csv", "size": 1024},
            format="json",
        )
        self.assertIn(
            file_init_response.status_code, [status.HTTP_201_CREATED, status.HTTP_400_BAD_REQUEST]
        )
        ...
        if file_init_response.status_code == status.HTTP_201_CREATED:
            ...
            self.assertIsNotNone(
                file_id,
                f"Response should have 'file_id' or 'id'. Got: {list(file_init_response.data.keys())}",
            )
            ...
            file_complete_response = self.client.post(
                ...
                {"ingestion_mode": "DATA_FIRST", "run_dq": True, "run_compliance": True},
                format="json",
            )
    ...
    def test_contract_first_onboarding_workflow(self):
        ...
        asset_response = self.client.post(
            "/api/v1/assets/",
            {"key": "contract-first-asset", "name": "Contract First Asset"},
            format="json",
        )
        self.assertEqual(asset_response.status_code, status.HTTP_201_CREATED)
        asset_id = asset_response.data["id"]
        ...
        if contract_response.status_code == status.HTTP_201_CREATED:
            ...
            file_init_response = self.client.post(
                "/api/v1/files/init/",
                {"name": "contract-first-data.csv", "content_type": "text/csv", "size": 2048},
                format="json",
            )
            self.assertIn(
    ...
    def test_asset_lifecycle_workflow(self):
        ...
        asset_response = self.client.post(
            "/api/v1/assets/",
            {"key": "lifecycle-asset", "name": "Lifecycle Asset", "status": "DRAFT"},
            format="json",
        )
        self.assertEqual(asset_response.status_code, status.HTTP_201_CREATED)
        asset_id = asset_response.data["id"]
        ...
        update_response = self.client.patch(
            f"/api/v1/assets/{asset_id}/", {"status": "ACTIVE"}, format="json"
        )
        self.assertIn(
        ...
        if update_response.status_code == status.HTTP_200_OK:
            ...
            # 3. Update to Public
            update_response = self.client.patch(
                f"/api/v1/assets/{asset_id}/", {"status": "PUBLIC"}, format="json"
            )
            self.assertIn(
                update_response.status_code, [status.HTTP_200_OK, status.HTTP_400_BAD_REQUEST]
            )
            ...
            # 4. Update to Retired
            update_response = self.client.patch(
                f"/api/v1/assets/{asset_id}/", {"status": "RETIRED"}, format="json"
            )
            self.assertIn(
                update_response.status_code, [status.HTTP_200_OK, status.HTTP_400_BAD_REQUEST]
            )
...
class ContractAssetIntegrationWorkflowTest(WorkflowRegressionTest):
    ...
    def test_attach_contract_to_asset_workflow(self):
        ...
        asset_response = self.client.post(
            "/api/v1/assets/",
            {"key": "attach-contract-asset", "name": "Attach Contract Asset"},
            format="json",
        )
        self.assertEqual(asset_response.status_code, status.HTTP_201_CREATED)
        asset_id = asset_response.data["id"]
        ...
        if contract_response.status_code == status.HTTP_201_CREATED:
            ...
            # Contract serializer uses 'asset' field (UUID), not 'asset_id'
            asset_field = contract_retrieve.data.get("asset") or contract_retrieve.data.get(
                "asset_id"
            )
            self.assertIsNotNone(asset_field, "Contract should have asset field")
            # Convert to string for comparison if needed
    ...
    def test_attach_dataset_to_asset_workflow(self):
        ...
        asset_response = self.client.post(
            "/api/v1/assets/",
            {"key": "attach-dataset-asset", "name": "Attach Dataset Asset"},
            format="json",
        )
        self.assertEqual(asset_response.status_code, status.HTTP_201_CREATED)
        asset_id = asset_response.data["id"]
        ...
        file_init_response = self.client.post(
            "/api/v1/files/init/",
            {"name": "dataset.csv", "content_type": "text/csv", "size": 4096},
            format="json",
        )
        self.assertIn(
            file_init_response.status_code, [status.HTTP_201_CREATED, status.HTTP_400_BAD_REQUEST]
        ...
        if file_init_response.status_code == status.HTTP_201_CREATED:
            ...
            attach_response = self.client.post(
                f"/api/v1/assets/{asset_id}/attach_dataset/", {"file_id": file_id}, format="json"
            )
