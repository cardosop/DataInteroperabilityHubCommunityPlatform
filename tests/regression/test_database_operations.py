...
class ModelCRUDTest(DatabaseOperationsRegressionTest):
    ...
    def test_tenant_crud(self):
        ...
        tenant = Tenant.objects.create(
            name=f"CRUD Tenant {uuid.uuid4().hex[:8]}", slug=f"crud-tenant-{uuid.uuid4().hex[:8]}"
        )
    ...
    def test_asset_crud(self):
        ...
        # Create
        asset = Asset.objects.create(tenant=self.tenant, key="crud-asset", name="CRUD Asset")
...
class DatabaseQueryTest(DatabaseOperationsRegressionTest):
    ...
    def test_pagination(self):
        ...
        # Create multiple assets
        for i in range(10):
            Asset.objects.create(tenant=self.tenant, key=f"asset-{i}", name=f"Asset {i}")
...
class JSONFieldOperationsTest(DatabaseOperationsRegressionTest):
    ...
    def test_jsonfield_storage(self):
        ...
        asset = Asset.objects.create(tenant=self.tenant, key="json-asset", name="JSON Asset")
    ...
    def test_jsonfield_query(self):
        ...
        asset = Asset.objects.create(
            tenant=self.tenant, key="json-query-asset", name="JSON Query Asset"
        )
...
class DatabaseTransactionTest(DatabaseOperationsRegressionTest):
        ...
        try:
            with transaction.atomic():
                Asset.objects.create(
                    tenant=self.tenant, key="transaction-asset", name="Transaction Asset"
                )
                # Force an error
    ...
    def test_transaction_commit(self):
        ...
        with transaction.atomic():
            Asset.objects.create(
                tenant=self.tenant, key="transaction-commit-asset", name="Transaction Commit Asset"
            )
...
class ForeignKeyRelationshipsTest(DatabaseOperationsRegressionTest):
    ...
    def test_asset_tenant_relationship(self):
        ...
        asset = Asset.objects.create(tenant=self.tenant, key="fk-asset", name="FK Asset")
    ...
    def test_contract_asset_relationship(self):
        ...
        asset = Asset.objects.create(
            tenant=self.tenant, key="contract-fk-asset", name="Contract FK Asset"
        )
    ...
    def test_cascade_delete(self):
        ...
        asset = Asset.objects.create(tenant=self.tenant, key="cascade-asset", name="Cascade Asset")
...
class DatabaseIndexesTest(DatabaseOperationsRegressionTest):
    ...
    def test_unique_constraints(self):
        ...
        Asset.objects.create(tenant=self.tenant, key="unique-asset", name="Unique Asset")

        # Try to create another asset with same key (should fail)
        with self.assertRaises(Exception):  # IntegrityError or ValidationError
            Asset.objects.create(tenant=self.tenant, key="unique-asset", name="Duplicate Asset")
    ...
    def test_indexed_queries(self):
        ...
        for i in range(10):
            Asset.objects.create(
                tenant=self.tenant, key=f"indexed-asset-{i}", name=f"Indexed Asset {i}"
            )
