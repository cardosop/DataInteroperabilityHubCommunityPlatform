"""
Health check endpoints for workflow engine service.
...
def healthz(request=None):
    ...
    Returns 200 OK if workflow engine process is running (no deep checks).
...
def ready(request=None):
    ...
    # Check Redis events connection (required for event bus)
    try:
        from hub.apps.core.redis_pools import get_redis_events_client
        r = get_redis_events_client()
        r.ping()
        checks['redis_events'] = 'ok'
    except Exception as e:
        checks['redis_events'] = f'unhealthy: {str(e)}'
        all_ready = False
